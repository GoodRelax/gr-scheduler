...
|---|---|---|---|
| TOOL-L1-008 | Part 1 | ヘッダー左端（ブランディングの左）に `[Fit]` を新設（機能＝`renderer.fitToContent()` 既存流用） | done：`main.buildChrome` が順序生成、`wirePaletteChrome` が click→fitToContent 配線。**配置は CR-015 で supersede**（ブランディング＋タイトルの後＝アクションツールバー先頭へ移動、`HEADER_LEFT_CONTROL_ROLES` は `HEADER_ELEMENT_ROLES` に統合して廃止）。機能は不変。単体：`tests/cr006-palette-header.test.ts`（Fit→P 隣接・ASCII）／`tests/header-model.test.ts`（CR-015 順序） |
| TOOL-L1-001/008 | Part 2 | ヘッダー左から2番目に `[P]`（`data-role=header-palette-toggle`）、パレット `[-]`（`togglePaletteMinimized`）と双方向連動・最小化時 aria-pressed=false（不活性/色変化） | done：`main.wirePaletteChrome` の `setPaletteMinimized` が `[P]` の aria-pressed/aria-expanded を同期、`[P]` click と `[-]` click が同一 `togglePaletteMinimized` を共有＝双方向。**配置のみ CR-015 で supersede**（`[Fit]` の直後、ツールバー2番目）。単体：`tests/cr006-palette-header.test.ts`（Fit→P 隣接）、挙動同期は e2e |
...
## CR-015 / CR-016 / DEF-010 (ヘッダー要素順序の改訂・プロジェクトタイトルの結線/編集/出力) 実装トレース

CR-015（ヘッダー要素順序の改訂：CR-006 Part 1/2 と TOOL-L1-008 を revise）、CR-016（タイトルの
インライン編集と MSPDI `<Name>` 出力）、DEF-010（タイトル表示がストア変更に追随しない）を1パスで実装。
スキーマ変更なし（`title` は既存の必須フィールド）。

| 改訂要求/契約 | 出典 | 内容 | 実装状態 |
|---|---|---|---|
| TOOL-L1-008（改訂, CR-015） | CR-015 | ヘッダー要素順序を `GR Scheduler +(c)` ブランディング → プロジェクトタイトル → `[Fit]` → `[P]` → SS → Load → Save → Light → Dark → Mono L → Mono D → Base V → Base I → Undo → Redo → AI → `?` へ改訂（CR-006 Part 1/2 のブランディング左端配置を supersede。`[R]` は `[P]` の誤記であり存在しない） | done：`header-model.HEADER_ELEMENT_ROLES`（17要素の単一SSOT）を新設し、`HEADER_CONTROL_ROLES` は同定数から branding/title を除いた派生値へ変更。`HEADER_LEFT_CONTROL_ROLES` は廃止し、`main.buildChrome` は `[Fit]`/`[P]` を右トールバー先頭へ role→element ルックアップで append（位置のハードコード無し）。挙動（`fitToContent` 配線・`[P]`↔パレット `[-]` 双方向同期）は不変。単体：`tests/header-model.test.ts`（順序・Fit/P がブランディングとタイトルの**後**・派生関係・重複無し）、`tests/cr006-palette-header.test.ts`（Fit→P 隣接に緩和） |
| TOOL-L1-008 / DATA-JSON（`title`） | DEF-010 | ヘッダーのタイトル表示がストアの変更に継続追随すること（従来は bootstrap と import の2箇所の時点呼び出しのみで、リネーム・Undo・Redo 後に陳腐化した） | done：`header-model.bindHeaderTitleText(source, setTitleText)` を新設（DIP：`getDocument`/`subscribe` の構造的インタフェースのみに依存）。`main.wireScheduleTitle` が購読し、`store.subscribe` が dispatch/undo/redo/`replaceDocument` の全経路で発火するため時点呼び出しを全廃。空タイトルの代替表示は共有定数 `HEADER_TITLE_PLACEHOLDER='(untitled schedule)'` 1箇所（構築時の初期値も同定数）。単体：`tests/cr016-project-title.test.ts`（初期描画／リネーム→Undo→Redo の3回追随／文書差替追随／空→プレースホルダ／購読解除） |
| TOOL-L1-008 / CURS-L1-005（編集様式踏襲） | CR-016 | タイトルのダブルクリックでインライン編集。Enter コミット／Escape 復帰（CR-007 コメント編集と同一セマンティクス）。コミットは Undo 可能なコマンドとして dispatch（CR-009 透かし UTC の再スタンプは正当） | done：`commands.setScheduleTitleCommand`（無変更なら同一参照＝履歴無し）と `commands.resolveTitleEditOutcome`（cancel=復帰／空白のみ=非コミット／無変更=非コミット／コミット時 trim）を新設。`main.wireScheduleTitle` が dblclick と Enter/F2（キーボード到達性 WCAG 2.1.1）で `<input>` を差し込み、Enter/blur=commit・Escape=cancel、編集中は `dataset.editing` で購読描画を抑止。単体：`tests/cr016-project-title.test.ts`（コミット→Undo→Redo／Escape 非 dispatch／trim・空白・無変更／title 以外不変／JSON 往復） |
| DATA-MSPDI（`<Name>` 追加） | CR-016 | Export で `<Project><Name>` にもプロジェクト名を出力（MS Project が「名前」として扱う要素）。Import は `<Title>` を正とし、欠落/空のとき `<Name>` へフォールバック。往復のロスレス性を維持 | done：`mspdi-codec.exportMspdi` が vendored `mspdi_pj12.xsd` の xsd:sequence 順（UID?, Name?, Title?）に従い `<Name>`→`<Title>` を出力。`projectScalarScope`（最初の `<Tasks>` より前）＋`projectTitleFromMspdi` を新設し、**タスクの `<Name>` をプロジェクト名と誤認しない**。単体：`tests/cr016-project-title.test.ts`（両要素出力・順序・XML エスケープ・サイドカー往復ロスレス・サイドカー無し往復・Title 優先・Name フォールバック・タスク名誤認防止） |

**CR-015/CR-016/DEF-010 ゲート**: tsc 0 err / eslint 0 err（`src tests`）/ vitest 776 pass・
5 fail（本作業の範囲外＝並行セッションが編集中の `plan-actual-geometry.ts`／`plan-actual-display.ts`
に起因、`tests/plan-actual-geometry.test.ts`・`tests/plan-actual-display-gating.test.ts`）。
本作業が触れた4ファイル（`header-model.ts`／`main.ts`／`commands.ts`／`mspdi-codec.ts`）関連の
テストは全緑（`tests/cr016-project-title.test.ts` 22 ケース新設を含む 50 pass）。
`document-schema-conformance.test.ts` は緑維持（スキーマ変更なし）。ヘッダーの実 DOM 配線
（要素の実配置・ダブルクリック編集）は `bootstrap()` の import 時実行により node で駆動不可のため、
純関数シーム（`HEADER_ELEMENT_ROLES`／`bindHeaderTitleText`／`resolveTitleEditOutcome`）で単体被覆し、
実 DOM 挙動は親セッションのライブ確認に委譲。

## CR-013 (Separate は行を伸ばす／新規実績を掴めるようにする) 実装トレース

CR-013 の Part 1（`separate` はバーを半分にせず**行（セクション）高**を伸ばす）と
Part 2（実績開始のみ＝ゼロ幅の実績が掴めない）を1パスで実装。スキーマ変更なし。

| 要求/契約 | Part | 内容 | 実装状態 |
|---|---|---|---|
| PLAN-L1-005 | Part 1 | `separate` で予定バー・実績バーともに**通常のバー高**を維持し、実績を予定の下段へ積む | done：`plan-actual-geometry.computePlanActualBars` の separate 分岐が両バーを `laneHeight` 全高で返し、実績を `laneTop + separateActualBarOffsetPx(laneHeight)` に配置。`SEPARATE_LANE_GAP_FRACTION` の意味を「レーン分割量」から「**バー高に対する上下間隙**」へ再定義。単体：`tests/plan-actual-geometry.test.ts`（全高維持・オフセット比例）、`tests/cr012-template-plan-actual.test.ts`（既存の上下関係は不変で緑） |
| ARCH-C-011（レイアウト） | Part 1 | 行高の決定は**レイアウト/ユースケース層**に置き、レンダラには置かない | done：`layout-engine.layoutRows` が `viewState.planActualStyle` ＋ `planActualDisplay` ＋ 行内アイテムから判定。`rowBandUnitHeight(laneCount, stackedActualLaneCount)` が実績を積む**レーン数ぶんだけ**増分（`separateActualLaneExtraUnitHeight()` = 通常バー高 ×(1+gap)）を加算し、同一行内の後続レーンも同量だけ下げる。`RowGeometry.stacksActualBars` で行ごとの判定を公開。`plan-actual-geometry` は純粋関数のまま |
| PLAN-L1-002 × PLAN-L1-005 | Part 1 | 表示フィルタと整合：`plan-only`／`actual-only`／`none` では余分な高さを確保しない | done：`plan-actual-display.stacksActualBarBelowPlan(item, style, display)` が `separate` ＋ 両側表示 ＋ `drawsActualBar(item)`（実績を持つ平板バーのタスクのみ。マイルストーン＝点、arrow/chevron/span・フェード形状は実績バーを描かない）のときだけ true。単体：`tests/cr013-separate-row-growth.test.ts` |
| PLAN-L1-005（描画/当たり判定） | Part 2 | 実績開始のみ（`actualEnd` 無し）の実績が**画面座標で最小幅**を持ち、どのズームでも掴めること | done：`MIN_ACTUAL_BAR_WIDTH_PX = 12`／`actualBarRenderWidthPx()` を新設し、`computePlanActualBars` の実績幅と `actualSideLaneRect()`（`actual-only` の単独グリフ）へ適用。内容グループは平行移動のみでスケールしないため world px == screen px＝ズーム非依存。`HitTester.itemGrabRects` が描画と同一の矩形（`computeItemDisplayedBars`）で当たり判定を作り、`separate` の下段実績バーも掴める（リサイズ辺は予定バー側のみ＝実績を掴んでも予定日付を誤って伸ばさない） |
| PLAN-L2-001（4象限） | Part 2 | 実績追加は `actualStart` のみを立て、既定値は**予定開始日**（ピクセルから日数を逆算しない） | done：`progress-line-builder.defaultActualStartDate(item)`（`actualStart ?? startDate`）を新設。プロパティパネルの `actual_end` 入力が `actualStart` 未設定のアイテムに対し同既定値を同時に適用（`dispatchPatch` にアイテム単位の補完フックを追加）。**単発の「実績を追加」アフォーダンス（パレット/ボタン）は未実装**：配置先が `main.ts`／`tool-palette.ts` で本作業の対象外のため、現状の投入経路はプロパティパネルの `actual_start`／`actual_end` |
| ARCH-C-021（Fit/描画更新） | Part 1 | 行が伸びる状態でも Fit と再レイアウトが追随すること | done：`svg-renderer.setViewState` が `planActualStyle`／`planActualDisplay` の変化でも `layoutDirty` を立てる（従来はズーム変化のみ）。左ペインは `onViewStateChange` 購読で同じ `computeRowGeometry` を再計算し整合。`viewport.measureItemsFitExtent`／`measureRenderedContentBottomPx`／`computeFitViewForItems` に任意引数 `planActual` を追加（省略時は従来どおり `overlap`）。**`main.ts` の Fit 呼び出しへ実際の view state を渡す結線は未実施**（対象外ファイル） |

**CR-013 ゲート**: tsc 0 err / vitest 801 pass・0 fail（77 files）/ eslint 0 err（`src tests`）。
新設 `tests/cr013-separate-row-growth.test.ts`（18 ケース：行の伸長・バー高不変・実績なし行の不変・
フィルタ別の非確保・`overlap` 回帰・zoomY 比例・レーン単位の伸長・下段レーンとの非干渉、
最小幅の 3 ズーム不変・実長スパンのズーム比例・当たり判定）。既存テストの更新は
`tests/plan-actual-geometry.test.ts`（旧「半分になる」期待）と
`tests/plan-actual-display-gating.test.ts`（同上）の2ファイルのみ。

## CR-014 (パレットのアイコン体系・配置の刷新: 線画化・並び順・比率) 実装トレース

CR-014 の Part 1（絵文字/文字グリフ → キャンバスと同一ビルダー由来の線画 SVG）・
Part 2（`LINE` グループラベル削除）・Part 3（TASK 左／MILESTONE 右）・
Part 4（白銀比によるボタン/グリフ/間隔の比率設計）を1パスで実装。スキーマ変更なし・
アクセシブルネーム／`data-role` は不変（見た目のみの変更）。

| 要求/契約 | Part | 内容 | 実装状態 |
|---|---|---|---|
| TOOL-L1-001 / ITEM-L1-001 | Part 1 | パレットの形状ボタンは**キャンバスと同一のパスビルダー**が生成する縮小 SVG を表示し、形状定義を二重化しない（SSOT） | done：新設 `adapters/ui/palette-icon.ts` が `task-glyph.taskGlyphPath`（arrow/chevron/span）・`fade-geometry.fadeTrapezoidPoints`（bar＝フェード 0 の矩形）・`item-geometry.milestoneShapePath`（マイルストーン12種すべて）を呼ぶ。`item-geometry` は `milestonePath(item,…)`／`milestoneShapeUsesEvenOdd(item)` を形状キー版 `milestoneShapePath(shape,…)`／`milestoneShapeKindUsesEvenOdd(shape)` へ委譲する形にリファクタ（既存シグネチャ不変）。`tool-palette.makeShapeGroup` が `createTaskShapeIcon`／`createMilestoneShapeIcon` でボタンを描画 |
| ITEM-L1-001（特殊7種） | Part 1 | CR-004 の 7 種特殊マイルストーン（file/box3d/floppy/cylinder/person/smiley/beer）を絵文字から線画へ | done：`SPECIAL_MILESTONE_CHOICES` は形状キーのみの配列となり、絵文字 `📄📦💾🛢👤🙂🍺` を全廃。塗りは `fill: none` ＋ `stroke: currentColor`（テーマ追従）、`fill-rule` はキャンバスと同じ `milestoneShapeKindUsesEvenOdd` 由来 |
| NFR-L1-005 | Part 1 | キャンバスに対応形状が無いコマンドも同一の視覚言語（線画）で統一 | done：`PALETTE_COMMAND_ICON_PATHS`（20×20 設計グリッド）に progress-line（イナズマ＝折れ線ジグザグ。`⚡` 廃止）／comment（`💬` 廃止）／add-box（`▢` 廃止）／watermark（`©` 廃止）／cursor-guide 4モード（`⃠✛│‖` 廃止）／grid-date・grid-category（`☰\|`・`≡` 廃止）を新規作図。`main.makeCommandIconButton` が `data-role`・`aria-label`・`title` を保ったまま差し替え |
| TOOL-L1-001 | Part 2 | 進捗線ボタン左の `LINE` グループラベルを削除（ボタンとアクセシブルネームは維持） | done：`main.buildChrome` の `makeCommandGroup('Line', …)` → `makeCommandGroup('', …)`（DOM のグループ枠は維持、キャプションのみ撤去） |
| TOOL-L1-001 | Part 3 | シェイプピッカーの並びを TASK 左／MILESTONE 右へ | done：`tool-palette.mountShapePicker` の `shapesRow.append(taskGroup, milestoneGroup)`。順序検証用に `data-role=task-shapes`／`milestone-shapes` を新設（既存 role の改名なし） |
| TOOL-L1-001 / NFR-L1-005 | Part 4 | アイコンボックス・グリフ・間隔の寸法を**白銀比（1:√2）**の等比ラダーで設計 | done：`palette-icon` に `SILVER_RATIO = Math.SQRT2` と `paletteSilverRungPx(n) = 20 / √2ⁿ` を定義。n=0 ボタン20px／n=1 グリフ14.14px／n=3 グループ間7.07px／n=4 グリフ左右インセット計5px／n=5 ボタン間3.54px。ボタン padding は残余リング `(n0−n1)/2`。`paletteProportionCss()` が定数から CSS を生成し `main.ensureCommandChromeStylesheet` が連結（グレー角丸のボタン外装は据置＝参考画像の黒枠は作図上の副産物として不採用） |
| ARCH-C-021（Fit） | — | CR-013 の残件：Fit 呼び出しへ実 view state を渡す結線 | done：呼び出し元は `main.ts` ではなく `adapters/render/svg-renderer.fitToContent` にあり、`computeFitViewForItems(…, estimateInnerLeftLabelExtentPx, this.viewState)` として `planActualStyle`／`planActualDisplay` を伝搬。`separate` で伸びた行を Fit が正しく収める |

**CR-014 ゲート**: tsc 0 err / vitest 816 pass・0 fail（78 files）/ eslint 0 err（`src tests`）。
新設 `tests/cr014-palette-icons.test.ts`（15 ケース：task 3形状のパスが `taskGlyphPath` と一致／
bar が `fadeTrapezoidPoints` 由来／マイルストーン12種のパスが `milestonePath` と一致／特殊7種の
even-odd 線画／span の塗り／絵文字・文字グリフ全廃と 16 ボタンの空ラベル／bespoke アイコンの
パスデータ ASCII 限定とジグザグ頂点数／TASK 群が MILESTONE 群より先（グループ順・ボタン順）／
`LINE` キャプション不在／白銀比ラダーの整合と CSS 反映／SVG の viewBox・装飾属性／
アクセシブルネームと `data-role` の回帰）。既存テストの更新は不要（0 ファイル）。

## DEF-011 (SVG エクスポートが実績バーを描かない) 実装トレース

CR-013 で `separate` の行高拡張がエクスポートにも波及した一方、エクスポータは実績バーを
描いていなかった（行だけ伸びて中身が空）。画面とエクスポートで「描くべき矩形」の判定を
共有させて解消。

| 要求/契約 | 内容 | 実装状態 |
|---|---|---|
| PLAN-L1-005（CR-013改訂） / ARCH-C-022 / IO-L1-003 | SVG 出力が画面と同じ予実描画規則に従うこと（overlap=重ね／separate=伸びた行内に上下並置） | done：新設 `domain/usecase/plan-actual-paint.ts` の `computeItemPlanActualPaint()` が「主グリフ矩形／主グリフの予実側／第2の実績バー矩形／マイルストーン実績マーカーの中心x」を返す純粋関数。既存の `computeItemDisplayedBars`・`actualSideLaneRect`・`drawsActualBar` を**合成**するのみでモード判定を二重定義しない。`svg-exporter` はこれを消費し、実績バー（`data-role="actual-bar"`／`data-plan-actual-side="actual"`）を出力 |
| PLAN-L1-002 / DEF-008 | `plan-only` / `actual-only` の表示フィルタをエクスポートでも尊重すること | done：`svg-exporter` がアイテム単位で `filterByPlanActualDisplay` を適用（`actual-only` では実績を持たないアイテムを出力しない）。片側のみのときは単独グリフが実績側に移動し `data-plan-actual-side="actual"` を帯びる（画面と同一） |
| CR-002 Part 1 | 淡い予定／鮮やかな実績の配色と、細線=予定／太線=実績の非色冗長化 | done：`displayFillColor` / `actualDisplayFillColor` と `planActualStrokeWidthPx` を画面と共有。予実が共存する場合のみ予定バーの輪郭を実績色＋細線に切替（実績なしバーは従来どおり `strokeColor` ＋ 1px＝回帰なし） |
| CR-002 Part 2 | マイルストーンは予定・実績の2マーカー＋リーダー線 | done：`milestoneActualCenterX` を用いてリーダー線→予定マーカー→実績マーカーの順に出力（`data-role="milestone-actual-marker"` / `milestone-plan-actual-leader"`） |
| DEF-009（ラベル重ね順） | 略称テキストが全バーより前面に描かれること | done：アイテムグループ内の出力順を「グリフ→実績バー→`<text>`」に固定（SVG は後勝ち＝テキストが最前面） |
| CR-013 Part 2 | 実績バーの 12px 最小幅フロア | done：共有の `actualBarRenderWidthPx`（`computeItemDisplayedBars` / `actualSideLaneRect` 経由）をそのまま利用。将来 `plan-actual-geometry` 側にクランプが追加されても自動追従 |
| ARCH-C-022（残作業） | 画面側も同ヘルパを消費して単一定義にする | not done（意図的）：`adapters/render/layers/item-layer.ts` は別エージェントが編集中のため未着手。推奨手順は DEF-011 記録の「残作業」節に記載 |

**DEF-011 ゲート**: tsc 0 err / vitest 868 pass・0 fail（80 files）/ eslint 0 err（`src tests`）。
新設 `tests/def011-svg-export-plan-actual.test.ts`（16 ケース：overlap の重ね／separate の
下段配置と伸びた行帯への収まり／plan-only の実績非出力／actual-only の単独出力とアイテム除外／
none の全非出力／実績なし行のバイト同一（回帰）／テキストが全 rect の後／配色と線幅／
3 ズームでの最小幅／マイルストーン実績マーカー、および実物の `ItemLayer` を fake DOM で
描画して**画面の矩形と出力の矩形が一致する**ことを style×display の 6 組合せで比較）。
既存テストの更新は 0 ファイル。
