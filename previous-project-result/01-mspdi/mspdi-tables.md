...
「断捨離」列は日程表用途でのスリム化における要否。**残す 8 / 削る 21**（全数は下記「構成の要点」）。

> ⚠️ **この列の判断根拠は `previous-project-result/` に無い。** 出所は前プロジェクトの中間分析 `mspdi-declutter-erd-ja.md`（725 行）で、
...
> **2026-08-04 に「採否」列を削除した。** ○/× だけを書く列で、**29 行すべてで「断捨離」列と一致していた**
> （機械で確認）。**同じ判断を 2 列で書いていた**ので、理由を併記できる「断捨離」列に一本化した。
> あわせて、その列の凡例だけが定義していた記号 `△`（**表に 0 件**）も消えた。

※ WorkingTime（勤務時刻）と Task_Baseline は不採用に確定（日粒度描画で時刻不要 / インラインの計画スナップショットは日程表コア外）。

| # | テーブル | 種 | 責務（一言） | カード | 断捨離 |
|---|---|:--:|---|---|:--:|
