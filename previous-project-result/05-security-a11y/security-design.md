...
## 引継ぎ時の注記（2026-07-26 追記）— **本文より先に読む**

**1. §3.2（SVG Import サニタイズ）と §3.5（PNG Import）は、対象機能が取り下げられた。**

`user-order.md` 項 14「アイコンのインポート（SVG / PNG）」は**取り下げ済み**（欠番）。
したがって **SVG / PNG の取込サニタイズは実装不要**であり、**攻撃面はその分減っている**。
検証すべき外部入力は **JSON と MSPDI XML の 2 つだけ**（項 62）。

> §3.2 / §3.5 は「もし将来 SVG/PNG 取込を復活させるなら何が必要か」の記録として残す。
> **今の要求には含まれない。**

**取り下げの影響を受ける節の全数**（**§3.2 と §3.5 には 2026-08-05 に節の冒頭へ 🚫 を付けた。
残りは本文に印が無いので、ここで一覧にする**）:

| 節 | 何が死んでいるか |
|---|---|
| 冒頭の要レビュー事項 | 「**SVG サニタイズ実装**」の実機検証 |
| §1.1 の表 | Import 対象に **SVG / PNG** が載っている |
| §2.2 信頼境界 | 「信頼しない」に **SVG / PNG** が載っている。脅威図の `F_SVG` / `F_PNG` ノードと辺 |
| §2.3 STRIDE | Tampering の「悪意ある **SVG**」と参照先「§3.1-**3.5**」 |
...
| §9 | ST-01〜04 / ST-11〜13 |
| §11 | **申し送り 1（DOMPurify 採用可否）は §3.2 のためだけの検討なので死んでいる。申し送り 4 のうち「SVG サニタイズの実機確認」も死ぬ**（**CSP の実機確認は生きている**） |
| §11 | DOMPurify の採用可否 |
| 付録 | A06 / A10 の §3.2 参照 |

**生きているのは §3.1（XSS）/ §3.3（JSON スキーマ・プロトタイプ汚染）/ §3.4（XXE・DOCTYPE 拒否）/
§3.6 の JSON・XML の行だけ / §4（CSP）/ §5（localStorage）/ §6（透かし）である。**

> ⚠️ **`img-src data:` は生きている。** §4 はこの指令の根拠を §3.5（PNG Import）に置いているが、
> **PNG Import が死んでも指令は要る** — **PNG 出力**（`user-order.md` 項 58）が
> SVG を `data:` URI として `<img>` に読み込み canvas へ描く経路を使うためである。
> `default-src 'none'` の下では `img-src` の許可が無いとこの読込が落ちる。
>
> **scheme は `data:` で決着 — 2026-08-02（ユーザー判断）。** §4 の推奨 CSP をそのまま使う。
>
> | | `data:`（採用） | `blob:`（不採用） |
> |---|---|---|
> | 実体 | URL 文字列に埋め込む | blob ストア。URL は短い取っ手 |
> | 大きさ | `encodeURIComponent` で約 2〜3 倍 | 膨らまない |
> | 解放 | **不要** | `URL.revokeObjectURL` が**必須**。漏れると文書の寿命まで残る |
> | `file://` | 生成元に依存しない | 生成元が不透明になる（**未検証**） |
>
> **採った理由**: ①**解放漏れという失敗の型が存在しない**。日程表は「直して出す」を繰り返すので、
> `blob:` の解放漏れは使うほど積み上がり、症状が出るのが遅い。②**`file://` で開くことが要件**であり、
> `data:` は生成元に依存しない。③**現行の推奨 CSP から変更ゼロ。**
>
> ⚠️ **符号化は `encodeURIComponent` を使う。`btoa` を使ってはならない** — `btoa` は Latin-1 しか
> 受け付けないので、**タスク名に日本語が入ると例外で落ちる**。
>
> **判定基準（骨格段階で実測する）**: `file://` で **50 行 / 1000 アイテム**の SVG を直列化し、
> `data:` URI で `<img>` が読めること。SVG 文字列は**約 300KB と見積もっている（未計測）**ので
> 上限に当たる見込みは薄いが、落ちたら `blob:` へ移す。**移す費用は CSP 1 行と符号化の関数だけである。**

**取り下げの影響を受けるのは SVG / PNG だけではない。**

| 廃止済みの概念 | 本文に残っている場所 |
|---|---|
| **略称**（`abbrev`。`user-order.md` 項 20 で廃止） | AS-4 / §3.1 の例 / C-02 / ST-09 |
| **絵文字**（`user-order.md` 項 11 は欠番） | §3.1 の絵文字アイコン処理 |

**2. 本文中の要求 UID（`IO-L1-006` / `ITEM-L2-001` / `NFR-L1-001` 等）は、前プロジェクトの仕様書の番号である。**

**その仕様書は `previous-project-result/` に入っていない**（`DISCARDED-ja.md` §3 で「参考」に分類）。
**各コントロールの内容は自己完結している**ので、**UID は無視して読んでよい**。
次期は自分の要求 UID を振り直し、§8 のトレーサビリティ表を作り替えること。

> ⚠️ **本文の `item59` / `item60` / `item61` も旧番号である**（計 9 箇所）。**現行の `user-order.md` の項番号とは一致しない。**
> 特に危険なのが `item60` — 本書では**共同編集（将来フェーズ）**を指すが、
> **現行の項 60 は「localStorage への自動保存とクラッシュ復旧」**である。共同編集は**現行の項 65-1**。
> **`itemNN` は読み替えず、前後の文が何を指しているかで判断すること。**

**3. 生きている要求は次のとおり。**

```
XSS 防止（描画は安全 API のみ。innerHTML 直挿し禁止）        §3.1
JSON Import のスキーマ検証・プロトタイプ汚染防御              §3.3
MSPDI XML の XXE 対策（外部エンティティ無効化）               §3.4
リソース上限（巨大 import による DoS 対策）                   §3.6
単一 HTML ビルドの CSP                                       §4
localStorage の取り扱い                                      §5
透かしのセキュリティ / プライバシ注記                         §6
```
...
### 2.2-1 機械向けインターフェース (2026-08-04 追記)
...
> **API 経由の変更も、UI と同じ検証を通る** (`../10-agent-interface/agent-interface-requirements-ja.md` A-7)。
> **API 専用の抜け道を作らない。** 迂回してはならないものの全数は同書 §3。
>
> ⚠️ **文書の埋め込みは実質 HTML のテンプレート処理である。**
> 埋め込む文字列中の `<` を JSON の Unicode エスケープ `\u003c` へ置き換えないと、
> タスク名に含まれる `</script>` が
> **早期にタグを閉じ、残りが HTML として解釈されて本文に出る** (対照実験で実測)。
> **§3.1 の XSS 防止と同じ土俵に載る**（`../10-agent-interface/agent-interface-spec-ja.md` §5-1）。
...
> 🚫 **この節は今の要求に含まれない（2026-08-05 に印を付けた）。**
> **SVG / PNG の取込は取り下げられた**（冒頭の注記）。本節は「**もし将来復活させるなら何が要るか**」の
> 記録として残している。**`§8` の C-03 / C-04 から来た場合も、実装対象ではない。**
...
> 🚫 **この節は今の要求に含まれない（2026-08-05 に印を付けた）。**
> **SVG / PNG の取込は取り下げられた**（冒頭の注記）。§3.2 と同じく**復活させる場合の記録**である。
...
- **`default-src 'none'` / `object-src 'none'` / `base-uri 'none'`** で余計な読込・
  base タグ乗っ取りを封じる。
- **`img-src data:`** は **PNG 出力**（`user-order.md` 項 58）で SVG を画像として読み込み
  canvas へ描くために必要。~~(§3.5 の PNG Import)~~ **取込は取り下げ済みだが出力は生きている**。
  **scheme は `data:` で決着**（**確定 2026-08-02**。比較と判定基準は本書冒頭の注記）。SVG アイコンは
  `<img>` ではなくインライン SVG ノードとして描画するため、`data:` を SVG 実行経路にしない。
  **符号化は `encodeURIComponent` を使う。`btoa` は日本語で落ちる。**
...
> ⚠️ **`file://` では上の「同一オリジン JS」が「そのマシンの全ローカルファイル」を意味する**
> (2026-08-04 追記。実測 ＝ `../10-agent-interface/agent-interface-open-items-ja.md` 決着-8)。
> **localStorage も IndexedDB も、フォルダが違っても同じ保管庫を共有する。**
> したがって**任意のローカル HTML ページが GRS の自動保存を読める。**
> ブラウザを完全に落として再起動しても残る。
>
> **帰結が 2 つある。**
> ① **自動保存のキーは文書ごとに分ける** (`grsched.autosave.<documentId>`)。分けないと
> 素の GRS・埋込文書つき GRS・古い複製が**同じ鍵を奪い合う**。
> ② **`file://` で保持したファイルハンドルも共有される**ため、
...
> `../04-performance/performance-notes-ja.md` §2-0）。**数字の大小で新旧を判断しないこと。**
