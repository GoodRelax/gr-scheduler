...
title: Agent Interface 仕様
description: GRS Agent API の契約。関数・エンベロープ・コマンド・拒否理由・監視・起動時投入。
...
- 日付: 2026-08-02 ／ **採用 2026-08-04**
- 対応する要求: `agent-interface-requirements-ja.md`
- ⚠️ **`authority` は持たない。** 用語の正は 4 文書のままである（`../README.md` §`authority`）。
- **記法は `../03-ui-naming/ui-parts-ja.md` §1-2 に従う**
...
| **公開点** | **`globalThis.grSchedulerAgentApi`** | **既定では存在しない。** 有効化されたときだけ現れる（`agent-interface-open-items-ja.md` 決定-4） |
...
**ユースケースは 1 つ、経路が 3 つである。** 名前は動詞句
（レビュー観点規約 **R2.1** — 関数は動詞 ＋ 目的語、真偽値は `is` / `has` / `can` / `should` で始める）。
...
> （語彙の重複は前プロジェクトのバグ根因＝ `../README.md` §0-1）。
...
**同時更新まわりの語**（`agent-interface-open-items-ja.md` 決定-6）:

| 英語 | 日本語 | 中身 |
|---|---|---|
| **`ConcurrentUpdate`** | **同時更新** | 2 者以上が同じ文書をほぼ同時に変えようとする現象 |
| **`BaseRevisionCheck`** | **基準版の照合** | 書き込み時に「読んだ版から動いていないか」を確かめ、動いていれば拒否する。**現行の方針** |
| **`AutomaticReconciliation`** | **自動調停** | 拒否せず両方を取り込む将来の選択肢（`agent-interface-open-items-ja.md` O-4。**延期中**） |
| **`ImportMerge`** | **取込マージ** | 取り込んだファイルを日程表へ合流させる（`user-order.md` 61 の 3 択）。**取込専用** |

> **`BaseRevisionCheck` は、業界では optimistic locking（楽観ロック）と呼ばれる方式である。**
> その名を採らないのは 2 つの理由による。① **何もロックしていない**（名が体を表さない）
> ② 「楽観」は**衝突がめったに起きないという見込み**を指すが、**実測では 1 セッションに 6 回発火した**（`A-3`）。
> **既存の識別子と語幹も一致する**（`baseRevision` / `stale-base-revision` / `expectedRevision`）。

> **使わない語**: **`Conflict`**（直訳が「競合」「衝突」になり、取込の語と重なる）／
> **`Collision`**（`grs-native-erd-ja.md` §5.3 が「UID 衝突」で使用中）／
> **`Resolution`**（「解像度」と重なる）／**裸の `Merge`**（広すぎる）／**裸の `Lock`**（ロックしていない）。

> `grs` のような略語を識別子にしない（`ui-parts-ja.md` §1-2「略語は識別子に入れない」）。
> **`Api` は残した** — `Lod` と違い camelCase に落ちても語として読めるため（同節が禁じているのは
> 「落ちた瞬間に意味が消える」略語である。同書は `UI` を語として使っている）。
> **ただし最終判断は命名の正（`ui-parts-ja.md`）が持つ。**
> **散文では GRS と書いてよい。**
...
**命名の品詞は純粋性で決まる。** 関数は動詞 ＋ 目的語にする（レビュー観点規約 **R2.1**・MUST）。
...
> **`readDocument()` を `document()` に改めてはならない**（**R2.1**）。
...
**既存の構造（`../02-data-model/grs-native-erd-ja.md`）は変えない。**
トップレベルに 2 つ足すだけである。
...
**保存する**（`agent-interface-open-items-ja.md` 決定-3）。`file://` ではファイルが往復の運び手になるので
...
> （`../02-data-model/grs-document-settings-ja.md` §2）、**`revisionStamp` は絵に影響しない**。
...
**2 つを分ける**（`agent-interface-open-items-ja.md` 決定-2）。
...
- **`note` / `notes` という語を使わない。** `Task.notes` が既にあり、
  **語彙の重複は前プロジェクトのバグ根因**である（`../README.md` §0-1）。
...
> `../02-data-model/grs-native-erd-ja.md`（データ構造の正）の管轄である。
> **この文書の側から正へ書き足さない。** 正へ入れる作業は**残作業として立ててある**
> （`../NEXT-STEPS-ja.md` **4-2**）。**`revisionStamp` に入れない** — 識別子は「版」ではないので、
...
- コマンド名は kebab-case の文字列判別値（`ui-parts-ja.md` §1-2）。
- 引数のプロパティ名は **`grs-native-erd-ja.md` の属性名と語幹を一致させる**。新語を作らない。
- **`update-*` は部分更新である — 確定 2026-08-06。**

  ```text
  キーが無い        その列を触らない
  キーがあり null   その列に null を代入する（＝既定へ戻す）
  キーがあり値      その値を代入する
  ```

  **`null` を受け付けない列に `null` を送ったら `invalid-argument` で拒否する**（§3 の `RejectionReason`。
  **新しい拒否理由は足さない**）。要求 A-9「例外を投げず、結果を値で返す」の範囲内である。
  **規則は既に上の実例が使っている** — `update-task` は `start` / `finish` だけを送り（＝書かなかった列は触らない）、
  `assign-task-to-group` は `"stackOrder": null` を明示的に送っている（＝`null` は値である）。**名前が無かっただけである。**
  これで**色を既定へ戻す口も足りる**（`../02-data-model/grs-native-erd-ja.md` §5.6 の疎な上書き）。
  **専用の `clear-*` コマンドや `reset: [...]` 配列は作らない** — 同じことを表す道が 2 つになり
  （`../user-order.md` 項 66「1 概念 1 語」）、列が増えるたびに増殖する。
  JSON Merge Patch の既知の落とし穴（「`null` を入れる」と「キーを消す」が区別できない）は、
  **GRS のコマンドにキー削除という操作が無い**ので起きない。
- **一覧はこれで確定ではない。** 実開発で UI の操作割当
  （`../03-ui-naming/ui-detail-spec-ja.md` §5）と**1 対 1**に整える。
...
| 2 | 文字列中の `<` を `\u003c` へ置き換える | **対照実験で確認済み。** 省くと `</script>` がタグを早期に閉じ、**JSON は解析不能になり、残りが HTML として本文に出る**（`agent-interface-open-items-ja.md` 決着-5） |
...
| 5 | **入れ口が運ぶのは文書だけ。** API の有効化を入れ口にも文書にも書かない | **権限をファイルで運ばない**（`agent-interface-open-items-ja.md` 決定-4）。転送された先では非公開のまま開く |
| 6 | **原本（ビルド成果物）は読み取り専用として扱う。** 出力先が原本と同じパスになりうる規則を作らない | **原本が壊れるとツールそのものが失われる。** 複製は何度でも作り直せるが、原本は 1 つしかない |
| 7 | **入れ口の既定値は `null`。初期テンプレートを入れ口に入れない** | **入れ口が空かどうかが「誰かが意図して文書を入れた」の唯一の判定材料**であり、起動順序の 4 分岐（§5-3）がこれに乗っている。下の警告を読むこと |

> ⚠️ **規約 7 を「似たものが 2 つあるから」と統合してはならない。**
> 初期テンプレートを入れ口に常駐させると、**普通に開くたびに「埋込文書あり」の状態**になる。
> 表示されるのは工場出荷のサンプル（別の `documentId`）なので、利用者の自動保存は
> **「別の文書のもの」と判定されて確認が出ず、未保存の作業がサンプルに追い越される。**
> **＝ クラッシュ復旧が壊れる**（`user-order.md` 60）。
...
**公開点は既定では存在しない**（`agent-interface-open-items-ja.md` 決定-4）。現れるのは次の 2 つの場合だけである。
...
| # | 規約 |
|:--:|---|
| 1 | **`file://` では機能しない（実測）。** パラメータは読めるが、**取得が拒否される** — 兄弟ファイルへの `fetch` は `TypeError: Failed to fetch`、`XMLHttpRequest` は network error（`agent-interface-open-items-ja.md` 決着-2）。**UI で理由を通知する** |
...
（`agent-interface-open-items-ja.md` 決着-1。Edge 151 で実測）。
...
**実測で確かめた挙動**（Edge 151 / `file://`。全数は `agent-interface-open-items-ja.md` 決着-1）:
...
### 5-3. 起動順序（`agent-interface-open-items-ja.md` 決定-5）
...
> （`../NEXT-STEPS-ja.md` 2-6）。**入れ口に置いてはならない**（§5-1 規約 7）。
> **エージェントが書き込むのは空の入れ口であって、初期テンプレートの場所ではない。**
...
### 6-1. 脅威モデルへ足した 1 段落（`security-design.md` §2）

**決定-4 により、次の 1 段落を `../05-security-a11y/security-design.md` の脅威モデルへ足した（2026-08-04）。**
**正はあちらである。以下は出典としての再掲であって、二重管理ではない。**
...
> 多人数編集（`user-order.md` 65-1）では、`A-3` の基準版の照合と `A-10` の revision スコープ監視が
> **そのまま土台になる**。2 者で成立した規約を人数だけ増やす形である。
> **ただし同時更新の方針（基準版の照合のみ／自動調停）は未決**（`agent-interface-open-items-ja.md` O-4。**語彙は決定-6 で確定**）。
