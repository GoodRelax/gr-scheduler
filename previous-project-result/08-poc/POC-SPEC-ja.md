...
description: 案比較 PoC の仕様。製品仕様ではない。食い違ったら本資産側が正。
tags: [poc]
...
- 位置づけ: **使い捨ての実験**。`previous-project-result/` の確定内容を変えるものではない。
  PoC で分かった事実だけを後で `previous-project-result/` に書き戻す。

> ⚠️ **本書は案比較 PoC（レイアウト 3 案 / ズーム 2 案）の仕様であり、製品仕様ではない。**（2026-08-01 追記）
>
> 書かれた時点（2026-07-30）以降に `previous-project-result/` 側で変わった規則がある。本書は
> **凍結した計測記録**なので追随させない。食い違ったら **`previous-project-result/` 側が正**である。
>
> | 本書の記述 | 現在の正 |
> |---|---|
> | 進捗マーカーは**固定ピクセルサイズ**（§3-5） | 文字サイズから導く（`../07-plan-actual/plan-actual-decisions-ja.md` §2-4-1） |
...
> | 横ズーム `Ctrl` / 縦ズーム `Shift` / スライダー（§6） | `Ctrl`＝両軸 / `Shift`＝横 / `Alt`＝縦。スライダーは置かない（`../03-ui-naming/ui-detail-spec-ja.md` §5-2） |
>
> **統合 PoC（`poc-integrated.html`）は本書ではなく `previous-project-result/` の確定内容に従う。**
...
// NOTE (2026-08-04): dead code in the PoC -- declared but never read, and it lists
// only 4 of the 5 shapes (milestone is missing). Do not copy it into the successor.
...
4. マイルストーンは最上段  ← ⚠️ **規則 3 と両立しない**（下記）
5. 機能上の上限は設けない。安全弁だけ持ち、到達したら画面に知らせる
   ※「最終段を再利用する」（重なりを許して押し込む）は廃止。絶対にやらない
6. 行の帯高は段数で決まる。行高固定を前提にしてはならない
```

> ⚠️ **規則 4 は規則 3 と両立しない**（2026-08-04 に記録。**PoC は直していない**）。
> **同じ日にマイルストーンが 2 本以上あると占有区間が重なる**ので、規則 3（貪欲・最も浅い段）は
> それらを別の段へ落とす。規則 4（最上段）はそれを許さない。**両方は同時に守れない。**
> **このケースは PoC の入力に実在する**（同日 5 本）。規則 2 の `milestone` 優先で
> 「先に置かれる＝浅い段に来やすい」だけが実際の挙動であり、**最上段は保証されていない**。
>
> **確定版はこの衝突を規則 4 の削除で解いている** —
> `../03-ui-naming/ui-detail-spec-ja.md` §6-1 が
> 「かつて `milestone` 優先があったが、**行を広く占めるものの固定が 1 つあれば全マイルストーンが押し上がる**」
> として退けている。**次期は規則 4 を持ち込まないこと。**
...
> 製品の正は `../03-ui-naming/ui-detail-spec-ja.md` §5-2 で、
...
生成器を単体で走らせて確かめた結果、**入力データそのものが Progress Marker の 4 記号のうち 3 つを出せない**ことが分かった
（記号は `( )` 未完了 / `(✓)` 完了 / `( \ )` 中断 / `(!)` 遅れ の **4 つ**。
`../07-plan-actual/plan-actual-decisions-ja.md` §2-4。**旧版は「4 状態を出せない」と書いていた** — 2026-08-04 訂正）。
