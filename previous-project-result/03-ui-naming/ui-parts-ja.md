...
- 素材: 前プロジェクトの用語集・GUI 木（**どちらも旧名のため `previous-project-result/` には無い**。`../DISCARDED-ja.md` §3）/ `../02-data-model/grs-native-erd-ja.md`（データモデル）
- 位置づけ: **UI パーツ名の確定版**。データ構造は `../02-data-model/grs-native-erd-ja.md` が正。
  **日英対応表は §2-1 が正**（全数をそこに置く。他の文書に同じ表を作らない）。
  予実・進捗まわりの設計は `../07-plan-actual/plan-actual-decisions-ja.md` が正。
...
（`../07-plan-actual/plan-actual-decisions-ja.md` §5-3）。
...
Bar   1 つの Task が占める横帯 1 本と、そこに描かれるもの全体を指す総称。
      ...
      根拠 2  マイルストーンも 1 タスクぶんの横帯を 1 本占有する
              （`../07-plan-actual/plan-actual-decisions-ja.md` §2-2）。
...
> **確認できる事実**: 「矢羽根」は `previous-project-result/` 内に **33 箇所**あり `user-order.md` を含む
...
| 深さ | **WBS** か **`TaskGroup`** かを必ず併記（`深さ` と単独で書かない）（`../07-plan-actual/plan-actual-decisions-ja.md` §5-3） |
...
> 目盛は 4 段階なので、しきい値は**その境目の数だけ 3 本**になる（`ui-detail-spec-ja.md` §6-3）。
...
前プロジェクトの GUI 木に、**抜けていた 4 件**（依存線・グリッド線・透かし・モーダル）と画面領域の定義を統合したもの。
**名前についてはこれが正**（他所で違う名前を見たらこちらが勝つ）。
⚠️ **全数ではない** — 2026-08-04 に決まった 4 つがまだ入っていない（木の直後の表）。
...
│   │       │                  07-plan-actual/plan-actual-decisions-ja.md §2-4
...
│   ├─ Canvas Overlays         Items から分離した重ね描き層（前の木では Items の中にあった）
...
> **`‼️`** = 前プロジェクトの GUI 木に**無かった**もの。**4 件**（`Grid Lines` / `Dependency Lines` / `Watermark` / `Modals`）で、
> 上の木に付いている `‼️` もちょうど 4 個である。核機能（依存線）まで抜けていたので、**木は必ず全数で持つ**こと。

> ⚠️ **この木は 2026-08-04 の時点で全数ではない。** 同日までに決まった次の 4 つがまだ入っていない。
> **実装前に木へ足すこと**（`../MILESTONES-ja.md` M0 の視覚モックで確定させる）。
>
> | 未収録 | 決まった場所 |
> |---|---|
> | **チャット欄**（画面上で AI と話す。会話は保存しない） | `../10-agent-interface/agent-interface-open-items-ja.md` 決定-2 |
> | **有効化の面**（Agent API は既定で非公開。人が明示的に有効化する） | 同 決定-4 |
> | **起動時の面**（保留中の用件を 1 枚に集約） | 同 決定-5 |
> | **`Toast`**（通知。表示位置・滞留時間・多重時の積み方が未定） | `../NEXT-STEPS-ja.md` 2-5 |
...
| ~~`progressStatus`~~ | **廃止** | 状態が構造化されて不要になった（`../07-plan-actual/plan-actual-decisions-ja.md` §9-5） |
...
| ~~`progressStatus`~~ | 進捗の自由文字列 | **廃止**（`../07-plan-actual/plan-actual-decisions-ja.md` §9-5） |
| `taskShape` / `milestoneShape` | タスク形状 | **`shapeKind` の 1 列に統合**（2 列に分けない）。※マイルストーンの**形の詳細**だけは `milestoneGlyph` に分ける（2026-07-30・`../07-plan-actual/plan-actual-decisions-ja.md` §2-2-2） |
...
| 区別 | 内容 |
|---|---|
| **日付 vs 掴み点** | 「開始日/終了日」＝データ、「開始点/終了点」＝画面で掴む場所。**混用禁止**。掴み領域の全数は `ui-detail-spec-ja.md` §4-1 |
...
根拠と却下案は `../07-plan-actual/plan-actual-decisions-ja.md` §2-2 / §2-3 が正。
掴み点・ラベル書式の詳細は **`ui-detail-spec-ja.md` §4**、
実績の入力規則（5 状態）は **`../07-plan-actual/plan-actual-decisions-ja.md` §1-3 が正**
（`ui-detail-spec-ja.md` §4-0 と `../02-data-model/property-mspdi-mapping-ja.md` §3-2 はそこを参照する）。
...
2. **用語集は次期が自分で作る。前プロジェクトの用語集は引き継がない**（`DISCARDED-ja.md` §3）。
   ...
   **用語の正は 4 つだけにする**: 本書（命名）／ `../02-data-model/grs-native-erd-ja.md`（データ構造）／
   `../07-plan-actual/plan-actual-decisions-ja.md`（予実・進捗）／
