...
description: "振る舞いの確定版。名前の正は ui-parts-ja.md、予実の正は 07-plan-actual。"
tags: [ui]
phase: planning
status: stable
---
...
- 位置づけ: **UI パーツ名の正は `ui-parts-ja.md`**、データ構造の正は `../02-data-model/grs-native-erd-ja.md`。
...
**UI パーツ木の正は `ui-parts-ja.md` §3 である。ここには複製しない。**
（同書の木は本節・`../07-plan-actual/plan-actual-decisions-ja.md` §9-6 の両方を含む上位集合であることを確認済み。
...
（`../04-performance/performance-notes-ja.md` E-11）。
...
> 🔴 **この節の正は `../07-plan-actual/plan-actual-decisions-ja.md`。**
> 本節はその要約であり、食い違ったら向こうが勝つ。

### 4-0. 実績の入力規則（5 状態）

**状態と各列の値は `../07-plan-actual/plan-actual-decisions-ja.md` §1-3 の表に従う。ここには複製しない。**
...
**寸法は `../07-plan-actual/plan-actual-decisions-ja.md` §3-2 に従う。ここには複製しない。**
...
**`../07-plan-actual/plan-actual-decisions-ja.md` §3-3 に従う。ここには複製しない。**
...
- **フォントサイズはタスク高の 80%。ただし下限 12px でクランプする。**（和文がこれを割ると読めない。基準は 1920x1080 / 全画面 / 拡大率 100%）
  **下限を割る文字が必要になったら、文字を縮めるのではなくグループ LOD（`TaskGroup`）の深さを 1 つ減らす**（`../07-plan-actual/plan-actual-decisions-ja.md` §2-4-1。**WBS ではない**）

#### 名称ラベルの配置

**`../07-plan-actual/plan-actual-decisions-ja.md` §6-1 の 4 段階に従う。ここには複製しない。**
...
これにより **本節の末尾で規定する「テキストは必ず最前面」（DEF-009 の再発防止）を覆さずに済む。**

> **依存線とラベルが重なることは許容する**（§4-7 / `../07-plan-actual/plan-actual-decisions-ja.md` §6-5）。
...
（`../07-plan-actual/plan-actual-decisions-ja.md` §2-2。確定 2026-08-01）。
...
**正は `../07-plan-actual/plan-actual-decisions-ja.md` §2-4 / §2-4-1 / §2-5 である。**
...
**正は `../07-plan-actual/plan-actual-decisions-ja.md` §7。** 下は所属を見るための要約で、
**不採用の理由（率は導出値であって人が入力する値ではない）はそちらだけに書く。**
...
**4 段階の順序は `../07-plan-actual/plan-actual-decisions-ja.md` §6-4 に従う。ここには複製しない。**
...
`../04-performance/performance-notes-ja.md` §4-1 の 4）。
**「0〜3」と書くと必ず破れる。**
...
したがって**固定の定数は次の 2 つ**である（下表の 3 行目は、その 2 つから導く値であって独立した定数ではない）。
...
- **実績に付けると同じ依存が実績の有無で動く。** 特にマイルストーンの実績は
  実績日の位置へ横にずれる（`../07-plan-actual/plan-actual-decisions-ja.md` §2-2）ので、
  付け先を決めないと絵が定まらない。
...
| **行をまたぐ** | **決着（2026-08-02・ユーザー判断）**。段が離れている場合と**同じ扱い**。横切ってよく、前面に描く（§4-3）。**縦の通り道の予約は要らない** — タスクの x は日付で決まるので避け先が無い場面があり、予約方式は原理的に解けない（`../07-plan-actual/plan-actual-decisions-ja.md` §6-4 の撤回記録） |
...
`actualMin`（`../07-plan-actual/plan-actual-decisions-ja.md` §2-4-1 の 16px）を必ず上回る。
...
**下限に当たったらどうするかは `../07-plan-actual/plan-actual-decisions-ja.md` §2-4-1 のとおり** — さらに縮めるのではなく、
...
6. **積み順（`stackOrder`）に機能上の上限は設けない。** 必要なだけ広げる。
   ...
   根拠は `../07-plan-actual/plan-actual-decisions-ja.md` §6-3、`user-order.md` 項 30-7。
...
> （`../07-plan-actual/plan-actual-decisions-ja.md` §5-2 / §5-3）。
...
**正は `../07-plan-actual/plan-actual-decisions-ja.md` §2-6。** ここは描画側の要件だけ書く。
...
- **描くときは 1 段の分岐で解く** — `描く色 = その列の指定 ?? テーマから解いた既定色`。
  **`null` はテーマ、値があればその値**（`../02-data-model/grs-native-erd-ja.md` §5.6 の疎な上書きが正）。
  ⚠️ **`fillColor` と `strokeColor` は独立に判定する。** 塗りだけ選んで輪郭はテーマに任せる、が普通に起きるので、
  片方に値があるからといってもう片方をテーマから外してはならない。行の色（`TaskGroup.color`）も同じ扱い。
  **解いた色を書き戻さない。**
  **値が `'transparent'` のときはその面を描かない**（下にある色が見える）。**塗りと輪郭を同時に透明にはできない**
  （値域と規則は `../02-data-model/grs-native-erd-ja.md` §5.6 が正）。
- **行の帯の色も同じ 1 段の分岐で解く。** 帯色の規則（**深さと行の位置**の 2 入力・縞の数え方）は
  `../02-data-model/grs-document-settings-ja.md` §4-2 が正。**ここには値も式も書かない。**
...
実測した値は `../07-plan-actual/plan-actual-decisions-ja.md` §2-6 の表にある。

### 6-1-3. 文字の色と縁取り — **確定 2026-07-31**

**正は `../07-plan-actual/plan-actual-decisions-ja.md` §2-7。** ここは描画側の要件だけ書く。
...
> **なぜ色を選び直す道を採らなかったか**: 文字色を固定したまま
> `../07-plan-actual/plan-actual-decisions-ja.md` §2-6 の色を使うと
...
> 詳細と性能の実測は `../07-plan-actual/plan-actual-decisions-ja.md` §2-7。
...
（`../07-plan-actual/plan-actual-decisions-ja.md` §2-4-1）は目盛にも掛かる。
...
- **判定では深さを 5 で頭打ち**にしてよい（6 段以上は実務でほぼ無い）。
  ただし**書き戻す値は頭打ちにしない**（階層が壊れる。`../07-plan-actual/plan-actual-decisions-ja.md` §5-2）。
- **小さい文書では全部描く**（現行は 200 アイテム以下）。ただしこれは**閾値ハックであって根治ではない**
  （`../04-performance/performance-notes-ja.md` N-4 / T-12）。次期は起動シーケンスで根治する。
...
**状態ごとの頂点は `../07-plan-actual/plan-actual-decisions-ja.md` §4-1 の表に従う。ここには複製しない。**
...
`../07-plan-actual/plan-actual-decisions-ja.md` §4-2）。
...
詳細は `../07-plan-actual/plan-actual-decisions-ja.md` §4。

> ⚠️ **性能上の注意**: 現行実装はこの頂点計算で**毎フレーム全アイテムを走査**していた（可視集合に絞っていない）。
> レビューで指摘され、**未解消のまま終わった**（`../04-performance/performance-notes-ja.md` T-1）。
> 次期は**可視集合に絞る**こと。
...
- **描画される矩形**（オーバーハング込み）で測る。論理矩形ではない
  （`../04-performance/performance-notes-ja.md` 罠 T-11）。含めるものの全数も同所 —
  実績バーの下方向 / マイルストーンの横方向 / Progress Marker の右方向 / はみ出す名称ラベル /
  **担当ラベルの左方向**（§4-2。担当 ＋ 完了率はバーの外側左に出る）。
  > ⚠️ **左方向を落とすと、右や下と違って取り返せない。** 右端と下端はスクロールで出せるが、
  > **行名ペインは不透明な固定ペインなので、その下に入った担当ラベルはどのスクロール位置でも出ない**
  > （§1）。**左のはみ出しの最大値を、行名ペインと日程本体の間の余白として確保する。**
  > **日付で確保してはならない** — ラベル幅は `zoomX` に比例しない（フォントは行高＝`zoomY` 由来）ので、
  > 低ズームで足りず高ズームで無駄になる。実測は `../04-performance/performance-notes-ja.md` T-11。
...
| # | 論点 | 扱い |
|:--:|---|---|
| ~~1~~ | ~~`CursorMode` と `CursorGuideMode` が前プロジェクトで**型が 2 つに分かれていた**~~（`ui-parts-ja.md` §5-3） | **確定**（2026-07-30）。型は 1 つ、名前は **`GuideCursorMode`**。`documentSettings` の項目は **`guideCursorMode`** |
