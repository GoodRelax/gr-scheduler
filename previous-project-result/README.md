...
| — | **`DECIDED-ja.md`** | **決まっていることの全数**。決定文書 8 本に散っている決定を 1 行ずつ引ける索引（171 行）。**結論と所在だけで、値は書いていない** | **最初に読む索引。** ⚠️ **索引であって正ではない**（`type: Index`）。食い違ったら所在の側が勝つ |
...
├── DECIDED-ja.md                    ★決まっていることの全数（索引 171 行。値は書いていない）
