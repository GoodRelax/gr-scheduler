...
| （この版） | 09-19 | 依存線を予定だけに付けた。予定が無いとき・予定を隠したときは描かず、構えても引けない（利用者の裁定。`FR-009` の実績への付け替えを覆す）。予定の開始・終了の上下の既定を 0 にした（7 問の ④、利用者の裁定）。依存線の表示の切り替えを、右のペインの一番上の「表示」に移した（隠すと構える・消すも押せない）。依存線の掴み代を線の縁から左右 2px にし、依存線のポインタを線の矢印（黒の縁・白の中）にした |
