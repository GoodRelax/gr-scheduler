...
|---|---|---|---|---|---|
| `===` 予定の開始 | 12 | 0 | 12 | — | `S-90`（外側と上下）。上下は 7 問の ④ |
| `===` 予定の終了 | 12 | 0 | 12 | — | `S-90`（同上） |
...
| ◆ ダミー | 0 | — | 0 | — | 新しい行 |
| `--->` 予定の開始 | — | — | 12 | 12 | `S-90`。左端を中心に（`JDG-185`）。上下は 7 問の ④ |
| `--->` 予定の終了 | — | — | 12 | 12 | `S-90`。`>` の中心を中心に。上下は 7 問の ④ |
| `--->` 実績の開始 | — | — | 0 | 12 | `S-91` |
| `--->` 実績の終了 | — | — | 0 | 12 | `S-91` |
| 依存線 | 6 | — | — | — | `S-137`（線の左右それぞれ） |
...
| ④ 予定の端の縦の張り出し | `===` と `--->` の予定の開始・終了の上下を 12px（`S-90`、GS-3）。4 つのセルに入る | 上下 0px | 予定の開始の掴み代の高さ: 推奨 42px（18 ＋ 12 × 2）、今まで 18px |
...
| 形 | 掴む所 | 横 | 縦 | ポインタ |
|---|---|---|---|---|
| `===` 予定 | 開始 ／ 終了 | 端から外側へ 12px | 予定バーの帯 ＋ 上下 12px（7 問の ④ の推奨） | ← ／ → 白抜き |
...
| `--->` 予定 | 開始 ／ 終了 | **軸の左端 ／ `>` の中心に中央合わせ**で 12px | 矢尻の高さ ＋ 上下 12px（7 問の ④ の推奨）。中線より上だけ | ← ／ → 白抜き |
...
**依存線が入る端での予定の開始**（②行、①から FS で入る線）: 線の入口の高さ ±6px は線が応える。予定の開始の掴み代（外側 12px）のうち、バーに近い 8px の列はそれ以外の高さで予定の開始が応える。**④ が推奨（上下 12px）なら高さ 30px、今まで（上下 0px）なら 6px** しか残らない。④（予定の端を上下に 12px 張り出す）は、依存線が付く端で効いてくる。バーから遠い 4px の列は、入口の手前で縦に折れる線の脚に取られる。見本の走りを仕様の 14px にしたのはこの測定のためである（8px の走りでは、予定の開始の掴み代の全部の列が上半分を取られていた）。
...
| （この版） | 09-19 | マーカーを出さないときのマイルストーン名を、菱形の幅の 25% の位置からにした（前の版は 75% の位置）。札の余白を 1 つの間隔から 4 つのつまみ（タスク名 開始 → 名前 ／ マーカー → 名前、マイルストーン名 マーカー → 名前、担当・進捗 → 開始）に分けた |
