...
| 掴み代を変える | 右のペインの各節の「掴み代」の表のつまみ（行 ＝ 掴む対象、列 ＝ その節の対象が持つ向き。外側・内側・上下・幅のうち） | 左のペインの絵と当たり判定が、見えたままその場で変わる。**1 本のつまみは 1 つの対象の 1 つの向きだけ**を動かす |
| 絵の大きさを変える | 右のペインの「全体」の節の「大きさ」と「進捗マーカー」 | ズーム・1 日の幅・バーの高さ・マーカーの描く径。掴み代の値は変わらない |
| 設定を探す | 右のペインは節に分かれている: 「全体」（表示・大きさ・共通の進捗マーカーと担当・進捗の札・描く順・応える順）→「=== タスク」→「---> タスク」→「◆ マイルストーン」→「依存線」→「7 問」。各節に、その対象の掴み代・札・ポインタが並ぶ | 利用者の指示（2026-09-19「設定ペイン内の各設定値がばらばらで見にくい。項目数はそのまま、領域を再整理しろ」）。つまみ・チェックの数は変えていない |
...
| ポインタの矢印 一辺 | 16px | 8〜32 | `S-249`（24 → 16、`JDG-181`）。タスクの端（=== と --->）と依存線のポインタ |
| ◆ 予定のポインタ ○ の径 | 12px | 4〜32 | 利用者の指示（2026-09-19）。最初の 16px の 75%。手のひらのポインタに代わる |
| ◆ 実績・ダミーのポインタ ● の径 | 8px | 4〜32 | 同上。最初の 16px の 50%。予定とは別の値 |
...
| 値 | 大きさ |
|---|---|
| フェードのポインタ | 10 × 10px（`JDG-191`）。三角の縦は横の 75%、縁は約 0.47px |
| ポインタの縁の太さ | 約 0.47px（依存線のポインタの 24 マスの箱で 0.7、16px のとき）。依存線・フェード・◆ の ○ ● で同じ |
| フェードの描く □ | 5 × 5px（`S-109`） |
...
| フェード | 入 ／ 出の掴み点 | 8 × 8px | — | **三角 10px、白の中・黒の縁。直角がポインタの位置。入は左上へ、出は右下へ広がる。縦は最初の 75%（24 マスの箱で 22 → 16.5）、縁は依存線のポインタと同じ太さ（約 0.47px）、角は丸め** |
| ◆ 予定 | 全体 | 予定の菱形の幅の 115% | 予定の菱形の高さ | **○**（白の中・黒の縁）径 12px |
| ◆ 実績 ／ ダミー | 全体 | 実績の菱形そのもの | 実績の菱形の高さ | **●**（黒の中・白の縁）径 8px |
...
| `f4146ca8` | 09-19 | 依存線のポインタの矢じりの幅と黒の縁を半分にし、縁を角張らせた（利用者の指示「予定と見間違えそうだ」） |
| （この版） | 09-19 | フェードのポインタの三角の縦を 75% にし、縁を依存線と同じ太さにした。マイルストーンのポインタを、予定 ○（12px）・実績 ● （8px）にした。設定のペインを「全体 → === → ---> → ◆ → 依存線 → 7 問」の節に分け、掴み代の表とポインタを節ごとに置いた（つまみの数は同じ） |
