...
| （① の行の右）コメントボックス「ここを確認」 | 本文の箱・引出し線・線先。線先は ① の「要件定義レビュー」の 18 日目を指す | 本文は箱の全体で掴む。引出し線は線から左右へ 3px、線先は 6px（タスクの上でも）。線先を引いて離すと、その日と行を指し、図の下にどのタスク・マイルストーンへのコメントかが出る（㊷、`JDG-249`） |
