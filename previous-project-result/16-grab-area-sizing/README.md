...
| 4 矢印（`--->`、再開日のある中断） | 3 段: 担当 : 進捗・マーカー・名前 ／ 予定の矢 ／ 実績の矢（`JDG-212`）。見出しに 3 段の縦幅と `===` のバーの高さを出す | 3 段が `===` のバーより低く収まること。札が動かず、予実の掴み代が中線で分かれること |
| ⑤ 縦に積んだ 3 本（「準備」は再開日のある中断） | 全部入りのタスク 3 本を、`JDG-148` の隙間（1px ＋ 依存線 1.5px ＋ 1px ＝ 3.5px）で 1 行に積んだ段 | 積んだとき、どれが応えるか（7 問の ⑤ ⑦ と `GS-1`）。隙間を通る依存線を掴めるか |
...
| ---> タスク | 縦の 3 段（名前の段 → 予定の線 `S-196`、予定の線 → 実績の線 `S-10`、線の太さ、矢じりの横幅、矢じりの縦幅）、掴み代、ポインタの見本 | XS ・ GA ・ PK |
...
| ② マーカーの押下 | **決まった: 表 T-021a**（`JDG-214`） | 表 T-021a の順（絵が変わるのは `PV-1` だけ） | 見本の仮の巡り（未着手 → 50% → 100% → 未着手） | 既定: 進行中 → 完了 → 中断 → 進行中 → 完了、実績は 2 日目から 12 日のまま。未着手の行は押すと 4 日目から 1 日の実績が立つ |
...
| ⑥ 「入る」の判定 | 未決 | 終了側だけ `S-91` を取る | `FR-002` のまま ／ マーカー ＋ 間隔 ＋ 名前 | ①行の名前が実績の中に入る最小の 1 日の幅: マーカーを出すと 終了側だけ 8px、`FR-002` のまま 10px、今までの形 7px。マーカーを隠すと 7 ／ 9 ／ 6px |
...
| 9 依存線の値の 2 点 | ② は**決まった: 見本を仕様の単位で書き直す**（`JDG-219`）。① は未決 | 引き入れ 16 ＞ 矢じり 9.6（画面で 10 ＞ 6px） | つまみで短くして試す | 下の「依存線の走りと矢じり」 |
...
| 再開日が未定（`===`） | マーカーのすぐ右（`S-23`）に 0.7 倍で薄く。名前はその右 | 「確認」: マーカー 432〜446、アイコン 448.5（9.8 × 9.8）、名前 464.3 |
| 再開日あり（`--->`） | 3 段目。腕を実績の線に、縦棒は下へ。実績の線は停止日で矢じり無しに止まる。掴み代は中線より下だけ | 「設計レビュー」: 停止日 288 → アイコン 324（17 日目）、掴み代 7.5 × 5.25 |
| 再開日が未定（`--->`） | 1 段目のマーカーのすぐ右。名前はその右 | マーカー 192、アイコン 202（5.25）、名前 213.25 |

- 引けば再開日が離した日になる（「準備」を +2 日で 39 日目）。停止日の翌日より前には置けない（左へ大きく引いても 34 日目で止まる）。未定のアイコンを引けば再開日を置く（「確認」を +3 日で 39 日目）。押すだけでは何も起きない。
...
2. 1 本の CR で、削除と、仕様案の表・定数・図・上位要求を同時に当てる（Step 1b）。⑥・9 ①と、仕様案 §8 の ㉔〜㉖（㉖ は再開アイコンの案）と群 E（㉗〜㉞。同じ CR に入れる要素の問い）の答えを待つ。ほかは決まった。
