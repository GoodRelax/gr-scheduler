...
| （① の行の右）コメントボックス「ここを確認」 | 本文の箱・引出し線・線先。線先は ① の「要件定義レビュー」の 18 日目を指す | 本文は箱の全体で掴む。引出し線は線から左右へ 3px、線先は 6px（タスクの上でも）。線先を引いて離すと、その日と行を指し、図の下にどのタスク・マイルストーンへのコメントかが出る（㊷、`JDG-249`）。指す先のタスクを動かしても、線先はついて行かない —— 日と行で持つ（`JDG-252`） |
...
| （この版の続き） | 09-20 | ハイライトボックスの掴み代を利用者の指定の値にした: 枠線から内と外へ 3px、四隅は隅から 6px（`JDG-248`。四隅とアンカーは同じ値なので、コメントボックスのアンカーも 6px）。四隅の値を「隅からの距離」で持つようにした。コメントボックスの「線が 3」の当て先は仕様案 §8 の ㊷ で問う |
| （この版の続き） | 09-20 | コメントボックスを見本に足した（㊷、`JDG-249`）: 本文は箱の全体、引出し線は 3px（タスクの上では描いた線だけ）、線先は 6px（タスクの上でも）。線先を「リリース判定」の上へ引くと「「リリース判定」へのコメント」と出る。行の無い所で離すと動かない。つまみは 45 本 |

## 追補: 縮小で消えるタスクを最小幅で描く（2026-09-23）

見本: [`min-width-lod-sample.html`](min-width-lod-sample.html)（1 ファイル。ダブルクリックで開く）。3 行に 1 年ぶんのタスク 85 件（1〜2 日の短いものが 81 件）とマイルストーン 3 件を置いた。描く幅は max(実際の幅, 最小幅) で、重なるタスクはアプリの段の割り当て（FR-003、表 T-038 の占有幅。`schedule-layout.ts` と同じく、開始日順に、占有幅が重ならない最初の段へ入れる。接するのはよい）のとおり下の段へ積む。1 段 ＝ 18.125px ＋ 隙間 3.5px（`VG-2`）、行の床 24.0px（`HF-19`）。ポインタを置くと、そのタスクが赤で囲まれ、名前と日付が出る。

**段の数の合計 ／ 3 行の縦幅の合計 —— 名前を占有幅に数える（今のアプリ。進捗マーカーあり）**

| 最小幅 ＼ 1 日の幅 | 0.25px | 0.5px | 1px | 2px | 4px | 8px |
|---|---|---|---|---|---|---|
| 今のアプリ（S-86 未満は描かない） | 7 段 ／ 154px（描いたのは 6 件） | 6 段 ／ 132px（7 件） | 4 段 ／ 91px（7 件） | 3 段 ／ 72px（7 件） | 3 段 ／ 72px（7 件） | 4 段 ／ 91px（21 件） |
| 4px | 59 段 ／ 1276px | 36 段 ／ 779px | 20 段 ／ 433px | 15 段 ／ 324px | 14 段 ／ 303px | 13 段 ／ 281px |
| 6px | 62 段 ／ 1341px | 37 段 ／ 800px | 21 段 ／ 454px | 16 段 ／ 346px | 14 段 ／ 303px | 13 段 ／ 281px |
| **8px** | 66 段 ／ 1427px | 37 段 ／ 800px | 22 段 ／ 476px | 16 段 ／ 346px | 14 段 ／ 303px | 13 段 ／ 281px |
| 10px | 68 段 ／ 1471px | 38 段 ／ 822px | 22 段 ／ 476px | 17 段 ／ 368px | 14 段 ／ 303px | 13 段 ／ 281px |
| 12px | 69 段 ／ 1492px | 38 段 ／ 822px | 23 段 ／ 497px | 17 段 ／ 368px | 14 段 ／ 303px | 13 段 ／ 281px |

**同じ —— 名前を占有幅に数えない（段は描いた形だけで決まる。名前は重なって描かれる）**

| 最小幅 ＼ 1 日の幅 | 0.25px | 0.5px | 1px | 2px | 4px | 8px |
|---|---|---|---|---|---|---|
| 4px | 15 段 ／ 324px | 12 段 ／ 260px | 7 段 ／ 154px | 4 段 ／ 91px | 3 段 ／ 72px | 3 段 ／ 72px |
| 6px | 16 段 ／ 346px | 13 段 ／ 281px | 10 段 ／ 219px | 6 段 ／ 132px | 4 段 ／ 91px | 3 段 ／ 72px |
| **8px** | 17 段 ／ 368px | 14 段 ／ 303px | 12 段 ／ 260px | 7 段 ／ 154px | 4 段 ／ 91px | 3 段 ／ 72px |
| 10px | 18 段 ／ 389px | 15 段 ／ 324px | 13 段 ／ 281px | 8 段 ／ 175px | 6 段 ／ 132px | 4 段 ／ 91px |
| 12px | 19 段 ／ 411px | 16 段 ／ 346px | 13 段 ／ 281px | 10 段 ／ 219px | 6 段 ／ 132px | 4 段 ／ 91px |

**最も短い的（px）—— 1 つのタスクが応え続けた横の長さの最小。左は名前を数える、右は数えない**

| 最小幅 ＼ 1 日の幅 | 0.25px | 0.5px | 1px | 2px | 4px | 8px |
|---|---|---|---|---|---|---|
| 4px | 9.25 ／ 5.25 | 10.5 ／ 7 | 11 ／ 5 | 8 ／ 4 | 8 ／ 4 | 10 ／ 8 |
| 6px | 10.25 ／ 7 | 11.5 ／ 7 | 12 ／ 7 | 9 ／ 6 | 9 ／ 7.25 | 10 ／ 8 |
| **8px** | 11.25 ／ 8.75 | 12 ／ 10 | 12 ／ 10 | 10 ／ 10 | 10 ／ 8 | 10 ／ 8 |
| 10px | 12 ／ 10.5 | 12 ／ 10.5 | 12 ／ 11 | 11 ／ 10 | 11 ／ 11 | 11 ／ 11 |
| 12px | 12 ／ 12 | 12 ／ 12 | 12 ／ 12 | 12 ／ 12 | 12 ／ 12 | 12 ／ 12 |

- 掴む: 60 の組のすべてで、描いた 88 件の全部に届き、どのタスクも自分の形の真ん中を指すと自分が応えた（88 / 88。隣の掴み代に奪われない）。掴み代は、描いた幅が 12px より細ければ中心に揃えた 12px。形の上ではその形のタスクだけが応え（`HT-1`）、形の外で重なったら中心が近い方が応える（`HT-4`、`item-hit-area.ts` の `beats`）ので、隣との中点で切れる。実物のポインタ（Playwright の mouse）でも、1 日 1px・最小幅 6px と 12px で 88 / 88 だった。
- 名前が段を決めている: 1 日 1px・最小幅 8px で、名前を数えると 22 段、数えないと 12 段。最小幅を 4 → 12px と変えて増える段（名前を数えるとき 1 日 1px で 20 → 23 段）より、名前の分（+10 段）がずっと大きい。
- 今のアプリは、1 年を 1 画面に収めると（1 日 1px 以下）85 件のうち 3〜4 件しか描かない。段が少ないのは、描かないからである。

**測り方**: 見本を Chromium（Playwright 1.62）で開き、つまみの値を入れて、見本が描いた段の数と行の縦幅を読んだ。掴みは、どの段の中心の高さでも左から右へ 0.25px 刻みでポインタの移動を送り（見本の pointermove の処理をそのまま通す）、応えたタスクと、応え続けた長さを拾った。続けて、描いた形の真ん中を 1 件ずつ指した。探り針は一時ファイルで書いたので、リポジトリには置いていない。

**推奨: 最小幅 8px**（見本の既定。掴み代の見本のフェードの掴み点 8px 角と ◆ の実績のポインタ 8px に揃う）。的は名前を数えても数えなくても、どの縮尺でも 8px 以上になる（6px では 6〜7px まで細る）。代償は、6px と比べて 1 日 1px で段が +1（名前を数えるとき）／ +2（数えないとき）、1 日 0.25px で +4 ／ +1。
