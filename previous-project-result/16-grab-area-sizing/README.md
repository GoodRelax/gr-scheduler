...
|---|---|---|---|
| ㉔ 進捗マーカーの大きさ | 上限を予定バーの高さにし、既定は 14px（仕様の単位で 22.4）のまま | 「全体」の「マーカーの描く径」 | つまみは 12〜28。予定バーを 20 にすると、マーカーも 20（描いて 12.5px ＝ バーと同じ）で止まる |
...
### 群 F —— 今の仕様と突き合わせて出た問い（㊱〜㊳。答え待ち）

㉔〜㉟ の答えを今の仕様と突き合わせた（仕様案 §9）。答えから決まらないものが 3 つ出たので、見本で比べられるようにした（右のペインの「7 問」）。確かめたい読み 4 つは仕様案 §8 の群 F にある。

| 問い | 中身 | 見本で比べる所 | 実測 |
|---|---|---|---|
| ㊱ 再開日が先のときの札 | 推奨 B: 札は実績のすぐ右、アイコンと重なるときだけアイコンの右（未定のアイコンでは今までどおり） ／ A: いつもアイコンの右 | 「準備」の再開アイコンを 50 日目へ引き、1 日の幅を 6px に | B: 停止日 324 のすぐ右にマーカー 324・名前 344、アイコン 420。A: マーカー 434・名前 454（アイコンの右）。再開日が 36 日目か未定なら、どちらもアイコンの右 |
| ㊲ 引ける進捗マーカーのポインタ | 推奨 A: 指のまま ／ B: 実績の終了の矢印 → 黒 | 「結合試験」のマーカー（ダミーの右） | B にすると、引けるマーカー（実績かダミーの右に立ち、中断でないもの）だけが矢印になる。実績の中のマーカーと ◆ のマーカーは指のまま |
| ㊳ 端点スパンの点の重なり | 推奨 A: 許す（既定で 1px） ／ B: 間を点の径に合わせて広げる | 「線だけの形」を端点スパンに（「設計レビュー」の開始は予実とも 6 日目） | 予定の開始の点と実績の開始の点の中心は 3px 離れ、半径 2px ずつなので 1px 重なる |
...
2. 1 本の CR で、削除と、仕様案の表・定数・図・上位要求を同時に当てる（Step 1b）。仕様案 §8 の問いは 2026-09-19 に全部答えが出た（最後は `JDG-228`〜`JDG-239`）。仕様案は、今の仕様の行と突き合わせて直した（仕様案 §9）。CR は、そこで出た ㊱〜㊳ と 4 つの読みの答え（仕様案 §8 の群 F）を待つ。
