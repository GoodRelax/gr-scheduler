...
| ② マーカーの押下 | 表 T-021a に決まった（`JDG-214`）が、後の指示「・ に戻れるようにしろ」で決め直す（仕様案 §8 の ㉟、`JDG-227`） | 案: 進み順の輪（未着手 → 進行中 → 完了 → 中断 → 未着手） | 表 T-021a（・ には戻らない） | 既定: 「要件定義レビュー」は 進行中 → 完了 → 中断 → 未着手（実績を覚える）→ 進行中（2 日目から 12 日に戻る）。表 T-021a では 8 回押しても未着手に来ない |
...
| 案 | 巡り | 重くなる押下 |
|---|---|---|
| **A（推奨・見本の既定）** 進み順の輪 | 未着手 → 進行中 → 完了 → 中断 → 未着手 | 1 日の作業の完了が 2 押し。中断からの再開も 2 押し |
...
- 実績は外しても消えない: 中断 → 未着手 で実績を覚え、次の押下で戻す（取り消しでも戻る）。覚えるのは開いているあいだだけ（案）。
...
2. 1 本の CR で、削除と、仕様案の表・定数・図・上位要求を同時に当てる（Step 1b）。仕様案 §8 の ㉔〜㉖（㉖ は再開アイコンの案）と群 E（㉗〜㉞。同じ CR に入れる要素の問い）と ㉟（マーカーの押下の巡り）の答えを待つ。⑥・9 ① は 2026-09-19 に決まった。ほかも決まった。
