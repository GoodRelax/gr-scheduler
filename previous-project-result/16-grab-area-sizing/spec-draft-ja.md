...
**群 H の答えで書き直す行**（`JDG-248`・`JDG-249`）: `GR-14`（R:3354）の場所に引出し線を足す。R:3436〜3443 に、ハイライトボックスの本体を枠線から 3px、コメントボックスの引出し線（引けば本文の箱が動く —— 本体と同じ `CM-51`）と線先（6px、タスクの上でも）を書く。R:3448 と `S-230`（S:306）を、ハイライトボックスの四隅だけの値に割る。残す: R:4397（引出し線は折れ線 1 種）、R:3441（線先は離した所の日と行で読む）、`AR-5`（R:3286。タスクを名指さない）、R:3445 付近（コメントボックスに四隅を持たせない、MUST NOT）。
