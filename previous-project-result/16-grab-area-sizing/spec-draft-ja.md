...
- 拠り所: 見本 [`grab-area-sizing-sample.html`](grab-area-sizing-sample.html)（規則はすべて実装して測ってある）と、[`rulings.md`](../../docs/development-records/rulings.md) の `JDG-180`〜`JDG-242`
...
どの表にも親の上位要求があり、どの上位要求も表に届く（§7 の表）。本文は `handoff.md` §1.2 で利用者が了承した案に、2026-09-19 の裁定（`JDG-194`〜`JDG-242`）を足したものである。
...
- 画像のポインタを描けない環境では、環境の形に替える（今の `PC-6` の `ew-resize` を残す）。三角・線の矢印・円・折れ矢印の替わりの形は ㊶ で決める。
...
仕様案の問い（群 A〜F）は 2026-09-19 にすべて答えが出た（`JDG-209`〜`JDG-242`）。読み直し（2026-09-19、確認役が 38 件を指摘し、本案と対応表を直した）で、利用者の答えが要るものが 3 つ見つかった（下の群 G）。その答えのあと、この案から 1 本の CR を書き、仕様の体 1 体で仕様へ当てる（`handoff.md` §1.2 の Step 1b）。1〜7・8・9 の事実・推奨・代償は `handoff.md` §1.2 が持つ。

### 群 G —— 読み直しで見つかった問い（2026-09-19。答え待ち）

| 問い | 中身 | 案 | 見本 |
|---|---|---|---|
| ㊴ | 依存線を隠しているあいだに、依存線を選ぶ・ラグを直す・消すことができるか —— 今の仕様の中で食い違う: 用語集の `IC-81` の注は「別の面に経路が残る」、`FR-049` は「別の面を置かない。戻してから行う」（`DFC-660`） | **推奨 A**: できない（`FR-049` のまま。戻してから行う。見えないものは掴めない —— `FR-108`）。`IC-81` の注を直す ／ B: 別の面（例えば属性の欄の依存の一覧）から行える。`FR-049` を直す | 見本は A（隠すと構えも消すもできない） |
| ㊵ | 暗い実績の中に描く名前の色（`handoff.md` の「`FR-109` に要る」） | **推奨 A**: 1 色のまま —— 濃い字に地の色の縁（見本の形。どの地の上でも読める）。表 LP に列を足さない ／ B: 実績の中では白い字にする（表 LP に「字の色」の列を足す） | 見本は A（「要件定義レビュー」の名前は実績の中） |
| ㊶ | 画像のポインタを描けない環境での、替わりの形 | **推奨**: 動く向きで選ぶ —— 端の箱の矢印・フェードの三角・◆ の実績とダミーの ●・再開の折れ矢印は `ew-resize`（今の `PC-6` のまま）、◆ の予定の ○ は `move`（横にも縦にも動く —— ㉛）、依存線の線の矢印は `pointer`（押して選ぶだけ） ／ 別案: どれも環境の既定の矢印 | — |
