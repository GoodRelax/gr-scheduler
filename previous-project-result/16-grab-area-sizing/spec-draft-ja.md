...
- 拠り所: 見本 [`grab-area-sizing-sample.html`](grab-area-sizing-sample.html)（規則はすべて実装して測ってある）と、[`rulings.md`](../../docs/development-records/rulings.md) の `JDG-180`〜`JDG-248`
...
どの表にも親の上位要求があり、どの上位要求も表に届く（§7 の表）。本文は `handoff.md` §1.2 で利用者が了承した案に、2026-09-19〜20 の裁定（`JDG-194`〜`JDG-248`）を足したものである。
...
| 3 | 注記（コメント／ハイライトボックス） | `GR-14`（残す。今は依存線と予定の端のあいだ。日程の形より先へ移す —— 群 B ⑫、`JDG-210`）。ハイライトボックスの本体は枠線の近く（枠線から内と外へ 3px。新しい行）と四隅（隅から 6px。`S-230`）だけで（値は `JDG-248`）、囲んだ内側は下のタスクへ素通しにする（`JDG-231`。今の R:3354・R:3435〜3437 を書き直す）⇒ 先に置いても、中のタスクから押しを取らない。描いた形の上では、枠は描いた線そのものでだけ応える（依存線と同じ。群 A ⑭）。コメントボックスは本体（本文の箱の全体。不透明に描くので素通しにしない —— `FR-108`）とアンカー（点から 6px。`S-230`）を持ち、四隅を持たない（今の MUST NOT のまま）。アンカーも、描いた形の上では描いた印そのものでだけ応える。コメントボックスの引出し線を掴めるようにするかは ㊷ で決める |
...
| 群 | 定数 | 既定 | 使う表 | 今の行（置く表） |
|---|---|---|---|---|
| 掴み代 | 表 GA の 41 の値（対象 × 向き） | 表 GA のとおり | GA | `S-90`（予定の端）・`S-91`（実績の端）・`S-92`（フェード）・`S-137`（依存線）を割る。ほかは新しい行。注記の四隅とアンカーの `S-230` は、自分の値 6px を持たせる（今は既定を「`S-137`」と書いている。値は同じ 6px。1 値 1 役。`JDG-248`） |
...
仕様案の問い（群 A〜F）は 2026-09-19 にすべて答えが出た（`JDG-209`〜`JDG-242`）。読み直し（2026-09-19、確認役が 38 件を指摘し、本案と対応表を直した）で見つかった 3 つ（群 G）も、2026-09-20 に答えが出た（`JDG-245`〜`JDG-247`）。その後の指示で注記の掴み代の値が決まり（`JDG-248`、群 H）、コメントボックスの読み 1 つ（㊷）だけが答え待ちである —— その答えのあと、この案から 1 本の CR を書き、仕様の体 1 体で仕様へ当てる（`handoff.md` §1.2 の Step 1b。新しいセッションで行う）。1〜7・8・9 の事実・推奨・代償は `handoff.md` §1.2 が持つ。
...
| 問い | 中身 | 案 |
|---|---|---|
| ㊷ | コメントボックスの「線が 3」をどこに当てるか。コメントボックスには四隅が無く（今の MUST NOT —— 本文の箱の大きさは本文で決まる）、線は本文の箱の縁と、アンカーへの引出し線（今の R:4397「折れ線 1 種」）である | **推奨 A**: 本文の箱は箱全体で掴む（不透明に描くので、ハイライトボックスのように中を素通しにすると、見えないタスクを掴むことになる —— `FR-108`）。引出し線は掴む場所にしない。「線が 3」はハイライトボックスの枠に当てる ／ B: 引出し線も線の左右 3px で掴め、引けば本文の箱を動かす（本体と同じ）。引出し線がタスクの上を通る所では、依存線と同じく描いた線そのものでだけ応える |
...
3. ㉗ の「枠の近くだけ」は、描いた形の上では描いた枠線そのものだけである（依存線と同じ。群 A ⑭）。コメントボックスのアンカーも同じ（表 T-023d の新しい並びの 3）。
