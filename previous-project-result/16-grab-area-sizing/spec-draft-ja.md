...
- STATEMENT: `GRS` は、ポインタの下で複数の掴み代が重なるとき、応答する対象を 1 つに決め、しかもその決め方を利用者が見た目から予測できるものにすること。決め方は表 HT に従う。日程表（行の領域と目盛）の上の押下と引く操作で、字の選択を始めてはならない（MUST NOT）。修飾キーを付けた割当の無い引く操作も同じである（今の `MK-12` の「ブラウザの既定動作に委ねる」に、この 1 つの例外を足す）。
...
- 打ち切った名前の説明（今の `EZ-6`、R:5861「どのタスクの上かは T-023d の順」）は「表 T-023d と表 HT」を指す。名前・担当は素の押下では掴みとして成立しない（並び 2）が、説明の引き金には数える（`UC-001` の拡張 4a）。
...
|---|---|---|---|---|
| ㉔ マーカー | `S-22`（S:93）、`RC-11`（R:6795）、`S-247` の注（S:369） | `TE-1`（R:3492。対応表のとおり） | — | `DM-3`（R:2804。表 T-240 の描く規則として残す）、`FR-094`（R:1212・1234）、S:60、`PI-5`（D:425） |
...
| ㉜ 形の上 | R:3408（表 HT の段 1 へ運ぶ） | 表 T-261（`GS-1` → 表 HT の段 1、`GS-2` → 段 2、`GS-4` → HT の順。`GS-3` は `JDG-197` で覆り、GA の値は隣で縮まないので、その MUST NOT は作りで保たれる） | — | R:3407、R:3409、`DS-7`（R:6393） |
...
**群 G の答えで書き直す行**: 用語集の `IC-81` の注（G:562。㊴）、`PC-6`（R:6449。替わりの形の一覧。㊶）、`FR-109` に 1 文（字は 1 色、地の色の縁。㊵）。
