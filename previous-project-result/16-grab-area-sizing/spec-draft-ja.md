...
| 4 | `--->` | 名前 | 箱の下端を、予定の形の上端から `S-196`（2px）上に置く（`OC-10`） | 字 × `S-233`（1.5）。字は 予定の帯 × 0.5715 × 0.90 × `S-9`（0.85）と `S-8` の大きい方 |
| 5 | `--->` | 予定の矢 | 名前の箱と `S-196` の下（`OC-10`） | 帯 p ＝ h × `S-15`（0.5）。線 ＝ p × `S-40`（0.20）を `S-41`〜`S-42` に収める（`LF-8`）。矢じり ＝ 線 × `S-45`（3.2）と 横幅 × `S-46`（0.4）の小さい方の長さで、高さも同じ（`LF-7`） |
| 6 | `--->` | 実績の矢 | 予定の帯の下端から `S-10`（2px）下（`LF-9`） | 帯 ＝ p × 0.5715。線は予定と同じ太さ、矢じりは同じ式 |
| 7 | `--->` | マーカー | 実績の矢の中心（今の `FR-002`） | 径 ＝ 名前の字の大きさ（`FR-094`） |
| 8 | `--->` | 予実の境（掴み代を分ける中線） | 2 つの矢の中心の中点 | — |
| 9 | ◆ | 予定の菱形 | 行の帯の中央 | h（幅も h） |
| 10 | ◆ | 実績・ダミーの菱形 | 同上 | h × 0.5715（幅も同じ。`LF-10`） |
...
- **群 A の答え（`JDG-209`）**: 裁定の無かった見本の数と形（`--->` の段の比 0.10 ／ 0.42 ／ 0.72、軸 × 3.2 を矢じりの高さに使うこと、◆ の実績の 0.7、線だけの形のマーカーの径と名前の大きさ）は、今の仕様の値へ戻した。4〜7 と 10 の式はどれも今の仕様の行であり、**新しい定数は無い**。見本の既定（バーの高さ 18px）で測った値: p 9px、線 1.8px、矢じり 5.76px、実績の帯 5.14px、名前の字 7.5px（`S-8` の床で決まる）、マーカーの径 7.5px。
- **7 の高さ**: 今の仕様は 2 か所で違うことを言う —— `FR-002` は線だけの形について「マーカーは実績の高さに立つ」、`LF-11` は形を分けずに「縦は予定バーの中心」（アプリは `LF-11` のとおり）。`===` では 2 つは同じ高さになるので、線だけの形について書いた `FR-002` を採った（`DFC-663`）。`LF-11` の中心に置くと、実績の開始が予定の開始と同じ日のとき、マーカーが予定の矢の左端（予定の開始の掴み代）の上に載る。
...
| 5 | `--->` | 出す | （問わない） | 基準の開始（実績の高さ。表 XS の 7） | マーカーの右端 ＋ 「タスク名 マーカー → 名前」（名前の段） |
...
| 5 | `===` | ダミーの開始 | ダミーの左端（＝予定の開始） | 外側 0 ／ 印の左半分（`JDG-37`。⚠️ 問い ①） | 実績 ＋ 0（群 B ①c） | 箱の矢印 ← 黒 |
| 6 | `===` | ダミーの終了 | ダミーの中心 | 印の右半分 ／ 外側 0（群 B ①b。`JDG-41` の「印の外へ広げない」のまま） | 実績 ＋ 0（群 B ①c） | 箱の矢印 → 黒 |
...
| 4. 同じ種別 | 掴み代の中心までの直線の距離が近い方が応える（`JDG-190`。⚠️ 7 問の ⑤）。距離が同じなら後に描いた方 |
...
- **`--->` のマーカー**: 実績の開始の上に立つが、種別は順 2（実績の外に立つもの）として扱う。`--->` の実績の開始の掴み代は軸の左端に中央合わせなので、左の半分（6px）がマーカーに取られずに残る。順 7 に置くと、径 7.5px のマーカーは 1.5px しか応えない。
...
| 形 | 進捗マーカーの描く径 | 14px（`--->` は名前の字の大きさ） | XS ・ LP ・ GA | `S-22`（16px、上限 16。描く比 0.625 が掛かる群）。⚠️ 見本の 14 を描いた px と読めば `S-22` ＝ 22.4 で上限を破り、仕様の値と読めばアプリは 8.75px で描く（問い 9 の ②）。`--->` は `FR-094` のまま（群 A） |
| 形 | ダミーの幅 | マーカーの径 × 0.5 | RF ・ GA | `S-247` |
| 形 | 実績の高さ・`--->` の段・◆ の実績の比 | 表 XS のとおり | XS | 今の `S-5`・`S-10`・`S-15`・`S-40`〜`S-46`・`S-196`・`S-233` のまま。新しい行は無い（群 A） |
...
CR（Step 1b）は、群 C の答えを待つ。群 A・B・D は 2026-09-19 に答えが出た。1〜7・8・9 の事実・推奨・代償は `handoff.md` §1.2 が持つ。残りは Step 1a の対応表（[`old-to-new-map-ja.md`](old-to-new-map-ja.md)）で見つかった。
