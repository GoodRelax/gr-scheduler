...
- 拠り所: 見本 [`grab-area-sizing-sample.html`](grab-area-sizing-sample.html)（規則はすべて実装して測ってある）と、[`rulings.md`](../../docs/development-records/rulings.md) の `JDG-180`〜`JDG-249`
...
⛔ **これは仕様ではない。** 仕様に着地するまでの下書きである。表の名（GA・HT・TY・RF・LP・PE・XS・PK）と行の番号は仮のものであり、CR で登録するまで仕様にも台帳にも書かない。接頭辞は登録済みの 153 と重ならないことを確かめてある（ポインタの表は、登録済みの `PTD` と 1 字違いになる `PT` を避けて `PK` とした）。
⭐ **2026-09-20、利用者の裁定で 表 HT を 2 つに割った** —— 応える**手順**の表（`HT`、4 行）と、同じ手順の中で**種別**が応える順の表（`TY`、9 行）。逐語と、字を選んだ理由は `CR-430` の冒頭と 0 節の ③ が持つ。
⭐ **2026-09-20、行番号を `3d342753`（`CR-429` が着地した木）へ写し直した** —— SAME 831 ／ SHIFTED 66 ／ 決まらないもの 0。本書の 6 行を書き換えた。
...
どの表にも親の上位要求があり、どの上位要求も表に届く（§7 の表）。本文は `handoff.md` §1.2 で利用者が了承した案に、2026-09-19〜20 の裁定（`JDG-194`〜`JDG-249`）を足したものである。
...
**FR-104 縮小しても、どれも指せる**
- STATEMENT: `GRS` は、表示をどこまで縮小しても、描いているタスク・マイルストーン・進捗マーカー・依存線のそれぞれを、ポインタで指して操作できるようにすること。そのために、各操作対象に、他の対象に取られない掴み代を持たせること。掴み代の形と大きさは表 GA、縦の段は表 XS、重なったときの応答は表 HT と表 TY に従う。
...
- STATEMENT: `GRS` は、ポインタの下で複数の掴み代が重なるとき、応答する対象を 1 つに決め、しかもその決め方を利用者が見た目から予測できるものにすること。決め方は表 HT と表 TY に従う。日程表（行の領域と目盛）の上の押下と引く操作で、字の選択を始めてはならない（MUST NOT）。修飾キーを付けた割当の無い引く操作も同じである（今の `MK-12` の「ブラウザの既定動作に委ねる」に、この 1 つの例外を足す）。
...
- STATEMENT: `GRS` は、ポインタが掴み代に入ったとき、ポインタを、何を掴むか（予定か、実績か、マーカーか、フェードか、マイルストーンか、依存線か）と、引いたら何がどちらへ動くかが分かる形に変えること。押しているあいだは、掴んだときの形を保つこと。どの掴み代でどの形かは表 GA のポインタ欄、形そのものは表 PK に従う。押す役と引く役を兼ねる進捗マーカー（表 PE の 8・9）は、押す役の形（指）とすること（`JDG-241`）。
...
- STATEMENT: `GRS` は、日程表の上の直接操作では、実績の日付を、利用者が実績（ダミーを含む）を掴んで意図して動かしたとき、または進捗マーカーを押す・引くとき（表 PE の 8〜10 と注「押下の巡り」）にだけ変えること。ほかの操作のついでに変えてはならない（MUST NOT）。どの操作が何を変えるかは表 PE に従う。属性の欄で入れる値と取り消しは、本要求の外である（今の R:2665）。
...
- STATEMENT: `GRS` は、そのフレームで描いていない編集対象 —— 表示を切り替えて隠した予定・実績・依存線・進捗マーカー・再開アイコン、LOD で描かないタスク（今の表 T-005a）、畳んだ注記、隠した行、変更前の予定の重ね（`FR-015`）、選んでいないタスクのフェードの掴み点（`FR-075`）—— を、掴めないようにし、ほかの操作で動かさないこと。表 RF と表 PE に従う。今の表 T-023a の結び（R:3273〜3276「当たり判定の対象はそのフレームで描いた編集対象に限る」）は本要求を指す（群 D、`JDG-211`）。
...
- ⚠️ 今の `FR-002` と二重にしない: `FR-002` は名称の字そのもの（打ち切り・予定日・9 点のアンカー）に縮め、置き場は本要求が持つ（群 D、`JDG-211`）。

**FR-110（新） 描く前後は 1 つの表で決める**
- STATEMENT: `GRS` は、日程表に描くものの前後（どれが手前に見えるか）を 1 つの表で決めること。名称・担当と進捗・進捗マーカーは、線と形より手前に描くこと（選択の枠・構えて引いている仮の依存線・範囲選択の矩形は、それよりさらに手前 —— 表 ZO の 10〜13）。描く前後を、応える順（表 HT）の根拠にしてはならない（MUST NOT）。前後は表 ZO に従う。
...
| GA → HT | 掴み代の一覧（種別・範囲・中心） | 応える手順は掴み代の上で決める |
| TY → HT | 種別の順（形の上 ／ 形の外の 2 列） | 手順 3 が読む唯一の表 |
...
**形の記号**（表の「形」の列と本文）: `===` は矩形（`SH-1`）と矢羽根（`SH-2`）、`--->` は矢印（`SH-3`）と端点スパン（`SH-4`）、◆ はマイルストーンの 15 の図形（`SH-5`）を指す（今の表 T-012）。
- 本案の「菱形」は ◆ の図形の代表であり、15 の図形のどれにも同じ規則を当てる。どの図形も一辺 h の正方形に収まる（今の `LF-10`）ので、「菱形の幅」は外接正方形の一辺、「菱形の中心」は外接正方形の中心と読む。形の上（表 HT の段 1）は描いた図形そのもの（今の R:3408 の「`SH-5` は図形そのもの」）である。
- 矢羽根（`SH-2`）は矩形と同じ行を当てる。端の基準点は外接矩形の左右の端（今のアプリもそう取る）、フェードの掴み点は今の表 T-012a の点 4 ／ 2 の日と上端・下端（`FD-5`: 矢羽根ではフェード長が切り込みの深さを置き換える）、「入る」は外接矩形の幅で数える（尖りで減らさない —— 札の置き場を形ごとに変えないため）。
- 端点スパン（`SH-4`）は矢印と同じ 3 段・同じ行を当てる（表 XS の注・表 GA の注）。
...
| 4 | `--->` | 1 段目: 担当 : 進捗・マーカー・名前 | 形の帯の上端 | 字の大きさ f（マーカーの径も f。`FR-094`）。f は 予定の帯 p（h × `S-15` 0.5。端点スパンは `S-16` 0.5）× 0.5715 × 0.90 × `S-9`（0.85）と `S-8` の大きい方 |
...
| 14 | `--->` | 3 段目: 未着手の印（ダミー） | 実績の線と同じ中心。予定の開始から | 線の太さ。横幅 ＝ マーカーの径（字の大きさ f）× 0.5。形は 1 稼働日の実績と同じ（今の `DM-4`）—— 矢じりは矢の長さの `S-46` 倍で頭打ち（字 7.5px なら 1.5px）、端点スパンなら両端の点（`JDG-232`） |
...
- **縦幅** ＝ f ＋ `S-196` ＋ 線 ＋ `S-10` ＋ 線 ＋（矢じりの縦幅 − 線）÷ 2 ≦ h（`FR-109`）。既定の値（仕様の単位。描く比 0.625 を掛けた px を括弧に）: f 12（7.5）、`S-196` 2（1.25）、線 2.8（1.75）、`S-10` 2（1.25）、矢じり 横幅 5.6 × 縦幅 5.6（3.5 × 3.5）⇒ 縦幅 23.0（14.375）＜ h 28（17.5）。間を 0 にすれば 19.0（11.875）。
  - 今の `LF-9`（D:1019「予定の縦幅に `actualGap` を加えただけ下げる」）は、表 XS 6（予定の線の下の縁 → 実績の線の上の縁 ＝ `S-10`）に書き直す。
  - 段の位置は、矢の横幅によらず取る（短い矢で矢じりの横幅が頭打ちになっても、段は動かない）。実績を隠しても、実績が無くても、3 段目は取っておく（`FR-049`、群 B ⑮）。
  - 端点スパン（`SH-4`）: 点の径は独立の設定値（既定 6.4、描いて 4px。`JDG-229`）であり、今の `LF-7` の「点の半径 ＝ 線 × `spanDotOfStroke`」と `S-47` を退役させる。縦幅の式の「（矢じりの縦幅 − 線）÷ 2」は「（端の印の縦幅 − 線）÷ 2」と読む（端の印 ＝ 矢じりの縦幅か点の径）⇒ 既定で 23.4（14.6）＜ h 28。⚠️ 点は両端にあるので、予定と実績の開始が同じ日に立つと、`S-10`（2）の間に上下から 1.8 ずつはみ出し、既定で 1.6（描いて 1.0px）重なる —— 重なりは許す（同じ日の端だけで、点の色で分かれる。`JDG-242`）。「はみ出すのは矢の終わりの 1 か所だけ」は `SH-3` に限る。
  ...
  - ⚠️ 1 段目の高さは字の大きさ f で数え、今の `OC-10` の「字 × `S-233`（1.5）」では数えない。描いた字の箱は、既定の書体で f の約 1.33 倍（見本で測った: f 7.5px の名前の箱 10px）なので、上下に約 1.25px ずつはみ出す。上は行と段の隙間（`VG-2` の 3.5px）が受ける。⇒ `OC-10`（R:1541）の 1 段目の数え方と `S-233`（S:292「字の大きさそのものを数えてはならない」）の使い道を書き直す。`VG-5`（R:1250「描いた端は `OC-6` / `OC-10` を含めた広がりの端」）と `LF-2`（D:1012）も、1 段目を字の大きさ f で数える文に直す。最初の行の最初の段の上には隙間が無い（`VG-1` は段の下にしか空けない）ので、字の箱が約 1.25px 目盛の下へ出る —— 今の表 T-012 の注（R:1090「最上段では目盛の下に隠れる」）と同じ扱いとする。
...
予定も実績も隠したときは、形を描かず、札は予定の位置に置く（3 の行。予定を隠しても札の基準は予定である）。
...
- **担当と進捗**: 1 枚の札にまとめ、右寄せで、描いている予定と実績のうち早い方の開始から「担当・進捗 → 開始」だけ左に置く。◆ は、基準の菱形の 25% の位置からその値だけ左。`--->` は 1 段目で、基準の開始（マーカーの左端）からその値だけ左 —— 1 段目は左から 担当 : 進捗 → マーカー → 名前 と並ぶ（`JDG-212`。`JDG-182` の要確認 —— `--->` の札の置き場 —— には、この 3 段で答えた）。
...
- **再開アイコン（`JDG-230` で決まった）**: 中断のあいだだけ描く（`FR-044`）。マイルストーンには描かず、掴み代も持たない（今の `LF-11` ／ `GR-8` の MUST NOT のまま —— 点は期間を持たないので中断も再開も無い）。
  - ⭐ **いつも実績の右に立つ**（利用者の裁定 2026-09-19、`JDG-226`）: 再開日があれば再開日に、未定なら停止日の翌日（実績のすぐ右）に立つ。すでに実績のある期間には立たない —— 再開日は停止日の翌日より前にならない（押し戻す。表 PE の 13）。再開日の既定（未定のときアイコンが立つ日）は、いつも停止日の翌日である。
  - 札の並びには、未定でも入らない（今の `OR-2` を未定にも広げる）。`--->` のアイコンは 3 段目に立つので、1 段目の札（担当 : 進捗 → マーカー → 名前）と重ならない。
  - ⭐ `===` で「入らない」とき（`JDG-240`）: 札（マーカーと名前）は実績のすぐ右に置き、その並びが再開アイコンに届くときだけ、アイコンの掴む箱（そのタスクのマーカーの径 —— 表 GA の 20）の右端から並べる。未定のアイコンは実績のすぐ右に立つので、いつも 実績 → アイコン → マーカー → 名前 になる（`JDG-226`）。再開日が先のときは 実績 → マーカー → 名前 …（破線）… アイコン となり、破線は札の下を通る（札は地の色の縁で読める）。「入る」とき、マーカーと名前は実績の中、アイコンは実績の右に立つ。「入る」の判定は実績の幅のまま（アイコンの幅は足さない）。
  - これで今の R:1584（アイコンのために並びの中に場所を空けない、MUST NOT）と `JDG-92`（名前を形の近くに出す）は保たれる —— 札をアイコンの右へ送るのは、重なるときだけである。「いつも アイコンの右」（`JDG-226` の逐語どおり）は退けた —— 再開日が 3 週間先なら札も 3 週間右へ離れ、R:1584 を覆すため（`JDG-240`）。
  ...
  - 取り込んだ再開日が停止日以前のとき（交換相手は過去の再開日を残しうる。R:2600）は、値は持ったまま（往復）、アイコンは停止日の翌日に描く。停止日の翌日より前の値は、引く操作でも属性の欄でも停止日の翌日へ押し戻す（拒まない —— 今の `FR-103` の「新しい拒み方を立てない」を保つ）。不変条件にはしない。
  - 実績を隠すと（`S-228`）、再開アイコンと破線も描かず掴めない（`FR-044` に 1 文。`DM-14` と同じ形）。
- **今の MUST を残す**（一括で消すと黙って落ちる。見本はまだ持たない）:
  - 基準が予定（表 RF の 3）のときは、フェードを除いた幅で「入る」を数え、名前は `fadeIn` の終わりから書く（今の `FR-002`、名称をフェードの上に重ねない）。マーカーを出すときは、`fadeIn` の終わりとマーカーの右端 ＋「マーカー → 名前」の遠い方から書く（今の R:1415〜1417）。
...
- 名前と札の字は 1 色 —— 濃い字に地の色の縁 —— とし、濃い実績の中でも変えない（`JDG-246`。縁でどの地の上でも読めるので、表 LP に字の色の列を足さない。`FR-109` に 1 文）。
...
| 20 | `===`・`--->`（◆ には無い） | 再開アイコン | アイコンの箱（左端 ＝ 縦棒 ＝ 再開日の左の境目。未定なら停止日の翌日の左の境目） | 箱 ＋ 外側 0（独立の定数。箱はそのタスクのマーカーの径 —— `===` は `S-22`、線だけの形は名前の字の大きさ（今の R:1234〜1235）。未定のアイコンは小さく描くが、掴む箱はマーカーの径のまま —— `JDG-39`、今の R:3462） | 同左。`--->` の 3 段目では中線より下だけ（`JDG-185`） | 再開の折れ矢印（表 PK の 9） |
...
- フェードの掴み点（順 1）は、`===` のダミーの右半分の上に約 0.25px の帯で重なることがある。そこはフェードが応える（今の C4 の「境目の右ではダミーが先」、R:3382 をこの帯について覆す。掴み点は選んだタスクにだけ出る —— `FR-075`）。
...
- 引いているあいだの絵と、引いたときに書かない規則（今の `FR-016` の散文、R:3524〜3548）は、本表の外に残す。ただし今の文は `GR-*` の ID を並べて持つので、ID を表 GA・PE の行へ付け替える（`GR-*` を消すと列挙が宙に浮く）。足すもの: マーカーを引くあいだ（表 PE の 8・9）は、置くことになる実績の終了を描いて示し、離すまで書かない。再開アイコンを引くあいだは、アイコンと破線を離す日へ描き直す。直すもの: R:3533 が「引いても追従しない行」に `GR-7`（マーカー）を数える文は、表 PE の 8・9 で偽になる。

### 4.5 表 HT（応える手順）と 表 TY（種別の順）—— どれが応えるか（`JDG-150`・`JDG-190`・`MK-9a`）

この 2 つの表は**日程の形の中の順**だけを持つ。今の 表 T-023d は消えず、日程の形でないもの（パレットと面の境の掴み帯、名前・担当のダブルクリック、注記、行見出し、基準日線、スクロールバー）の順を持ち、日程の形の行を「表 HT に従う」の 1 行で持つ（下の表 T-023d の新しい並び）。

⭐ **2 つの表に割ってある**（利用者の裁定 2026-09-20。逐語は `CR-430` の冒頭）—— **表 HT** は「どの手順で 1 つに決まるか」（4 行）、**表 TY** は「同じ手順の中でどの種別が先か」（9 行）に答える。⛔ 語「段」は表 XS（線だけの形の縦の段）にだけ使い、本表の側は「**手順 1〜4**」と呼ぶ —— 用語集の `A-1` は「段」に 7 義を置き、そのどれでもない意味を持ち込まないためである（`CR-430` の決定 4。`A-1` には 表 XS の 1 義を足す）。

4 つの手順で決める。前の手順で 1 つに決まれば、後の手順は読まない。

| 手順 | 規則 |
|---|---|
| 手順 1 形の上 | 描いた形の上では、その形のタスクの掴み代だけが応える（`GS-1`）。描いた形は形ごとに読む（今の R:3408 の「予定と実績を描いた範囲の矩形」を置き換える。`JDG-236`）: `===`（と矢羽根）は描いた予定・実績・ダミーのバーそれぞれ、`--->` と端点スパンは予定の線・実績の線それぞれの帯（矢じり・点の縦幅を含む）、◆ は描いた図形、進捗マーカーと再開アイコンは描いた箱。形と形のあいだ（`--->` の 2 本の線のあいだ、短い実績の右の空き）は形の外である。依存線とハイライトボックスの枠は、形の上では描いた線そのもの（線の中心から 線の太さ ÷ 2。群 A ⑭）でだけ応える —— 依存線の描いた線は本線と矢じりであり、選んだ線は `S-178`（2）倍の太さで描いてその太さで取り、地の色の縁は含めない（今の R:3410〜3411・`SL-8`） |
| 手順 2 形の外の縦 | 形の外で 2 つ以上のタスクの掴み代に入ったら、描いた形が上下に近いタスクの掴み代だけを残す（今の `GS-2`。群 B ⑪）。依存線はタスクではないので残る |
| 手順 3 種別の順（表 TY） | 下の順で、先に来る種別の掴み代が応える。ただし進捗マーカーと再開アイコンの描いた箱の上では、実績の端の次にマーカー・アイコンが応え、ほかの種別 —— フェード（群 B ⑩、`JDG-178`）・依存線・予定の端・◆ の予定・本体 —— はその後へ回る（今の C8、R:3428「マーカーの描いた形の上では予定の端点より先」）。実績の開始がマーカーの左半分を取るのは `JDG-187` のまま |
| 手順 4 同じ種別 | 掴み代の中心までの直線の距離が近い方が応える（`JDG-190`・`JDG-217`）。距離が同じなら、日付の遅い方 —— 終了の側 —— が応える（今の `TE-1`・`TE-4` の「決められないときは終了」を広げる。描いた前後では決めない —— `FR-110`） |

**表 TY —— 種別の順**（上ほど先。手順 3 が読む）

| 順 | 形の上 | 形の外（隙間・外側の帯） |
|---|---|---|
| 1 | フェード | フェード（`JDG-218`。`GS-4` の例外） |
| 2 | 進捗マーカー（実績の外に立つ）・再開アイコン（この順） | 依存線（`GS-4`） |
| 3 | ダミー | 進捗マーカー（実績の外に立つ）・再開アイコン（この順） |
| 4 | 実績の端（◆ の実績を含む） | ダミー |
| 5 | 依存線 | 実績の端 |
| 6 | 予定の端 | 予定の端 |
...
- **段 3 のただし書き（群 B ⑩、今の C8）**: マーカーの描いた箱の上では、マーカーがフェードの掴み点・予定の端・本体に勝つ（`JDG-178` のまま。今の R:3428）。見本で測った: 実績が予定より 1 日早く始まると、予定の開始の外側 12px がマーカーの右半分の下に入る。ただし書きがフェードだけだった版では、右半分 7px のうち 5px で予定の開始が応えた（広げて 0px）。実績の開始に立つマーカーの左半分は、順 4 の実績の開始が順 7 のマーカーより先なので、実績の開始が応える（`JDG-187` のまま）。見本で測った: ① の行の点（予定の開始 ＋ 2、バーの上端 ＋ 3）は フェード入 → マーカー、⑤ の「作業」のフェード入を 1 日入れた点は フェード入 → マーカー に変わった。
...
**表 T-023d の新しい並び** —— 上ほど先。

| 順 | 対象 | 今の行 |
|---|---|---|
| 1 | パレットの掴み帯・面の境の掴み帯 | `GR-19` ・ `GR-22`（残す） |
| 2 | 名前・担当（ダブルクリックだけ。素の押下では成立しない） | `GR-10` ・ `GR-11`（残す） |
| 3 | 注記（コメント／ハイライトボックス） | `GR-14`（残す。今は依存線と予定の端のあいだ。日程の形より先へ移す —— 群 B ⑫、`JDG-210`）。ハイライトボックスの本体は枠線の近く（枠線から内と外へ 3px。新しい行）と四隅（隅から 6px。`S-230`）だけで（値は `JDG-248`）、囲んだ内側は下のタスクへ素通しにする（`JDG-231`。今の R:3354・R:3435〜3437 を書き直す）⇒ 先に置いても、中のタスクから押しを取らない。描いた形の上では、枠は描いた線そのものでだけ応える（依存線と同じ。群 A ⑭）。コメントボックスは、本体（本文の箱の全体。不透明に描くので素通しにしない —— `FR-108`）、引出し線（線から左右へ 3px。描いた形の上では描いた線そのものでだけ応える）、線先（アンカー。線先から 6px。描いた形の上でも 6px —— タスクやマイルストーンの上に置いて、どれへのコメントかを示す取っ手だから）を持ち、四隅を持たない（今の MUST NOT のまま）。本体と引出し線を引けば本文の箱が動き（`CM-51`）、線先を引けば指す先が変わる（`CM-50`。離した所の日と行で読む —— 今の R:3441 と `AR-5` のまま。タスクを名指す列は足さない。指す先のタスクを動かしても、線先はついて行かない —— `JDG-252`）（`JDG-249`） |
| 4 | **日程の形 —— 表 HT に従う** | `GR-1`〜`GR-7`・`GR-9`・`GR-12`・`GR-13`・`GR-15`・`GR-17`・`GR-18` を消して、この 1 行にする。再開アイコン `GR-8` は HT の中へ |
| 5 | 行見出しの行 | `GR-20`（残す） |
| 6 | 基準日線 | `GR-16`（残す。掴み代は今の `S-137` の 6px のまま —— `JDG-199` が 2px にしたのは依存線だけである。`S-137` を依存線と基準日線に割った後、基準日線の行として残る） |
| 7 | スクロールバーのつまみ | `GR-21`（残す） |

- 打ち切った名前の説明（今の `EZ-6`、R:5874「どのタスクの上かは T-023d の順」）は「表 T-023d と表 HT」を指す。名前・担当は素の押下では掴みとして成立しない（並び 2）が、説明の引き金には数える（`UC-001` の拡張 4a）。

### 4.6 表 PE —— 押す・引くの効果（`JDG-186`〜`JDG-188`・`JDG-196`・`JDG-204`）

- タスクに属する行（1〜11）は、押して離せばそのタスクを選ぶ（今の `SL-2`。それまでの選択は置き換える）。依存線（12）は線を選ぶ。再開アイコン（13）は押しても何も起きない（`JDG-222` の ⑥、`JDG-230`）。選択に含まれるものの本体を引けば、選択の全部が動く —— 横は同じ日数、縦は同じ行数（`SL-7`）。離した後も全部が選ばれたまま（`JDG-233`）。行の数は画面に描いた行で数え（今の `HB-3` と同じ）、最初の行より上か最後の行より下へ出るものがあれば、全体がそこで止まる。1 つだけ引いても同じ。選択にハイライトボックスが混ざっても全体が止まる（`HB-3` の「動かさず `RS-44` を告げる」は、ボックスだけを引いたとき）。今の `HM-10`（R:1779「移るのは掴んだ `Task` だけ」）と R:3528 を、選択の全部と ◆ の予定へ広げる（WBS の子は元の行に残る —— `HM-10` のその MUST は残す）。選択に含まれるタスクの端・フェード・マーカー・ダミー・再開アイコンを引いたときは、選択を掴んだ 1 つに絞り、それだけを動かす（今の `SL-7a`）。⚠️ 今のアプリは、離したときに選択を掴んだ 1 つに縮め、縦に移すのも掴んだ 1 つだけ（`DFC-664`）。
...
| 12 | 依存線 | 選ぶ | 何もしない | — |
| 13 | 再開アイコン | 何も起きない | 再開日（`resume`）を離した日にする。停止日の翌日より前で離したときは停止日の翌日へ押し戻す（拒まない）。`resumeValid` を真にする（`FR-044`）。未定のアイコンは停止日の翌日に立ち、引けばそこから数えて再開日を置く | — |
...
- 中断のあいだ、マーカーを引いても何もしない（行 10。引ける場面を中断の外に限った —— `JDG-234`・`JDG-226`）。停止日を動かすのは実績の終了の端（行 3）である。
- マーカーの押下で 完了 → 中断 にすると、再開日は未定になり、アイコンは停止日の翌日に小さく薄く出る。中断 → 未着手（下の「押下の巡り」）で再開日を消し、アイコンは消える。
- 乗せたときに濃くするもの（今の `FR-013` R:2942 の未着手のマーカー、`FR-048` R:4447 のダミー）は、表 HT でいま応えている掴み代のものとする —— 実績の開始がマーカーの左半分を取る所では、マーカーを濃くしない。
- 表示していない予定・実績は、どの行でも動かない（`FR-108`）。依存線を隠しているあいだは、選ぶ・ラグを直す・消すことができない —— 表示に戻してから行う（`FR-049` のまま。`JDG-245`）。用語集の `IC-81` の注（G:562「別の面に経路が残る」）を、これに合わせて直す（`DFC-660`）。
...
- 画像のポインタを描けない環境では、動く向きを示す環境の形に替える（`JDG-247`。今の `PC-6` の `ew-resize` を広げる）: 端の箱の矢印・フェードの三角・◆ の実績とダミーの ●・再開の折れ矢印は `ew-resize`、◆ の予定の ○ は `move`（横にも縦にも動く —— 表 PE 6）、依存線の線の矢印は `pointer`（押して選ぶだけ）。
- ポインタの画像には表示の倍率を掛けない（今の 表 T-264 の結びの MUST を残す）。
- 依存線の道具を構えているあいだは、端のポインタを当てない（今の 表 T-264 の結びの ①。構えている合図は `IN-2` の作図の合図）。
- ◆ の「掴めることの合図」（今の `IN-2`）は、○ ／ ● に置き換える。
- 引ける進捗マーカー（表 PE の 8・9）のポインタは、指のまま（押す役）とする。押す方が多く、押せば状態が巡ることを示すためである。引く役（実績の終了の矢印 → 黒）の形は退けた（`JDG-241`。`FR-106` に 1 文足した）。
...
| 7 | 進捗マーカー・再開アイコン | `ZO-3` を線より手前へ移す（群 B 8、`JDG-210`。今の `ZO-3` は線より奥）。マーカーは不透明な下地を持つ（今の `ZO-3`、R:2715 のまま） |
...
- 目盛は自分の帯に描くので、本表に入れない。最上段の字の箱が目盛の帯に約 1.25px 掛かる所は、今の表 T-012 の注（R:1090「最上段では目盛の下に隠れる」）のとおり目盛の下に隠れる（表 XS の注と同じ）。
- 行 ID: 今の `ZO-1`〜`ZO-6`・`ZO-1a` はほかの行から引かれている（R:2203 の `SL-3` → `ZO-6`、R:2755 の `GD-7` → `ZO-1a`、R:3312 の `MK-9a` → `ZO-4`）ので、ID を変えずに順だけを直し、新しい行は `ZO-7` から付ける。
...
設定の値の正は `docs/spec/_source/settings.json` であり、表 T-201・T-206、文書の型、既定の値は `npm run gen` で作り直す（検査 21〜22）。名前は用語集の表 T-104（鍵の行）と `display-words.json` が持つ。⚠️ 保存した文書は既定と同じ値でも全項目を持つ（今の R:5005）—— 既定を変えても、これまでに保存した文書は保存したときの値のまま開く（例: マーカーの既定を 22.4 にしても、保存済みの文書の `markerSize` は 16）。退役させる行と鍵（下の一覧）は、古い文書では知らない鍵として保たれる（今の `IX-3`・`OP-6`）。新しい鍵を持たない古い文書は、その鍵の既定で開く。

⭐ **`S-181`（`commentBoxPad`）の上限の理由を書き直す**（`CR-430` の決定 5）—— 今の文は上限を `labelPad`（`S-31`）に揃え「同じ『文字の周りの余白』」と書くが、本案は `S-31` の意味を「タスク名 開始 → 名前」に替える ⇒ 「上限は 表 T-201 が札に置く px の値（`S-31` ほか）と同じ 30px とする —— 別の上限を立てる理由が無い」に直す。**数は 30 のまま。**

**退役させる行と鍵**（使い手がどれも本案で書き直されるか消える）: `S-20`・`K-20`（`S-19` の 3 役を割る）、`S-23`・`K-23`（使い手 R:1392・1394・1397・1581・1585・2934 はどれも書き直すか消す）、`S-40`〜`S-42`・`K-40`〜`K-42` と `S-45`・`K-45`（使い手は `LF-7`・`LF-8` だけ —— 線だけの形の線と矢じりは独立の値になる）、`S-47`・`K-47`（点の径が独立の値になる）。`S-46`（矢じりの頭打ち）は残す。
...
| 群 | 定数 | 既定 | 使う表 | 今の行（置く表） |
|---|---|---|---|---|
| 掴み代 | 表 GA の 41 の値（対象 × 向き） | 表 GA のとおり | GA | `S-90`（予定の端）・`S-91`（実績の端）・`S-92`（フェード）・`S-137`（依存線）を割る。ほかは新しい行。注記の `S-230` は、ハイライトボックスの四隅だけの行にし、自分の値 6px を持たせる（今は既定を「`S-137`」と書き、コメントボックスのアンカーと共にしている —— アンカーの役は下の「コメントボックスの線先」の行へ割る。1 値 1 役。`JDG-248`・`JDG-249`） |
| 掴み代 | コメントボックスの引出し線 | 線から左右へ 3px | T-023d の 3 | 新しい行（T-206。`JDG-249`） |
| 掴み代 | コメントボックスの線先 | 線先から 6px | T-023d の 3 | 新しい行（T-206。今の `S-230` のアンカーの役を割る。`JDG-249`） |
| 掴み代 | ハイライトボックスの枠 | 枠線から内と外へ 3px | T-023d の 3 | 新しい行（T-206。1 値 1 役。本体を枠の近くにしたのは `JDG-231`、値は `JDG-248`） |
...
| 形 | 進捗マーカーの描く径 | 22.4（14px。`--->` は名前の字の大きさ） | XS ・ LP ・ GA | `S-22` を書き直す（T-201。`JDG-228`）: 既定 16 → 22.4、上限 `actualMin` → `actualMin ÷ actualOfPlan`（`S-4` の下限と同じ式 ＝ 予定バーが縦の倍率でどこまで縮んでも取る高さ。既定で 28）、上限の理由を「予定バーより大きくならないので、行の外へ出ない」に。下限 `fontMin` は残す。`RC-11`（R:6809 の「骨格で実物を見て選び直す」）を閉じる。`S-247` の注（S:369 の 10px ／ 5px）を 14px ／ 7px に。⚠️ 上限を `basePlanHeight` そのものにすると、それを上げてマーカーも上げたあと縦の倍率を下げたとき、予定は床で止まりマーカーだけがはみ出す（マーカーは倍率に追随しない。R:1211）ので、床の式にした（`JDG-228` の読みとして利用者が確かめた。見本は縦の倍率を持たず、予定バーを床より低くもできるので、床と予定バーの高さの低い方に結んでいる） |
...
| 依存線 | 線の太さ | 2.4（1.5px） | 経路 | `S-18`（今の既定 1.5。範囲の上限を「矢じりの幅 ÷ 2」に結び直す —— 矢じりの付け根が線の 2 倍より細くならないため。今の結び先 `S-19` は長さだけになる）（T-201） |
...
| 再開アイコン | マーカー → 未定のアイコン | —（使わない） | LP | `S-23` のこの用途は消す（未定のアイコンも札の並びに入らない。`JDG-226`）。ほかの使い手も残らないので、`S-23`・`K-23` を退役させる（§5 の頭の一覧） |
| 掴み代 | 再開アイコンの外側 | 0 | GA | 新しい行（T-206。今の `GR-8` は `S-22` の箱そのもの。未定のアイコンも同じ箱 —— 今の R:3462 を「描いた大きさによらず、そのタスクのマーカーの径（線だけの形は名前の字の大きさ）」と書き直す） |
...
| 積んだ行の応え方 | ⑤ の行を、応えた掴み代で塗り分けて。**引く刻みは 1px**（`CR-430` の決定 6） | HT ・ TY |
...
仕様案の問い（群 A〜F）は 2026-09-19 にすべて答えが出た（`JDG-209`〜`JDG-242`）。読み直し（2026-09-19、確認役が 38 件を指摘し、本案と対応表を直した）で見つかった 3 つ（群 G）も、2026-09-20 に答えが出た（`JDG-245`〜`JDG-247`）。その後の指示で注記の掴み代の値が決まり（`JDG-248`、群 H）、コメントボックスの問い（㊷）にも答えが出た（`JDG-249`）。**決めていないことは無い** —— 次は、この案から 1 本の CR を書き、仕様の体 1 体で仕様へ当てる（`handoff.md` §1.2 の Step 1b。新しいセッションで行う）。1〜7・8・9 の事実・推奨・代償は `handoff.md` §1.2 が持つ。

### 群 H —— 注記の掴み代（2026-09-20）

利用者の指示「ハイライトボックス、コメントボックスのつかみシロは線が3、角が6としろ。」（`JDG-248`）を当てた: ハイライトボックスの枠線は内と外へ 3px、四隅は 6px。四隅とアンカーは今の仕様で 1 つの値（`S-230`）を共にしているので、コメントボックスのアンカーも 6px になる。

| 問い | 中身 | 案 |
|---|---|---|
| ㊷（`JDG-249` で決まった） | コメントボックスの「線が 3」をどこに当てるか。コメントボックスには四隅が無く（今の MUST NOT —— 本文の箱の大きさは本文で決まる）、線は本文の箱の縁と、アンカーへの引出し線（今の R:4397「折れ線 1 種」）である | **推奨 A**: 本文の箱は箱全体で掴む（不透明に描くので、ハイライトボックスのように中を素通しにすると、見えないタスクを掴むことになる —— `FR-108`）。引出し線は掴む場所にしない。「線が 3」はハイライトボックスの枠に当てる ／ B: 引出し線も線の左右 3px で掴め、引けば本文の箱を動かす（本体と同じ）。引出し線がタスクの上を通る所では、依存線と同じく描いた線そのものでだけ応える ／ **答え**（利用者は A・B の外で決めた）: 本体は箱の全体で掴む。引出し線（線から左右へ 3px）と線先（線先から 6px）も掴める。線先を掴んで、どのタスクやマイルストーンへのコメントかを示せる。引出し線を引けば箱が動く（B の読み）。線先はタスクの上に置くものなので、タスクの上でも 6px で応える（群 F の読み 3 を線先について上書きする） |

### 群 G —— 読み直しで見つかった問い（2026-09-20 に答えが出た。どれも推奨どおり）

| 問い | 中身 | 案 | 見本 |
|---|---|---|---|
| ㊴ | 依存線を隠しているあいだに、依存線を選ぶ・ラグを直す・消すことができるか —— 今の仕様の中で食い違う: 用語集の `IC-81` の注は「別の面に経路が残る」、`FR-049` は「別の面を置かない。戻してから行う」（`DFC-660`） | **決まった A**（`JDG-245`）: できない（`FR-049` のまま。戻してから行う。見えないものは掴めない —— `FR-108`）。`IC-81` の注を直す ／ B: 別の面（例えば属性の欄の依存の一覧）から行える。`FR-049` を直す | 見本は A（隠すと構えも消すもできない） |
| ㊵ | 暗い実績の中に描く名前の色（`handoff.md` の「`FR-109` に要る」） | **決まった A**（`JDG-246`）: 1 色のまま —— 濃い字に地の色の縁（見本の形。どの地の上でも読める）。表 LP に列を足さない ／ B: 実績の中では白い字にする（表 LP に「字の色」の列を足す） | 見本は A（「要件定義レビュー」の名前は実績の中） |
| ㊶ | 画像のポインタを描けない環境での、替わりの形 | **決まった**（`JDG-247`）: 動く向きで選ぶ —— 端の箱の矢印・フェードの三角・◆ の実績とダミーの ●・再開の折れ矢印は `ew-resize`（今の `PC-6` のまま）、◆ の予定の ○ は `move`（横にも縦にも動く —— ㉛）、依存線の線の矢印は `pointer`（押して選ぶだけ） ／ 別案: どれも環境の既定の矢印 | — |

### 群 F —— 今の仕様と突き合わせて出た問い（2026-09-19 に答えが出た。どれも推奨どおり）

| 問い | 答え | 記録 | 当てた所 |
|---|---|---|---|
| ㊱ 再開日が先のときの札 | B: 札は実績のすぐ右に置き、並びが再開アイコンに届くときだけアイコンの右へ送る（未定のアイコンでは 実績 → アイコン → マーカー → 名前）。破線は札の下を通る。今の R:1584 と `JDG-92` を保つ。退けた A: いつもアイコンの右 | `JDG-240` | 表 LP の再開アイコンの段・§9 |
| ㊲ 引ける進捗マーカーのポインタ | A: 指のまま（押す役）。退けた B: 実績の終了の矢印 → 黒 | `JDG-241` | §2 の `FR-106`・表 PK の注 |
| ㊳ 端点スパンの点の重なり | A: 許す（既定で 1px。同じ日の端だけ）。退けた B: 間を点の径に合わせて広げる | `JDG-242` | 表 XS の注 |

読み（2026-09-19 に利用者が「全部あってる」と確かめた）:

1. ㉔ の「予定バーの高さ」は、予定バーが縦の倍率で縮みきった高さ（`actualMin ÷ actualOfPlan`、既定 28）である —— 倍率を下げても、マーカーが行の外へ出ないため（§5）。
2. ㉜ の「線ごとの帯」は端点スパンにも当てる（`JDG-236`）。`===` も描いたバーごとに読むので、実績が予定の外へ出た所の上下は形の外になる（今の仕様では形の上。表 HT の段 1）。
3. ㉗ の「枠の近くだけ」は、描いた形の上では描いた枠線そのものだけである（依存線と同じ。群 A ⑭）。コメントボックスの引出し線も同じ。⚠️ 線先（アンカー）だけは、後の `JDG-249` でタスクの上でも 6px で応えるに変わった（表 T-023d の新しい並びの 3）。
...
| 群 | 答え | 記録 | 当てた所 |
|---|---|---|---|
| A | 仕様へ戻す | `JDG-209` | 見本を今の仕様の値へ直した（⑬ ◆ の実績 0.5715、⑭ 形の上の線は描いた太さだけ、⑯ `--->` の断面、⑰ マーカーは実績の高さ、⑱ 線だけの形のマーカーは名前の字の大きさ、⑲ 「 : 」と 0.85 倍）。表 XS・LP・HT・定数を直した。⑰ は今の仕様の中で `FR-002` と `LF-11` が食い違うので `FR-002` を採った（`DFC-663`）。⚠️ その後、⑯ の線と矢じりは `JDG-221` で、⑰ のマーカーの段は `--->` について `JDG-212` で覆った |
...
| 問い | 答え | 記録 | 当てた所 |
|---|---|---|---|
| ㉔ 進捗マーカーの大きさ | 上限を予定バーの高さにし、既定は 22.4（14px） | `JDG-228` | §5（上限の式は群 F の読み 1。利用者が確かめた） |
...
㉔〜㉟ の答えを当てるとき、今の仕様のどの行を書き直し・消し・残すかを、体に今の仕様を読ませて数えた（12 の答えで 119 行。⭐ **行番号は 2026-09-20 に `3d342753` へ写し直した**（`CR-429` の着地後）。R ＝ `01-04-requirements.md`、D ＝ `05-07-design.md`、S ＝ `_assets/tbl-settings.md`、G ＝ `_assets/tbl-glossary.md`）。上の §2・§4・§5 は、この表に合わせて直してある。⚠️ S と G の表は生成物であり、正は `docs/spec/_source/settings.json` と `display-words.json` である（§5 の前文）。

| 答え | 書き直す | 消す（行き先へ運ぶ） | 足す | 残す（読み直しただけ） |
|---|---|---|---|---|
| ㉔ マーカー | `S-22`（S:93）、`RC-11`（R:6809）、`S-247` の注（S:369） | `TE-1`（R:3492。対応表のとおり） | — | `DM-3`（R:2804。表 T-240 の描く規則として残す）、`FR-094`（R:1212・1234）、S:60、`PI-5`（D:425） |
| ㉕ 端点スパンの点 | `LF-7`（D:1017）、`LF-8`（D:1018） | `S-47`（S:118）、`K-47`（G:231） | 点の径の行（T-201 と鍵） | `OC-10`（R:1541）、`SH-4`（R:1072） |
| ㉖ 再開アイコン | `FR-044`（R:2857・2861・2862 に 1 文ずつ）、R:3462、R:3514、`LF-11`（D:1021）、`LF-13`（D:1023）、`OC-4`（R:1535 の「場所」）、R:1554、R:3508〜3513（`GR-8`・`GR-12` の名を表 HT の順へ）、R:3458（◆ に再開アイコンを当てない —— GA 20 の「◆ には無い」へ） | `GR-8`（R:3350。HT の順 2 と GA 20 へ）、R:1585 | 薄さの行、色の行（表 T-236）、破線の太さと色、表 T-245 の再開アイコンの行、`S-228` の 1 文 | `OR-2`（R:1562。未定にも広げる）、R:1584（㊱ B、`JDG-240`）、`S-23`・`S-25`〜`S-29`、`K-28`・`K-29` |
...
| ㉘ `--->` のダミー | R:3372・R:3374・R:3476、`S-180`（S:290）、`S-131`（S:129）を「`--->` を除く」文に | 表 T-240 の掴みの行 `DM-5`・`DM-6`・`DM-10`・`DM-13`（GA・HT へ）。表 T-244（`TE-1`・`TE-4` → 表 HT の段 4、`TE-2` → 表 PE 4、`TE-3` → 表 GA の注） | 表 XS 14、GA 21・22 | 表 T-240 の描く規則 `DM-1`〜`DM-4`・`DM-7`〜`DM-9`・`DM-12`・`DM-14`（R:2802〜2814） |
...
| ㉜ 形の上 | R:3408（表 HT の段 1 へ運ぶ） | 表 T-261（`GS-1` → 表 HT の段 1、`GS-2` → 段 2、`GS-4` → HT の順。`GS-3` は `JDG-197` で覆り、GA の値は隣で縮まないので、その MUST NOT は作りで保たれる） | — | R:3407、R:3409、`DS-7`（R:6407） |
...
**群 H の答えで書き直す行**（`JDG-248`・`JDG-249`）: `GR-14`（R:3354）の場所に引出し線を足す。R:3436〜3443 に、ハイライトボックスの本体を枠線から 3px、コメントボックスの引出し線（引けば本文の箱が動く —— 本体と同じ `CM-51`）と線先（6px、タスクの上でも）を書く。R:3448 と `S-230`（S:306）を、ハイライトボックスの四隅だけの値に割る。残す: R:4397（引出し線は折れ線 1 種）、R:3441（線先は離した所の日と行で読む）、`AR-5`（R:3286。タスクを名指さない。線先をタスクに追従させないことは、利用者が 2026-09-20 に確かめた —— `JDG-252`）、R:3445 付近（コメントボックスに四隅を持たせない、MUST NOT）。

**群 G の答えで書き直す行**: 用語集の `IC-81` の注（G:562。㊴）、`PC-6`（R:6463。替わりの形の一覧。㊶）、`FR-109` に 1 文（字は 1 色、地の色の縁。㊵）。
...
- 読み直し（38 件）から、前の裁定と今の仕様で決まるものを書いた: 表 HT の段 3 のただし書きを予定の端・本体・依存線へ広げる（C8、R:3428）／ 段 4 の同点は終了の側（`TE-1`・`TE-4`）／ `--->` のダミーは 1 日の実績の形（`DM-4`）／ 再開日の下限は押し戻す（`FR-103`）／ 再開アイコンの箱はそのタスクのマーカーの径（R:1234〜1235）／ `S-18` の上限は矢じりの幅 ÷ 2 ／ `S-230` は 6px ／ 退役の一覧（§5）／ 表 ZO の行 ID ／ `===` のダミーの上のフェードの 0.25px は、フェードが先。
- `FR-109` の「3 段 ≦ 矩形」は端点スパンにも掛かる（既定で 23.4 ＜ 28 で満たす。§4 の頭の「形の記号」の定めによる）。
- ほかの裁定の要確認のうち、見本の振る舞いで決まるもの（`JDG-194` の値の結び付き、`JDG-200`・`JDG-206` のポインタの向きと押さえる点、`JDG-207` の縁の角）は、見本のとおりに書いた —— 見本が振る舞いの正本である（`handoff.md` §1.2）。

**古い裁定との食い違い**（`rulings.md` に記す）:

| 裁定 | 答え | 扱い |
|---|---|---|
| `JDG-92`（名前を形の近くに） | ㉖ の札の並び | ㊱ B で保たれる（`JDG-240`） |
| `JDG-39`（再開アイコンの当たりはマーカーと同じ大きさ） | ㉖ の GA 20 | 保つ（群 F の読み 4。利用者が確かめた） |
...
**対応表（`old-to-new-map-ja.md`）**: 本節の表に合わせて直した（2026-09-19）—— 表 T-240 は描く規則を残して掴みの行だけ消す、表 T-244・T-261 は行ごとに行き先へ運ぶ、`GR-8` は消して HT と GA 20 へ、R:1585 を足す案は消す、表 PE の番号を今の番号に、ポインタの表は PK。
