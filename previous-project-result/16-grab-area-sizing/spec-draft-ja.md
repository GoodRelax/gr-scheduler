...
- 拠り所: 見本 [`grab-area-sizing-sample.html`](grab-area-sizing-sample.html)（規則はすべて実装して測ってある）と、[`rulings.md`](../../docs/development-records/rulings.md) の `JDG-180`〜`JDG-247`
...
どの表にも親の上位要求があり、どの上位要求も表に届く（§7 の表）。本文は `handoff.md` §1.2 で利用者が了承した案に、2026-09-19〜20 の裁定（`JDG-194`〜`JDG-247`）を足したものである。
...
| 群 | 定数 | 既定 | 使う表 | 今の行（置く表） |
|---|---|---|---|---|
| 掴み代 | 表 GA の 41 の値（対象 × 向き） | 表 GA のとおり | GA | `S-90`（予定の端）・`S-91`（実績の端）・`S-92`（フェード）・`S-137`（依存線）を割る。ほかは新しい行。注記の四隅の `S-230` は、自分の値 6px を持たせる（今は既定を「`S-137`」と書いている。値は同じ 6px。1 値 1 役） |
...
仕様案の問い（群 A〜F）は 2026-09-19 にすべて答えが出た（`JDG-209`〜`JDG-242`）。読み直し（2026-09-19、確認役が 38 件を指摘し、本案と対応表を直した）で見つかった 3 つ（群 G）も、2026-09-20 に答えが出た（`JDG-245`〜`JDG-247`）。**決めていないことは無い** —— 次は、この案から 1 本の CR を書き、仕様の体 1 体で仕様へ当てる（`handoff.md` §1.2 の Step 1b。新しいセッションで行う）。1〜7・8・9 の事実・推奨・代償は `handoff.md` §1.2 が持つ。
