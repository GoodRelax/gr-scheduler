...
| 10 | `===` | 再開アイコン（再開日あり） | 腕を実績の帯の中心線に置く（箱の中心 ＝ 中心線。箱の下端はマーカーの下端に揃う —— `LF-13`）。縦棒は再開日の左の境目 | 箱 ＝ マーカーの径 |
...
- **再開アイコン（案）**: 中断のあいだだけ描く（`FR-044`）。マイルストーンには描かず、掴み代も持たない（今の `LF-11` ／ `GR-8` の MUST NOT のまま —— 点は期間を持たないので中断も再開も無い）。
  - ⭐ **いつも実績の右に立つ**（利用者の裁定 2026-09-19、`JDG-226`）: 再開日があれば再開日に、未定なら停止日の翌日（実績のすぐ右）に立つ。すでに実績のある期間には立たない —— 再開日は停止日の翌日より前に置けない（表 PE の 11）。再開日の既定（未定のときアイコンが立つ日）は、いつも停止日の翌日である。
  - 札の並びには、未定でも入らない（今の `OR-2` を未定にも広げる）。⭐ `===` で「入らない」とき、札の基準の終了を**再開アイコンの右端**まで延ばす —— 並びは 実績 →（破線）→ 再開アイコン → マーカー → 名前 となり、マーカーと名前は破線とアイコンの上に載らない。「入る」とき、マーカーと名前は実績の中、アイコンは実績の右に立つ。「入る」の判定は実績の幅のまま（アイコンの幅は足さない）。`--->` のアイコンは 3 段目に立つので、1 段目の札（担当 : 進捗 → マーカー → 名前）と重ならない。今の仕様は、アイコンと名前が重なりうるのに何も決めていない（`OR-1` の 4 つにアイコンは入らない）。
  - ⚠️ 消えるもの: 今の表 T-243 の結び（未定のアイコンは札の並びに入り、名前はその右端から数える）と、`S-23` のうち「マーカー → 未定のアイコン」の用途。前の案（`JDG-222` の ③: 未定はマーカーのすぐ右、`--->` は 1 段目）も退く。
...
| 20 | `===`・`--->`（◆ には無い） | 再開アイコン | アイコンの箱（左端 ＝ 縦棒 ＝ 再開日の左の境目。未定なら停止日の翌日の左の境目） | 箱 ＋ 外側 0（独立の定数。箱はマーカーの径 ＝ 今の `GR-8` の `S-22` と同じ大きさ。未定のアイコンは箱も × `S-25`） | 同左。`--->` の 3 段目では中線より下だけ（`JDG-185`） | 再開の折れ矢印（表 PK の 9） |
...
| 6 | ◆ の予定 ／ 実績 | — | その日付だけ | — |
| 7 | 進捗マーカー（実績の右に立つ） | 状態を 1 つ進める（下の注「押下の巡り」。案 ㉟） | 実績の終了を動かす | — |
| 8 | 進捗マーカー（それ以外） | 状態を 1 つ進める（下の注「押下の巡り」。案 ㉟） | 何もしない | — |
| 9 | フェードの掴み点 | — | 入 ／ 出の日数 | — |
| 10 | 依存線 | 選ぶ | 何もしない | — |
| 11 | 再開アイコン（案） | 何も起きない | 再開日（`resume`）を離した日にする。停止日の翌日より前には置けない。`resumeValid` を真にする（`FR-044`）。未定のアイコンは停止日の翌日に立ち、引けばそこから数えて再開日を置く | — |
...
- 中断のあいだ、マーカーを引いても何もしない（行 8）—— 「入らない」ときマーカーは再開アイコンの右に立ち、「入る」ときは実績の中に立つので、どちらも実績の右端に接しない。停止日を動かすのは実績の終了の端（行 3）である（`JDG-226`）。
...
- ⭐ **押下の巡り（案 ㉟。利用者の指示 2026-09-19「・ に戻れるようにしろ」、`JDG-227`）**: 未着手 → 進行中 → 完了 → 中断 → 未着手 の輪（作業の進み順）。
  ...
  - 4 つの状態の 1 本の輪では、「未着手 → 完了」（`FR-011` の 1 押しの完了。`PV-1`）と「進行中 → 完了」（`PV-2`）を両方 1 押しに保ったまま未着手へ戻ることはできない（輪の中で完了の 1 つ前に立てる状態は 1 つだけ）。案は、最も多い押下である「進行中 → 完了」を 1 押しに残す。
  - 未着手 → 進行中: 覚えている実績があれば戻し、無ければ予定の開始に 1 日の実績を置く。中断 → 未着手: 実績を外し、開いているあいだ覚える（保存しない。取り消しでも戻る）。◆ は 未着手 ⇄ 完了 の 2 つを巡る（完了 → 未着手 で実績の日を覚えて外す）。
  - `PV-4` の MUST NOT（未着手へ戻さない）は、利用者の指示そのもので覆る —— どの輪を採っても同じであり、問い ㉟ で決めるのは輪の形だけである。
  - 代償（進み順の輪）: 1 日の作業の完了は 2 押し（未着手 → 進行中 → 完了）。中断からの再開も未着手を通る 2 押し。`FR-011` の「未着手を 1 押しで完了」、`JDG-214`、`JDG-187` の「押下で絵も実績も変えるな」の一部を覆す。図 F-018 は描き直す（図を 2 枚にしない）。見本は ② の選択で表 T-021a にも戻せる（比べるため）。
...
| マーカーの状態図 | 押下の巡り（㉟）が決まれば、今の 図 F-018 を描き直す（図を 2 枚にしない） | PE |
...
CR（Step 1b）は、㉔〜㉖、群 E（㉗〜㉞）、㉟ の答えを待つ。群 A・B・C・D は 2026-09-19 に答えが出た。1〜7・8・9 の事実・推奨・代償は `handoff.md` §1.2 が持つ。残りは Step 1a の対応表（[`old-to-new-map-ja.md`](old-to-new-map-ja.md)）で見つかった。
...
| ② マーカーの押下 | 表 T-021a のまま（A）。⚠️ 後の指示「・ に戻れるようにしろ」で巡りを決め直す（下の ㉟） | `JDG-214` ／ `JDG-227` |
...
| ㉖ | 再開アイコンの案（表 XS 10〜13・LP・GA 20・HT・PE 11・PK 9・ZO 4 と 7）をこのまま採るか。要点: いつも実績の右に立つ（再開日があれば再開日、未定なら停止日の翌日。`JDG-226` で利用者が決めた）／停止日から破線で繋ぐ／未定は小さく（`S-25`）薄く（0.55）／札の並びに入らない —— `===` で名前が入らないとき 実績 → アイコン → マーカー → 名前／ポインタは再開アイコンと同じ折れ矢印（小さめの 12px。利用者の指定）／実績かマーカーを隠すと描かず掴めない／`--->` の中断中の実績は停止日で矢じり無しに止まる | 案のとおり |
| ㉟ | マーカーの押下の巡り（`JDG-227`。表 PE の注「押下の巡り」）: 進み順の輪 未着手 → 進行中 → 完了 → 中断 → 未着手 を採るか。`PV-4` の MUST NOT は指示そのもので覆る（どの輪でも）。この輪の代償は 1 日の作業の完了が 2 押し、中断からの再開も 2 押し（`FR-011` の「未着手を 1 押しで完了」を覆す）。ほかの輪（未着手 → 完了 → 中断 → 進行中 → 未着手）は「進行中 → 完了」が 2 押しになる —— 最も多い押下が重くなるので採らない | 進み順の輪（表 T-021a にも切り替えられる） |
