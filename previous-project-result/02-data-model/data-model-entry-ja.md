...
> 🔴 **実績・進捗の部分は `../07-plan-actual/plan-actual-decisions-ja.md` が上書きする。**
...
- **マルチバー** = `TaskGroup` ＋ `TaskGroupMember`（**GRS 専用・非 export**）
...
> （`../03-ui-naming/ui-parts-ja.md` §4）。**数は目視で書かないこと。**
>
> 書かないものが 4 つある。
>
> | 書かないもの | 理由 |
...
> `taskUid: 3` は人が選んだ色なので**`themeHue` を変えても動かない**
> （ただし `themeMonochrome` は描画時に効くので**無彩色で描かれる**。`grs-document-settings-ja.md` §4-2）。
> **両方の形をわざと 1 つずつ載せてある。**
...
| `"wbsParentUid": 1` が 3 件 | **WBS**。企画・設計・検証は「製品A」の子 |
| `taskGroupMembers` が 3 件で同じ `groupId` | **マルチバー**（`TaskGroupMember`）。**1 行に 3 つのタスク**が載る＝製品の核 |
...
|:--:|---|--:|--:|--:|---|
| 2（マイルストーン） | 5/15 の 1 点 | **0** | **0** | **100** | 予定の期間が 0 のマイルストーンは `actualDuration` も 0。率は **0 か 100 のみ**（`../07-plan-actual/plan-actual-decisions-ja.md` §1-5） |
...
2. **【決着】テキスト列は `name` ＋ `notes` の 2 つだけ**（ユーザー確定 2026-07-26）。現行仕様はテキスト列を 5 つ持っていた（略称・正式名称・説明・備考・メモ）が、MSPDI 側は `Name` と `Notes` の 2 つしかない。**`abbrev`（略称）は廃止し、アイコンに描くラベルは `Task.name` を使う**。`fullName` / `description` / `remarks` も廃止。詳細は `property-mspdi-mapping-ja.md`。
