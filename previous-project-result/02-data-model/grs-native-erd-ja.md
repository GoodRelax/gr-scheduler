...
> 🔴 **予実・進捗まわりは `../07-plan-actual/plan-actual-decisions-ja.md` が上書きする。**
...
- **Resource / Assignment は「軽量ネイティブ」**（§5.5）。**担当者名をバーに表示する**ため、`Resource` の 5 列（`Name` を出し、`Type`/`IsCostResource` で出す相手を絞り、`CalendarUID` で暦を辿る）と `Assignment` の 3 列、**計 8 列だけ**を理解し、**残りは全て Carry** で温存する。資源管理（工数・コスト・平準化）は引き続き非対象。
  **担当者の割当は GRS で編集し MSPDI へ書き戻す**（§5.5a-2）。**そのために増える列は無い。**
...
**出自の保持（確定・別テーブルに持つ）**: **`TaskOrigin{ task_uid, source_project_uid, source_uid, import_session_id }`** に保持する（**`Task` には置かない**）。
...
**表示の経路**: `Task` →（`Assignment.task_uid`）→ `Assignment` →（`resource_uid`）→ `Resource.name` をバーに表示。`type=1`（作業）のみを担当者として扱い、材料資源は除外する。**MVP で割当の追加・編集・解除を行い、MSPDI へ書き戻す**（§5.5a-2）。
...
**情報源は `Assignment` → `Resource.name` だけ**である。`Task` に自由文字列の担当者プロパティは**持たない**（§4 原則 6「Task 無汚染」。**列は増えない**）。**編集は `Assignment` と `Resource` の行に対して行う**（§5.5a-2）。
...
### 5.5a-2 担当者の編集と MSPDI への書き戻し（確定 2026-08-05）

**要求**: `../user-order.md` 項 69。**GRS で追加・編集した担当者が MSPDI に反映される。**

**編集の単位は割当（`Assignment`）である。`Task` に担当者の列は足さない**（§4 原則 6「Task 無汚染」）。

#### 何ができるか

| 操作 | 何が起きるか |
|---|---|
| **選ぶ** | 文書内の `Resource` のうち **`type = 1` かつ `is_cost_resource ≠ 1`** のものから選び、`Assignment` を作る |
| **新しい名前を入れる** | 同名の `Resource` があればそれを使い、無ければ `Resource` を作ってから `Assignment` を作る（下記） |
| **名前を変える** | `Resource.name` を書き換える。**その資源を使う全タスクの表示が変わる**ので、対象の件数を人に知らせる |
| **解除する** | その `Assignment` の行を削除する。**`Resource` は残す**（他のタスクが使っていることがある） |

**同名の判定は §5.4 の自動統合と同じ**（`Name` が非空かつ NFKC 正規化 ＋ trim 後に完全一致）。
**判定を 2 つ持たない。**

#### 新規に作る行の中身

```
Resource      uid  = uid_high_water_mark + 1        §5.3
              name = 入力された文字列
              type = 1                              担当者として表示されるために要る
              他は全列 null                          §5.5d-4
Assignment    uid  = uid_high_water_mark + 1
              task_uid / resource_uid を埋める
              他は全列 null
```

#### 編集しないもの

**`type` / `is_cost_resource` / `calendar_id` / 割当率 `Units` は編集しない。** 取り込んだ値をそのまま保つ。
資源管理（工数・割当率・稼働率・コスト）は**引き続き対象外**である（`../user-order.md`「やらないこと」）。

**どの `Assignment` からも参照されなくなった `Resource` を自動削除しない。**
削除は人が明示的に行い、連鎖は §5.5c に従う。

#### なぜ列が増えないか

**軽量ネイティブの 8 列（`Resource` 5 ＋ `Assignment` 3）で足りる。** 編集に要る列は全て既に Own / Consume である。

新規行は Carry を持たないので、**`null` でない列だけが書き出される**（§5.5d-4）。
正本 XSD で確かめたところ、**`Resource`（子要素 71）も `Assignment`（子要素 265）も必須は `UID` だけ**なので、
これで **XSD 妥当な XML になる**（`Assignment` は `TaskUID` / `ResourceUID` すら `minOccurs="0"` である）。
出典 `https://schemas.microsoft.com/project/2007/mspdi_pj12.xsd`。

> ⚠️ **MS Project 側の挙動は未検証である。** 上は「妥当な XML になる」ことまでしか言っていない。
> **MS Project が `Units` / `Work` を勝手に埋めるかどうかは XSD からは分からない。**
> 埋めるなら `Task` の Carry（工数・コスト）と食い違い、**往復無損失が壊れる。**
> 実機で確かめること ＝ `../OPEN-ITEMS-ja.md` **4**。
...
### 5.5d Carry ストア設計（確定・エンティティ別バッグ ＋ 入口/出口の検査）
...
**依存線の経路は保存しない（確定）**: 依存線は**全て自動配線**で、**ユーザーは手操作しない**。**経路の規則は `../03-ui-naming/ui-detail-spec-ja.md` §4-9 が正**（2026-07-30 確定）。**ここには幾何を書かない。** ~~9 点アンカー・折れ点 0-3・重なり最小化~~ という旧記述は**誤り**。したがって経路は毎回エンジンが算出すれば足り、**`DependencyRoute` テーブルは廃止**した（当初案を削除）。`Dependency` が持つのは論理（先行/後続・種別・ラグ）のみ。
...
**疎な上書きパターン（確定）**: 「原則自動・人が触る場合あり」の列は、**全件保存せず `null`=自動 / 値あり=人の上書き**とする。既定は常にエンジンが算出し、上書きは例外として少数だけ保存される。該当: `TaskVisual.nameAnchor`/`nameAlign`、`TaskGroup.height`、`TaskGroupMember.stack_order`、**`TaskVisual.fillColor`/`strokeColor`**、**`TaskGroup.color`**、**`HighlightBox.strokeColor`**（§5.8）。**`null` を何で埋めるかは列ごとに違う**（下記「`null` の解決先」）。
...
- **色を既定へ戻す操作は `null` を代入する**ことである。UI の入口は `../NEXT-STEPS-ja.md` 2-4 で決める。
  **機械向けの口では `update-*` に `null` を送る**（部分更新。`../10-agent-interface/agent-interface-spec-ja.md` §3-1）。

**色の値域は 3 種 — 確定 2026-08-06**

```text
null           選んでいない → テーマから解く
'transparent'  人が透明を選んだ
'#rrggbb'      人が色を選んだ
```

- **`null` は「選んでいない」であって「透明」ではない。** 透明は**選んだ結果**なので値の側に置く。
  `'transparent'` は文字列判別値なので kebab-case（`../03-ui-naming/ui-parts-ja.md` §1-2。1 語なので区切りは現れない）。
- **塗りと輪郭を同時に透明にはできない。** 後から選んだ側を優先し、他方をテーマへ戻す。
  両方を透明にすると**そのタスクが画面から消えて掴めなくなる**。`../user-order.md` 項 25 の
  「色以外にも識別手段」は**線の太さと形状**だが、**どちらも図形が描かれることが前提**である。
  **塗りだけ透明（＝輪郭だけの図形）は正当な表現なので許す。**
- **透明の要素は「その下にある色」を相手にコントラストを判定する**（下限 3:1 ＝
  `../07-plan-actual/plan-actual-decisions-ja.md` §2-6）。
  ⚠️ **未検証である。次期の色 PoC で実測する。推測で値を書かない。**
- ⚠️ **SVG 出力で透明をどう書くか（`fill="none"` か `fill="transparent"` か）は決まっていない。**
  本資産に記述が無い。次期が決める。
...
| 案 | 却下理由 |
|---|---|
| 空文字 `''` | 「書き忘れ」と区別できない（§5.5d-4 と同じ理由） |
| 8 桁 hex `#00000000` | アルファを導入すると**半透明を止める理由が無くなる**。半透明は下地と混ざるので、コントラストの下限が下地しだいで壊れる。`../user-order.md` 項 23 は透明 1 段しか求めていない |
| 真偽の列 `fillTransparent` を新設 | **色を決める場所が 2 つになる**（`../user-order.md` 項 66「1 概念 1 語」違反）。`fillColor='#fff'` かつ `fillTransparent=true` という矛盾状態を表現できてしまう |
| `null` を「透明」に入れ替える | **致命的。** 既定が `null` なので、**色を選んでいない全タスクが一斉に透明になる** |

**`null` の解決先は列ごとに違う — 確定 2026-08-06**

**疎な上書きの型は 1 つのまま**である。違うのは「`null` を何で埋めるか」だけである。

| 列 | `null` の解決先 |
|---|---|
| `TaskVisual.fillColor` / `strokeColor` | **テーマ**（`themeHue` から解く） |
| `TaskGroup.color` | **テーマ**（同上。行の帯の色 ＝ `grs-document-settings-ja.md` §4-2） |
| `HighlightBox.strokeColor`（§5.8） | **注記用の固定色**（テーマから独立） |

- **注記だけテーマから外す理由**: `../user-order.md` 項 45 の目的は「**一部を強調**」である。
  テーマ色相に追随させると**日程表と同じ色になって強調にならない**。
  `../07-plan-actual/plan-actual-decisions-ja.md` §2-6 が依存線とイナズマ線を
  「**テーマ色相からも離す。予定・実績と同系色だと本体に溶ける**」として固定色にしたのと**同じ判断**である。
- **常に具体色を焼き込む案は採らない。** 解いた結果を保存すると、規則を直したときに古い値が残って
  **規則と絵が食い違う**（設定値キー `progressLineColor` を廃止したのと同じ事故。
  **廃止の記録は `grs-document-settings-ja.md` §4-1**、保存しない原則は同 §4-2）。
- **`HighlightBox.strokeColor` に透明は無い。** 枠線が**この注記の唯一の描画要素**なので、
  透明にすると注記そのものが消える。**塗りと輪郭を同時に透明にできないのと同じ理由**である。
- **`Comment` には色の列が無い**（`leaderShapeKind` / `text` / 位置だけ ＝ §5.8）。**決めるのは 1 列だけ**である。

> ⚠️ **注記用の固定色の値は未定である。** 制約だけ先に置く —
> **依存線の橙・イナズマ線・`themeHue` のいずれとも色相を離し、ライト / ダークの両方で地に対し 3:1 以上。**
> **次期の色 PoC で確定させる。推測で書かない。**

> **この決定が開いた未決 5 件は、すべて決着した — 確定 2026-08-06（ユーザー判断）。**
>
> | # | 何が未決だったか | 決着 | 所在 |
> |:--:|---|---|---|
> | 1 | 「透明」をどの値で表すか | **`'transparent'`**。`null`（選んでいない）とは別の値である。**塗りと輪郭を同時に透明にはできない** | 本節「色の値域は 3 種」 |
> | 2 | 行の帯の色の正がどこにも無い | **`themeHue` から解く。正は設定値の側**。入力は**深さと行の位置**の 2 つで、**縞は行の位置で数える** | `grs-document-settings-ja.md` §4-2 |
> | 3 | `themeMonochrome` を on にしたとき人の指定色はどうなるか | **描画時のフィルタである。** 保存値は変えず、**人の指定色も無彩色で描かれる** | 同 §4-2 ／ `../07-plan-actual/plan-actual-decisions-ja.md` §2-6 |
> | 4 | `HighlightBox.strokeColor` は疎な上書きにするか | **する。ただし `null` の解決先は注記用の固定色**（テーマから独立） | 本節「`null` の解決先」／ §5.8 |
> | 5 | 機械向けの口で色を既定へ戻す方法 | **`update-*` は部分更新である。** `null` を送れば既定へ戻る。**新しいコマンドも新しい拒否理由も作らない** | `../10-agent-interface/agent-interface-spec-ja.md` §3-1 |
>
> ⚠️ **残るのは 4 件で、いずれも規則ではなく実測・表記である** — 透明時のコントラスト判定 ／
> モノクロ時のコントラスト ／ 注記用の固定色の値 ／ SVG 出力で透明をどう書くか。
> **次期の色 PoC が出す。推測で埋めない。**
...
> **`../03-ui-naming/ui-detail-spec-ja.md` §6-1 が正。ここには複製しない。**
...
>   （`../03-ui-naming/ui-detail-spec-ja.md` §4-9）。詳細は下の差分表。
...
2. **唯一の根拠が消えた。** 特別扱いの理由は `user-order.md` 項 30-4 の括弧書き
   ...
   （`../03-ui-naming/ui-detail-spec-ja.md` §4-9）、**マイルストーンが上でも下でも配線は同じ形になる**。
...
| `TaskVisual.fillColor` | `null` / `'transparent'` / 色 | **`null`=テーマから解く**（人が選んでいない）。値=人が選んだ塗り色。**透明も選んだ値である** |
| `TaskVisual.strokeColor` | `null` / `'transparent'` / 色 | **`null`=テーマから解く**。**`fillColor` とは独立に判定する**。ただし**両方を同時に透明にはできない** |
| `TaskGroup.color` | `null` / `'transparent'` / 色 | **`null`=テーマから解く**。値=人が選んだ行色 |
| `HighlightBox.strokeColor` | `null` / 色 | **`null`=注記用の固定色**（テーマから独立）。**透明は選べない** — 枠線がこの注記の唯一の描画要素だから |
...
> 項目を足すときは台帳に追記する。**名前の正は `../03-ui-naming/ui-parts-ja.md` §2-1-6。**
...
（`../07-plan-actual/plan-actual-decisions-ja.md` §2-4。記号が増えて見にくいときのため）。
...
| `cursorGuideMode` | **`guideCursorMode`** | UI パーツ名の改名（`Cursor Guide` → **`Guide Cursor`**）を**データ項目にも及ぼす**。UI 名とデータ名を食い違わせない（項 66）。改名の理由は `../03-ui-naming/ui-parts-ja.md` §2-1-3 |

#### 廃止した項目 — **確定 2026-07-30**

| 廃止した項目 | 理由 |
|---|---|
| `planActualStyle`（`'overlap'` \| `'separate'`） | **上下分離表示そのものを廃止した**（`user-order.md` 項 52 は欠番）。予定バーの高さ > 実績バーの高さ で幾何的に解き、**幅がない 2 タスク形状（`arrow` / `endpointSpan`）だけ実績を下にずらす**。**マイルストーンは実績日の位置へ横にずらす**（確定 2026-08-01）。切替の設定は要らない。詳細は `../07-plan-actual/plan-actual-decisions-ja.md` §2-3 |
...
HighlightBox   ハイライトボックス
  ...
  strokeColor           枠線の色: null=注記用の固定色 / 値=人の指定（疎な上書き・§5.6）
                        ※透明は選べない（枠線がこの注記の唯一の描画要素）
...
> **MVP で割当の追加・編集・解除を行い、MSPDI へ書き戻す**（§5.5a-2）。割当率 `Units` は Carry のため**表示も編集もしない**。
...
> 根拠は `../07-plan-actual/plan-actual-decisions-ja.md` §1-1。
...
> 見分けが付かない。**穴は中身で呼ぶ**（`../03-ui-naming/ui-parts-ja.md` §1-3）。
...
当初、Carry 内の UID 参照（`Assignment.TaskUID` 等 3 つ）が UID 振り直しで壊れる問題に対し、「旧UID→新UID の再マップ表で該当フィールドを書き換える」方式を検討した。
...
| # | 論点 | 決着 |
|---|---|---|
| ~~1~~ | ~~**Carry ストアの設計**~~ | **解決済み**（§5.5d・エンティティ別バッグ「臭いものに蓋、入口と出口で検査」）。Drop=0 は機械検証の結果になった |
...
- MSPDI 事実・ERD: `../01-mspdi/mspdi-tables.md`, `../01-mspdi/mspdi-core-tree.md`（断捨離の経緯 ERD は **`previous-project-result/` に無い**。`../DISCARDED-ja.md`）
...
| 6 | 依存線の経路 | `DependencyRoute` に保存（自動＋手動上書き） | **廃止（保存しない）** | 依存線は**全自動配線で人が触らない**ため、毎回算出すれば足りる。保存すると再計算結果との二重管理＝ドリフト。経路の規則は `../03-ui-naming/ui-detail-spec-ja.md` §4-9 |
...
| 18 | **深い WBS（6 段以上）の扱い** | ①5 段にクランプ＋警告 → ②原レベルを退避して export で復元 → ③クランプしたまま export する | **クランプしない**（取り込んだ深さをそのまま保持し、そのまま書き戻す） | ② の副問題は 3 つ（復元は文書順に依存するため兄弟並べ替えで親が変わる／レベル飛びが生じる／「未編集」の判定単位が定義できない）。③ はこの 3 つを消すが**代わりにデータを捨てる**。**クランプしなければ 3 つとも発生せず、データも失われない**。深さは `wbs_parent_uid` の木から数えるだけなので、退避も復元も判定も要らない。5 段の根拠だった「推奨 3 段に対するマージン」は `user-order.md` 項 27 ＝ **`TaskGroup` の要求**であり、WBS へ流用していた（`../07-plan-actual/plan-actual-decisions-ja.md` §5-3「WBS と `TaskGroup` は別物」）。項 36 は「**書き戻す値は頭打ちにしない**」と明記している |
...
| 15 | **Carry の格納方法** | 「不透明に温存する」とだけ宣言（**格納設計なし**） | **エンティティ別バッグ ＋ 入口/出口の検査** | 影文書案（原 XML 丸ごと保持）は往復に最強だが**マージで破綻**し JSON が不透明になる。バッグ案は**入れ忘れで漏れる**（実際 `WeekDay.TimePeriod` で発生）。→ バッグに**自己検証と要素まるごと退避**を足して漏れを構造的に潰した。「**臭いものに蓋。ただし受ける時と出る時に検査する**」 |
