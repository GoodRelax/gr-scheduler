...
> 🔴 **本書の §2-2 / §3 / §4 は `../07-plan-actual/plan-actual-decisions-ja.md` が上書きする。**
> 変更の全数は次のとおり。
>
> | 本書の記述 | 確定 |
> |---|---|
...
| 4 | MSPDI に無い GRS 固有項目の格納は **固定枠の `ExtendedAttribute` ＋ 衝突検出 ＋ 未使用枠への退避**を採る（§4 で 3 案比較） |
...
| 8 | ~~`Stop` / `Resume` は MSPDI へ写さず拡張領域に置く~~ → **撤回**。**`Stop` / `Resume` / `ResumeValid` は MSPDI ネイティブで往復する**（Own）。拡張領域を使うのは `fadeInDays` / `fadeOutDays` の **2 つだけ**。`stop` は保存せず export 時に算出する（`../07-plan-actual/plan-actual-decisions-ja.md` §1-1 / §10-1 / §10-2） |
...
| `stop` | — | `Task/Stop` | `xsd:dateTime` | **保存しない** | 中断時の実績バー右端と同じ値。**export 時に算出して書く**（`../07-plan-actual/plan-actual-decisions-ja.md` §1-1 / §10-1）。**中断しているときだけ書く**（中断していないタスクに書くと相手が分割と誤解する） |
| `milestone` | bool | `Task/Milestone` | `xsd:boolean` | **Own** | マイルストーン判定 |
| 担当者（`Task` の列ではない） | — | `Resource/Name` ← `Assignment` 経由 | `xsd:string` | **Own**（`Resource.Name`） | `Task` に自由入力欄は持たない。**割当の追加・編集・解除はできる**（`grs-native-erd-ja.md` §5.5a-2）。**経路の `Assignment.TaskUID` / `ResourceUID` は Consume**。両者を混ぜないこと |
...
| 割合 | `PercentComplete` | `xsd:integer` | **期間**の完了率（**整数・0 以上**。XSD に上限は無く、GRS も頭打ちにしない。`../07-plan-actual/plan-actual-decisions-ja.md` §1-4） |
...
| 表示 | 担当者名 | `Assignment` → `Resource.Name` から導出。**割当は編集できる**（`grs-native-erd-ja.md` §5.5a-2）。`Task` の列ではない |
...
> **上の「管理する項目」と「不変条件」は `../07-plan-actual/plan-actual-decisions-ja.md` §1-1 / §1-2 に従う。**
> 本節が独自に持つのは**「持たない項目」（GRS が受け取らない MSPDI 要素）**だけである。

**入力規則（5 状態）**

**`../07-plan-actual/plan-actual-decisions-ja.md` §1-3 の表に従う。ここには複製しない。**
...
完了                       → ActualFinish 必須。RemainingDuration は 0。
                             ...
                             （正は ../07-plan-actual/plan-actual-decisions-ja.md §1-4 / §10-1）
...
固定枠方式を基本とする方針は決まった。以下は**その中で決めるべき 8 点**である。
...
これを判定に入れないと、**往復ごとに退避が起きて枠が毎回変わる**（**空き枠を動的に確保する方式**と同じ非決定性に落ちる）。
...
#### JSON 往復の検査（2026-07-31 追加）

**`documentSettings` は MSPDI に出ない**ので、上の 5 件（MSPDI 往復）は設定値の影響を受けない。
...
3. **`Stop` / `Resume` / `ResumeValid` は MSPDI ネイティブで往復する**（Own）。**拡張領域には置かない**（§3-4 #8 は撤回した）。
   `stop` は**保存せず** export 時に算出する。`../07-plan-actual/plan-actual-decisions-ja.md` §1-1 / §10-1 が正。
