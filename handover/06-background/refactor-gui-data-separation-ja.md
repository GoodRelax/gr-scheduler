...
> | `progressRatio` | **`percentComplete`**（**整数・0 以上**。上限を型に持たせない） |
...
> 🔴 **本節の手順は実行しない。** 本プロジェクトは反省・引継モードに移行し、
> **コード / 仕様書はフリーズ、CR は起票しない**。`CR-017` は**起票されていない番号**であり、
> 記録番号は無視してよい（`../README.md` §0-4）。
> 凍結済みのゲート（`strictdoc` / `playwright` ほか）も回さない。
> **次期は自分のプロセスを回すこと。** 以下は当時こう提案した、という記録である。

これは三状態モデルを含む**再アーキテクチャ**であり、単発の変更要求を超える、という認識だった。当時の提案:

1. 本文書をベースに設計を起こす (GUI コンポーネント木 = 設計、
   二層データモデル = データ形式契約、命名 = 用語集)。
2. 影響が広いので親の変更要求を立て、子タスクに分解する。三状態モデル・用語正規化・注記分離は子。
3. 既存の 55+ 要求と V 字トレースへの影響を評価してから着手。
4. 各節目で全ゲートを回す。
