...
> | 用語集を `docs/glossary.md` に新設する | **作らない。** 用語の正は 4 つだけ（`handover-ui-parts-ja.md` / `grs-native-erd-ja.md` / `handover-plan-actual-decisions-ja.md` / `grs-document-settings-ja.md`） |
>
...
- `00-overview.sdoc` §4 は、後続の設計作業で**リンクだけの節へ置き換える**。
  本セッションではまだ触らない（未レビューの仕様変更を入れず、ゲートを緑に保つため）。
- 副作用: 生成される仕様 HTML から用語表が消える。公開方法は §7 Q-5 で確認したい。

> 🔴 **この手順は実行しない。** `.sdoc` と生成ゲートは凍結済みで、`CR-017` は**起票されていない番号**である
> （`../README.md` §0-4）。当時こう提案した、という記録として読む。
...
## 7. 残る質問（当時、これが埋まれば変更要求を起票できるとした）
