...
AA 適合に要るのは A + AA = 50
  ...
  方針あり  14  満たし方が handover にある。実装はまだ無い
  要対応    21  満たし方が書かれていない ← 残作業
```

**2.5.5 Target Size は AAA なので AA 判定に数えない**（1-1 ③ でタッチを対象外にしたので確定）。

> ⚠️ **「方針あり 14」は「適合 14」ではない。** 前プロジェクトの実装は引き継がないので、
> **50 基準のうち実装として存在するものは 1 つも無い。**
>
> ⚠️ **要対応 21 件は次期の実装作業であって、handover の欠落ではない。**
> **何を満たす必要があるかは全数そろった。** 適合を宣言できるのは実装して検証したあとである。
