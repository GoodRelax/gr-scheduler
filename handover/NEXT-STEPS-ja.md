...
type: Open Items
title: 実開発ステップ別の欠落一覧
description: 次期の実開発で埋めるべき 19 件を、どのステップで埋めるかまで割り当てた一覧。
tags: [open-items]
phase: packaging
status: stable
---
...
| | |
|---|---|
| **決まっていること** | **方針**＝「文字をさらに縮めるのではなく、**グループ LOD（軸B・`TaskGroup`）の深さ**を 1 つ減らす」（`07-plan-actual/handover-plan-actual-decisions-ja.md` §2-4-1・§5-3。軸は 2026-08-02 に確定）。フォント下限は 12px。**式を決めるときの制約 2 つも確定**（2026-08-02）— ①**分岐数を定数に焼き込まず実際の行数から求める** ②**発動点 `zoomY = 0.536` は定数にせず `basePlanHeight` / `fontOfActual` / `fontMin` から導く** |
...
> ✅ **「縦の余地を予約する」ことは既に確定している** — `07-plan-actual/handover-plan-actual-decisions-ja.md` §6-4
> （2026-08-02）。計算順序の手順 1（最小間隔の制約）に**行を跨ぐ縦の通り道を含める**。
> 根拠は PoC の実測で、案 C は**行内 57/57 が 1 パスで無交差なのに行跨ぎは 9/9 が破綻**した
> （`08-poc/POC-RESULTS-ja.md` §6-2）。原因は最小間隔が横方向にしか作られていなかったこと。
>
> ⚠️ **確定したのは「余地を予約する」ことだけで、経路の引き方（上の ①〜⑤）は未解決のままである。**
> 縦の余地は**解き方が何であれ要る**ので先に確定させた。**横だけの制約で組むと同じ破綻を踏む。**
