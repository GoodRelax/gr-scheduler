...
>
> ⚠️ **Firefox はこの機に入っていない**（2026-08-02 時点）。動作確認の工程を回す段で導入が要る。

### 1-2. WCAG 2.1 AA の適用範囲

| | |
|---|---|
| **決まっていること** | **AA を満たすこと自体は確定**（条件付きプロセスとして有効）。前プロジェクトが実装した範囲の対応表が `05-security-a11y/a11y-wcag21-aa-checklist.md` にあり、**14 行**（A / AA 12 件 ＋ AAA から参考採用 2 件）。`aria-live` / フォーカス順 / コントラスト等、個別の実装方針も同書にある |
| **決めること** | **同書は部分集合であって全数ではない。** WCAG 2.1 の **A / AA 全基準を列挙して同表と差分を取り**、どこまでを適合範囲とするかを決める。あわせて **2.5.5 Target Size（AAA）を AA 判定に数えるか**を決める（掴み代 6px・フェード掴み点 15×15px の根拠がこれに依存する。1-1 の ③ と同じ判断） |

> **「あの表が全部緑なら AA 適合」ではない。** 適合を宣言する前に全数の突合が要る。
> これはヒアリングで**適合範囲を要求として確定させる**話なので、ステップ 1 に置く。
...
| **決めること** | ① **算出規則を書いた節が無い**（余白の取り方・縦横どちらを優先するか・上下限に当たったときの振る舞い） ② **Fit で最下行が半分切れる**ことがある（縦ズームの下限に先に当たる。`08-poc/POC-RESULTS-ja.md` §7） |

> ✅ **「Fit と LOD しきい値の衝突」は先に片づいている** — `04-performance/handover-performance-notes-ja.md` 罠 **T-14**（2026-08-02）。
> **ズーム値を量子化する場所は LOD 閾値に対して切り下げる。** 算出規則が決まる前でも守れるので、規則を待たずに入れた。
