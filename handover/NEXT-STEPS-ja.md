...
### 6-2. ~~行をまたぐ依存線~~ → **一部決着（2026-08-02）**

**「行をまたぐ」と「段が 2 つ以上離れる」は決着した。**
垂直線はタスクを**横切ってよい**。避けようとしない（タスクの x は日付で決まるので避け先が無い場面がある）。
**読めることは重ね順で担保する** — 依存線は**バーより前面・名称ラベルより背面**
（`03-ui-naming/handover-ui-detail-spec-ja.md` §4-3 / §4-9）。**経路は 5 パターンのまま。**

| | |
|---|---|
| **決まっていること** | FS の経路 5 パターン・アンカー 2 点・走り・折れ点 0〜4・段間の通路（同 §4-9）。**横切りの扱いと重ね順**（同 §4-3） |
| **決めること** | 同 §4-9 の未決表に**残り 3 件** — ① 同じ段で後続が先行より**前に**始まる（完全な逆行） ② **マイルストーン**が先行 / 後続（幅 0 で右端＝左端） ③ **FS 以外**（`Type` 0=FF / 2=SF / 3=SS。アンカーが右辺・左辺でなくなる） |

> **③ は核機能である**（`user-order.md` 項 49「依存の種別（FS / SS / FF / SF）とラグを持てること」）。
> ①② は FS の中の端の場面なので、③ を決めるときに一緒に片づく可能性が高い。
