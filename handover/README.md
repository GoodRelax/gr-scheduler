...
| **10** | **`07-plan-actual/handover-plan-actual-decisions-ja.md`** | **予実・進捗の確定設計**。MSPDI の `Stop`/`Resume`/`ResumeValid` をそのまま使うモデル・4 状態・形状・Progress Marker・イナズマ線の頂点規則・Undo・命名 | **この領域はここが正**。⚠️ 8b と 9b の予実まわりを**上書きする**（同書 §11 に差分の全数） |
...
│   └── mspdi/
│       ├── mspdi_pj12.xsd           ★正本（MS Project 2007 MSPDI スキーマ。pj12 = Project 12 = 2007）
...
│                                    MSPDI 準拠モデル・4 状態・形状・Progress Marker
│                                    イナズマ線の頂点規則・Undo・命名
