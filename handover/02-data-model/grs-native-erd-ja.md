...
> **用語**: **外部 WBS マスタ**（以後「外部マスタ」）= **GRS の外側で WBS 構造を保持し、MSPDI を生成・再取込する対向ツール**。特定の製品を指す語ではない。往復規約は `grs-data-model-ja.md` §6 が正で、そこに**どの挙動を実機で検証したか**も記してある。
...
> ⚠️ **明示許容の損失（重要）**: この決定により、**6 段以上を含む文書では「深さ」の情報が失われる**。export すると **外部マスタの WBS も 5 段にフラット化される**（ユーザーが WBS を触っていなくても）。
