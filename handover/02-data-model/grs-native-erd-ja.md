...
- **文書外との衝突は GRS の責任範囲外**。具体的には「GRS が export した UID が、その後 iQUAVIS 側で採番された別タスクとぶつかる」ケース。**別ツールでの ID 衝突検査**を前提とする。
...
- 追加しないとマスタの更新が取り込めない
- **どの行（`TaskGroup`）に入れるか**は import の器の初期化ポリシー（`grs-data-model-ja.md` §7.1「サマリ配下の葉をそのサマリの器へ」）を再取込にも適用する
- 器が特定できない場合の落とし先（「既定行」）は**未決**（§8F ⑥⑦ で扱う）
...
> **通知が必須な理由**: 消さない方針なので、放置すると export 時に iQUAVIS 側で「存在しない UID」として**新規作成＝削除したはずのタスクが復活**する（ゾンビ）。通知がこれを防ぐ唯一の手段。
...
| **階層の妥当性** | `OutlineLevel` が「先頭=1・増分 ≤ +1」を満たすか | 復元を諦めて全体再生成＋警告（§5.6 クランプ対策） |
...
#### Lv5 クランプ（確定）— 潰したまま export する

**6 段以上は 5 段にクランプし、export でも復元しない。** クランプ後の木が GRS における**唯一の真実**（軸A）となる。

```
元:    工程(5) > 詳細(6) > 手順(7)
GRS:   工程(5), 詳細(5), 手順(5)      ← 6 段以上は 5 段目に並ぶ（本来の親子が兄弟になる）
export: そのまま Lv5 として出力       ← 復元しない
```

**クランプで変わるのは「親子関係」だけ**である。MSPDI の参照はすべて **UID** であり、UID はクランプしても不変なので:

| | 影響 |
|---|---|
| 依存関係（`PredecessorLink.PredecessorUID`） | **無傷**（UID 参照） |
| 担当（`Assignment.TaskUID` / `ResourceUID`） | **無傷**（UID 参照） |
| 開始・終了日／実績／進捗／メモ／中断 | **無傷**（Task の列で階層と無関係） |
| **親子関係（`OutlineLevel`）** | **6 段以上が 5 段目にフラット化される** |

**採用理由**: 推奨 3 段に対して**5 段はマージンを持たせた上限**であり、6 段以上は運用上やりすぎ。5 段にクランプしても実務上問題ないという判断。復元機構（原レベル退避／真の親の保持）を持たないことで、**順序依存・レベル飛び・「未編集」の判定単位という 3 つの副問題が発生しない**（実装・テストが単純に保たれる）。

> ⚠️ **明示許容の損失（重要）**: この決定により、**6 段以上を含む文書では「深さ」の情報が失われる**。export すると **iQUAVIS 側のマスタの WBS も 5 段にフラット化される**（ユーザーが WBS を触っていなくても）。
> - これは本設計で**唯一の明示的な Drop**である（→ §8D）。
> - したがって **import 時の提示は「情報」ではなく「予告」**とする: 「**この文書は N 段の階層を含みます。5 段に統合され、書き戻すと元の階層も 5 段になります**」と、影響を明示して取込の可否を選ばせる。
...
> 主要な入力元は iQUAVIS 等の**第三者生成 MSPDI** であり、同じカスタムフィールド枠を既に使っている可能性が現実にある。固定枠のみだと**他ツールのデータを黙って壊す**。
...
**WBS 深さクランプと export（④⑤ で確定・当初案は撤回）**

当初は「原 `OutlineLevel` を Carry に退避し、WBS 未編集なら export で復元する」としていたが、**復元しない**（クランプしたまま出力する）に確定した。→ §5.5e

> **当初案を撤回した理由**: 復元は「文書順に依存する」ため、別枝の兄弟並べ替えで親が変わる／復元値と再生成値が隣接するとレベル飛びが生じる／「未編集」の判定単位が定義できない、という 3 つの副問題を生む。**復元しなければ、これらは発生しない**。5 段はマージンを持たせた上限であり、6 段以上のフラット化は運用上許容できるという判断。
...
| # | 承認済み要求 | 本設計 | 差分の性質 |
|---|---|---|---|
| 1 | **ALIGN-L2-004（Must）**: サブレーン割当順は「**下から上**」に**固定**。最上段にマイルストーンが置かれる | `stack_direction`（`up`/`down`）を**ユーザーが選べる**（既定 `up`） | **既定値は要求と一致**。`down` を選んだ場合のみ要求から外れる |
| 2 | 同上「**最上段＝マイルストーン**」 | `stack_order`（`null`=自動 / 値=人の指定）で**任意の段に固定できる** | 人が非マイルストーンを最上段に固定すると要求から外れる |

**どちらを正とするか（記録）**: **本設計を正とする**。理由:

- **ALIGN-L1-001（Must）**「同種マイルストーンを同じ高さに揃える」「共有ベースラインへスナップ」は、**ユーザーが指定した縦位置の意図**を前提としており、自動規則だけでは実現できない。`stack_order` はこの要求を満たすために必要。
- したがって ALIGN-L2-004 の「固定」は、**ALIGN-L1-001 と両立させるために「既定は下から上」と読み替える**のが整合的。次プロジェクトで要求を書き直す際は「**既定は下から上。人の明示指定が優先**」とするのが妥当。

> **積み順規則における優先順位**（§5.6 の再掲・この差分の実体）:
> `stack_order`（人の指定）> `milestone` 優先（ALIGN-L2-004）> `start` 昇順 > `finish` 降順 > `uid` 昇順
...
| H-6 | **iQUAVIS 側の並行採番**との衝突 | **GRS の責任範囲外**（文書外の一意性は担保しない）。**別ツールで ID 衝突を検査**する前提とする |
...
- **適用範囲**: **単一 MSPDI の未編集往復**、かつ **WBS 深さ ≤ 5** の文書。
- **明示的に許容する損失（Drop）は 2 つだけ**:
  1. **WBS 深さ 6 段以上のフラット化**（§5.5e）。クランプして復元しないため、**深さの情報が失われ、書き戻すとマスタの階層も 5 段になる**。import 時に予告して選ばせる。
  2. **マージ時の取込側 Carry の欠落**（§5.4）。Project メタ「既存を保持」や Calendar/Resource の自動統合で発生する。
- 上記 2 件以外に Drop は無い（入口の自己検証と出口の往復同一性で機械検証・§5.5d）。
- **Drop**: なし（Drop=0）。
...
| ~~2~~ | ~~`stack_direction` / `stack_order` と ALIGN-L2-004 の矛盾~~ | **記録済み**（§5.6・本設計を正とし、次プロジェクトで要求を「既定は下から上・人の明示指定が優先」と書き直すのが妥当と結論） |
...
> **#1 は解決済み**。キー（親＋`ordinal`）・粒度（フィールド単位 / 要素まるごと）・順序（`ordinal`）・`null` と既定値の区別（Own を nullable）を §5.5d で確定した。残る 2〜6 は下記の順で着手する（影響度順）。

**未決事項は全て解消**（①〜⑨）。次プロジェクトで再検討すべき点は §8C（マージの詳細判断）と `grs-data-model-ja.md` §8（設計判断の変遷）に記録済み。
