...
description: "設計判断の変遷と却下案の記録。確定は grs-native-erd-ja.md。生きているのは §8 だけ。"
tags: [data-model]
phase: planning
status: deprecated
---
...
>
> **本書のうち生きているのは §8（設計判断の変遷）だけである。**
> 旧 §7.1-1 / §7.1-2（器の既定名・生成規則／移動時の追随規則）は
> **2026-08-02 に `grs-native-erd-ja.md` §5.5g へ移設した。**
...
> | 節 | 性質 |
> |---|---|
> | **§8** | **設計判断の変遷**（何を試し、なぜ変えたか）。却下案の理由が要るときに読む。**ここだけが生きている** |
> | ~~§7.1-1 / §7.1-2~~ | **`grs-native-erd-ja.md` §5.5g へ移設した**（2026-08-02）。本書には残っていない |
...
GRS は階層を **2 つの独立した軸**で持つ（§6 の帰結。確定版は `grs-native-erd-ja.md` §5.5g。旧 §4.5「TaskGroup に階層一元化」は**これで置換**）。

- **軸A: WBS 構造ツリー** — MSPDI `OutlineLevel` に対応。**`Task.wbs_parent_uid`（隣接リスト）**で保持し、外部マスタへ **export する**。**明示的 WBS 編集のみ伝播**（§6）。**深さに上限は無い**（§4.6）。
...
```mermaid
erDiagram
    Task ||--o{ Task : "wbs_parent_uid (軸A WBS 上限なし)"
...
- 各群は名前 `label` を 1 つ持つ。「大/中/小/対象」は `parent_id` の**木の深さ**で決まる（複数ラベル列は持たない）。器（軸B）の入れ子は 5 段まで。
...
- `Task` に `row_id`/`group_id` を**足さない**（MSPDI 核の汚染＋依存逆流を避ける）。所属は `TaskGroupMember{group_id, task_uid}` で表す。**`task_uid` は UNIQUE＝1 タスクは高々 1 行**（器に入っていない場合の落とし先は **`grs-native-erd-ja.md` §5.5g** で確定）。
...
### 4.5 【置換済み】WBS 階層は Task、視覚グルーピングは TaskGroup（2 軸）

> ⚠️ **置換済み**（変遷 §8-2）。確定版は `grs-native-erd-ja.md` §5 / §5.5g。
...
- **取り込んだ WBS を潰さないので、深さによる Drop は無い。** import 時の予告ダイアログも要らない。
...
#### 7.1-1 / 7.1-2 — **`grs-native-erd-ja.md` §5.5g へ移設した（2026-08-02）**

**既定行（器）の生成規則・既定表示名・追加列・二軸の片方向追随・遷移の扱いは、
`grs-native-erd-ja.md` §5.5g が正。** 本書には残していない。

> **なぜ移したか**: 本書は `type: Background`（README §00 =「採用しない」）で読む順にも載っていないのに、
> **8 か所がここを「正」と名指ししていた**。README §00 の「用語の正は 4 つだけ」を守るため、
> データの正（`grs-native-erd-ja.md`）へ物理的に移した。**判断の中身は変えていない**（2026-07-26 のユーザー確定のまま）。
