...
> ## 本書のどこを読むか（2026-07-26 追記）
>
> | 節 | 性質 |
> |---|---|
> | **§7.1-1 / §7.1-2** | **確定仕様**（既定行の生成規則・既定名／二軸の片方向追随）。他に書いた場所は無い |
> | **§8** | **設計判断の変遷**（何を試し、なぜ変えたか）。却下案の理由が要るときに読む |
> | §1〜§7 のその他 | **設計の過程**。確定版は `grs-native-erd-ja.md`。**食い違ったら確定版が勝つ** |
>
> 本書に残る「要 change-manager」「Step2/3」等の記述は**前プロジェクト内部の工程の話**であり、次期には適用しない。
...
flowchart LR
    ...
    B <-->|"XML"| C["MSPDI ファイル<br/>(外部 WBS マスタ等)"]
```
...
- **軸A: WBS 構造ツリー** — MSPDI `OutlineLevel` に対応。**`Task.wbs_parent_uid`（隣接リスト）**で保持し、外部マスタへ **export する**。**明示的 WBS 編集のみ伝播**（§6）。深さ ≤ **Lv5**。
...
erDiagram
    ...
    Task {
        ...
        int fadeInDays "← ExtendedAttribute（拡張領域・§5.5f）"
        int fadeOutDays "← ExtendedAttribute（拡張領域・§5.5f）"
    ...
    TaskGroup {
        ...
        string label "行/見出し名（大/中/対象は深さで決まる）"
        int order "兄弟内の並び順"
...
- 各群は名前 `label` を 1 つ持つ。「大/中/小/対象」は `parent_id` の**木の深さ**で決まる（複数ラベル列は持たない）。正規化・5 段までスケール。
...
- `Task` に `row_id`/`group_id` を**足さない**（MSPDI 核の汚染＋依存逆流を避ける）。所属は `TaskGroupMember{group_id, task_uid}` で表す。**`task_uid` は UNIQUE＝1 タスクは高々 1 行**（器に入っていない場合の落とし先は **§7.1-1** で確定）。
...
- ※ 前プロジェクトの承認済み要求は「下から上」固定だったが、**本設計（既定 `up` ＋ `stack_order` 優先）を正とする**。次期は最初からこう書く。
...
> **削除した旧記述について**: 本節にあった Mermaid 図と表（「Export は群の深さから `OutlineLevel` を再生成」「Task は階層を持たない」「保存は TaskGroup 一本」）は**確定版と正反対**のため削除した。誤って実装すると軸独立性が全否定され外部マスタの WBS を破壊する。

### 4.6 階層の最大深さ

- **推奨 3 段（大・中・対象）、ハードキャップ 5 段**。UI/LOD は最大 5 段前提で設計（有界）。
- MSPDI import で 6 段以上: **5 段にクランプし、export でも復元しない**（`grs-native-erd-ja.md` §5.5e で確定）。
  - **クランプで変わるのは親子関係だけ**。MSPDI の参照は全て UID なので、**依存・担当・日付・実績は無傷**。
  - ただし **export すると外部マスタの WBS も 5 段にフラット化される**（未編集でも）。→ import 時は「情報」ではなく**予告**として提示し、取込の可否を選ばせる。
  - これは本設計で**明示的に許容する 2 つの Drop の 1 つ**（もう 1 つはマージ時の Carry 欠落）。
- **軸B（行の器 `TaskGroup`）の ≤Lv5 は別制約**として維持する（レイアウトの有界性を担保するのはこちら）。
...
- **MSPDI 発行**（export）= Reconstruct 値を**その場で計算して焼き込む**（MSPDI は自己完結スナップショットの思想。外部マスタ等の素朴な読み手のため、Duration/OutlineNumber 等も計算して出力する＝安全側）。
- → 「派生を保存しない（ドリフトなし）」と「MSPDI は自己完結（派生を出す）」は**別成果物なので両立**。

---

## 6. 外部マスタとの双方向往復と WBS 保全（明示編集の定義）
...
```
1. 子を持つタスク S       → 器 TaskGroup(S) を作る。S の直下の葉を member に入れる。S 自身も member（§4.1）
...
**なぜ装飾しないか**: 器はサマリタスクと 1:1 で、**サマリ自身のバーもその行に載る**（§4.1）。
...
#### 7.1-2 二軸の追随は**片方向** — **確定（2026-07-26 ユーザー確定）**
...
`collapsed` と member の `stackOrder` は**保つ**。作り直すとユーザーが作った視覚情報が WBS 操作で消えてしまう。
...
- **出自保持（フラット化しない）**。`TaskGroup` は視覚のみで各 Task の `wbs_parent` を触らないため、別々の WBS 枝のタスクを 1 行の器に混ぜても、**export は各 Task を自分の WBS 位置で出す**。器（TaskGroup）は落ちるので外部マスタは通常の WBS だけを受け取る（マルチバーの混在を知らない）。→ **§6 の非伝播原則を構造的に保証**。
...
| 18 | **深い WBS（6 段以上）の扱い** | 5 段にクランプ＋警告。のちに「原レベルを退避して export で復元」する案へ | **クランプしたまま export する（復元しない）** | 復元は**文書順に依存**するため、兄弟並べ替えで親が変わる／レベル飛びが生じる／「未編集」の判定単位が定義できない、という副問題を生む。**復元しなければ全部発生しない**。5 段はマージンを持たせた上限で 6 段以上は運用上やりすぎ、という判断。**クランプで変わるのは親子関係だけ**（参照は全て UID なので依存・担当・日付は無傷）。深さ情報の損失は**明示許容の Drop** として記録し、import 時に予告する。無限階層案は実装・テスト困難で却下 |
