...
- **これは説明用の実例である。スキーマではない。**
  機械可読なスキーマは**まだ 0 件**（`../NEXT-STEPS-ja.md` 5-1）。
...
> 本物は**常に全項目を書き出す**（`../02-data-model/grs-document-settings-ja.md` §2）。
> 実例の目的は「`revisionStamp` と `changeLog` が**どこに座るか**」を示すことなので、
> 既存の完全な実例（`../02-data-model/handover-data-model-entry-ja.md` §3）を**複製していない**。
> **正はあちらである。**
...
> **セットで初めて意味を持つ**。片方だけ通ると、通知（`../NEXT-STEPS-ja.md` 2-5 の 8 系統）で
> 説明できない中途半端な文書が残る。
...
**トライアルの API を GRS の名前に置き換えたもの。** コードは写さない（`../README.md` §0-1）。
...
| 項目 | 状態 |
|---|---|
| `documentSettings` の全項目 | **正は `../02-data-model/grs-document-settings-ja.md`。複製しない** |
| 機械可読なスキーマ | **0 件**（`../NEXT-STEPS-ja.md` 5-1） |
