...
- **記法は `../03-ui-naming/handover-ui-parts-ja.md` §1-2 に従う**
...
> （語彙の重複は前プロジェクトのバグ根因＝ `../06-background/refactor-gui-data-separation-ja.md`）。
...
- **`note` / `notes` という語を使わない。** `Task.notes` が既にあり、
  **語彙の重複は前プロジェクトのバグ根因**である（`../06-background/refactor-gui-data-separation-ja.md`）。
...
- **一覧はこれで確定ではない。** 実開発で UI の操作割当
  （`../03-ui-naming/handover-ui-detail-spec-ja.md` §5）と**1 対 1**に整える。
