...
- `shapeKind`: `'bar' | 'chevron' | 'arrow' | 'endpointSpan' | 'milestone'`。
- **上下に幅がある形状種**（`bar` / `chevron`）→ 実績を**内側に重ねる**。
...
> **`poc-glyphs-reference.html`（字形の基準）**で全状態を描いて確かめてある。
