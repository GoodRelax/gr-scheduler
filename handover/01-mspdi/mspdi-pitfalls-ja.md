...
- 関連: `mspdi-core-tree.md`（構造の概観）/ `mspdi-tables.md`（全 29 テーブルの責務）/ `../02-data-model/grs-mspdi-field-ledger-ja.md`（取捨選択の実例）
...
| **`elementFormDefault`** | **`qualified`**（属性は無いので `attributeFormDefault` は既定の `unqualified`） |
| **トップレベル要素** | **`Project` の 1 つだけ** |
| Project 直下スカラー | **63**（＋ 容れ物 7 ＝ 全 **70**。うち必須は 2 つだけ・罠 B-2） |
| Task | 91 スカラー ＋ 子要素 5 ＝ 全 **96**（必須は `UID` のみ） |
| Resource | 65 スカラー ＋ 子要素 6 ＝ 全 **71**（必須は `UID` のみ） |
| Assignment | 61 スカラー ＋ **201 の空予約枠** ＋ 子要素 3 ＝ 全 **265**（必須は `UID` のみ） |
| エンティティ（テーブル）総数 | 29（中核 6 ＋ 衛星 23） |

> **要素の宣言順（全数）と最小妥当文書は `mspdi-core-tree.md`「XSD の要素順と最小妥当文書」節が正。**
> 上表の内訳は 2026-08-02 に XSD から再抽出して**全数が一致することを機械で確かめた**。
...
### B-5. 省略はできるが**並べ替えはできない**

**事実**: `Project` / `Task` / `Resource` / `Assignment` は**すべて `xsd:sequence`** である（XSD 実測 2026-08-02）。B-1 のとおりほぼ全てが `minOccurs=0` だが、**出す要素は宣言順に並んでいなければならない**。

**何が起きるか**: 「どれも省略できる」から「順不同でよい」と読み違える。**意味で分類した資料の順に書き出すと非妥当になる** — 例えば `mspdi-core-tree.md` のコアツリーは【メタ情報】【スケジュール方針】【通貨・表示】で括ってあり、実際の順（`CurrencyCode` は 20 番目で `CalendarUID` 22 番目より前）とは違う。スキーマ検証を掛けずに出力すると**相手ツールで開くまで気づかない**。

**対処**: **持っている要素だけを宣言順に出す。** 順序の全数は `mspdi-core-tree.md`「XSD の要素順と最小妥当文書」節にある。export は**必ずスキーマ検証を通してから**完成とする。

---

### B-6. `Calendars` を出すなら `Calendar` を 1 つ以上入れる

**事実**: `Calendars` の子 `Calendar` は **`minOccurs=1`**（XSD 実測 2026-08-02）。`Project` 直下の容れ物 7 つのうち、**中身が必須なのはこれだけ**である（`Tasks`/`Resources`/`Assignments` の子は `minOccurs=0`）。

**何が起きるか**: 「容れ物は空でも出しておく」という素直な実装が、**`<Calendars/>` だけで非妥当**になる。カレンダーを使わないツールほど踏みやすい。

**対処**: **中身が無いなら `<Calendars>` 要素そのものを出さない**（`Calendars` 自体は `minOccurs=0`）。
