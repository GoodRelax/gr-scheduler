...
出口と入口を別々の定数にすると必ず食い違うので、**倍率 1 つから両方を導くこと**。

#### 5 パターン

`x1 = 先行の右端 + 走り` / `x2 = 後続の左端 − 走り` と置くと、分岐は 1 つで足りる。
...
| # | 状況 | 折れ点 | 経路 |
|:--:|---|:--:|---|
| **①** | 同じ段・間隔あり | **0** | 右辺 → 左辺（水平 1 本） |
| **②** | 下の段・間隔あり | **2** | 右辺 → 中点 → 下降 → 左辺 |
| **③** | 上の段・間隔あり | **2** | 右辺 → 中点 → 上昇 → 左辺 |
| **④** | 下の段・**日程が重なる** | **4** | 右辺 → 右へ走り → 通路へ下降 → **左へ戻る** → 下降 → 左辺 |
| **⑤** | 上の段・**日程が重なる** | **4** | 右辺 → 右へ走り → 通路へ上昇 → **左へ戻る** → 上昇 → 左辺 |
...
**根は 3 つだけ**（`basePlanH` / `fontMin` / `pxPerDayAt1x`）。残りは全部そこから落ちてくる。
...
graph LR
    ...
    depRunOfArrow["走り倍率<br/>depRunOfArrow = 2"]
    depRun["走り<br/>depRun"]
    depWidth["太さ<br/>depWidth"]
    laneGap["段の間隔<br/>laneGap"]
    zoom["ズーム"]

    depArrowLen -- "x" --> depRun
    depRunOfArrow -- "x" --> depRun
    zoom -. "追随しない" .-> depRun
    zoom -. "追随しない" .-> depWidth
...
graph TD
    ...
    fontMin -- "下限 = fontMin ÷ (actualOfPlan x fontOfActual)" --> basePlanH
    actualOfPlan --> basePlanH
    fontOfActual --> basePlanH
...
| フォントが下限に張り付かない | `basePlanH ≧ fontMin ÷ (actualOfPlan × fontOfActual)` | 等倍でも文字が下限。レベルを減らすしかなくなる |
| 走りが三角形より長い | `depRunOfArrow > 1` | 垂直線の先端に三角形が直接乗り、矢印に見えない |
...
53 個すべてを個別に変えられ、**範囲は他の設定に追随して引き直される**。
