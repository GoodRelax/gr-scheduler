...
| ボタン | 役割 |
|---|---|
| `Fit` | **縦横とも**全体が収まるようにズームとスクロールを合わせる（§5-2） |
...
- **フォントサイズはタスク高の 80%。ただし下限 12px でクランプする。**（和文がこれを割ると読めない。基準は 1920x1080 / 全画面 / 拡大率 100%）
  **下限を割る文字が必要になったら、文字を縮めるのではなくグループ LOD（軸B・`TaskGroup`）の深さを 1 つ減らす**（`../07-plan-actual/handover-plan-actual-decisions-ja.md` §2-4-1。**軸A ではない**）
...
避ける場所が無い**。確定した 5 パターン自身が `x2 < x1` のとき「右へ出て段間の通路を左へ戻る」＝
**横切りを織り込んだ設計**である（§4-9）。
...
**段が 2 つ以上離れていても、行をまたいでも、経路は 5 パターンのままである。**
...
| 段が **2 つ以上離れている** | **決着（2026-08-02・ユーザー判断）**。垂直線は途中の段のタスクを**横切ってよい**。依存線をバーより前面に描くので線は読める（§4-3）。**経路は 5 パターンのまま** |
...
`../08-poc/poc-integrated.html` の「マルチバー」タブの `P00 依存線の 5 パターン` が 5 例を 1 枚で描く。
...
```
 ── 縦の寸法（1 本の鎖。上流を変えれば下流が全部動く）────────────────
    ...
    最小フォントサイズ  ──▶  予定の縦幅の下限
        ...
        グループ LOD（軸B）の深さを 1 つ減らす（§4-2 / §6-5 / 07 §2-4-1）
...
グループ LOD（軸B・`TaskGroup`）の深さを 1 つ減らす（§6-5）。下限は**引き金**であって、そこで固まる値ではない。
...
| フォントサイズ | 実績の縦幅 | 同上 |
| 予定の縦幅の下限 | 最小フォントサイズ | 下限を割ったら**グループ LOD（軸B）の深さ**を 1 つ減らす（§6-5） |
...
5. **マイルストーンは `stackOrder` の小さい側**に来る（下のタスク群から上のマイルストーンへ依存線が立ち上る構図）。
6. **積む向き**（`stackDirection`）は**文書全体**の設定。既定は `'up'`。
   **`stackOrder` → 画面の y への写像だけが変わる** — **確定 2026-08-02（ユーザー判断）**

   | `stackDirection` | `stackOrder = 0` の位置 | 積む向き |
   |---|---|---|
   | **`'up'`**（既定） | 行の**下端** | 下から上へ（ALIGN-L2-004 の「下から上」に一致） |
   | **`'down'`** | 行の**上端** | 上から下へ |

   - **手順 2〜5 は向きに依存しない。** `stackOrder` の割当は同じで、**描くときの y だけを反転**する。
   - したがって **`'down'` でも「マイルストーンは積みの先頭」は保たれる**（画面上は下端に来る）。
   - **`'down'` は ALIGN-L2-004（下から上に固定）から外れる**が、要求は
     「**既定は下から上。人の明示指定が優先**」と読み替え済み（`../02-data-model/grs-native-erd-ja.md` §5.6）。既定は要求と一致する。
...
> ```
>
> **器（軸B）は export しない**ので、これで失われるものは無い。7 段の WBS は
...
> ⚠️ **現行の既定値 1 / 4 / 14 は、上の目安を 3 つとも満たしていない。**
> 既定値の決め直しは `../NEXT-STEPS-ja.md` ステップ 4（既定値の確定）で扱う。
