...
| 掴み領域 | 場所 | 操作 |
|---|---|---|
| **開始点** | バー左端（中央高さ） | 開始日を変える（リサイズ） |
| **終了点** | バー右端（中央高さ） | 終了日を変える（リサイズ） |
| **バー本体** | 開始点と終了点を除いた中間 | 平行移動 |

- **予定側の端点は予定日を、実績側の端点は実績日を書き換える。**
- 両方表示で実績の端点が掴めない字形があるため、**モードで解く**（§4-4）。

**字形固有の掴み点（予定側のみ）**

| 掴み点 | 場所 | 出る字形 |
|---|---|---|
| **フェードイン** | バー左上の角 | **矩形と矢羽根のみ** |
| **フェードアウト** | バー右下の角 | **矩形と矢羽根のみ** |

> **矢印と細線スパンにはフェード掴み点を出さない**（ユーザー確定）。字形として端をぼかす意味がない。
...
| 掴み領域 | 内容 |
|---|---|
| **依存アンカー** | 引出し口。外接矩形の **9 点**（0 = 左上 〜 8 = 右下） |
