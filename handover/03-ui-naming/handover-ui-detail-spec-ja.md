...
| 図形（マイルストーン） | 〇 △ ▽ □ ☆ 五角形 六角形 ※畳んで隠れており、展開して選ぶ。形状種（`shapeKind`）はタスクと同じ 1 列 |
...
- **フォントサイズはタスク高の 80%。ただし下限 10px でクランプする。**（和文が 10px 未満で急に読めなくなる）
...
**その形状種だけ実績を下にずらす**（局所的な分離）。文書全体のモードではなく、形状種ごとの描き方である。
