...
2. 縦を解く — 軸B（TaskGroup）の深さを 5..1 で試す（07 §2-4-1 と同じ方式）
     d = 5, 4, 3, 2, 1 の順に
...
> 保証するのは「**軸A と軸B の深さを試したうえで、収まる最大のズームに着地する**」ことだけである。
...
| | 入らないときに減らせるか |
|---|---|
| **縦** | **減らせる。** 軸B の深さを畳めば行数が減る。「入らない」は畳みきった後の話 |
...
| # | ケース | 深さを畳んで救えるか |
|:--:|---|---|
| **V-1** | 行数が多い | **救える。これが軸B の深さ減の本来の役目** |
...
> 最優先事項 3「1 画面でスクロールなしに見られる」は、**軸B で畳んで初めて成り立つ**。
