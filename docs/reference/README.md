...
| フォルダ | 中身 | 取得手順の所在 |
|---|---|---|
| `mspdi/` | 版ごとに分ける。`pj12/`（`mspdi_pj12.xsd` —— MS Project 2007 の MSPDI スキーマ。そこから作った ERD の要約 `_erd-part-M*.md` と `mspdi-erd-ja.md` も置く）／ `pj15/`（`mspdi_pj15.xsd` —— MS Project 2013 のスキーマ。**公式 URL が無く**、Project 2013 SDK の `.msi` から取り出す）／ `learn-docs/`（MS Learn 要素リファレンス 390 本。⚠️ **どちらの版とも一致しない**ので、版を決める根拠にしない）／ `LICENSE`（learn-docs の CC-BY-4.0） | **`previous-project-result/01-mspdi/mspdi/README.md`** — 入手手順・照合ハッシュ・ライセンスの正 |
| `wcag/` | `wcag21-rec.html`（WCAG 2.1 原文） | `wcag/README.md` |
| `w3c/` | `xml-c14n10.html` / `xml-c14n11.html`（Canonical XML 1.0 と 1.1 の原文）、`xmlschema-2.html`（XML Schema Part 2: Datatypes。表 T-228 の `NR-4` が指す標準字句表現） | `w3c/README.md` |
