...
（利用者の裁定 2026-09-12、`CR-371`）—— ⚠️ **この 1 文が無いと、下の「表に無い接頭辞を本文で使ってはならない（MUST NOT）」が行 ID にも掛かって読める。**  
⭐ 実測 2026-09-12: `docs/spec` は行 ID を **1,891 行・接頭辞 124 個**持ち、本表には **1 つも入っていない**。⇒ その空白から `PD-` の衝突（表 T-023a の行 対 台帳の未裁定）が生まれ、**生きた仕様の指し先が 3 箇所剥がされた**。 略号は初出のここでのみ展開する。
...
### ⛔ 散文に数を書くときは、機械が読み直せる形で書く（**利用者の裁定 2026-09-12**）
