"""Alternate four probes between built trees on one machine on one day (record 17 method, DFC-610 A/B).
...
Probes, in this order inside every run:
  ...
  dependency-drag  ../frame-time-ab-2026-09-16/extra-zoom-row-axis-and-dependency-drag/dependency-drag.mjs --tree <tree>
...
(run 1: T1 T2 T3, run 2: T2 T3 T1, run 3: T3 T1 T2), so no tree always goes first.
...
def command_of(probe, tree):
    ...
    if probe == "dependency-drag":
        return ["node", os.path.join(OLD, "dependency-drag.mjs"), "--tree", tree]
    return ["node", os.path.join(tree, "tools", "probe", "examples", "lm-19-frame-time-baseline.mjs")]


PROBES = ("zoom-row-axis", "first-notch", "dependency-drag", "lm-19")
