...
4. ⛔ ⭐ **いま止まっているのはここである** —— 設定値 6 個を原稿へ移す
     exportCanvas / importMinDate / importMaxDate /
     planActualGuidePattern / rulerFont / rulerHeight
     ⭐ 裁定 ㊸（CR-175）どおり「生成器の射程を広げる」。値を写さない
     ⚠️ rulerFont と rulerHeight は文字サイズの段ごとの 3 つ組（fontScaleSizes と同じ形）
     済めば startup-template.json は 1 コマンドで出る
5. .json を npm run gen と gen:check へ繋ぐ
6. 縦線の 1（UF-33）から順に書く。段は 05 の 4. に従う
7. 試験は別のエージェントに docs/spec だけを読ませて書かせる（04 の 1.）
```
