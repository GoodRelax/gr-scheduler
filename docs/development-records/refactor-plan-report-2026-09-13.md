...
- **段 0 と段 1 の到達（2026-09-14）:** 段 1 は済んだ。段 0 は ①②③ を満たし、④⑤ の直しが入った。この巡の出来事は `git log 8bf6aca..7f7463d` の本文が持つ。
- **次の一手（新しいセッションで、この順）:**
  1. `git status` で `refactor` にいることを確かめ、`handoff.md` の §0 を回す。⛔ 本書の数を信じず、実物で測る。
  2. **段 0 の出口に残るものを台帳から引く。** ⛔ 行の一覧を本書に写さない。`handoff.md` の §3 の問いを打ち、次の行の状態を確かめる。
     - `DFC-568`（ハイライトボックスの四隅と本体、コメントボックスのアンカー）: 仕様は着地した（`eceac3d`・`7f7463d`）。実装と、仕様だけを読む体の試験を取り込む。実装の体が自分で決めた点への反証を読んでからコミットし、出荷ビルドで押す。⚠️ コードは `c9c1aaa` でコミット済み。読みの直し（四隅とアンカーを離した位置で読む、`JDG-71`）は、設計の穴が 2 つ見つかったため（`docs/development-records/defects.md` の `DFC-568` の行）取り込まず、次の巡に回した。作業木 2 つを残してある（`.claude/worktrees/agent-a8d904163fb22b4c1/` と `.claude/worktrees/agent-a5abfa1729e7c53e7/`）。
     - `実測待ち` の行: 出荷ビルドを押した結果を、押した条件とともに `実物確認` の欄へ書き、`実測済` へ動かす。⚠️ `7f7463d` の台帳ではまだ `実測待ち` であり、押しの結果は台帳にもコミットの本文にも無い。`DFC-571` は押しで判定できなかったので、判定の手から決める。
     - `DFC-577`（取り込みが `IV-21` を見ない）: `validate-imported-document.ts` は割るファイルでも書き直すファイルでもない。⇒ 判断: 記録 7 節の a（段 0 で直す）に当たる。
     - `DFC-573`（`LM-19` の数）: `裁定待ち`。問う前に `handoff.md` の §0.2 の 3 手を打つ。
     - `DFC-574`（開く道がファイル名を書かない）: 所在は `frame-loop.ts` なので、記録 7 節の b（段 7 で領域を移した直後に直す）。⇒ 段 0 の出口の条件に入れない。記録 7 節の表に追記で載せる。
     - `DFC-560`: `8e340f8` で段 0 の波から外した。② が `Agent API` の公開の型に触るので、変更要求が要る。
     - `DFC-566`（色の欄）: `裁定待ち`（`PND-494`）。段 6 の入口の条件である。
     - `実測済` の行が `defects.md` に残っている（`7f7463d`）。巡を閉じる前に `fixed-defects.md` へ刈り取る（`JDG-32`。数え方は `handoff.md` の §9）。⚠️ 前の版の §9 の awk は `D-` しか拾わず、`DFC-` の行があっても 0 を返していた。
     - 押しで見えた 4 件（対話欄がフォーカスを取らない／f と p が落ちる／確定した行が対話ログに出ない／依存線がマイルストーンのマークを覆う）: `DFC-578`〜`DFC-581` として `a9ae60d` で行を立てた（状態 `未検討`）。
  3. **段 0 を閉じる。** 状態表の出口の条件に加え、段の出口の e2e（記録 6 節の「全段で」）と `check.sh` の exit 0 を満たしたら、状態表を ✅ にする。そのうえで `main` を `refactor` の先端へ早送りして push する（JDG-61。記録 14 節）。段のタグは利用者が打つ。
...
- ⬜ 残り: ④ 割っても残る場所の欠陥のうち `DFC-568`（コメント／ハイライトボックスの四隅の掴み）。
