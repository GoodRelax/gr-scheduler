...
- **次の一手（新しいセッションで、この順）:**
  ...
  - ⭐ **2026-09-15 早朝の到達:** `CR-378`（仕様 ＋ コード波 ＋ 試験）と `CR-379` 2b-A（仕様）を、ともに `wave-cr378-379` へ当てた。**段 2a・段 2b の出口の条件「変更要求が当たり、検査が緑」は、検査 39 を除いて満たした。** ⚠️ **検査 39（MUST 節で試験の裏づけが無いもの）は、この枝で 1369 → 1376（+7）に上がった** —— `CR-378` の 6 件（`EG-5`・`EG-6`・表 T-247 の前文・`JF-2`。段 8 より前にコードが満たせないか、判断が要るもの）と `CR-379` の 1 件（5.5 の条項）である。⛔ **この +7 を段 0 と同じ基準線として是とするかは、利用者の裁定を待つ**（`docs/development-records/baseline-moves-need-user-ok.md` と同じ形 —— 基準線を動かす直しは体へ振らず、利用者に問う）。⭐ `DFC-588`（`DomScreenSurface` がヘルプの名簿 `.json` を読んでいたこと。`JF-1` / `JF-3` に触れる）は直した（実測待ち）。`DFC-589`（1 つの表の中で行 ID が重複しても、どの検査も赤くならないこと）は記録した（未修正）。
