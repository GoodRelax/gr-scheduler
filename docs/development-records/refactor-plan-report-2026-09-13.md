...
- **ブランチとタグの進め方が決まった**（JDG-60・61。記録の 14 節）。
- **次の一手（この順）:**
  1. 本書と記録を `restart` にコミットして push する。
  2. **利用者が**そのコミットに `After-Cleanup` のタグを打つ（断捨離の完了と計画まで）。
  3. `main` をそのコミットまで早送りして push する。
  4. `main` の先端から `refactor` ブランチを切って push する。
  5. **利用者が**同じコミットに `Before-Refactor` のタグを打つ。
  6. `refactor` で、`dist/index.html` の作り直し（JDG-51）と段 0・段 1 を始める。仕組み化（記録の 11 節）は段 0 の並行作業である。
- **段の流れ:**
