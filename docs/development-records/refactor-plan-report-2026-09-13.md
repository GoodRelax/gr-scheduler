...
- **次の一手（新しいセッションで、この順）:**
  ...
  2. **段 0 の残り物を閉じる（出口の条件ではない）。** ⛔ 行の一覧を本書に写さない。`handoff.md` の §3 の問いを打ち、次の行の状態と、塞いでいるものを確かめる。
     ...
     - `DFC-582`（開く道が取り込みの検証の拒否を告げない）: `仕様待ち`。`frame-loop.ts:2594-2596` が拒否を黙って捨て、Agent API は空の `refusals` を返す。`T-220` を鍵にする拒否をどう言うかの条項が仕様に無い。⛔ 利用者に問う前に `handoff.md` の §0.2 の 3 手を打つ。
