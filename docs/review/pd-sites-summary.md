...
| 2 | `ruled-elsewhere-baseline.txt:38:23` | `PD-3` → **`PND-3`** | 基準線は検査 40 が**印字したもの**を記録しており、その「裁定済の PD」は台帳側である。⚠️ **DFC-468 が名指した既知の偽陽性**であり、巡 1 で `DFC-292` の綴りが変わると**印字が変わって基準線がずれる** |
...
| ファイル | 何を持つか |
|---|---|
| `check-ruled-elsewhere.py:138` | `TABLE_ROW_PD = re.compile(u'表 T-023a の [`]?PD-\\d+[`]?')` ⛔ **DFC-468 の免除そのもの。直さないと免除が効かなくなる** |
