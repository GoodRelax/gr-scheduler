...
- 検査 16 と 検査 41 —— ⛔ **台帳が「検査の側の欠陥」と記録している**（`DFC-344` / `DFC-345` の型）。正しい `npm run gen` のたびに鳴っていた
...
| | |
|---|---|
| **外すと何が失われるか** | ⛔ **失われる実績が在る。**台帳自身が **`DFC-06` と `DFC-43` は自動試験が緑なのに実物で落ちた**と記録しており、そのために刈り取り済みの行にも戻り道が要る、と書いている。**432 行のうち 387 行（89.6%）が実測の証拠を持っている** —— これは他のどの仕掛けも持っていない情報である |
...
| **4** | **台帳の 1 行を短くする**（実測の証拠は残し、経緯の散文を削る） | 432 行 × 1,680 字。期 2 で 19,884 行 | ⚠️ **経緯。**⛔ **戻り道（`DFC-06` / `DFC-43`）は残さねばならない** |
