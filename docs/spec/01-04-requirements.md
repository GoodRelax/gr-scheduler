...
| ST-11 | 人が指定した積み順が同じ段で衝突したときは、**先に置かれた側を残し、あぶれた側が落ちた先を人に知らせること** |
...
**STATEMENT**: 作成者が行見出しツリーで階層を移したとき、システムは、その変更を WBS にも反映し、かつ `UID` を保持すること。一方、**タスクバーを別の行へ移す操作では WBS を変えてはならない（MUST NOT）。** **自分の子孫を親にする移動を受け付けてはならない（MUST NOT）。** **階層を移したとき、行の器を作り直してはならない（MUST NOT）** —— 更新するのは親だけとし、**行の名前・色・高さ・畳み状態・積み順を保つこと（MUST）。** 作り直すと、階層を 1 段動かすたびに見た目が失われる。**人が手で作った器は WBS の移動に追随しない。** **兄弟どうしの並べ替えができ、その順序も WBS へ伝わること（MUST）** —— 親子の変更だけでは、同じ親の下の順番を直せない。
...
**RATIONALE**: 項目 1 つずつを別の要求にすると 22 件の要求になり、追跡だけが重くなる。**画面上でタスクを動かしたときは、プロパティの日付を追随させること（MUST）** —— 同じ値を 2 つの面から編集できる以上、片方だけが動く状態を作らない。
...
**STATEMENT**: プロパティパネルに出す中身は、**最後に行われた操作**で決めること。設定を開いても選択を解除せず、もう一度同じ入口を押したら直前の選択物へ戻すこと。**選択を解除しても設定へは移らないこと（MUST NOT）。** いま何を出しているかを、押下状態とパネル先頭の見出しで示すこと。
...
**ORIGIN**: UC-001 / UC-002 / UC-003 / UC-005 の全手順（横断）

**RATIONALE**: **誤操作への備えは取り消しの 1 本に絞る。** 確認ダイアログを増やす代わりにこれが要る。**例外は行の削除だけである**（`FR-032`）—— 連鎖して消える範囲が大きく、取り消しで戻せても「何が消えたか」を人が把握できないため。**例外を増やさない（MUST NOT）。**破壊的な操作（連鎖削除・取込による上書き）を戻せることは、適合範囲に含めたアクセシビリティの基準でもある。**`Agent API` 経由の変更も同じ履歴に積み、呼び出し 1 回を 1 段とすること（MUST）** —— 口が 2 つあっても履歴は 1 つである。
...
**STATEMENT**: 作成者が整列を求めたとき、システムは、選ばれたタスクの開始日または終了日を同じ日付へ揃えること。**システムが自動で横へ動かしてはならない（MUST NOT）。**

**ORIGIN**: UC-001

**RATIONALE**: **縦と横で仕組みが違う。** 縦は段割当（FR-003）の結果として自動で揃うが、**横は人が実行する。** 日付は人が決めるものであり、システムが動かしてよいものではない。
...
**タスクの本体をドラッグしたとき、予定の日付だけをずらし、実績の日付を変えてはならない（MUST NOT）。** 行をまたぐ移動では予定も実績も新しい行へ移るが、**どちらの日付も変わらない（MUST NOT）。** —— **一度入力された実績は、担当者が置いた事実である。** 予定を動かす操作で書き換わると、それを置いた者以外が気づかないうちに履歴が変わる。**実績の日付を変える経路は実績バーの端点だけとすること（MUST）**（表 T-023d）。
...
**STATEMENT**: `Task` が遅れていると判定されたとき、システムは、遅れの量を**日数**として読める形で示すこと。

**ORIGIN**: UC-006 手順 3

**RATIONALE**: 進捗マーカーの `(!)` は遅れの**有無**しか伝えない。どれだけ遅れているかは、イナズマ線の頂点の位置と、この日数が担う。
...
**RATIONALE**: 見出しと目盛が本体と一緒に流れると、**どの行のどの日付を見ているのかが分からなくなる。** 拡大したときこそ手がかりが要る。**行見出しパネルは日程の描画領域の左側に置くこと（MUST）。** **`App Header` は、視認できる範囲で最小の高さに描くこと（MUST）** —— 上部に使った高さはそのまま日程表から引かれる。
...
**RATIONALE**: 割当 1 つずつを要求にすると 15 件になる。表で持つ。**修飾キーの付いた入力は画面全体で既定動作を止める**（表 T-023 の MK-10）—— 止め忘れると `Ctrl+S` がブラウザの HTML 保存にすり替わる。
...
| SK-2 | すべてのアイテムを選択する | `Ctrl+A` |
| SK-3 | 選択（アイテム / 依存線 / 注記）を削除する | `Delete` / `Backspace` |
| SK-4 | コピーする | `Ctrl+C` |
| SK-5 | 貼り付ける | `Ctrl+V` |
| SK-6 | 元に戻す | `Ctrl+Z` |
...
> ⚠️ **員数が引継ぎ資産の中で食い違っている。** アイコン案は「現行 7 個 ＋ 新設 6 個 ＝ 13 個」と記すが、`ui-detail-spec-ja.md` §5-1 に実在する編集の割当は **9 個**で、足すと 15 になる。**本表は実在が確かめられた 9 個と、新設と宣言された 6 個を合わせた 15 行である。**「13」という数は採らない。**引継ぎ資産側の書き戻しが済んだら数え直すこと。**
...
**保証しないのは全体についてであって、下限が無いという意味ではない。表示を最も縮めたとき、最も浅い階層（WBS の深さ 1 と `TaskGroup` の深さ 1）のすべてが 1 画面に収まること（MUST）。** ここまで落ちても収まらないなら、`GL-002` は達成されていない。**保証しないと明記するのは、保証しようとすると LOD の段階を無限に下げることになり、読めない絵で「収まった」と主張することになるためである。**
...
**RATIONALE**: 切り替えの対象は担当ラベル・完了率ラベル・依存線・進捗マーカー・イナズマ線・罫線 2 種・変更前の予定・予実の表示など 14 項目あり、**1 つずつを要求にすると 14 件になる。** 値と既定は表 T-202 が持っているので、要求はそこを指す。
...
**RATIONALE**: `FR-022` は「同じタスクが来たときに 3 択で選ばせる」ことしか定めていなかった。**合流は 1 つの操作ではなく、判別・衝突・重複・取消のそれぞれに規則が要る。** 1 つずつを要求にすると 5 件になるので表で持つ。**間違えると外部 WBS マスタ側のデータを壊す。**
...
**STATEMENT**: 担当ラベルを描くとき、システムは、**作業資源だけ**を対象とし、材料資源・費用資源・名前が空の資源を出さないこと。複数が割り当てられているときは、**先頭 1 名と残りの人数**で示すこと。
...
**階層の循環（あるタスクの親が、その子孫である状態）を検出して取り込まないこと（MUST NOT）** —— `FR-005` が禁じているのは作成者が画面で行う移動だけで、**外から来た循環は素通りする。循環では深さが確定しないので、ネストの深さの上限でも検出できない。** **資源の上限（ファイルサイズ・件数・ネストの深さ）を持ち、超えた入力は取り込まずに知らせること（MUST）。部分的に適用してはならない（MUST NOT）** —— 途中まで取り込んだ状態は、利用者から見て「壊れた文書」と区別がつかない。
...
**STATEMENT**: システムは、**操作の切れ目に、溜まった編集をまとめて 1 回**自動保存し、異常終了した次の起動で復旧を申し出ること。**編集のたびに書いてはならない（MUST NOT）。****保存された内容が壊れているとき、黙って破棄してはならない（MUST NOT）** —— 復旧できないことを人に知らせること。
...
**STATEMENT**: 文書が更新されたとき、システムは、**あらゆる更新で 1 ずつ増える整数**と、最後に書いた者と時刻を文書に持たせ、書き出す JSON にも載せること。
...
**STATEMENT**: `Agent API` は、**表 T-035 の規約**を満たすこと。

**ORIGIN**: UC-012 手順 2 〜 4、拡張 3a

**RATIONALE**: `FR-028` は「値で返す」ことしか定めていなかった。**規約 1 つずつを要求にすると 8 件になるが、いずれも「文書の一貫性を壊さずに機械へ口を開ける」という同じ目的の具体である。** 前プロジェクトの実測で、基準版の食い違いは 1 セッションに 6 回起きている。
...
**STATEMENT**: 人が `Agent API` を画面上で有効にできること。**有効であるあいだ、そのことが画面上に見えていること（MUST）。**
...
| NT-4 | 起動時の保留中の用件 | **1 枚に集約して出すこと。** 復帰・復旧の確認・`Agent API` の有効化を別々の面で順に出さない |
...
**ヘルプは全機能を示すこと（MUST）。基準環境（表 T-025 の MC-6）では 1 画面に収め、スクロールを要さないこと。** それより低い画面ではスクロールを許す。**「全機能」と「1 画面」は両立しうると実測されている**ので、どちらかを諦める前に割り付けを見直すこと。
...
**RATIONALE**: 1 つずつを要求にすると 5 件になるが、いずれも「**操作できるものが操作できるように見え、誤って確定しない**」という同じ原則の具体である。表で持つ。
...
**RATIONALE**: いずれも範囲外だが（表 T-002 の S-12）、**設計の余地だけは残すと決めている。** 機械向けの窓口が 1 つに揃っていれば、サーバー側はその薄い包みで済む。**逆に UI が直接ストアを書き換える形にすると、後から窓口を生やすことになり、そこは取り返せない。**
...
**親も子も持たない UID は無い。** すべての `GOAL` が `USE_CASE` または `FUNC_REQ` に受けられ、すべての `USE_CASE` と `FUNC_REQ` と `NON_FUNC_REQ` が親を持つ。**この性質は機械で検査する**（`docs/spec` を export して親を持たないノードを数える）。
