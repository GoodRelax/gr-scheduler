...
| MK-13 | ダブルクリック | **対象ごとに定めること（MUST）** —— コメントボックス ＝ 本文の編集（`FR-097`）／タスク（名称ラベルと本体のどちらでも） ＝ 名称の編集／担当ラベル ＝ 担当者名の変更／行見出し ＝ 行名の編集。⛔ **プロパティパネルを開く経路を本行に置いてはならない（MUST NOT）**（利用者の裁定 2026-08-27）—— **パネルの中身は `FR-072` が「最後に行われた操作」で決めており、選ぶ操作がすでにその操作である。**⚠️ **名称ラベルと本体を 1 つに畳んだのは、`NL-1` がラベルを形状の中に描くからである** —— 2 行に分けると同じ操作が表の 2 か所に並ぶ | — |
