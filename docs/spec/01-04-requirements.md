...
| G-2 | `TaskGroup` | `Task` が載る器。入れ子で階層になる（**`TaskGroup` の深さで最大 5 段**）。画面に描いたものは行（`Rows`）。⚠️ **WBS の深さはこれと別物で、上限を持たない** |
| G-3 | `TaskGroupMember` | どの `Task` がどの `TaskGroup` に載るかの対応 |
| G-4 | `stackOrder`（積み順） | 1 つの `TaskGroup` の中で縦に積む順。**自動で決まる値であり、人が指定する経路を持たない**（表 T-014 の ST-6） |
...
- 1a. 作成者が合流を選ぶ。
  - システムは現在の文書へ足し、同じタスクが両側にあるときは上書き・別として取込・取込をやめる、から選ばせる。
...
| HM-3a | **移動後の深さが `TaskGroup` の 5 段を超える移動を受け付けてはならない（MUST NOT）。** 部分木は**移動後の最深部**で測る。拒んだことを人に知らせる（`FR-076` の NT-1） |
...
**STATEMENT**: 作成者が行見出しパネルで行（`TaskGroup`）を選んだとき、システムは、その行の名前・色・高さをプロパティパネルに出して編集できるようにし、指定されたらその指定で帯を描くこと。**色**の指定が無い行は、**深さと行の位置**からテーマ色に基づいて解いた帯色で描くこと。**高さ**の指定が無い行は、段数から自動で決めること（`FR-003` の ST-9）。
...
**本要求の入口は、未着手のあいだ進捗マーカーの役も兼ねる**（表 T-021 の `PM-1a`）。したがって**未着手のタスクの状態を変える経路は、画面上で直接掴む経路のうちでは本要求の入口が唯一である**（表 T-023d の `GR-9`）。⚠️ **プロパティパネル（`FR-006` の表 T-016）と `Agent API`（`FR-028`）は値として編集する別の面であり、ここでいう経路には数えない**（`FR-011` と同じ切り分け）。
...
**STATEMENT**: システムは、実績バーの右端の外側に進捗マーカーを出し、**表 T-021 の記号**で状態を示すこと。**予定だけを表示しているときは、予定バーの右端の外側に出すこと（MUST）** —— 依存線が「予定の幾何に付く。予定を表示していないときに限り実績の幾何に付ける」と定めているのと同じ形である（`FR-009`）。表 T-038 の `OC-3` と表 T-023d の `GR-7` の場所も同じに読み替える。押されたときは、**表 T-021a に従って状態を巡らせること。**
...
| PM-4 | `(!)` | 遅れ | **導出**。**進行中のまま** `finish` を過ぎている / 未着手のまま `start` を過ぎている / 中断のまま `resume` を過ぎている。**完了したものを遅れとしない** —— 3 条件はいずれも状態を名指しする |
...
**RATIONALE**: **基準日を動かしても、バー・日付・完了率のどれも動かない**（`FR-011`）。動くのはイナズマ線と遅れ判定だけである。縦線を常時表示にしないのは、遅れが進捗マーカーと遅れの日数で読めるためで、線を増やすと画面が狭くなる。**基準日は設定値ではなく文書のデータである。**
...
**3 行は表 T-021 の `PM-4` の 3 条件と 1 対 1 に対応する。**
...
| MK-6 | 何にも当たらない場所での素の左ドラッグ（構えなし） | **範囲選択。** `Shift` 併用で既存の選択に追加する |
| MK-7 | **`Ctrl` だけを伴う**ドラッグ / 中ボタンドラッグ | パン |
| MK-8 | **アイテム**の上での素の左ドラッグ | **そのアイテムを動かす。** そのアイテムが選択に含まれているときは、**選択されている全部を動かすこと（MUST）** |
...
**STATEMENT**: ポインタが使われていないとき、システムは、**表 T-030 の操作**をキーボードだけで完了できるようにすること。

**ORIGIN**: UC-007 / UC-004

**RATIONALE**: `NFR-008` は「主要な操作をキーボードだけで」とだけ述べており、**何が主要な操作かを定めていなかった。** 表で定めることで、適合の判定ができるようになる。
...
**表 T-030 — キーボードだけで完了できること**

| 行 ID | 操作 |
| --- | --- |
| KB-1 | アイテム間をフォーカス移動する |
| KB-2 | フォーカス中のタスクを 1 日 / 1 行ずらす |
| KB-3 | フォーカス中のタスクの端を動かして期間を変える |
| KB-4 | ズームし、等倍へ戻す |
| KB-5 | **依存線の作成を最後まで完了する** |
| KB-6 | **選択を作り、広げ、解除する**（`FR-081`）。ポインタの範囲選択に対応する経路が無いと、`FR-032` / `FR-033` / `FR-034` がキーボードだけでは使えない |
| KB-7 | **フォーカスの対象に依存線とコメントボックス・ハイライトボックスを含める** —— 表 T-036 の `SK-3` が `Delete` でそれらを消すと定めている以上、選ぶ経路が要る |
| KB-8 | **構えを選び、解除する**（表 T-023b の全数） |
| KB-9 | **キャレットを置き、動かす** —— `SK-1` の「構えているものをキャレット位置に置く」が前提にしている |
| KB-10 | **フェード点を動かす**（`FR-075`）、**名称ラベルの位置を変える**（`FR-002`） |
| KB-11 | **スクロールとパンを行う** —— `MK-9` はズームについてだけ「ホイールを唯一の経路にしてはならない」と定めており、パンには対応物が無かった |
| KB-12 | **`Dual Cursor` の 2 本を置き、動かす**（`FR-082`） |
...
**変更は取り消しの対象とすること（MUST）**（表 T-027 の `UN-3`）。**暦を変えると期間の計算がすべて動く**ので、影響する `Task` の件数を知らせること（MUST）。
...
**RATIONALE**: `GL-002`（全体日程を 1 画面で見られる）を受ける操作が、ユースケースにも要求にも無かった。**測る対象は描画の実寸とし、算入するものは表 T-038 に従うこと（MUST）** —— 段割当（`FR-003`）と同じ表を使う。**2 か所で別々に数え上げてはならない（MUST NOT）。** **日付の範囲だけで測ると、左の張り出しが行見出しパネルに隠れ、どのスクロール位置でも見えなくなる。**
...
**表 T-202 の表示可否は、当該の要素を描くすべての要求の状態条件に加わること（MUST）。ただし操作の入口を閉じてはならない（MUST NOT）** —— 隠した要素を編集する経路まで消えると、`progressMarkerVisible` を `false` にしただけで着手済みタスクの状態を変える手段が無くなる。**隠している要素の掴み領域は残してはならない（MUST NOT）** —— 描いていないものが当たると、何も無い場所でドラッグしたのに既存の要素が動く（表 T-023a）。**代わりに、隠した要素が持っていた操作を、プロパティパネル（表 T-016）または当該要素を対象とする別の面から行えるようにすること（MUST）。面の形は Chapter 5 が決める。** ⚠️ **表 T-016 は `Task` の属性表であり、依存線の行を持たない。** したがって `dependencyVisible` を `false` にしたときは、依存線を選ぶ（表 T-023c の `SL-1`）・ラグを編集する（`FR-009`）・削除する（表 T-036 の `SK-3`）経路を別の面に残すこと（MUST）。キーボードだけの経路（表 T-030 の `KB-7`）も同じである。 表示の切り替えは取り消しの対象外（`UN-7`）なので、失った操作を履歴から戻せない。 例えば `FR-014`（イナズマ線）は「基準日が置かれているとき」と書いているが、`progressLineVisible` が `false` のあいだは描かない。**この 1 文で一括して閉じる** —— 個別の要求文に条件を書き足すと、表 T-202 に行を足すたびに要求の追記が要る。
...
**取り込みでは `TaskGroup` の深さ上限で受け付けを拒んではならない（MUST NOT）。** 6 段目以降にあたる階層には**器を作らず、その配下のタスクを 5 段目の行に載せること（MUST）**。上限が掛かるのは人が階層を作るときだけである（`FR-004`）。**WBS の深さはそのまま保たれる**ので、書き出せば元の階層が戻る。
...
**同じ機能の入口を画面上の 2 か所に置いてはならない（MUST NOT）。** どちらを押しても同じことが起きる面が 2 つあると、利用者は違いを探して迷う。**例外は表示言語の切替だけである**（`FR-038`）—— **メニューが読めない人が探す機能なので、入口 1 か所では届かない。** 例外を増やすときは、ここに理由を書いて足すこと。
...
**ORIGIN**: GL-006 / WCAG 2.1 の 2.1.1 / 表 T-004 の LM-2
