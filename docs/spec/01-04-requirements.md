...
**STATEMENT**: `GRS` は、文書が持つ暦に従って非稼働日（週末・祝日）を稼働日と区別して描くこと。**期間の計算に用いる「稼働日」も同じ暦に従うこと。1 日を何時間と決め打ちしてはならない（MUST NOT）。** **稼働日を交換相手の期間（時間の量）へ換算するときは `Project.minutesPerDay` に従い、それが空のときは表 T-209 の `S-128` に従うこと（MUST）** —— この MUST NOT が禁じているのは実装が根拠なく数を選ぶことであって、**仕様が定め文書が上書きできる既定は決め打ちではない。**
...
**バイト列をそのまま突き合わせて合否としてはならない（MUST NOT）** —— 属性の並び・要素の間の空白と改行・自己終了タグの綴り方・名前空間の接頭辞は、書き出す側を変えれば揺れる。バイト一致を求めると、取り込んだ文字列を丸ごと抱えて書き戻すほかに手が無くなり、合流（`FR-056`）も編集も成り立たない。⚠️ **どの正規化を用いるかは Chapter 6.1 が持つ** —— 本要求が課すのは、比べる前に双方へ同じ正規化を当てることだけである。
