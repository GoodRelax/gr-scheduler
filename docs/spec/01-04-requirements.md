...
**薄い掴みシロ（`FR-043`）は、掴まれるまで「人が置いた」に数えない（MUST NOT）** —— 描いてあるだけで文書には無い値だからである。**進捗マーカーの操作（`FR-013`）による状態の確定は「人が置いた」に数える（MUST）** —— 上の MUST NOT は**既に実績を持つタスク**について述べたもので、実績がまだ無いタスクには当てはまらない。**未着手のタスクを 1 押しで完了にできること（MUST）** —— 1 日で終わる作業は日程表で頻出するので、2 手を要求すると道具として重い。⚠️ **未着手の印は `FR-043` が定める。選択してからマーカーを探す 2 手にしない。置く値は表 T-021a に従うこと。**
...
**STATEMENT**: `Task` が未着手であるあいだ、`GRS` は、実績の入力を始める掴みシロを**2 つ**、実績の開始点と終了点として**薄く**タスクの上に示し、**どちらが掴まれたときも実績開始日と実績期間（`actualDuration`）と `resumeValid`（`true`）を**置くこと（MUST）。**掴みシロは掴むだけとし、押して状態を巡らせてはならない（MUST NOT）** —— 状態を巡らせるのは進捗マーカーである（`FR-013`）。
...
**掴んで置く値は、実績開始日 ＝ 予定の開始日、実績期間（`actualDuration`）＝ `_assets/tbl-settings.md` の `S-129`、`resumeValid` ＝ `true` とすること（MUST）。**
...
**そのタスクが遅れていると判定されたときは、進捗マーカーを `(!)`（表 T-021 の `PM-4`）にすること（MUST）** —— 未着手のまま `start` を過ぎたタスクは `UC-006`（遅れを見つける）が最も見つけたい対象である。⚠️ **`(!)` は進行中・中断の遅れと同じ記号であり、位置も同じである** —— **未着手であることは実績バーの有無が担う。** 未着手のタスクは実績バーを持たず、薄い掴みシロを 2 つ持つだけである。
...
**未着手のマーカーと、実績入力のダミー（`FR-043`）は薄く描き、ポインタが乗っているあいだだけ濃くすること（MUST）** —— 作法は表 T-051 の `HF-6` と同じである。濃さの値は `S-131`。**色は実績バーの色を継ぎ、独立した色を保存しない**（`FR-041`）。⚠️ **薄さで進行中と区別してはならない（MUST NOT）。形が意味を担う**（`FR-030`）。⚠️ **遅れの `(!)` は薄くしない** —— `PM-4` が勝つ状態であり、`UC-006` が最も見つけたい対象だからである。
...
⚠️ **ポインタが乗ったことで濃さが変わる UI パーツと、待ち時間で出る説明は「描く内容が変わる」に当たる** —— 表 T-051 の `HF-6`（行の操作子）、`FR-053`（コマンドパレット）、`FR-043` の掴みシロと未着手のマーカー、`FR-092` の `EZ-2`（アイコンの説明）である。**これらは本 MUST NOT の対象ではない。**
