...
| XO-5 | 日付を自動で決める計算系 | 日付は人が決める。クリティカルパス計算と資源平準化は範囲外である（S-10） |
...
5. 作成者が表示と書式の設定を開き、表示の切り替え・テーマ・行の色と高さを整える。
6. システムは指定に従って描き直す。
...
| NL-4 | 右に出しても干渉しない | **そのまま右に出す。** 干渉は段割当が解く —— 外へ出したラベルは表 T-038 の `OC-1` により占有幅に入っているので、`ST-3` の貪欲割当が干渉しない段を選ぶ。**ここでずらす規則を別に置くと、ラベル配置 → 占有幅 → 段割当 → 干渉判定 → ラベル配置 の循環になる** |
...
| ST-7 | 段数に機能上の上限を設けない。**ただし 1 つの `TaskGroup` あたり 256 段を安全弁とし、達したらエラーを出して処理を止めること（MUST）。** 黙って切り捨てても、重ねて押し込んでもならない（MUST NOT） |
...
| OC-8 | 遅れの日数の表示（`FR-047`）と期限の印（`FR-045`） | 右 |
...
| HR-1a | 畳んだときの中身 | **畳んだ `TaskGroup` の `Task` を親の行に載せて描くこと（MUST）** —— 「畳んだ内容を親へ集約する」（`UC-002` 手順 5）の意味はこれである。**縦ズームが駆動するグループ LOD（表 T-005a の L-3）と同じ絵になること（MUST）** —— 手で畳んだときと自動で畳まれたときで絵が違うと、どちらの状態か読めない。段割当（`FR-003`）と全体表示の測定（`FR-055`）も集約後の姿で行う |
...
**項目名は英語表記とすること（MUST）。** **`stop`（中断日）をモデルの列として持ってはならない（MUST NOT）** —— 中断したときの実績バーの右端と同じ値なので、書き出すときに算出する。⚠️ **ただし取り込んだ `Stop` は Carry として保つこと（MUST）** —— 算出値が原値と 1 日でも違うと、**編集していないタスクの値を書き換える**ことになり（表 T-033 の `EX-2`）、未編集の往復も壊れる（`FR-021`）。**人がそのタスクの実績を編集したときに限り、算出値へ置き換える** —— `FR-012` の完了率と同じ規則である。
...
**進捗マーカーの操作（`FR-013`）による状態の確定は「人が置いた」に数える（MUST）** —— 上の MUST NOT は**既に実績を持つタスク**について述べたもので、実績がまだ無いタスクには当てはまらない。**未着手のタスクを 1 押しで完了にできること（MUST）** —— 1 日で終わる作業は日程表で頻出するので、2 手を要求すると道具として重い。⚠️ **未着手では進捗マーカーが出ていない**（表 T-021 の PM-1 は選択時のみ表示）ので、**`FR-043` の実績入力の入口がマーカーを兼ねること（MUST）** —— その入口は未着手のあいだ常に見えており、**掴めば実績開始日を置き、押せば状態を巡らせる。** 選択してからマーカーを探す 2 手にしない。**置く値は表 T-021a に従うこと。**
...
**STATEMENT**: `Task` が未着手であるあいだ、システムは、実績の入力を始める入口を**未着手と分かる見た目**でタスクの上に示し、掴まれたときに実績開始日を置くこと。
...
⚠️ **`start` または `finish` を持たない `Task` を受け付けてはならない（MUST NOT）** —— 積み順（`ST-2`）・遅れの量（表 T-021b）・完了率のいずれもが両方の存在を前提にしている。取り込む入力は `FR-023` の検証で弾く。
...
**STATEMENT**: システムは、実績バーの右端の外側に進捗マーカーを出し、**表 T-021 の記号**で状態を示すこと。**実績バーが無いとき（未着手）、および予定だけを表示しているときは、予定バーの右端の外側に出すこと（MUST）** —— 依存線が「予定の幾何に付く。予定を表示していないときに限り実績の幾何に付ける」と定めているのと同じ形である（`FR-009`）。表 T-038 の `OC-3` と表 T-023d の `GR-7` の場所も同じに読み替える。クリックされたときは 未完了 → 完了 → 中断 → 未完了 と巡らせること。
...
| PM-1a | （マーカーを出さない） | 未着手 | **`FR-043` の実績入力の入口がその役を兼ねる** —— 同じ機能の入口を画面上の 2 か所に置かない（`FR-029`） |
| PM-2 | `(✓)` | 完了 | 完了 |
...
**未着手のタスクでは PM-1 が出ていないので、状態を巡らせる入口は `FR-043` の実績入力の入口が兼ねる**（`FR-011`）。

**PM-4 は他の行と必ず同時に成立する。** 3 つの条件はいずれも PM-1 または PM-3 に当てはまる状態を名指ししているためである。**マーカーは 1 つしか出ないので、PM-4 が成立したときは PM-4 を出すこと（MUST）。** 遅れの発見は独立したユースケース（`UC-006`）として立っており、状態の表示より優先される。

**表 T-021a — マーカーを押して状態を確定したときに置く値**
...
| 行 ID | 条件 | 結果 |
| --- | --- | --- |
| PD-1 | 中ボタンドラッグ、または `Ctrl` ＋ 左ドラッグ | **パン。** 構えと当たりによらず優先する |
...
| AR-5 | コメントボックス | **その位置にコメントを置く**（`FR-019`） | 当たった対象を指す先にしてコメントを置く |
...
| MK-12 | 割当の無い修飾キー付きドラッグ（`Alt`、`Ctrl` ＋ `Shift`） | 何もしない。ただし MK-10 によりブラウザの既定動作は止める |
...
| KB-9 | **キャレットを置き、動かす** —— `SK-1` の「構えている図形をキャレット位置に置く」が前提にしている |
...
**保証しないのは全体についてである。下限は別に定める。表示を最も縮めたとき、最も浅い階層（WBS の深さ 1 と `TaskGroup` の深さ 1）のすべてが 1 画面に収まること（MUST）。** ここまで落ちても収まらないなら、`GL-002` は達成されていない。**保証しないと明記するのは、保証しようとすると LOD の段階を無限に下げることになり、読めない絵で「収まった」と主張することになるためである。**
...
**表 T-202 の表示可否は、当該の要素を描くすべての要求の状態条件に加わること（MUST）。ただし操作の入口を閉じてはならない（MUST NOT）** —— 隠した要素を編集する経路まで消えると、`progressMarkerVisible` を `false` にしただけで着手済みタスクの状態を変える手段が無くなる。**隠している要素の掴み領域は残してはならない（MUST NOT）** —— 描いていないものが当たると、何も無い場所でドラッグしたのに既存の要素が動く（表 T-023a）。**代わりに、隠した要素が持っていた操作をプロパティパネル（表 T-016）から行えるようにすること（MUST）。** 表示の切り替えは取り消しの対象外（`UN-7`）なので、失った操作を履歴から戻せない。 例えば `FR-014`（イナズマ線）は「基準日が置かれているとき」と書いているが、`progressLineVisible` が `false` のあいだは描かない。**この 1 文で一括して閉じる** —— 個別の要求文に条件を書き足すと、表 T-202 に行を足すたびに要求の追記が要る。
...
**STATEMENT**: 2 つ目以降の MSPDI を合流させるとき、システムは、**表 T-032 の規則**に従うこと。
...
| MG-9a | **取込の最中に来た次の取込を受け付けてはならない（MUST NOT）** —— 3 択を問うている間に別のファイルが落ちると、`MG-6` が復帰先とする「取込前」がどの時点か決まらなくなる。先の取込が終わるまで待たせる |
...
**行の名前が行見出しパネルの幅に収まらないときは、打ち切って全文をツールチップで出すこと（MUST）** —— 名称ラベルと同じ扱いである（表 T-013 の `NL-2`）。深さのインデント（`S-37`）ぶん使える幅が狭くなるので、深い段では必ず起きる。
...
**ORIGIN**: UC-011 手順 1（表 T-036 の SK-10 に割当があるのに、開く操作を定める要求が無かった）

**RATIONALE**: **`Ctrl+O` は表 T-036 の `SK-10` に載っているのに、「開く」を定めた要求が 1 件も無かった。** 表 T-034 は**起動時**の順序だけを扱い、合流の規則（`FR-022` / `FR-056`）は**2 つ目以降の MSPDI** に限られる。**編集途中の文書を開いたまま別のファイルを開いたとき、未保存の編集が黙って捨てられるのかどうかが決まらなかった。**
...
| OP-2 | 経路 | ファイル選択、およびドラッグ＆ドロップ（表 T-008 の R-1）。**ドロップは日程の描画領域の判定順序（表 T-023a）を適用しない面として扱う** |
| OP-3 | 現在の文書 | **置き換える。** 合流したいときは取込（`FR-022`）を使う —— **開くと合流は別の操作である** |
| OP-4 | 未保存の編集 | **捨てる前に確認を求めること（MUST）。** 黙って捨ててはならない（MUST NOT）。取り消しの履歴は引き継がない（`LM-9` と同じ理由） |
| OP-5 | 検証 | 経路によらず `FR-023` の検証を通すこと（MUST） |
| OP-6 | JSON を読んだとき | 文書のデータと `documentSettings` を復元すること。**欠けている設定値は既定値で補い、知らないキーは捨てずに保つ**（往復で失わないため） |
...
**STATEMENT**: 文書が更新されたとき、システムは、**1 ずつ増える整数**と、最後に書いた者と時刻を文書に持たせ、書き出す JSON にも載せること。**版数を上げるのは、文書に保存される値を変える更新すべてとすること（MUST）。** 表 T-027 の対象行に加えて、文書名（`FR-035`）・文書の基本情報（`FR-074`）・テーマ（`FR-041`）・行の色と高さ（`FR-042`）も含む。**版数を上げないのは、文書に保存されない操作だけである** —— 選択・構えの変更・フォーカスの移動。⚠️ **ズーム・スクロール・パン・表示の切り替え・`Dual Cursor` の位置・パネル幅は文書に保存される**（表 T-202 / T-203）**ので版数を上げる。** 取り消しの対象外であることとは別である。**取り消しは「人が戻したい編集か」、版数は「読んだものが古くなったか」を判定しており、目的が違う。**
