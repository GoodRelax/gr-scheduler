...
**STATEMENT**: 作成者が行（`TaskGroup`）に色を指定したとき、システムは、その色で帯を描くこと。指定が無い行は、**深さと行の位置**からテーマ色に基づいて解いた帯色で描くこと。

**ORIGIN**: UC-002

**RATIONALE**: 入力は 2 つある —— 深さは構造（見出しの行かどうか）、行の位置は縞である。**縞は行の位置で数えること（MUST）。上書きの有無で数えてはならない（MUST NOT）** —— 指定の無い行だけで数えると、**1 行に色を付けた瞬間にその下の全行の縞が反転し**、操作と結果が対応しなくなる。
...
**RATIONALE**: **誤操作への備えは取り消しの 1 本に絞る。** 確認ダイアログを増やす代わりにこれが要る。破壊的な操作（連鎖削除・取込による上書き）を戻せることは、適合範囲に含めたアクセシビリティの基準でもある。**`Agent API` 経由の変更も同じ履歴に積み、呼び出し 1 回を 1 段とすること（MUST）** —— 口が 2 つあっても履歴は 1 つである。
...
**RATIONALE**: 文書名は保存にも出力にも出る。画面から変えられないと、別の手段が要ることになる。
...
**STATEMENT**: 利用者がパネルの境界をドラッグしたとき、システムは、行見出しパネルとプロパティパネルの幅を変えること。**2 つの幅の上限は互いに依存するので、片方だけを見て判定してはならない（MUST NOT）。**
...
**ORIGIN**: UC-007（要望 39。表 T-011 の V-7 が吸収済みと記していたが要求が無かった）

**RATIONALE**: **`FR-011` と `FR-012` はどちらも「稼働日」で計算すると述べているのに、稼働日が何かを定める要求が無かった。** 暦は取り込んだ MSPDI が持っており、曜日ごとの稼働可否と例外日からなる。**繰り返し指定の例外日（毎年の元日など）は実日付へ展開しないので、未対応であることを利用者に知らせること。**
...
**RATIONALE**: `GL-002`（全体日程を 1 画面で見られる）を受ける操作が、ユースケースにも要求にも無かった。**保証しないと明記するのは、保証しようとすると LOD の段階を無限に下げることになり、読めない絵で「収まった」と主張することになるためである。**
...
**STATEMENT**: 利用者が表示の切り替えを求めたとき、システムは、**`_assets/tbl-settings.md` の表 T-202 に挙げた項目**をそれぞれ切り替えられるようにすること。
...
**予定と実績の表示は 3 値の列挙とし、両方を非表示にできてはならない（MUST NOT）** —— バーが 1 本も出ない画面は故障に見える。**担当ラベルと完了率ラベルの既定が非表示である**のは、低ズームでの段数膨張の主因がこの 2 つだと実測されたためで、`GL-002` を守るための判断である。
...
**RATIONALE**: 証跡として意味があるのは「誰がいつ画面に出していたか」である。**作者の名前が他人の画面に出るのは誤りである。** **書き出した SVG / PNG にも透かしは載るので、開いた者の名前が第三者へ伝わりうる。そのことを利用者に示し、画像に載せるかどうかを選ばせること（MUST）**（制限事項 LM-8）。 日時は `YYYY-MM-DDThh:mm:ssZ`（UTC）で書く。**MSPDI 内部の日付はタイムゾーンを持たないローカル時刻なので別扱いであり、混同してはならない（MUST NOT）。** 非表示にするには合言葉を求め、SHA-256 のハッシュ比較で照合する。**生の合言葉をコード・モデル・出力に保存してはならない（MUST NOT）。**
...
| AG-6 | 監視は「**自分が最後に読んだ版より後の変更だけ**」を通知すること。対話に書いただけでは起きない |
...
**RATIONALE**: `GL-006` を受ける経路がアイコンとツールチップしか無く、**操作の全体像に到達する道が 1 本も無かった。** 一覧は表 T-023 と同じ内容を人が読む形で出すものであり、別の規則を作らない。
...
**STATEMENT**: システムは、将来サーバーを介した共同編集を足すとき、**`Agent API` の関数群を作り直さずに包める形**であること。

**RATIONALE**: 共同編集と権限設定は範囲外だが（表 T-002 の S-12）、**設計の余地だけは残すと決めている。** 機械向けの窓口が 1 つに揃っていれば、サーバー側はその薄い包みで済む。**逆に UI が直接ストアを書き換える形にすると、後から窓口を生やすことになり、そこは取り返せない。**
