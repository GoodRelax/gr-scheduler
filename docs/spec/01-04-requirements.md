...
| CN-9 | **画面とデータモデルの分離** | **画面（GUI）とデータモデルを層として分離する。同じものを画面側とデータ側で別の名前で呼ばない。** 前プロジェクトで不具合が止まらなかった根因がこれである。層の具体は Chapter 5 が決めるが、**分離すること自体は破れない制約**とする |
...
| R-9 | D-1 | OS のクリップボード | 画像 | 書き出し（`FR-033` / 表 T-024 の IO-6） | **送信のみ・読まない**（検証の対象にならない）。⚠️ **出て行く経路なので `LM-8`（透かしに載る名前が第三者へ伝わりうる）はここにも効く** |
...
> **V-13 は取りこぼしではない。** 最優先事項は**目標**として Chapter 1.3 が、作り方の原則は**表記と命名の規約**として Chapter 1.9 が、**層の分離**は制約として表 T-003 の CN-9 が受けている。欠番 3 件は要望として取り下げられたものである。
>
...
| ST-7 | 段数に機能上の上限を設けない。**ただし 1 つの `TaskGroup` あたりの段数に安全弁を置き、達したらエラーを出して処理を止めること（MUST）。** 黙って切り捨てても、重ねて押し込んでもならない（MUST NOT）。**数えるのは、人がその `TaskGroup` に置いた `Task` が同時に重なる段数である** —— 畳んだ配下は描かないので（表 T-015 の `HR-1a`）、畳みで段数が増えることはない。段数は `_assets/tbl-settings.md` の `S-89` が持つ |
...
| OC-3 | 進捗マーカー | 右（実績バーの右端の外側） |
| OC-4 | 再開アイコン | 右（マーカーのさらに外側） |
...
**STATEMENT**: 作成者が行見出しツリーで階層を移したとき、システムは、**表 T-015a の規則**に従うこと。
...
**RATIONALE**: 項目 1 つずつを別の要求にすると、表の行数ぶんの要求になり追跡だけが重くなる。**画面上でタスクを動かしたときは、プロパティの日付を追随させること（MUST）** —— 同じ値を 2 つの面から編集できる以上、片方だけが動く状態を作らない。
...
**RATIONALE**: **誤操作への備えは取り消しの 1 本に絞る。** 確認ダイアログを増やす代わりにこれが要る。**例外は行の削除だけである**（`FR-032`）—— 連鎖して消える範囲が大きく、取り消しで戻せても「何が消えたか」を人が把握できないため。**例外を増やさない（MUST NOT）。**破壊的な操作（連鎖削除・取込による上書き）を戻せることは、適合範囲に含めたアクセシビリティの基準でもある。**`Agent API` 経由の変更も同じ履歴に積み、呼び出し 1 回を 1 段とすること（MUST）。ただし積むのは表 T-027 の対象行に当たる変更だけである** —— 表示の切り替えのように対象外の操作は、`Agent API` から呼んでも履歴に残らない（表 T-035 の AG-10）。 —— 入口が UI と `Agent API` の 2 つあっても、履歴は 1 つである。
...
**STATEMENT**: 作成者がタスクを選んでコピーし貼り付けたとき、システムは、それらを複製すること。行見出しツリーでは、選ばれた `TaskGroup` を**部分木ごと**複製すること。

**ORIGIN**: UC-001 / UC-002

**RATIONALE**: 同じ形の工程を対象ごとに横へ並べるのが日程表の主要な作業である。1 つずつ引き直させると、作図ソフトに対する利点が消える。

**複製に使う置き場はアプリの中に持つこと（MUST）。OS のクリップボードから読み込んではならない（MUST NOT）** —— **外から来た文字列をタスクとして解釈すると、データの正確性を保証できない。** 表 T-008 の経路が増えないので、検証の対象も増えない（`FR-023`）。**画像の書き出しだけは OS のクリップボードへ送る**（表 T-024 の IO-6。送るだけで読まない）。
...
**RATIONALE**: **縦と横で仕組みが違う。** 縦は段割当（FR-003）の結果として自動で揃うが、**横は人が実行する。** 日付は人が決めるものであり、システムが動かしてよいものではない。**揃え先を「最後に選んだタスク」とするのは、選ぶ順序で人が基準を決められるからである** —— 最も早い日や最も遅い日に寄せると、どれに揃うかを人が選べない。**したがって選択は順序を保つこと（MUST）**（`FR-081`）。範囲選択は順序を作れないので、**範囲選択だけで整列を実行させてはならない（MUST NOT）** —— 基準が決まらない。
...
**RATIONALE**: 分母は予定の期間である。`actualStart` が `start` より後でも分母は変わらない。**完了したら 100 を下回らせない**（`actualFinish` があって 100 未満は相手ツールで矛盾する）。型は整数・0 以上とし、**100 を超える値を丸めない** —— 予定より長くかかって完了したタスクは 100 を超える。**上限を課す理由は無い**（`LM-7` は 100 超の値が相手へ渡る前提で書かれている）。
...
**RATIONALE**: 割当を 1 つずつ要求にすると追跡だけが重くなる。表で持つ。**同じ操作に 2 つの答えが立たないよう、割当の前に判定順序（表 T-023a）を置く。** 以前は「行の上でドラッグ」と「背景の上でドラッグ」という**場所**で書き分けていたため、行の空き領域が両方に当たっていた。
...
| PD-3 | **何かに当たった**（判定の順と優先は MK-9a） | **そのものへの操作。** ⚠️ **構えが依存線のときは表 T-023d を適用せず**、当たったタスクの**左半分 / 右半分**で依存の端点を決める（`FR-009`） —— 端点の掴み代で判定すると低ズームで掴めなくなる |
...
| GR-9 | 実績入力の入口 | 未着手のタスクの上 | 掴めば `actualStart` を置き、押せば状態を巡らせる（`FR-043`） |
...
**RATIONALE**: 割当 1 つずつを要求にすると、表の行数ぶんの要求になる。表で持つ。**修飾キーの付いた入力は画面全体で既定動作を止める**（表 T-023 の MK-10）—— 止め忘れると `Ctrl+S` がブラウザの HTML 保存にすり替わる。
...
| 行 ID | 操作 | 割当 |
| --- | --- | --- |
| SK-1 | **構えているもの**（依存線を除く。依存線は `SK-1a`）をキャレット位置に置く | `Enter` |
| SK-1a | 依存線を構えているとき、フォーカス中の対象を依存の端点にする | `Enter` |
...
**RATIONALE**: 狭い画面に最大の情報量を出すという方針（`GL-002`）の最も直接的な手段である。**入って戻れないと道具として使えない**ので、同じ入口で解除できることを要求に含める。`Esc` が全画面から戻る鍵として効くかどうかはブラウザ側の挙動であり、**アプリに消費対象が無いときだけブラウザへ渡る**（表 T-028 の IN-4）。⚠️ **構えているあいだは消費対象があるため渡らない。** 構えたまま全画面にした利用者は `Esc` を 2 回押すことになり、**1 回目で構えが解ける**（IN-4a）。
...
| DC-3 | 測る | 2 本の間の日数を示す |
| DC-4 | 出る | 同じ入口の再押下、カーソルなしの入口、または `Esc` |
...
**RATIONALE**: 切り替えの対象は表 T-202 が持つ。**1 つずつを要求にすると表の行数ぶんの要求になる。** 値と既定は表 T-202 が持っているので、要求はそこを指す。
...
**STATEMENT**: 閲覧者が透かしの設定を求めたとき、システムは、**透かしに出す名前**を入力させ、`localStorage` へ保存すること。**名前が設定されていないまま日程表を開いたときは、設定を促すこと（MUST）。** 作成者が**透かし解除パスワード**を設定したときは、その SHA-256 だけを保存し、**生の値を保存してはならない（MUST NOT）。**
...
**名前は文書に保存してはならない（MUST NOT）** —— **作成者の名前が他人の画面に出るのは誤りである**（`FR-020`）。読む人の環境に属する値なので `localStorage` へ置く（`S-99a`）。**既定の透かし解除パスワードは表 T-207 が持ち、利用者が設定しなければそれを使う。**
...
**RATIONALE**: `FR-022` は「同じタスクが来たときに 3 択で選ばせる」ことしか定めていなかった。**合流は 1 つの操作ではなく、判別・衝突・重複・取消のそれぞれに規則が要る。** 1 つずつを要求にすると表の行数ぶんになるので表で持つ。**間違えると外部 WBS マスタ側のデータを壊す。**
...
**100 を超える値をそのまま出すこと（MUST）** —— `FR-012` は予定より長くかかって完了したタスクが 100 を超えると定め、**上限を課さないと決めている。** ここで 100 に丸めると、その決定が画面で無効になる。
...
**RATIONALE**: **見えているものが成果物になることが、この道具の前提である。** 書き出しで別の絵が出るなら、画面で整える作業そのものに意味が無くなる。`FR-025` は出力サイズと文字サイズを文書に保存することまでを定めていたが、**見た目が一致することを求める要求が無かった。** 収まらないときに縮小して収める（`FR-025`）のは倍率の話であり、**描くものの取捨選択を変えてよいという意味ではない。**
...
| OP-5 | 検証 | 経路によらず `FR-023` の検証を通すこと（MUST） |
...
**RATIONALE**: `FR-028` は「値で返す」ことしか定めていなかった。**規約 1 つずつを要求にすると表の行数ぶんになるが、いずれも「文書の一貫性を壊さずに `Agent API` を開ける」という同じ目的の具体である。** 前プロジェクトの実測で、基準版の食い違いは 1 セッションに 6 回起きている。
...
| NT-3 | 破壊的な結果を伴うとき | 対象の**件数**を添えること（連鎖削除・取込による上書き・担当者名の波及） |
...
**文字サイズの変更は目盛にも及ぶこと（MUST）** —— 目盛の文字と目盛の帯の高さがこれに追随し、**その保存値が書き換わる**（`_assets/tbl-settings.md` の `S-3` / `S-2`）。⚠️ **目盛だけ据え置くと、読む人が文字を大きくする手段が目盛に無くなる。** 目盛の粒度の判定もこれに従う（表 T-205 の注）。
...
**RATIONALE**: 1 つずつを要求にすると表の行数ぶんになるが、いずれも「**操作できるものが操作できるように見え、誤って確定しない**」という同じ原則の具体である。表で持つ。
...
| IN-4a | **構えているあいだは `Esc` に消費対象があるため、ブラウザへ渡らない。** 全画面から `Esc` で戻る経路（`FR-071`）はブラウザ側の挙動に委ねているので、**構えたまま全画面にした利用者は `Esc` を 2 回押すことになる。** そのことを `FR-071` の前提として書くこと（MUST） |
