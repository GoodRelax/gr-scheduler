...
| HF-6 | **操作子は、その行の名前にポインタが乗っているあいだだけ描くこと（MUST）** —— 常に描くと、日程より操作子が目立ち、行の名前ともぶつかる。⛔ **描かないあいだも、確保する場所を変えてはならない（MUST NOT）** —— 規則と理由は `FR-085` が持つ。⚠️ **以前は「薄く描き、乗っているあいだだけ濃く」と定めていた**（利用者の裁定、2026-08-25）|
...
**各段が刷るものを次のとおりとすること（MUST）**（利用者の裁定 2026-08-26）。**年の段は年の数字、月の段は月の数字、週の段はその週の始まりの日の数字、日の段は日の数字と曜日**を 1 段に並べる。⛔ **月を語で書いてはならない（MUST NOT）** —— **数字なら言語で幅が変わらず、`S-83` が両言語で 1 つの値でいられる。**⛔ **週の段に曜日を書いてはならない（MUST NOT）** —— 週の始まりは `Project.weekStartDay` が 1 つに決めているので（`FR-054`）、**どの刻みにも同じ曜日が並ぶ。**⭐ **曜日の語がどこに住むかは `FR-038` が持つ。**
