...
| HR-6 | 非表示 | 行を隠す。親の非表示グループタブから戻せること。**親を持たない最上位の行は、行見出しパネルの根に置く非表示グループタブから戻せること（MUST）** —— `FR-085` が最上位の行を許しているためである。**隠した状態を文書に保存すること（MUST）** —— 保存しないと、書き出して読み直したときに隠した行が戻り、`WY-1` が成立しない。**隠した行の配下の行と、その行に載っている `Task` を描いてはならない（MUST NOT）。配下の `Task` を親の行に載せ替えてはならない（MUST NOT）** —— 理由は `HR-1a` と同じである。⚠️ **畳みと違い、グループ LOD と同じ絵になることを求めない** —— 隠しは人が駆動するので、倍率で戻してはならない（MUST NOT） |

**表 T-051 — 行ごとの折り畳みの操作面**

| 行 ID | 規則 |
| --- | --- |
| HF-1 | 行見出しパネルの各行に、**開く操作子と、その行自身を閉じる操作子と、配下をすべて閉じる操作子を 1 つずつ**置く。⚠️ **3 つ目は 2026-08-30 の利用者の裁定で足した** —— ⛔ **それまで 表 T-015 の `HR-4` は入口を 1 つも持たなかった**|
...
| HF-10 | **行見出しパネルの最上部の右端に、すべての行を開く操作子を 1 つ置くこと（MUST）** —— 表 T-015 の `HR-1` である。⛔ **最上位の行が自分を畳むと、それを開く操作子がどこにも無くなる**（`HF-3`）。⛔ **ピン止めの操作子と重ねてはならない（MUST NOT）** —— `FR-098` はピン止めした行を画面の上端に置き、`HF-4` はその操作子を右端に寄せるので、置き場が衝突する。⚠️ **倍率も表示位置も動かさない** —— 動かすのは `FR-055` であり、あちらとは別の操作である |
