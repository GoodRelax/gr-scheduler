...
| HF-5 | **行の名前の文字サイズにかかわらず、操作子を同じ大きさで描くこと（MUST）。名前が操作子より大きいときは、名前の上端に揃えず、そこから下げて揃えること（MUST）。中央で揃えてはならない（MUST NOT）** —— 中央にすると名前の文字サイズが変わるたびに操作子の位置が動き、`HF-4` が横で禁じたことを縦で許すことになる。**下げる量は `_assets/tbl-settings.md` の表 T-206 の `S-139` が持ち、その行の名前の文字サイズに比例させること（MUST）** —— 名前の大きさは深さで変わるので（表 T-201 の `S-36` と `S-38`）、絶対値で下げると深さごとに見え方が食い違う |
