...
1. 作成者が MSPDI ファイルを開き、現在の文書を置き換えるか合流させるかを選ぶ。
...
**STATEMENT**: **表示倍率または実効フォントサイズが変わったとき**、システムは、タイムルーラーの粒度を 1 日あたりの表示幅から決まる 4 段階へ切り替えること。**しきい値は表 T-205（`_assets/tbl-settings.md`）の 3 本に従うこと。**
...
**しきい値の 3 本は固定値とし、実行時に導出してはならない（MUST NOT）。** 実装が値を書き換えてはならない（MUST NOT）。**3 本は目盛のフォントが基準の 12px であるときの px/day であり、判定は実効フォントサイズで割った比で行うこと（MUST）** —— 判定式は `pxPerDay ÷ (実効フォントサイズ ÷ 12) ≧ しきい値` とする。**実効フォントサイズとは、そのとき目盛を描いている `rulerFont` の px** である。⚠️ **これは値を導出することではない** —— 保存する 3 本は固定のままで、判定に入る前に文字倍率を打ち消すだけである。
...
**RATIONALE**: タスク LOD の判定は WBS の階層の深さで行う。**倍率を刻む箇所（ホイール・ボタン・保存値の丸め）は、しきい値をまたぐ側へ丸めてはならない（MUST NOT）** —— 1 刻みで表示量が跳ねる。実測では 1 ノッチで行が 9 から 21 に増えている。**判定では深さを `S-125` で頭打ちにしてよいが、書き戻す値を頭打ちにしてはならない（MUST NOT）** —— 階層が壊れる。
...
**名前は文書に保存してはならない（MUST NOT）** —— 理由は `FR-020` が持つ。読む人の環境に属する値なので `localStorage` へ置く（`S-99a`）。**既定の透かし解除パスワードは表 T-207 が持ち、利用者が設定しなければそれを使う。**
...
| MG-8 | 「上書き」を選んだ | **見た目（色・形状・名称ラベルの位置）と、どの行に載っているかを保つこと（MUST）。** 置き換えるのは取込元が持つ値だけである。**行をまるごと差し替えると、マルチバーの配置が取込のたびに失われる** |
...
⚠️ **`WY-1` は JSON が縦横の倍率と縦横の表示位置を持つことを要する** —— `zoomX` / `zoomY`（`S-75` / `S-76`）と `scrollDate` / `scrollGroupId`（`S-77` / `S-78`）がそれに当たり、`FR-024` が全項目を書き出すと定めているので満たされる。**この 4 つを保存対象から外してはならない（MUST NOT）。**`FR-025` は出力サイズと文字サイズを文書に保存することまでを定めていたが、**見た目が一致することを求める要求が無かった。** 収まらないときに縮小して収める（`FR-025`）のは倍率の話であり、**描くものの取捨選択を変えてよいという意味ではない。**
