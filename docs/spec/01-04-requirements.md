...
| OP-10 | **表示位置が `null`、または指す行が存在しないとき** | **`FR-055` の全体表示が選ぶ倍率と表示位置にすること（MUST）。このとき `HF-8` を働かせてはならない（MUST NOT）** —— `null` は「人がまだ場所を決めていない」を表す値であって、欠けているのではない（`OP-6` の補完に当たらない）。起動のたびに畳みを捨てると、`HR-6` が `WY-1` のために保存させた状態が消える |
