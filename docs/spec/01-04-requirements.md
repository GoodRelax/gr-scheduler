...
| RC-13 | **新しく起こしたアイコンの図形**（`_assets/tbl-glossary.md` の 図 F-019。表 T-109 の `IC-3`（差し替え）と `IC-58` 〜 `IC-70` の 14 個） | **実物で見比べていない。** 2026-08-21 の裁定で起こしたばかりであり、用途が伝わるかを実物で確かめていない | `FR-098` の「図形で描くこと（MUST）」と 表 T-051 の `HF-1`、表 T-023b の `AR-4` は満たせるようになったが、伝わらない図形は `FR-029` の「用途を言葉ではなくアイコンで伝える」を果たさない。外すのではなく**見比べて選び直す**対象である | **残す。骨格で実物と見比べて選び直す。** ⛔ **本仕様書の中で発明しない** —— 図形の決定は利用者の裁定である |
