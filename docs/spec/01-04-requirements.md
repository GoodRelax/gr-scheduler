...
⚠️ **ポインタが乗ったことで濃さが変わる UI パーツと、待ち時間で出る説明は「描く内容が変わる」に当たる** —— 表 T-051 の `HF-6`（行の操作子）、`FR-053`（コマンドパレット）、`FR-043` の掴みシロと未着手のマーカー（`FR-013`）、`FR-092` の `EZ-2`（アイコンの説明）、`FR-037`（スクロールバーの説明）である。**これらは本 MUST NOT の対象ではない。**
...
| EZ-2 | **アイコンにポインタを合わせて一定時間が経ったら、そのアイコンの説明を出すこと（MUST）。説明の後ろに、その行の割当も出すこと（MUST）**（利用者の指示 2026-08-29「Tips にショートカットキーも表示しろ」）—— **割当が指すものも、語をどこから取るかも `FR-036` と同じであること（MUST）。同じ対を 2 通りに組み立ててはならない（MUST NOT）。** **字の大きさは 表 T-206 の `S-204` が定める係数で決めること（MUST）。** 待ち時間は `_assets/tbl-settings.md` の `S-124` が持つ | 本要求。作法は表 T-028 の `IN-3` |
...
| IN-3 | ツールチップは、次の 3 つをすべて満たすこと（MUST）。**消せること** —— ポインタもフォーカスも動かさずに消す手立てがあること。**ポインタを乗せられること** —— 説明そのものの上へポインタを移しても消えないこと。**引き金が外れるまで出ていること** —— ポインタまたはフォーカスがその対象から外れるか、人が消すか、その内容が有効でなくなるまで、消してはならない（MUST NOT）。⭐ **裏を返せば、引き金が外れたら消してよい** —— 出す引き金は `FR-092` の `EZ-2` と `FR-037` が持つ。⚠️ **`FR-085` は引き金ではなくなった** —— 打ち切った行名は `…` で示し、全文はパネルを広げて見る |
