...
⚠️ **ポインタが乗ったことで濃さが変わる UI パーツと、待ち時間で出る説明は「描く内容が変わる」に当たる** —— 表 T-051 の `HF-6`（行の操作子）、`FR-053`（コマンドパレット）、`FR-043` の掴みシロと未着手のマーカー（`FR-013`）、`FR-092` の `EZ-2`（アイコンの説明）、`FR-085`（打ち切られた行の名前の説明）、`FR-037`（スクロールバーの説明）である。**これらは本 MUST NOT の対象ではない。**
...
| IN-3 | ツールチップは、次の 3 つをすべて満たすこと（MUST）。**消せること** —— ポインタもフォーカスも動かさずに消す手立てがあること。**ポインタを乗せられること** —— 説明そのものの上へポインタを移しても消えないこと。**引き金が外れるまで出ていること** —— ポインタまたはフォーカスがその対象から外れるか、人が消すか、その内容が有効でなくなるまで、消してはならない（MUST NOT）。⭐ **裏を返せば、引き金が外れたら消してよい** —— 出す引き金は `FR-092` の `EZ-2` ／ `FR-085` ／ `FR-037` が持つ |
