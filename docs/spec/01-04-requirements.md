...
| UN-4 | 対象 | 階層の変更 |
| UN-14 | 対象 | **行（`TaskGroup`）の追加・削除・名前の変更、行の色と高さの変更、および畳みと非表示の変更**（`FR-085` / `FR-032` / `FR-042` / `FR-004` / 表 T-015 の `HR-6`）。⚠️ `UN-1` は `Task`、`UN-4` は階層であり、器そのものの生成と消滅を含まない。⚠️ **畳みと非表示が `UN-7` でないのは、どちらも `TaskGroup` の列だからである** —— `UN-7` が対象外とするのは 表 T-202 の行であって、日程データの群の列ではない |
...
| UN-16 | 対象外 | **見る場所の割り付けと出力の設定** —— パネル幅（`FR-052`）・ピン止め（`FR-098`）・PNG の倍率（`FR-025`）。⚠️ **保存することと戻せることは別である** —— `UN-12` が `Dual Cursor` の位置で同じ形を既に採っている。いずれも読む人の都合であって、日程の内容ではない |
...
**掴んで置く値は、実績開始日 ＝ 予定の開始日、実績期間（`actualDuration`）＝ `_assets/tbl-settings.md` の `S-129`、`resumeValid` ＝ `true` とすること（MUST）。** ⚠️ **マイルストーンは例外である** —— 実績バーを持たないので（表 T-023d の `GR-15`）、**ダミーは点として 1 つだけ出し、実績期間は `S-130` とすること（MUST）**。位置と当たり判定は 表 T-023d の `GR-18` が持つ。

**終了点の掴みシロは、予定の開始日から `S-129` ぶん進んだ稼働日に置くこと（MUST）** —— **暦に従う**（`FR-054`）。暦日で進めると非稼働日に置くことになる。**開始点を掴んだときは終了点をその既定の位置で、終了点を掴んだときは開始点を予定の開始日で確定させること（MUST）** —— 片端だけが決まった状態を作らない。⚠️ **重なったときに掴めるのは開始点である**（表 T-023d の順）。**終了点を開始点まで動かして長さを 0 にすることは受け入れる** —— そのあと長さを直すのはプロパティパネル（表 T-016 の `PR-5`）である。
...
| GR-9 | 実績の開始点のダミー | 未着手のタスクの上、予定の開始日（当たり判定の大きさは `_assets/tbl-settings.md` の `S-93`） | 掴めば `actualStart` と `actualDuration` を置く（値は `FR-043`）。**押して状態を巡らせない（MUST NOT）** —— 巡らせるのは進捗マーカーである（`FR-043`） |
| GR-17 | 実績の終了点のダミー | 未着手のタスクの上、`S-129` ぶん進んだ稼働日（当たり判定は `S-93`） | 掴めば `actualDuration` を置く（`actualStart` は予定の開始日で確定。`FR-043`）。⚠️ **`GR-9` より下に置く** —— 重なったら開始点が勝つ |
...
**マイルストーンの図形の入口は、一覧を開くまで出さないこと（MUST）。** 開閉の状態は 表 T-206 の `S-142` が持ち、既定は閉じている。⭐ **使う頻度が低い入口をたたんで、日程を隠す面積を減らす**（`GL-002`）。⛔ **`Esc` で閉じてはならない（MUST NOT）** —— 重ねて開く「面」ではないので `S-99g` に当たらず、表 T-028 の `IN-4` の階層を 1 つ増やしてはならない。
