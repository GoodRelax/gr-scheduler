...
| LM-14 | **ファイルを直接開いた状態（`file://`）では、上書き保存と自動保存が働かないことがある** | 単一ファイルをローカルで開く形では保存先への権限が保たれず（`FR-060`）、保管庫の読み出しを拒否するブラウザもある（`FR-038`）。**表 T-003 の `CN-1` と `NFR-004` の「全機能が動く」判定からは、上書き保存・自動保存に加えて、`localStorage` を保管先とする値（表 T-206 の別枠）も除く** —— 全数は表 T-206 の別枠の行、すなわち「別枠。`localStorage` に置く。」と明記した行である。 代わりに `FR-060` が起動時に権限の復帰を申し出る |
...
| LM-17 | **まだ 1 度も保存していない文書は、自動保存の枠を互いに共有する** | **鍵は 表 T-227 の `DI-1` であり**（`FR-061`）、**その 3 つ組は 表 T-211 の既定のファイル名・`Project.name`・`Project.id` である。**⛔ **起動テンプレートから始めた文書はこの 3 つが揃うので、2 つ目が 1 つ目の自動保存を上書きする。** ⚠️ **クラッシュ復旧で戻るのは後のものだけである。**⭐ **受け入れたうえで名乗る**（利用者の裁定、2026-08-25）—— 1 度保存すればファイル名が分かれ、枠も分かれる |
...
**STATEMENT**: `GRS` は、画面の各部を**表 T-031 の規則**でスクロールさせること。**スクロールバーの操作でも表示位置を変えられるようにすること（MUST）。** **表示位置が変わったときは、`_assets/tbl-settings.md` の表 T-203 の `S-77` と `S-78` が新しい表示位置を指すようにすること（MUST）** —— 読む側の規則は表 T-024a の `OP-10` が持つ。⚠️ **その値をいつ書き出すかは `FR-026` が持つ。**
...
**書き込む回数に上限を設けない。** —— **同じ鍵を上書きするだけであり、回数を重ねても保管庫は増えない** —— 履歴を保管庫に入れないと決めてあるからである（表 T-004 の `LM-9`）。⚠️ **上限を持つのは間隔（`S-112`）だけである。**
...
**取り違えないための鍵は、表 T-227 の `DI-1` とすること（MUST）。** ⚠️ **以前は `DI-1` を MUST NOT で禁じていた** —— ⛔ **禁じた理由は「`DI-1` はファイル名を含むが、1 度もファイルへ保存していない文書にも自動保存は掛かる」ことただ 1 つであり、既定のファイル名を定めてその理由を消した**（`_assets/tbl-settings.md` の 表 T-211。利用者の裁定、2026-08-25）。⚠️ **まだ 1 度も保存していない文書どうしは `DI-1` の 3 つ組が揃って同じ鍵になる** —— 帰結は 表 T-004 の `LM-17` が持つ。
...
**負けた自動保存の扱い**: 同じ文書で、**自動保存の刻印が開いた文書のそれと 1 つでも違えば確認を求めること（MUST）**。⛔ **どちらが新しいかで判定してはならない（MUST NOT）** —— 壁時計は逆行しうるうえ、取り消しで戻った文書は「新しくない」ので黙って捨てられる（`FR-063`）。**別の文書なら触らない。** 壊れていれば通知したうえで退避する。
...
**RATIONALE**: **`FR-063` の 2 つの刻は「いつ動いたか」であって、形式の版ではない。混同してはならない。** 形式の版が無いと、以前の版が書いた自動保存を読めるかどうか判定できず、**壊れた文書と古い文書の区別がつかない。**
...
⚠️ **本表が答えるのは上書きの問いだけである。** 自動保存が使う鍵は `FR-061` が持つ。
...
| NT-5 | **受け付けたうえで注意を伝えるとき** | **操作を止めないこと（MUST）。** 受け付けた事実と注意を同じ面で示し、**`NT-1`（受け付けないとき）と見分けがつく形にすること（MUST）** —— 同じ見た目だと、通ったのか弾かれたのかが読めない。⚠️ **`NT-2` の対象とする** —— 読み終える前に消えてはならない。対象は「通知する」「示す」とだけ書いた要求のうち、入力を受け付けているもの（例: `FR-054` の繰り返し例外日、表 T-033 の `EX-3`、`FR-061` の平文で残ること、`FR-065` の渡した参照、`FR-020` の証跡であること、表 T-032 の `MG-10` / `MG-11`） |
...
| RS-17 | 自動保存した内容が壊れていて、復旧できない | `NT-3a` | `FR-026` |
| RS-18 | 自動保存から復旧したが、取り消しの履歴は戻らない | `NT-4` | 表 T-004 の `LM-9` |
...
| QN-6 | 開いた文書と刻印の違う自動保存が残っているとき | 挙げない —— 捨てるのは自動保存であって、文書のものではない | `FR-062` |
| QN-7 | 異常終了する前に自動保存した編集が残っているとき | 挙げない —— 復旧するかどうかの問いであり、この時点で失うものは無い | `FR-026` |
