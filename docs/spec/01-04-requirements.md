...
⚠️ **2 つの欄を持つのは、`OP-1` が取込で受け付ける 2 行だけである** —— 判別（表 T-024a の `OP-12`）はその 2 行のためにあり、書出だけの形式は判別されない。**`IO-7` も欄を持たない** —— 埋め込まれた文書を読むのは表 T-034 の `BT-1` であって、開く道ではない。**書出のときに添える名前は本表が持たない。**
