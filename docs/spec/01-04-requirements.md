...
⚠️ **ポインタが乗ったことで濃さが変わる UI パーツと、待ち時間で出る説明は「描く内容が変わる」に当たる** —— 表 T-051 の `HF-6`（行の操作子）、`FR-053`（コマンドパレット）、`FR-043` の掴みシロと未着手のマーカー（`FR-013`）、`FR-092` の `EZ-2`（アイコンの説明）である。**これらは本 MUST NOT の対象ではない。**
...
| MC-8 | **測る区間** | ズーム・スクロール・パン・ドラッグ・範囲選択を行っている間、および**ポインタの移動が描き直しを起こしている間**。⚠️ **何も変わらないポインタの移動は含めない** —— 描き直しが起きないので測る母数が無い（`NFR-010`）。⚠️ **ポインタが乗ったことによる濃さの変化と、待ち時間で出る説明だけの描き直しも含めない**（`FR-048` が挙げる 4 つ）—— **日程を作り直さないので、本要求が測ろうとしている負荷が現れない** |
