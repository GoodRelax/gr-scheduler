...
| GR-20 | 行見出しパネルの行 | **行の左端に敷く掴み代**（幅は `_assets/tbl-settings.md` の 表 T-206 の `S-138`）| 掴めば行を動かす（表 T-051 の `HF-15`）。⛔ **ピン止めしている行は掴めないこと（MUST NOT）** —— `FR-098` が留めた行をパネルの先頭へ上げるので、**上げられた位置で掴むと、木の順ではなく描く順を触ることになる。**⚠️ **実測で、留めた行を引くと画面は 1px も動かないまま親を 2 つまたいだ** |
