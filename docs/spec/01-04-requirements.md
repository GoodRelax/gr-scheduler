...
| LM-5 | **往復無損失を保証するのは、1 つのファイルを合流させずに未編集で往復した場合に限る** | 2 つのファイルが同じ要素について別々の持ち回り分を抱えていると、片方しか書き戻せない場面が生じる。**合流の利点（`FR-056`）と引き換えに受け入れる**（`FR-021`） |
...
| LM-14 | **ファイルを直接開いた状態（`file://`）では、上書き保存と自動保存が働かないことがある** | 単一ファイルをローカルで開く形では保存先への権限が保たれず（`FR-060`）、保管庫の読み出しを拒否するブラウザもある（`FR-038`）。**`NFR-004` の「全機能が動く」判定からは、上書き保存・自動保存に加えて、`localStorage` を保管先とする値（表 T-206 の別枠）も除く** —— 透かしに出す名前（`FR-086`）と `Agent API` の有効化の記録（`FR-065`）がそこに当たる。 代わりに `FR-060` が起動時に権限の復帰を申し出る |
...
**STATEMENT**: `Task` にフェードイン日数またはフェードアウト日数が入っているとき、システムは、**表 T-012a の形**でバーの端を細らせて描くこと。作成者がその日数を、表 T-023d の `GR-1` / `GR-2` の掴み点で編集できるようにすること。
...
**STATEMENT**: 作成者がテーマの色相・明暗・モノクロを選んだとき、システムは、**色を指定していない `Task` と `TaskGroup`・行の帯・地の色**をそれに追随させて描くこと。
...
| SL-5 | 全部選ぶ | `Ctrl+A`（表 T-036 の SK-2） |
| SL-6 | 解除する | 何にも当たらない場所をクリックする（表 T-023 の MK-11） |
...
**STATEMENT**: `Task` が中断しているあいだ、システムは、作成者が再開予定日を画面上で置き、置いた後に動かせるようにすること。**再開日を未定のままにもできること。**
...
**RATIONALE**: 分母は予定の期間である。`actualStart` が `start` より後でも分母は変わらない。型は整数・0 以上とし、**下限も上限も課さないこと（MUST NOT）** —— 予定 100 日のタスクを 80 日で終えれば 80、120 日かかれば 120 である（利用者判断、2026-08-13）。**完了したかどうかは `actualFinish` の有無が表すのであって、完了率が 100 かどうかではない**（表 T-019）。**これにより予定と実績の工数の差が完了率から直に読める。**
...
**構えは持続すること（MUST）。** 続けて何本も引けなければ道具として重い。**解除は `Esc` かパレットの同じ入口の再押下とすること（MUST）。** 構えの各値は排他であり、依存線を構えれば図形の構えは外れる。**`Dual Cursor` モード（表 T-029a）に入るときも構えを外すこと（MUST）** —— 同モードは作成・移動・編集を受け付けないので、構えたまま入れると何も起きない構えが残る。
...
| MK-13 | ダブルクリック | **対象ごとに定めること（MUST）** —— 名称ラベル ＝ 名称の編集／担当ラベル ＝ 担当者名の変更／タスク本体 ＝ プロパティパネルを開く／行見出し ＝ 行名の編集 |
...
| GR-12 | 予定バー本体 | 端点を除いた中間 | **予定の日付だけを平行移動する。実績の日付は変えない**（`FR-011`）。**縦に動かしたときは、落とした位置が指す `TaskGroup` へ載せ替えること（MUST）** —— `TaskGroupMember` だけを付け替え、WBS は変えない（表 T-015a の HM-3） |
| GR-13 | 依存線 | 線の上 | 選ぶ |
...
**STATEMENT**: システムは、画面の各部を**表 T-031 の規則**でスクロールさせること。
...
| SC-4 | スクロールバー | **横・縦とも常時表示する。内容が収まっていても消さない** |
...
**RATIONALE**: 細目は表に置く（1.9）。**修飾キーの付いた入力は画面全体で既定動作を止める**（表 T-023 の MK-10）—— 止め忘れると `Ctrl+S` がブラウザの HTML 保存にすり替わる。
...
| IO-2 | JSON | 取込 / 書出 | 本ツールの主データ形式。機械連携 | 再現の規則は `FR-024`。**RFC 8259 が BOM を禁じている** |
