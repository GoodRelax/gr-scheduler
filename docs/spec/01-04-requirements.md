...
**一覧の各項目は、その行の説明・割り当てられたキー・入口の図形の 3 つを、この順に並べること（MUST）。**⭐ **図形は説明の右に置くこと（MUST）**（同指示「説明の右にアイコンを付けろ」）。⛔ **図形を持つのは、表 T-109 がその行にちょうど 1 つの入口を置いている行だけである** —— **同表の `構え` の欄が 1 つの構えに複数の入口を与えることがあり**（`AR-2` に 4 つ、`AR-3` に 8 つ）、**そのうちどれを代表とするかを述べる行がどこにも無い。**⇒ **選べば `FR-029` が図形の正と定める 図 F-019 と 表 T-109 の外で選んだことになる。**⚠️ **キーを持つのは 表 T-036 の行だけであり、持たない行はその場所を空ける。**

⛔ **一覧に出す語を本要求や表から直に取ってはならない（MUST NOT）** —— `FR-038` が「画面に刷る語は言語ごとの辞書 1 つに持つ」と定めており、**各行の説明はその辞書が行 ID で持つこと（MUST）。**⚠️ **キーの綴りは語ではない** —— `Ctrl+S` はどの言語でも同じなので、表 T-036 の `割当` の欄から運ぶ。
