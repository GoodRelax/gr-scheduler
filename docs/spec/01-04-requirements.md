...
**深さの上限は `FR-004` に従う（`TaskGroup` の深さ 5 段）。上限に達している親の下に作らせてはならない（MUST NOT）。**
...
**階層は推奨 3 段、最大 5 段とする。** 「大 / 中 / 小」という専用語を持たず、**深さで表す**。

⚠️ **この 5 段は `TaskGroup` の深さである。WBS の深さではない**（表 T-006b の A-4 に従って併記する）。

**WBS の深さをクランプしてはならない（MUST NOT）。** 取り込んだ元データが 6 段以上でも、階層は `Task.wbs_parent_uid` がそのまま持ち、書き出すときに深さから算出して戻す。**浅くすると往復で構造が壊れ、`FR-021` と正面から衝突する。** 5 で頭打ちにしてよいのは **LOD の判定のときだけ**である（`FR-018`）。**非表示が増えてもグループ罫線を太くしてはならない（MUST NOT）。**
...
| 行 ID | 事項 | 規則 |
| --- | --- | --- |
| SL-1 | 対象 | **タスク・依存線・ハイライトボックス・コメントボックス。** 行（`TaskGroup`）は対象に含めない |
...
**RATIONALE**: タスク LOD の判定は WBS の階層の深さで行う。**倍率を刻む箇所（ホイール・ボタン・保存値の丸め）は、しきい値をまたぐ側へ丸めてはならない（MUST NOT）** —— 1 刻みで表示量が跳ねる。実測では 1 ノッチで行が 9 から 21 に増えている。**判定では深さを 5 で頭打ちにしてよいが、書き戻す値を頭打ちにしてはならない（MUST NOT）** —— 階層が壊れる。
...
**達成は次の 2 つで判定すること（MUST）**（利用者判断、2026-08-13）—— どちらも機械で確かめられる。

| 行 ID | 判定 |
| --- | --- |
| WY-1 | **編集 → JSON へ書き出し → 初期化 → その JSON を読み込み** を行ったとき、**書き出す前と同じ画面になること。** 倍率と表示位置を含む |
| WY-2 | **同じ JSON を読み込み、全体表示（`FR-055`）を行ってから書き出したとき、同じ SVG / PNG が出ること** |
