...
| HF-18 | **配下に畳み込んでいる行があるとき、その行数を行に示すこと（MUST）**（利用者の裁定 2026-08-30）—— ⭐ **`HF-12` が段 0 について定めるものを、行について定めたものであり、示さないと何が読めなくなるかも同行が持つ。**⭐ **その行自身にも印を付けること（MUST）** —— **数だけでは、どの行が抱えているかを目で追うのに読む必要がある。**⭐ **色は 表 T-236 の `S-153` とする** —— **注意であって不良ではない。**⛔ **`HF-6` の対象ではない** —— **ポインタが乗っているあいだだけでは、抱えている行を探して回ることになる** |
