...
⚠️ **アイコンの員数を本要求の散文に書かない** —— 名簿が動くたびに要求の追記が要る。**員数は 表 T-109 が自ら数えて持つ。** 同表のうち図形をまだ持たない行の選び直しは、表 T-026 の `RC-13` が持つ。
...
| RC-13 | **図形を持たないアイコン**（`_assets/tbl-glossary.md` の 表 T-109 の `IC-58` 〜 `IC-61`。行の折り畳みの開く / 閉じる、ピン止め、依存線の構え） | **どこにも図形が無い。** 前プロジェクトの図案は行見出しパネルとパレットの構えの入口を描いておらず、本仕様書もそれを写していない | `FR-098` の「図形で描くこと（MUST）」と 表 T-051 の `HF-1`、表 T-023b の `AR-4` が満たせない。外すのではなく**描いて選び直す**対象である | **残す。骨格で描いて実物と見比べて選び直す。** ⛔ **本仕様書の中で発明しない** —— 図形の決定は利用者の裁定である |
