...
| 行 ID | 形式 | 方向 | 用途 | 備考 |
| --- | --- | --- | --- | --- |
| IO-1 | MSPDI XML | 取込 / 書出 | 外部 WBS マスタとの交換 | **往復無損失**（`FR-021`）。文字コードは表 T-003 の `CN-5`。拡張子は `.xml`、先頭の非空白 1 文字は `<`（使い道は表 T-024a の `OP-12`）|
| IO-2 | `GRS JSON` | 取込 / 書出 | 本ツールの主データ形式。機械連携 | 再現の規則は `FR-024`。文字コードは表 T-003 の `CN-5`。拡張子は `.json`、先頭の非空白 1 文字は `{`（使い道は表 T-024a の `OP-12`）|
