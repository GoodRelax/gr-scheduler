...
**`S-127` の上限に達しているときは、新しいピン止めを受け付けずに知らせること（MUST）。既にピン止めした行を自動で外してはならない（MUST NOT）** —— 人が固定した行が黙って流れると、スクロールして初めて気づくことになる。**知らせ方は `FR-076` の `NT-1` に従う。** ⚠️ **表 T-014 の `ST-7` のように処理を止めない** —— あちらは「届いたときは入力か操作の異常である」と自ら書く安全弁であり、こちらは**人が普通に届く上限**である。`FR-085` が深さの上限で採っている形と同じにする。
...
| EX-6 | フェード日数を書き出す枠は `_source/mspdi-custom-fields.json` の名簿とし、**その順に空きを探すこと（MUST）。** **取込元が使っている枠を上書きしてはならない（MUST NOT）。** 空き枠へ移して知らせ、空き枠が無ければ書き出さずに知らせること（MUST）。⭐ **取込元が使っているかどうかは、定義の側の `Alias` が名簿のものと一致するかで見分けること（MUST）** —— **値の側は番号しか持たない**（`mspdi_pj12.xsd:2254`）。取り込むときも同じ見分け方を用い、**合わなければ解釈せずに `carryElements` へ残すこと（MUST）**（表 T-053 の `DF-2`） |
...
**階層の循環（あるタスクの親が、その子孫である状態）を検出して取り込まないこと（MUST NOT）** —— `FR-005` が禁じているのは作成者が画面で行う移動だけで、**外から来た循環は素通りする。循環では深さが確定しないので、ネストの深さの上限でも検出できない。** **受け入れる日付の範囲を表 T-214 に従って持つこと（MUST）。日付の列が「文書が使えない日付」を持つ入力を、黙って取り込んではならない（MUST NOT）** —— **「文書が使えない日付」とは、日として読めない値（空文字を含む）と、表 T-214 の範囲の外にある値をいう。** ⚠️ **空文字を特例にしない** —— 列が空を許すときの空は `null` であり（`FR-024` の契約）、空文字はその契約の外にある。 **取り込む前に、どの行のどの列が使えないかを知らせ、次の 2 つから選ばせること（MUST）** —— **(1) その行を落として残りを取り込む、(2) 読み込みを中止する。** ⛔ **どちらも選ばれないまま取り込んではならない（MUST NOT）。** **(1) で落ちるものは 表 T-050 の `CD-1` に従うこと（MUST）** —— ⭐ **取り除く連鎖を 2 通り持たない。** 知らせ方は `FR-076` の `NT-1` と `NT-3` に従う。 ⚠️ **資源の上限（下記）はこの 2 択の対象ではない** —— 上限を超えたときに落とすべき行を名指しできないからである。**日付は行と列を名指せる。それが扱いを分ける根拠である。** **資源の上限（ファイルサイズ・件数・WBS のネストの深さ・解釈しない要素のネストの深さ）を `_assets/tbl-settings.md` の表 T-211 に従って持ち、超えた入力は取り込まずに知らせること（MUST）。部分的に適用してはならない（MUST NOT）** —— 途中まで取り込んだ状態は、利用者から見て「壊れた文書」と区別がつかない。
...
**負けた自動保存の扱い**: 同じ文書で、**自動保存の刻印が開いた文書のそれと 1 つでも違えば確認を求めること（MUST）**。⛔ **どちらが新しいかで判定してはならない（MUST NOT）** —— 壁時計は逆行しうるうえ、取り消しで戻った文書は「新しくない」ので黙って捨てられる（`FR-063`）。**別の文書なら触らない。** 壊れていれば知らせたうえで退避する。
...
⭐ **知らせが運ぶ理由は 表 T-233 の行とすること（MUST）。同表に無い理由を運んではならない（MUST NOT）** —— 理由の語は `FR-038` の辞書が持ち、辞書は行 ID で引く。⚠️ **行の無い理由に落ち先を与えるのが `RS-15` である** —— 落ち先が無いと、`NT-1` の「文字で示すこと（MUST）」と `NT-3a` の「次に取れる手段を添えること（MUST）」を果せない知らせが出る。⛔ **行を足すときは、辞書の原稿にも項を足すこと（MUST）** —— 生成器が本表から名簿を起こすので、片方だけを書けば黙らずに落ちる。
...
| RS-9 | 知らせを配っている最中である | `NT-1` | 表 T-067 の `WS-2` |
