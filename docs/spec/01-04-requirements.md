...
| SO-12 | Out-of-scope | サーバー通信・認証・共同編集。設計の余地だけ残す |
...
| LM-2b | **すべての操作をキーボードだけで完了することはできない**（WCAG 2.1 の 2.1.1 を適合範囲から外す） | **中核がドラッグで日程を描くことだからである**（`CN-4` はマウスとキーボード）。バーを引く・範囲を囲う・依存線を結ぶは、いずれもポインタの連続した位置を入力とする操作であり、鍵に写しても同じ道具にならない。**キーボードは速さの手段として残す**（`FR-070` の表 T-036）。⚠️ **これにより、ポインタを使えない利用者は本ツールを使えない。** 受け入れたうえで名乗らない（確定 2026-08-13） |
| LM-2a | 1.4.12（行間・字間の上書き）は**参考に留め、適合範囲に数えない** | **ラベル幅を概算する方針（実測もキャッシュもしない）と両立しない。** 概算は文字の実寸を見ないので、読む人が字間を広げた瞬間に見積もりが外れる。実測へ切り替えると描画のたびに計測が走り、`GL-003`（操作が引っかからない）を直接崩す。**最優先事項を守るほうを選ぶ**（確定 2026-08-12） |
...
| LM-11 | **異常終了すると、直近の数秒ぶんの編集が失われる** | 自動保存を操作の切れ目にまとめる帰結である（`FR-026`）。**編集のたびに書くとスクロールやドラッグの最中にフレームを落とす**ので、`GL-003` を守るほうを選ぶ |
...
| LM-12 | **既定の透かし解除パスワードは秘密として機能しない** | 成果物に埋め込む定数なので、配布物を読めば復元できる（表 T-207）。**透かしはアクセス制御ではなく証跡である**（`FR-020` が画面上でもそう示すことを求めている）。**この機能が意味を持つのは将来サーバーと接続したときである** |
| LM-14 | **ファイルを直接開いた状態（`file://`）では、上書き保存と自動保存が働かないことがある** | 単一ファイルをローカルで開く形では保存先への権限が保たれず（`FR-060`）、保管庫の読み出しを拒否するブラウザもある（`FR-038`）。**表 T-003 の `CN-1` と `NFR-004` の「全機能が動く」判定からは、上書き保存・自動保存に加えて、`localStorage` を保管先とする値（表 T-206 の別枠）も除く** —— 透かしに出す名前（`FR-086`）と `Agent API` の有効化の記録（`FR-065`）がそこに当たる。 代わりに `FR-060` が起動時に権限の復帰を申し出る |
| LM-13 | **透かしはコントラストの適合範囲に数えない** | 透かしは薄く重ねると定めており（`FR-020`）、`NFR-007` の 4.5 : 1 と両立しない。**透かしが求めるのは可読性ではなく証跡**であり、画面を複写されたときに誰がいつ出した画面かが判別できればよい。濃さは表 T-207 の `watermarkOpacity` が持つ |
...
| | 書き方 |
| --- | --- |
| 良い | 「〜の判定は表 T-XXX に従うこと。」／「〜を構成するクラスは図 F-XXX に従うこと。」（**番号は例示であり、実在の表や図を指していない**） |
...
⚠️ **規則が 2 か所にあると、片方を直したときにもう片方が残り、そこで 2 つの文が食い違う。重複は必ず矛盾に育つ。** 本書はこの型の欠陥を繰り返し作っている。**`_assets/tbl-settings.md` の `S-105` が正しい書き方の例である** —— 規則を持たず「派生する色は保存しない方針（§4）に従う」と指すだけで済ませている。
...
| 行 ID | 語 | 書き方 |
| --- | --- | --- |
| A-1 | 段 | **5 義ある。** ①積み順 ②WBS の深さ ③`TaskGroup` の深さ ④ズームのノッチ ⑤目盛の帯の中の段（`rulerHeight` の内訳）。必ずどれかを併記する。「N 段」と単独で書かない |
...
| R-9 | D-1 | OS のクリップボード | 画像、および `Agent API` へ渡す文書（`FR-068`） | 書き出し（`FR-033` / 表 T-024 の IO-6） | **送信のみ・読まない**（検証の対象にならない）。⚠️ **出て行く経路なので `LM-8`（透かしに載る名前が第三者へ伝わりうる）はここにも効く** |
...
| 行 ID | 持たないもの | 理由 |
| --- | --- | --- |
| XO-1 | サーバーとネットワーク通信 | 単一 `.html` で完全にオフラインで動くこと（CN-1・CN-6）。将来拡張として設計の余地だけ残す |
...
>
> ⚠️ **将来の拡張（項 65）は 5 つある。** `NFR-012` が受けているが、ユースケースは持たない —— **いま作らないものにユースケースを立てない**という判断である。
...
5. システムは畳んだ `TaskGroup` の配下の行を隠し、非表示にしたものは親の非表示グループタブから戻せるようにする。
...
3. システムは、引き出した辺と引き入れた辺の組合せから種別を決めて依存を作り、ラグを既定値で置く。
4. システムは種別から両端のアンカーを決め、表 T-018a の規則で線を引く。手順 2 へ戻り、続けて引ける。
5. 作成者が構えを解いて、ラグを変える。**種別を変えるときは依存を引き直す**（辺の組合せが種別を決めるため、種別だけを選ぶ入口は設けない）。
6. システムは経路を引き直す。

**EXTENSIONS**:

- 2a. 引き入れる先が受け付けられない（同じタスクへ結ぼうとした、既に同じ 2 つの間に依存がある、タスクでもマイルストーンでもないものへ引き入れた）。
  - システムは依存を作らず、理由を示して引きかけの矢印を捨てる。構えは解かない。
- 2b. 作成者が引き出した後、引き入れる前に `Esc` を押す。
  - システムは引きかけの矢印だけを捨てる。構えは解かない（表 T-028 の IN-4）。
...
**STATEMENT**: 閲覧者が基準日を指定し、システムがイナズマ線と、タスクごとの遅れの印で、どこが遅れているかを示すこと。

**SCENARIO**:

1. 閲覧者が基準日を置く。
2. システムは、動いているべき日を過ぎているタスクと進行中のタスクに頂点を打ち、上端から下端まで途切れない 1 本の折れ線を引く。
3. システムは遅れているタスクの印を、遅れを表す形にする。未着手のタスクでは実績入力の入口がその形をとる。
...
- 5a. 閲覧者が修飾キーを押さずに、何にも当たらない場所をドラッグする。
  - システムは範囲選択を始める。パンはしない（表 T-023a の PD-5）。
...
**STATEMENT**: 作成者が日程表の任意の場所にコメントとハイライトボックスを置き、システムがそれらを行の識別子で結び付けて描くこと。
...
- 1a. 作成者がコメントの本文を入力する、または後から書き換える。
  - システムは本文を保持し、コメントボックスの中に描く。
...
**STATEMENT**: 作成者が秘密の日程を画面に出すとき、システムが誰がいつ出していたかを示す透かしを重ね、消すには透かし解除パスワードを求めること。
...
3. 作成者が透かしを消そうとする。
4. システムは透かし解除パスワードを求め、ハッシュを照合して一致したときだけ消す。
...
3. システムは現在の文書を置き換えるか合流させるかを問い、作成者が選ぶ。
4. 作成者が日程を編集する。
5. 作成者が MSPDI として書き出す。
6. システムは保持していた項目を戻して書き出す。編集していなければ、元と同じものが出る。

**EXTENSIONS**:

- 1a. 作成者が置き換えではなく合流を選ぶ。
...
**STATEMENT**: 作成者が別の日程のファイルを開いて合流を選び、システムが現在の文書へ足して、同じものが両側にあるときの扱いを作成者に決めさせること。

**SCENARIO**:

1. 作成者がファイルを開き、置き換えではなく合流を選ぶ。
2. システムは、取り込んだファイルの出自を判別する。
3. システムは、同じタスクが両側にあるものを集め、扱いを 1 度だけ問う。
4. 作成者が、示された扱いのどれかを選ぶ。
5. システムは現在の文書へ足し、消えたもの・残したものを知らせる。

**EXTENSIONS**:

- 1a. 開いたのが本ツール自身の形式（JSON）である。
  - システムはタスクだけでなく、行の器・依存・注記・担当者も足す。文書の設定が食い違うときは、タスクとは別に問う。
- 3a. 前回「別のものとして取り込む」を選んだタスクが、また届いた。
  - システムは再び複製せず、前回と同じものとして扱う。
- 4a. 作成者が取込をやめた。
  - システムは文書を取込前と完全に同じ状態へ戻す。
- 5a. 前回は届いていたのに、今回届かなかったタスクがある。
  - システムは一覧で示し、作成者が選んで消せるようにする。
...
> **`UC-010`（外部 WBS マスタとやりとりする）から分けたものである**（利用者判断、2026-08-13）。
> **合流はもう形式に依らない** —— 入口は「開く」1 つで、`MSPDI` も `JSON` も同じ経路から入る（`FR-087` の `OP-2`）。
> `UC-010` に置いたままだと、**`JSON` の合流まで「外部 WBS マスタとのやりとり」に紐づいてしまう。**
...
5. システムは操作の切れ目に自動保存し、異常終了した次の起動で復旧を申し出る。
...
- 1b. 作成者が、開いている文書を捨てて新しく始める。
  - システムは未保存の編集を確認したうえで文書を捨て、初期表示用のテンプレートを出す。
...
> **本ユースケースは仕様と設計まで書き、実装は後の節目へ回す**（確定 2026-08-12）。
...
**STATEMENT**: 作成者がタスク形状を構えた状態で、どのアイテムにも当たらない場所をドラッグしたとき、システムは、構えている形状でその期間のタスクを作ること（表 T-023a の PD-4）。**作ったタスクは、ドラッグを始めた縦位置が指す `TaskGroup` に載せること（MUST）** —— `TaskGroupMember` を作る契機はここである。**タスク形状の全数は表 T-012 に従うこと。** ドラッグせずにクリックしたときは、**開始日と終了日が同じタスクを作ること（MUST）。**

**ORIGIN**: UC-001 手順 1 〜 2

**RATIONALE**: 形状は 1 つずつが別の要求ではなく、同じ描き分けの値である。表で持つ。**作るものはコマンドパレットで構えているものが決める。システムが形状を勝手に読み替えてはならない（MUST NOT）** —— 期間がゼロでもタスクはタスクであり、`minShapeWidth` の幅で描く。
...
**STATEMENT**: 作成者がタスクまたはマイルストーンを選んだ状態でパレットの形状を押したとき、システムは、**選んでいるものすべての形状をその形状に変えること。** ただし**表 T-012 の `SH-1` 〜 `SH-4` と `SH-5`（マイルストーン）の間で変えてはならない（MUST NOT）。**

**ORIGIN**: UC-001 手順 1 / UC-003 手順 3（形状は作るときにしか選べなかった）

**RATIONALE**: **`shapeKind` は表 T-016 の `PR-11` として持っているのに、置いたあとで変える手段が要求として無かった。** 形は最初から決まっているとは限らず、描いてから読み直して変える作業は普通に起きる。**マイルストーンとタスクを跨がせないのは、点と期間で持つデータが違うためである** —— 期間を持つタスクをマイルストーンにすると終了日の行き先が無く、逆は期間が決まらない。**実績の形状は変えられない。予定と必ず同じである**（`FR-001`）。
...
⚠️ **描いた直後のタスクは選択されたまま残る。** したがって続けて別の形を描きたいときは、**`Esc` で構えを解いてから**、何にも当たらない場所をクリックして選択を外し（表 T-023 の `MK-11`）、パレットを押す。⚠️ **構えたまま何にも当たらない場所をクリックすると、`PD-4` により期間 0 のタスクが作られる。** `MK-11` は構えていないときにだけ効く。**描いた直後に別の形を押す操作は「形を間違えたので変える」を意味する**（利用者判断、2026-08-13）。**選択されていることは色以外の手掛かりでも見えている**（`FR-081` の `SL-8`）ので、押す前にどちらが起きるかを読める。
...
**STATEMENT**: 作成者がマイルストーンを置くとき、システムは、**8 種の図形**（〇 六角形 五角形 ◇ □ ☆ △ ▽）から選べるようにすること。

**ORIGIN**: UC-001 手順 1 / UC-003 手順 3

**RATIONALE**: 表 T-012 はタスク形状の 1 値として `'milestone'` を持つだけで、**その中のどの図形を使うかを人が選べることを定めていなかった。** マイルストーンは日程表で最も注目される印であり、**種類を描き分けられないと、節目の意味の違いを形で伝えられない**（`FR-030` が求める「色だけで伝えない」の実現手段でもある）。
...
**STATEMENT**: 縦と横で別々の倍率が掛かっているとき、システムは、**マイルストーンの図形・矢じり・角丸・破線・透かしを歪ませずに描くこと（MUST NOT）。**

**ORIGIN**: UC-007 手順 1 〜 2 ／ GL-002

**RATIONALE**: 異方性ズームは本ツールの機能なので、**素直に図形へ倍率を掛けると、◇ が潰れて菱形になり、円が楕円になる。** 形が意味を担う（`FR-030`）以上、形が歪むことは意味が壊れることである。角丸（`FR-019`）と依存線の太さは個別に非追随と定めていたが、**横断の要求として持っていなかった。**
...
**STATEMENT**: システムは、日程表を描くときの寸法 —— バーの縦幅・形状ごとの縦幅の比・段と行の間隔・依存線・進捗マーカーと再開アイコン・ラベル・形状の細部・イナズマ線・余白 —— を、**`_assets/tbl-settings.md` の表 T-201 に従って決めること（MUST）。** 同表に無い寸法を実装が独自に持ってはならない（MUST NOT）。

**ORIGIN**: UC-007 手順 4 ／ GL-002

**RATIONALE**: **他の値の表（表 T-205 〜 T-215）はすべて要求から名指しで指されているのに、最大の表である表 T-201 だけが指されていなかった。** 行 ID でもキー名でも要求文書に一度も現れない行が 41 あり、`importSeq` で見つかったのと同じ型が、その規模で残っていた。
...
| 予定の縦幅（`basePlanHeight`） | **ラベルの文字サイズがバーの高さから導かれること。** `FR-077` は「タスクの高さに比例させた文字」と事実として述べるだけで、比例させることを求めていない |
| 依存線の寸法（太さ・矢じり・走り） | `FR-079` の RATIONALE が「依存線の太さは個別に非追随と定めていた」と述べるのに、**その「個別に定めた」箇所が要求に無かった** |

⚠️ **本要求は新しい規則を作らない。** 1.9 の「細目は表に置き、要求は 1 文でその表を指す」を、表 T-201 にも適用しただけである。**表の側は 1 行も変えていない。**
...
**STATEMENT**: `Task` にフェードイン日数またはフェードアウト日数が入っているとき、システムは、**表 T-012a の形**でバーの端を細らせて描くこと。作成者がその日数を、表 T-023d の `GR-1` / `GR-2` の掴み点で編集できるようにすること。**掴み点は選択しているタスクにだけ出すこと（MUST）** —— 常時出すと、フェードを使っていないタスクにも点が並ぶ。寸法と条件の値は表 T-210。

**ORIGIN**: UC-001 / UC-003

**RATIONALE**: 「この辺りから始まる」「この辺りで終わる」という**確定していない日付**を表す唯一の手段である。表 T-016 の PR-14 にデータ項目としては載っており、**拡張領域を使う 2 つはこれだけ**なのに、描く要求も編集する要求も無かった。⚠️ **「ぼかす」は視覚効果（blur）とも読めるが、そうではない。** 端を斜めに落として細らせる —— **形が意味を担う**ので、ぼかしでは低いズームやモノクロで消える（`FR-030`）。
...
| FD-6 | 切り詰め | `fadeIn` を `[0, 期間]` に丸めた後、`fadeOut` を `[0, 期間 − fadeIn]` に丸める（**`fadeIn` が勝つ**） |
...
**STATEMENT**: 作成者がタスクを作った直後、およびあとから求めたとき、システムは、そのタスクの名称をその場で入力・変更できるようにすること。

**ORIGIN**: UC-001 手順 3

**RATIONALE**: **`UC-002` 手順 1（行の名前）は `FR-085` が受けているのに、`UC-001` 手順 3（タスクの名称）を受ける要求が 1 件も無かった。** 入口は表の行（表 T-023 の `MK-13`、表 T-023d の `GR-10`）と表 T-016 の `PR-1` にしかなく、**細目を持つ表だけがあって、それを要求する文が無い状態だった。**
...
**STATEMENT**: システムは、**`nameAnchor` / `nameAlign` の指定が無いとき**、タスクの名称ラベルを**表 T-013 の順で最初に成立した置き方**で配置すること。**指定があるときはその指定に従うこと。**
...
**STATEMENT**: システムは、ラベルが占める幅を、**全角 2・半角 1 で数えた単位数 × フォントサイズ × `labelCoef`** で概算すること。**文字の実寸を測ってはならない（MUST NOT）。測った結果を蓄えてはならない（MUST NOT）。**

**ORIGIN**: UC-001 手順 4 ／ GL-003

**RATIONALE**: **この方針は 3 か所から「既にあるもの」として参照されていたのに、定める要求がどこにも無かった** —— `LM-2a`・表 T-026 の `RC-3`・`_assets/tbl-settings.md` の `S-30` である。

⚠️ **とくに `LM-2a` は、WCAG 2.1 の 1.4.12（行間・字間の上書き）を適合範囲から外す唯一の根拠としてこの方針を挙げている。** 定める要求が無いということは、**適合範囲を狭めた根拠が仕様に存在しない**ということである。
...
| ST-11 | （廃止。人が段を指定する手段を設けないので、指定どうしの衝突が起きない） |
...
| OC-6 | 下へずらした実績バー（矢印・端点スパン） | 下（縦の占有） |

**日付の範囲だけで数えてはならない（MUST NOT）。** 算入を漏らすと、段割当では重なりを見逃し、全体表示では画面外へ切り落とす。

**算入するかどうかは、表示の切り替えでタスクの位置が動くかどうかで決めている**（利用者判断、2026-08-13）。

| | 扱い | なぜ |
| --- | --- | --- |
| 進捗マーカーと再開アイコン（`OC-3` / `OC-4`） | **常に算入しない** | **表示を切り替えてもタスクの位置が動かないようにするため。** イナズマ線を占有幅に数えないのと同じ考え方である |
| 担当ラベルと完了率ラベル（`OC-2`） | **表示しているときだけ算入する** | **担当者名・完了率・タスク名は重要な情報であり、重なると極端に読みにくくなる。** 出しているあいだは場所を確保する |
...
**STATEMENT**: 作成者が行見出しパネルで操作したとき、システムは、`TaskGroup` を新しく作り、名前を付け、あとから名前を変えられるようにすること。**作った行は既存のどの `TaskGroup` の子にもでき、どこにも属さない最上位にもできること（MUST）。**

**ORIGIN**: UC-002 手順 1

**RATIONALE**: **`UC-002` の手順 1「作成者が行見出しパネルで `TaskGroup` を作り、名前を付ける」を受ける要求が無かった。** `FR-004` は畳む・展開する・表示を切り替えるだけ、`FR-005` は既にある階層を移すだけで、**行を新しく作る経路がどこにも定められていなかった。** それでいて表 T-014 の `ST-4` は「1 か所へ集めたいときは人が専用の行を作る」と、`FR-058` は「上限が掛かるのは人が階層を作るときだけである（`FR-004`）」と、**存在しない要求を前提にして書かれていた。** 消す要求（`FR-032`）だけがあって作る要求が無い状態だった。

**深さの上限は `FR-004` に従う（値は表 T-211 の `S-125`）。上限に達している親の下に作らせてはならない（MUST NOT）。**

**行見出しパネルで行（`TaskGroup`）を選べること（MUST）。複数を選べること、および選択を解除できること。** ⚠️ **これは表 T-023c が定める描画領域の選択とは別の集合である**（同表の `SL-1` が行を対象から外している）。表 T-015 の `HR-3` 〜 `HR-5`・`FR-032`・`FR-033`・`FR-042` が前提にしている「選ばれた行」は本規則が与える。
...
**WBS の深さをクランプしてはならない（MUST NOT）。** 取り込んだ元データが 6 段以上でも、階層は `Task.wbs_parent_uid` がそのまま持ち、書き出すときに深さから算出して戻す。**浅くすると往復で構造が壊れ、`FR-021` と正面から衝突する。** `S-125` で頭打ちにしてよいのは **LOD の判定のときだけ**である（`FR-018`）。**非表示が増えてもグループ罫線を太くしてはならない（MUST NOT）。**
...
**STATEMENT**: 作成者が行見出しパネルで階層を移したとき、システムは、**表 T-015a の規則**に従うこと。
...
**STATEMENT**: 作成者がタスクを選んだとき、システムは、**表 T-016 の項目**をプロパティパネルに出し、**同表が読み取り専用と記した項目を除いて**編集できるようにすること。
...
| PR-17 | `milestoneGlyph` | マイルストーン形状 | `shapeKind` が `'milestone'` のときだけ有効。置いた後も変えられること（`FR-078`） | 無い（JSON のみ） |
...
**STATEMENT**: 利用者が選択または設定の入口を操作したとき、システムは、プロパティパネルに出す中身を**最後に行われた操作**で決めること。**選択が解除されたときは、直前に出していた中身を残し、見出しで「選択なし」と示すこと（MUST）。**設定を開いても選択を解除せず、もう一度同じ入口を押したら直前の選択物へ戻すこと。**選択を解除しても設定へは移らないこと（MUST NOT）。** いま何を出しているかを、押下状態とパネル先頭の見出しで示すこと。
...
**STATEMENT**: 作成者がテーマの色相・明暗・モノクロを選んだとき、システムは、**色を指定していない `Task` と `TaskGroup`・行の帯・地の色・依存線とイナズマ線と注記の固定色**をそれに追随させて描くこと。**依存線とイナズマ線は同じ固定色とし、文書に保存しないこと（MUST NOT）。**
...
**RATIONALE**: **色のテーマはプロジェクトのテーマカラーであり、作った人の決めごとである。** 明暗と同じ枠に入るので文書に保存する。**モノクロは描画の段で効くので、人が指定した色も無彩色で描かれる。保存値は変わらないので、戻せば色も戻る。** `FR-007` は「指定しなければテーマ色に追随する」とだけ述べており、その追随先を人が決められることを定めていなかった。
...
**STATEMENT**: 作成者が行見出しパネルで行（`TaskGroup`）を選んだとき、システムは、その行の色と高さをプロパティパネルに出して編集できるようにし、指定されたらその指定で帯を描くこと。⚠️ **行の名前はここで扱わない** —— 名前を付ける・変える入口は `FR-085` の行見出しパネル 1 つとする（`FR-029`）。**色**の指定が無い行は、**深さと行の位置**からテーマ色に基づいて解いた帯色で描くこと。**高さ**の指定が無い行は、段数から自動で決めること（`FR-003` の ST-9）。
...
**STATEMENT**: 作成者が取り消しを求めたとき、システムは、直前の編集を取り消し、取り消した編集をやり直せるようにすること。**取り消しの対象は表 T-027 に従うこと。**

**ORIGIN**: UC-001 / UC-002 / UC-003 / UC-005 の全手順（横断） / UC-012 手順 4 / UC-013 手順 5

**RATIONALE**: **誤操作への備えは取り消しの 1 本に絞る。** 確認ダイアログを増やす代わりにこれが要る。**例外は行の削除だけである**（`FR-032`）—— 連鎖して消える範囲が大きく、取り消しで戻せても「何が消えたか」を人が把握できないため。**例外を増やさない（MUST NOT）。**破壊的な操作（連鎖削除・取込による上書き）を戻せることは、適合範囲に含めたアクセシビリティの基準でもある。**`Agent API` 経由の変更も同じ履歴に積み、呼び出し 1 回を 1 段とすること（MUST）** —— 入口が UI と `Agent API` の 2 つあっても、履歴は 1 つである。**何を積み、何を積まないかは表 T-035 の `AG-10` が定める。**
...
| UN-13 | 対象 | **文書全体の設定の変更** —— テーマ（`FR-041`）・文字サイズ（`FR-039`）・暦（`FR-088`）・行の色と高さ（`FR-042`）・文書名（`FR-035`）・文書の基本情報（`FR-074`）。**`UN-7` の「表示の切り替え」とは別である** —— あちらは `FR-049` が扱う表 T-202 のトグルに限る |
| UN-4 | 対象 | 階層の変更 |
| UN-5 | 対象 | 注記（コメント・ハイライトボックス）の追加・変更・削除 |
| UN-6 | 対象 | 取込による上書き |
...
**STATEMENT**: 作成者が選択の操作を行ったとき、システムは、**表 T-023c の規則**で選択を作り、広げ、解除すること。

**ORIGIN**: UC-001 手順 5 / UC-003 手順 1 / UC-004 手順 2（選択を作る手段が定められていなかった）

**RATIONALE**: **`FR-032`（削除）・`FR-033`（複製）・`FR-034`（整列）・表 T-036 の `SK-3` はいずれも「選ばれた」複数を前提にしているのに、その複数を作る要求がどこにも無かった。** 入口は `Ctrl+A` の全選択しか定義されておらず、一部を選ぶ手段が仕様に存在しなかった。
...
| 行 ID | 事項 | 規則 |
| --- | --- | --- |
| SL-1 | 対象 | **タスク・依存線・ハイライトボックス・コメントボックス。** 行（`TaskGroup`）は対象に含めない。⚠️ **本表が定めるのは日程の描画領域での選択である。行の選択は別の集合であり、規則は `FR-085` が持つ** |
...
**ORIGIN**: UC-001 / UC-002（削除の手順が無かった）

**RATIONALE**: 作る要求だけがあって消す要求が無かった。**連鎖して消えるものを黙って消すと、利用者は何が失われたかを知る手段を持たない。**
...
**STATEMENT**: 作成者が整列を求めたとき、システムは、選ばれたタスクの開始日または終了日を、**最後に選んだタスクの日付へ**揃えること。**システムが自動で横へ動かしてはならない（MUST NOT）。**

**ORIGIN**: UC-001

**RATIONALE**: **縦と横で仕組みが違う。** 縦は段割当（FR-003）の結果として自動で揃うが、**横は人が実行する。** 日付は人が決めるものであり、システムが動かしてよいものではない。**揃え先を「最後に選んだタスク」とするのは、選ぶ順序で人が基準を決められるからである** —— 最も早い日や最も遅い日に寄せると、どれに揃うかを人が選べない。**選択が順序を保つことと、順序を作らない選択で整列を実行させないことは、表 T-023c の `SL-7b` が定める。**
...
**STATEMENT**: 作成者が文書名を選んだとき、システムは、その場で編集できるようにすること。

**ORIGIN**: UC-011

**RATIONALE**: 文書名は保存にも出力にも出る。画面から変えられないと、別の手段が要ることになる。**ブラウザのタブにも、いま開いている日程表が判る題名を出すこと（MUST）** —— 同じ機で 2 つの文書を同時に開くことが実際に起きる構成なので、タブの見出しが同じだと選べない。
...
**STATEMENT**: 作成者が依存線を構えて 2 つのタスクを結んだとき、システムは、**引き出した辺と引き入れた辺の組合せから表 T-018 の種別を決め**、同表のアンカーで線を引くこと。経路は表 T-018a に従うこと。
...
**依存の種別ごとにラグを保持し、作成者が編集できること（MUST）。既定値と単位は `_assets/tbl-settings.md` の表 T-213 が持つ。** ⚠️ **ラグは値として保持するだけで、日付を自動で動かさない** —— 日付を動かす計算系は範囲外である（表 T-009 の `XO-5`）。経路の規則も増やさない。
...
⚠️ **「他の図形と極力かぶらない経路」という言い方を要求から外した。** 「極力」には合否の線が引けず、RC-6 が交差の増加を既に受け入れているため上限を置く道も無い。**意図としては残すが、検証するのは RT-1 〜 RT-4 である。**
...
**STATEMENT**: システムは、実績バーの左端を `actualStart`、右端を `actualStart` に `actualDuration` を稼働日で加えた日とし、**人が置いていない限り両端を動かさないこと（MUST NOT）。** 実績の置き方は表 T-012 の最終列に、**重ね順は表 T-020 に従うこと。**
...
**STATEMENT**: 予定と実績が時間軸上で重ならなくなったとき、システムは、**表 T-020a の規則**で実績から予定へ補助線を引くこと。
...
| GD-2 | 上下の幅がある形状（矩形・矢羽根） | **実績バーの上端と下端の高さから、予定側へ水平な細い点線を 2 本**引く |
...
| GD-6 | 依存線との区別 | **依存線は実線で矢じりを持ち、補助線は点線で矢じりを持たない（MUST）。** 色だけで区別してはならない（`FR-030`） |
...
**STATEMENT**: `Task` が未着手であるあいだ、システムは、実績の入力を始める入口を**未着手と分かる見た目**でタスクの上に示し、掴まれたときに実績開始日を置き、**押されたときに状態を巡らせること（MUST）。**
...
**STATEMENT**: `Task` が中断しているあいだ、システムは、作成者が再開予定日を画面上で置き、置いた後に動かせるようにすること。**再開日を未定のままにもできること。****中断のあいだは再開アイコンを描くこと（MUST）。再開日が未定のときは表 T-201 の `S-25` に従って別の見た目にすること（MUST）** —— 表 T-021 の `PM-3` が中断の 2 種を 1 つの記号に畳んでいるので、`PA-3` と `PA-4` を絵で見分ける手段は再開アイコンしか無い（`FR-030`）。

**ORIGIN**: UC-005 拡張 3a

**RATIONALE**: 表 T-019 の 5 状態のうち 2 つ（PA-3 / PA-4）は再開予定日の有無で分かれるが、**その日付を人が置く手段が定められていなかった。** 未定のままにできることが「中止」を表す唯一の経路である（専用の概念を作らないため）。
...
**RATIONALE**: 分母は予定の期間である。`actualStart` が `start` より後でも分母は変わらない。型は整数とし、**0 〜 100 に丸めてはならない（MUST NOT）** —— 予定 100 日のタスクを 80 日で終えれば 80、120 日かかれば 120 である（利用者判断、2026-08-13）。**完了したかどうかは `actualFinish` の有無が表すのであって、完了率が 100 かどうかではない**（表 T-019）。**これにより予定と実績の工数の差が完了率から直に読める。**

⚠️ **本要求の算出規則は、変更耐性を特に高く保つこと（MUST）** —— **書き出した完了率を受け取った側がどう解釈するかは未検証であり**（`LM-7`）、**後から式を変える判断があり得る**（利用者判断、2026-08-13）。設計は、この式を 1 か所に閉じ込め、**呼ぶ側が式の中身に依存しない形にすること**（設計の基準は `FR-092` の `EZ-5` が持つ）。
...
⚠️ **表 T-033 の `EX-5` が定める「中身のない行」は、この禁止の対象外とすること（MUST）。** 中身のない行は日付を持たないが、**タスクとして画面に出さず**、書き出しで元の位置と形のまま戻すだけなので、上の 3 つのどれも走らない。**中身のない行が含まれることを理由にファイルごと拒んではならない（MUST NOT）** —— `FR-023` は超えた入力を部分適用せずに拒むと定めており、対象外にしておかないと**中身のない行が 1 つあるだけで取込が丸ごと失敗し、`FR-021` の往復無損失も同時に落ちる。**
...
⚠️ **予定の期間が 0 の `Task` では割り算を行ってはならない（MUST NOT）。完了率は 0 か 100 のどちらかだけとする。** 対象はマイルストーンに限らない —— **`UC-001` 拡張 2a が作る「開始日 ＝ 終了日のタスク」も期間 0 である。** 期間は開始日と終了日の差とし、**端を含む日数と取り違えないこと（MUST NOT）** —— 含めると期間 0 が存在しなくなり、この規定が空振りする。
...
**STATEMENT**: システムは、実績バーの右端の外側に進捗マーカーを出し、**表 T-021 の記号**で状態を示すこと。**予定だけを表示しているときは、予定バーの右端の外側に出すこと（MUST）** —— 依存線がどちらのバーに付くかの定め（`FR-009`）と同じ形である。表 T-038 の `OC-3` と表 T-023d の `GR-7` の場所も同じに読み替える。押されたときは、**表 T-021a に従って状態を巡らせること。**
...
| PV-3 | 完了 | 中断 | `actualFinish` を空にする。`resume` は置かない（再開日未定） |
| PV-4 | 中断 | 未着手 | `actualStart` / `actualDuration` / `actualFinish` / `resume` をすべて空にする |

**PV-1 で置いた値は「人が置いた」に数える（`FR-011`）。** 以後その両端はシステムが動かさない。

**色だけで区別してはならない（MUST NOT）。形が意味を担うこと（MUST）。**

**進捗マーカーの大きさは実績バーに追随させ、実績バーより大きく描いてはならない（MUST NOT）。** ⚠️ **`markerOfFont` × `fontOfActual` が 1 を超えるとこの禁止を破る** —— 径はフォント基準、フォントは実績基準なので鎖が掛かる。上限は `_assets/tbl-settings.md` の `S-21` が持つ。 低いズームでは実績バーが数 px まで縮むので、**追随させないと注釈が本体より大きくなり、何の印なのか読めなくなる。**
...
**STATEMENT**: `Task` に期限（`deadline`）が入っているとき、システムは、それを**終了日とは別の印**として日程表の上に示すこと。

**ORIGIN**: UC-003

**RATIONALE**: 期限は「この日までに終える」という独立した約束であり、予定の終了日とは別物である。**期限の超過を遅れの判定に含めない** —— 遅れは表 T-021 の `PM-4` と表 T-021b の 3 条件だけで判定する。期限は外部との約束であって進捗の遅れとは別の量であり、混ぜると `UC-006` の判定が二重になる。表 T-016 の PR-10 にデータ項目としては載っていたが、**描く要求が無かった。**
...
**STATEMENT**: 閲覧者が基準日を置いたとき、システムは、その日付を基準として遅れを判定すること。**基準日は後から動かせること。基準日の縦線は表示と非表示を切り替えられ、常時表示にしてはならない（MUST NOT）。**

**ORIGIN**: UC-006 手順 1

**RATIONALE**: **基準日を動かしたときに何が動かないかは `FR-011` が定める。** 動くのはイナズマ線と遅れ判定だけである。縦線を常時表示にしないのは、遅れが進捗マーカーと遅れの日数で読めるためで、線を増やすと画面が狭くなる。**基準日は設定値ではなく文書のデータである。**
...
**STATEMENT**: `Task` が遅れていると判定されたとき、システムは、遅れの量を**表 T-021b の起点と終点**から**稼働日**で数えて示すこと。

**ORIGIN**: UC-006 手順 4

**RATIONALE**: 進捗マーカーの `(!)` は遅れの**有無**しか伝えない。どれだけ遅れているかは、イナズマ線の頂点の位置と、この日数が担う。⚠️ **起点と単位を定めていなかったので、同じタスクでも実装ごとに違う日数が出た。** 基準日から数えるのか実績バーの右端から数えるのかで値が変わり、暦日か稼働日かでも変わる。**稼働日で数えるのは `FR-011` と `FR-012` が同じ単位を使っているからである。**
...
**STATEMENT**: 閲覧者が変更前の予定を読み込んだとき、システムは、それを別ファイルとしてグレーで重ねて描くこと。**現在の文書との対応は `UID` で照合すること（MUST）。片側にしか存在しない `Task` は描いてはならない（MUST NOT）。** 重ねる相手の `TaskGroup` の構造が違うときは、現在の文書の行に従うこと。
...
**STATEMENT**: システムは、ポインタとキーボードの操作を**表 T-023 の割当**で受け付けること。**ズームの倍率は表 T-203 の `S-75` / `S-76` が持つ範囲へ丸めること（MUST）。1 ノッチで動く倍率は表 T-201 の `S-53` に従うこと。**

**ORIGIN**: UC-007 手順 1・5、拡張 1a・5a

**RATIONALE**: 細目は表に置く（1.9）。**同じ操作に 2 つの答えが立たないよう、割当の前に判定順序（表 T-023a）を置く。** 以前は「行の上でドラッグ」と「背景の上でドラッグ」という**場所**で書き分けていたため、行の空き領域が両方に当たっていた。
...
**当たり判定の対象は、そのフレームで描いた編集対象の要素に限ること（MUST）。** 変更前の予定（`FR-015`）は表示のみで編集できないので対象外とする。LOD で描かなかった `Task`・非表示にした行・畳まれて隠れたコメントも対象外である。**描いていないものが当たると、何も無い場所でドラッグしたのに既存の要素が動く。**
...
| AR-5 | コメントボックス | **その位置にコメントを置く**（`FR-019`） | そのものへの既定操作（既存が勝つ）。⚠️ **当たった対象を指す先にしてコメントを置く経路は持たない** —— `PD-3` の例外は依存線の構えだけである。コメントの指す先は日付と行の識別子で持つ（`FR-019`）ので、対象を指名する必要がない |
...
| MK-9a | 掴む対象が重なった | **優先順位を定めること（MUST）** —— 掴み領域の全数と優先順位は表 T-023d が持つ。定めないと、同じ場所を押すたびに違うものを掴む |
| MK-10 | **本ツールが割り当てた**修飾キーの付いた入力 | **ブラウザの既定動作を画面全体で止めること（MUST）。割り当てていない組合せを止めてはならない（MUST NOT）** —— `Ctrl+P`（印刷）や `Ctrl+F`（検索）まで奪うと、ブラウザの機能が使えなくなる。 キャンバスの上だけでは足りない —— `Ctrl+S` が素通りすると、利用者の保存操作がブラウザの HTML 保存にすり替わる |
...
**上の行ほど優先すること（MUST）。** 掴み代の値は `_assets/tbl-settings.md` の表 T-206 が持つ。
...
| GR-15 | 実績のマイルストーン | 実績の図形の上 | `actualStart` を動かす。**マイルストーンは実績バーを持たないので `GR-5` / `GR-6` に当たらない** |
| GR-13 | 依存線 | 線の上 | 選ぶ |
...
**STATEMENT**: システムは、画面の各部を**表 T-031 の規則**でスクロールさせること。**スクロールバーの操作でも表示位置を変えられるようにすること（MUST）。**
...
**スクロールバーを消してはならない（MUST NOT）** —— 消えるとキャンバスの幅が変わり、再レイアウトが走る。
...
**STATEMENT**: 利用者がパネルの境界をドラッグしたとき、システムは、行見出しパネルとプロパティパネルの幅を変えること。**2 つの幅の上限は互いに依存するので、片方だけを見て判定してはならない（MUST NOT）。** **行見出しパネルの幅を 0 にできてはならない（MUST NOT）** —— 表 T-031 の SC-3 が常時表示を求めており、幅 0 はそれを破る。
...
**STATEMENT**: システムは、コマンドパレットを浮遊させ、作成者がドラッグで動かせるようにすること。**選択されていないあいだは薄く透明に描くこと。**
...
**STATEMENT**: システムは、**表 T-036 の割当**でショートカットキーを受け付けること。
...
| 行 ID | 操作 | 割当 |
| --- | --- | --- |
| SK-1 | （廃止。キーボードだけで図形を置く経路は持たない。表 T-026 の `RC-10`） | — |
| SK-1a | （同上） | — |
...
> ⚠️ **員数が引継ぎ資産の中で食い違っている。** アイコン案は「現行 7 個 ＋ 新設 6 個」と記すが、`ui-detail-spec-ja.md` §5-1 に実在する編集の割当は、アイコン案が「現行」とした数を上回る。**本表は実在が確かめられたものと、新設と宣言されたものを合わせた全数である。**行数は本表が持ち、散文に書かない。**引継ぎ資産への書き戻しは未処理である。**
...
**STATEMENT**: **表示倍率または実効フォントサイズが変わったとき**、システムは、タイムルーラーの粒度を 1 日あたりの表示幅から決まる 4 段階へ切り替えること。**1 日あたりの表示幅は、表 T-201（`_assets/tbl-settings.md`）の `S-1` に `zoomX` を掛けた値とすること（MUST）。****しきい値は表 T-205 のしきい値の行（`S-83` 〜 `S-85`）に従うこと。**
...
**しきい値の 3 本は固定値とし、実行時に導出してはならない（MUST NOT）。** 実装が値を書き換えてはならない（MUST NOT）。**しきい値を 1 つの配列にまとめてはならない（MUST NOT）** —— 隣どうしが互いを縛る（Month ≦ Week ≦ Day）ので、どの要素にどの範囲が掛かるかを書く場所が無くなる。**しきい値は目盛のフォントが表 T-201 の `S-8`（和文の可読下限）であるときの px/day であり、判定は実効フォントサイズをその値で割った比で行うこと（MUST）** —— 判定式は `pxPerDay ÷ (実効フォントサイズ ÷ `S-8`) ≧ しきい値` とする。**実効フォントサイズとは、そのとき目盛を描いている `rulerFont` の px** である。⚠️ **これは値を導出することではない** —— 保存する 3 本は固定のままで、判定に入る前に文字倍率を打ち消すだけである。

**目盛の帯の高さは、目盛の段階が変わっても動かさないこと（MUST）。** 段階が変わっても外枠の高さは同じで、中の組み方だけが変わる。**高さが段階で変わると `zoomX` → 目盛の段階 → 帯の高さ → 使える高さ → `zoomY` という循環がもう 1 本増える。** ⚠️ **`fontScale` で帯の高さが変わることはこの禁止に当たらない** —— 禁じているのは**目盛の段階**による変化であり、`fontScale` はズームと独立なので循環を作らない。
...
**STATEMENT**: 作成者が暦を編集したとき、システムは、**稼働する曜日・例外日（休業日）・週の始まり**を変えられるようにし、変更を文書の暦へ反映すること。

**ORIGIN**: UC-007（`FR-054` は暦に従って描くことだけを定め、暦を人が直す入口が無かった）

**RATIONALE**: **設定値表の `S-107` が「必要なら MSPDI を取り込むか、**非稼働日として個別に置く**」と、置く経路の存在を前提にして書かれていた。** 既定の暦は祝日を持たない（持つと特定の地域を前提にすることになる）ので、**自社の休業日を入れる手段が無いと、既定のままでは日数が実態と合わない。**
...
**STATEMENT**: システムは、文書が持つ暦に従って非稼働日（週末・祝日）を稼働日と区別して描くこと。**期間の計算に用いる「稼働日」も同じ暦に従うこと。1 日を何時間と決め打ちしてはならない（MUST NOT）。**

**ORIGIN**: UC-007（要望 39。表 T-011 の V-7 が吸収済みと記していたが要求が無かった） ／ UC-005

**RATIONALE**: **`FR-011` と `FR-012` はどちらも「稼働日」で計算すると述べているのに、稼働日が何かを定める要求が無かった。** 暦は取り込んだ MSPDI が持っており、曜日ごとの稼働可否と例外日からなる。**MSPDI を取り込んでいない文書にも暦があること（MUST）** —— 表 T-033 の EX-1 は取り込まずに作った文書も相手のツールで開ける形で書き出すことを求めており、**MSPDI には暦が要る。** 既定の暦は `_assets/tbl-settings.md` の表 T-209 が持つ。**取り込んだ暦があればそれを文書の暦とする。** **繰り返し指定の例外日（毎年の元日など）は実日付へ展開しないので、未対応であることを利用者に知らせること。** **週の目盛は週の始まりの日付を書き、週の始まりも文書の暦に従うこと（MUST）** —— 曜日を決め打ちしない。
...
**STATEMENT**: 日付罫線を表示しているとき、システムは、**タイムルーラーが出している最も細かい段と同じ間隔**で縦線を引くこと。**1 日あたりの表示幅によらず一定の間隔で引いてはならない（MUST NOT）。**

**ORIGIN**: UC-007 手順 3（表 T-202 の `S-67` と用語辞書の `U-17` が存在を前提にしているのに、描く要求が無かった）

**RATIONALE**: **`S-67` の既定が `false` である理由が「縦線を目盛の段階に連動させる規定がまだ無い」というものだった** —— つまり**規定が無いこと自体が既定値を決めていた。** 連動させないと `px/day` が 1 を割った時点で 1px ごとに線が立ち、**線が面になって日程が読めなくなる。**
...
**「一定数以下なら全部描く」というしきい値を設けてはならない（MUST NOT）** —— 前プロジェクトの場当たりであり、本プロジェクトは起動の手順で根治する。設定として残すと根治しない。

**グループ LOD は深いほうから順に畳むこと（MUST）。** 縦の倍率を下げるにつれ、`TaskGroup` の深さが**深い段から順に**描かれなくなる。**下限は `FR-055` が、畳まれた行の描き方は表 T-015 の `HR-1a` が持つ。** しきい値は `_assets/tbl-settings.md` の表 T-205 の `S-87` / `S-88` が持ち、**公比が 1 を超えることがこの順序を保証する**（`S-88`）。
...
**STATEMENT**: システムは、画面に描く文字を可読の下限より小さくしないこと。**下限を割るときは、文字を縮めるのではなく、表示する行の量を減らすこと（MUST）。**

**ORIGIN**: UC-007 手順 4 ／ GL-002

**RATIONALE**: 縦のズームを縮めると、タスクの高さに比例させた文字はいくらでも小さくなる。**読めない文字で埋まった画面は、情報量が多いのではなく 0 である。** `FR-025` は書き出しのときの下限を扱っていたが、**画面表示側の下限が要求として無かった。**
...
**STATEMENT**: 閲覧者が全体表示を求めたとき、システムは、1 つの操作で、縦横の倍率と表示位置を全体が収まる側へ合わせること。**ただし必ず収まることを保証しない。収まらない軸にはスクロールを残すこと。** **描くものが 1 つも無い文書では、倍率を等倍に戻し、表示位置を今日へ合わせること（MUST）** —— 実寸が 0 だと収める倍率が決まらない。

**ORIGIN**: UC-007 ／ GL-002

**RATIONALE**: `GL-002`（全体日程を 1 画面で見られる）を受ける操作が、ユースケースにも要求にも無かった。**測る対象は描画の実寸とし、算入するものは表 T-038 に従うこと（MUST）** —— 同表の見出しが、段割当（`FR-003`）と本要求で数え上げを分けないことを定めている。**日付の範囲だけで測ると、左の張り出しが行見出しパネルに隠れ、どのスクロール位置でも見えなくなる。**
...
⚠️ **そこまで落としても 1 画面に収まるとは限らない**（利用者判断、2026-08-13）—— 深さ 1 の `TaskGroup` が多い文書や、1 つの行に人が多くの `Task` を積んだ文書では、**どの倍率でも収まりようがない。** そのときスクロールを残すことは STATEMENT が既に定めている。 **`GL-002` の達成は、表 T-025 の `MC-7` が定める目標規模の文書で収まることをもって判定する。** **保証しないと明記するのは、保証しようとすると LOD の段階を無限に下げることになり、読めない絵で「収まった」と主張することになるためである。**
...
**STATEMENT**: 利用者が求めたとき、システムは、全画面表示に入ること。**同じ入口で解除できること。** 入口は画面上のボタンとキーの両方に持つこと。
...
**STATEMENT**: 作成者がコメントまたはハイライトボックスを構えて置いたとき、システムは、それを作り、その位置を日付と**行の識別子**で持つこと。**置く手段は表 T-023b の構え（AR-5 / AR-6）とする。****行を順番で参照してはならない（MUST NOT）。**

**ORIGIN**: UC-008 手順 1 〜 4、拡張 2a・4a

**RATIONALE**: **ハイライトボックスの線色を指定でき、指定が無ければ注記用の固定色で描くこと（MUST）。** その固定色は、テーマの色相・依存線・イナズマ線のいずれからも離した色とする —— 注記が日程の要素と見分けられなくなる。**ハイライトボックスに透明を選ばせてはならない（MUST NOT）** —— 枠線が唯一の表現なので、透明にすると何も残らない。**コメントは引出し四角と折れ線の 2 種から選べること（MUST）** —— 指す先が近いときと遠いときで、読みやすい形が違う。順番で持つと並べ替えで別の行を指す。**コメントは点を指し、ハイライトボックスは範囲を囲む。必要な情報が違うので別のものとして持つ。** 吹き出しのずれだけは画面上の距離で持ち、ズームしても見た目の距離が変わらないようにする。角丸の半径もズームによらず一定に描く。指している行が畳まれたらコメントも隠し、囲んだ範囲の一部が非表示になったら見えている行だけを囲う。
...
**STATEMENT**: 閲覧者がカーソルを有効にしたとき、システムは、**表 T-029 の 3 種**をそれぞれ独立に出し分けること。

**ORIGIN**: UC-007（要望 40〜43。表 T-011 の V-7 が吸収済みと記していたが要求が無かった）
...
| 行 ID | 名前 | 何をするもの | 備考 |
| --- | --- | --- | --- |
| CU-1 | `Today Line`（本日線） | 今日の日付に縦線を 1 本引く（固定） | **表示状態を文書にも `localStorage` にも保存しない**（今日は読む人の環境である）。**起動のたびに表示から始め、その場で切り替えられること（MUST）。** 表 T-202 に行を持たないので `FR-049` の対象ではない |
| CU-2 | `Dual Cursor`（デュアルカーソル） | 任意の縦線を 2 本立て、**その間の日数を測る** | **2 本の日付を持つ**（持たないと測った日数が再現しない） |
| CU-3 | `Guide Cursor`（ガイドカーソル） | ポインタに追従する補助線 | **4 モード排他** —— なし / 十字 / 縦 1 本 / 縦 2 本 |

**描く内容が変わらないとき、描き直してはならない（MUST NOT）。** **ポインタの移動それ自体は描く内容を変えない** —— 変えるのは、**ポインタに追従する線がある場合だけ**である（ガイドカーソル、および `Dual Cursor` の追従している側。表 T-029a の DC-1）。追従する線が 1 本も無いのに毎フレーム描き直すと、`GL-003` を崩す。
...
**STATEMENT**: 閲覧者が `Dual Cursor` を有効にしたとき、システムは、**表 T-029a の規則**で 2 本の縦線を置き、動かせるようにすること。

**ORIGIN**: UC-007（`FR-048` は 3 種を「出し分ける」ことしか定めておらず、2 本を置く手段が無かった）

**RATIONALE**: **測る道具なのに、測る位置を人が決める手段が要求として無かった。** `dualCursor` は 2 つの日付を持つと定めながら（`tbl-settings.md` の S-65）、その日付をどう入れるかがどこにも書かれていない。
...
| 行 ID | 事項 | 規則 |
| --- | --- | --- |
| DC-1 | 入る | パレットの `Dual Cursor` の入口を押す。**既定では A がポインタに追従し、B は画面の中央に置く** |
...
**STATEMENT**: 利用者が表示の切り替えを求めたとき、システムは、**`_assets/tbl-settings.md` の表 T-202 のうち、表示の可否を持つ項目**をそれぞれ切り替えられるようにすること。⚠️ **同表には切り替えでない項目も含まれる**（積む向き・文字サイズ・取込の連番・デュアルカーソルの 2 日付）ので、**全項目をトグルとして扱ってはならない（MUST NOT）。**
...
**STATEMENT**: 閲覧者が透かしの設定を求めたとき、システムは、**透かしに出す名前**を入力させ、`localStorage` へ保存すること。**名前が設定されていないまま日程表を開いたときは、設定を促すこと（MUST）。** 作成者が**透かし解除パスワード**を設定したときは、その SHA-256 だけを保存すること（生の値の扱いは `FR-020`）。

**ORIGIN**: UC-009 拡張 2a

**RATIONALE**: **`UC-009` の拡張 2a「開いた者の名前が設定されていない → システムは名前の設定を促す」を受ける要求が無く、名前を人が設定する手段そのものが要求に無かった。** `tbl-settings.md` の `S-99a` は保管先を書いているだけである。`GL-007`（秘密の日程を画面に出した証跡が残る）は `FR-020` 1 本で支えられているので、**名前が入る経路が無ければ目標がそのまま未達になる。**
...
| IO-2 | JSON | 取込 / 書出 | 本ツールの主データ形式。機械連携 | 再現の規則は `FR-024`。文字コードは表 T-003 の `CN-5` |
...
**STATEMENT**: 合流が選ばれ（`FR-087` の `OP-3`）、同じタスクが現在の文書と読んだファイルの両方にあるとき、システムは、「上書き / 別のものとして取り込む / 取込をやめる」から選ばせること。**取込側に無かったタスクを消してはならない（MUST NOT）。**

**ORIGIN**: UC-014 手順 3 〜 4

**RATIONALE**: 残したうえで知らせるだけにする。勝手に消すと、合流のたびに他の担当者の作業が失われる。本要求が受け持つのは、**合流を選んだ後に同じタスクが両側にあった場面**だけである（入口と選ばせ方は `FR-087` の `OP-2` / `OP-3`）。
...
**STATEMENT**: 読んだファイルを現在の文書へ合流させるとき（`FR-087` の `OP-3`）、システムは、**表 T-032 の規則**に従うこと。

**ORIGIN**: UC-014 手順 1 〜 5、拡張 1a・3a・4a・5a

**RATIONALE**: `FR-022` は「同じタスクが来たときに 3 択で選ばせる」ことしか定めていなかった。**合流は 1 つの操作ではなく、判別・衝突・重複・取消のそれぞれに規則が要る。** 細目は表に置く（1.9）。**間違えると外部 WBS マスタ側のデータを壊す。**
...
| 行 ID | 場面 | 規則 |
| --- | --- | --- |
| MG-1 | 取り込んだファイルの出自 | 同じ外部 WBS マスタの再取込か、別のマスタかを判別して既定の扱いを変える。**判別できないときは安全側（別のものとして扱う）に倒すこと（MUST）** |
| MG-2 | 同じタスクが来た | 「上書き / 別のものとして取り込む / 取込をやめる」から選ばせる（`FR-022`） |
...
| MG-8a | **JSON で**「上書き」を選んだ | **見た目と行の所属も取込元の値で置き換えること（MUST）** —— JSON はこれらを持つので「取込元が持つ値だけを置き換える」に含まれる。対象の全数は `MG-12` |
| MG-9a | 取込の最中に次のファイルが来た | **受け付けない。規則は `FR-087` の `OP-8` が持つ** —— 入口が「開く」1 つになったので、`OP-8` が別の開く操作も含めて上位で定めている |
| MG-9 | 3 択を問う粒度 | **取込 1 回につき 1 度だけ問い、衝突した全タスクへ一括して適用すること（MUST）。** 数百件を 1 件ずつ問うと、答え切る前に人が諦める |
| MG-10 | 「別のものとして取り込む」を選ばせる前 | **そのタスクが元の外部 WBS マスタへ戻せなくなることを示すこと（MUST）** |
...
| MG-12 | JSON を合流させた | **本ツール固有のもの（`TaskGroup`・`TaskGroupMember`・依存・注記・担当者）も合流の対象とすること（MUST）** —— MSPDI が持たないものなので、`MG-1` 〜 `MG-11` だけでは受け皿が無い。**文書の設定（`documentSettings`）が衝突したときは、タスクとは別に「上書き / 既存を保持 / 取込をやめる」から選ばせること（MUST）**（`MG-4` と同じ形。利用者判断、2026-08-13）。⚠️ **設定の項目ごとに問うてはならない（MUST NOT）** —— 問う粒度は `MG-9` に従い、`documentSettings` 全体へ一括して適用する |
...
**STATEMENT**: MSPDI を書き出すとき、システムは、**表 T-033 の規約**に従うこと。

**ORIGIN**: UC-010 手順 5 〜 6

**RATIONALE**: `FR-021` は「情報を落とさない」ことだけを述べており、**相手のツールが開けるかどうか**と**編集していない部分を触らないこと**を定めていなかった。往復無損失は、この 2 つが揃って初めて成立する。
...
**STATEMENT**: MSPDI を取り込んだとき、システムは、タスクが**自動的に行に載って見える状態**にすること。
...
**行の名前が行見出しパネルの幅に収まらないときは、打ち切って全文をツールチップで出すこと（MUST）** —— 名称ラベルと同じ扱いである（表 T-013 の前に置いた打ち切りの前処理）。深さのインデント（`S-37`）ぶん使える幅が狭くなるので、深い段では必ず起きる。
...
**STATEMENT**: 担当ラベルを描くとき、システムは、**作業資源だけ**を対象とし、材料資源・費用資源・名前が空の資源を出さないこと。複数が割り当てられているときは、**資源名の昇順で先頭 1 名と残りの人数**で示すこと（同名は `UID` の昇順）。
...
**STATEMENT**: 完了率ラベルを描くとき、システムは、`percentComplete` の値を**整数と百分率の記号**で示すこと。**丸めて示してはならない（MUST NOT）。**

**ORIGIN**: UC-003 手順 4（担当ラベルには `FR-059` があるのに、完了率ラベルには描く要求が無かった）

**RATIONALE**: **`UC-003` 手順 4 は「担当ラベルと完了率ラベルを表示の設定に従って出す」と 2 つを並べているのに、受ける要求が担当ラベルにしか無かった。** 表 T-038 の `OC-2` は 2 つを占有幅に算入しており、`S-61` は既定値を持つ。**出す場所も値の形も決まっていなかった。**
...
#### 画面に見えているものを、そのまま書き出す

**Type**: FUNC_REQ
**UID**: FR-080

**STATEMENT**: 画像として書き出したとき、システムは、**画面に見えているものと同じ絵**を出すこと。表示の切り替え・ズームの段階・LOD による増減の結果を、書き出しでも同じにすること。

**ORIGIN**: 最優先事項 5（WYSIWYG） ／ GL-004 ・ UC-011

**RATIONALE**: **見えているものが成果物になることが、この道具の前提である。** 書き出しで別の絵が出るなら、画面で整える作業そのものに意味が無くなる。

**達成は表 T-041 で判定すること（MUST）**（利用者判断、2026-08-13）—— どちらも機械で確かめられる。

**表 T-041 — WYSIWYG の達成判定**

| 行 ID | 判定 | 判定に含めないもの |
| --- | --- | --- |
| WY-1 | **編集 → JSON へ書き出し → 初期化（`FR-095`）→ その JSON を読み込み** を行ったとき、**日程の描画領域が書き出す前と同じになること。** 倍率と表示位置を含む | **読む人の環境に属する表示状態**（全数は表 T-206）。本日線は表 T-029 の `CU-1` により起動のたびに表示から始まるので、消した状態は再現しない |
...
**STATEMENT**: 作成者が画像を書き出したとき、システムは、表 T-024 の `IO-3`・`IO-4`・`IO-6` に従って出力し、出力サイズと文字サイズを文書に保存すること。収まらない場合は縮小して収め、**文字が下限を割るときは警告すること（MUST）。**
...
**STATEMENT**: 利用者が文書を開いたとき、システムは、**表 T-024a の規則**でそのファイルを読み、現在の文書をどうするかを決めること。

**ORIGIN**: UC-011 手順 1（表 T-036 の SK-10 に割当があるのに、開く操作を定める要求が無かった） / UC-010 手順 1

**RATIONALE**: **`Ctrl+O` は表 T-036 の `SK-10` に載っているのに、「開く」を定めた要求が 1 件も無かった。** 表 T-034 は**起動時**の順序だけを扱い、合流の規則（`FR-022` / `FR-056`）は**合流を選んだ後**を扱う。**編集途中の文書を開いたまま別のファイルを開いたとき、未保存の編集が黙って捨てられるのかどうかが決まらなかった。** ⚠️ **本要求が合流への入口も兼ねる**（`OP-2` / `OP-3`）ので、`UC-010` 手順 1「作成者が MSPDI ファイルを開き…」を受けるのも本要求である。
...
| 行 ID | 事項 | 規則 |
| --- | --- | --- |
| OP-1 | 受け付ける形式 | JSON と MSPDI XML（表 T-024 の IO-1 / IO-2） |
| OP-2 | 経路 | ファイル選択、およびドラッグ＆ドロップ（表 T-008 の R-1）。**ドロップは日程の描画領域の判定順序（表 T-023a）を適用しない面として扱う。** ⚠️ **入口は本要求の「開く」1 つとし、取込（合流）に別の入口を設けてはならない（MUST NOT）** —— **1 つ目のファイルも 2 つ目以降も同じ経路で入る**（利用者判断、2026-08-13）。アイコンの正（`FR-029`）が持つ「開く」も 1 つである |
| OP-3 | 現在の文書 | **読んだ内容をどう扱うかを人に選ばせること（MUST）** —— **置き換える**（現在の文書を捨てて新しい日程を出す）か、**合流させる**（現在の文書へ足す）かの 2 つとする。**どちらになるかをシステムが勝手に決めてはならない（MUST NOT）。** 合流を選んだ後の規則は `FR-022` と `FR-056` が持つ |
...
| OP-6 | **置き換えを選んで** JSON を読んだとき | 文書のデータと `documentSettings` を復元すること。**欠けている設定値は既定値で補い、知らないキーは捨てずに保つ**（往復で失わないため）。⚠️ **合流を選んだときは復元しない** —— 現在の文書へ足す動作と両立しないので、表 T-032 の `MG-12` に従う |
| OP-7 | 版の判別 | 形式の版は `FR-073` に従って判別する |
| OP-9 | **重ねる用途で開いたとき** | **現在の文書を置き換えも合流もしないこと（MUST NOT）** —— `OP-3` の 2 択のどちらでもない。以後の扱いは `FR-015` が持つ |
| OP-8 | 取込の最中 | **取込または別の開く操作が進行中のあいだは受け付けないこと（MUST NOT）** —— 「取込前の状態」が決まらなくなる（表 T-032 の MG-6） |

⚠️ **`FR-024`（JSON の書き出し）の対になる要求である。** 書き出しだけがあって読み込みが無い状態だった。
...
**STATEMENT**: システムは、**操作の切れ目に、溜まった編集をまとめて 1 回**自動保存し、異常終了した次の起動で復旧を申し出ること。**編集のたびに書いてはならない（MUST NOT）。** **「操作の切れ目」は、編集が 1 つ以上溜まった状態で無操作が続いた時間で定めること（MUST）。** その時間は `_assets/tbl-settings.md` の表 T-211 が持つ。**保存された内容が壊れているとき、黙って破棄してはならない（MUST NOT）** —— 復旧できないことを人に知らせること。
...
**STATEMENT**: 作成者が開いたファイルを保存するとき、システムは、**同じファイルへ上書きできるようにすること。**
...
**STATEMENT**: システムは、自動保存の成否を画面上で示すこと。**同じ機で別の文書や複製を開いても、自動保存が互いを取り違えてはならない（MUST NOT）。**

**ORIGIN**: UC-011 手順 5

**RATIONALE**: `FR-026` は保存と復旧だけを扱い、**状態が画面から読めることを定めていなかった。** 保存されているかどうかが分からない編集は、利用者に保存操作を繰り返させる。取り違えについては、**単一ファイルをローカルで開く形では、同じ機の全ローカルページが同じ保管庫を共有する**という制約が効く（制限事項 LM-6）。
...
**STATEMENT**: 起動したとき、システムは、**表 T-034 の順**で最初に開く文書を決めること。**負けた自動保存を黙って捨ててはならない（MUST NOT）。**

**ORIGIN**: UC-011 拡張 1a・5a

**RATIONALE**: `FR-027`（テンプレート）と `FR-026`（復旧）が別々に「最初に出るもの」を定めており、**どちらが勝つかが決まっていなかった。** 順序が決まっていないと、復旧できたはずの作業がテンプレートに上書きされる。
...
**STATEMENT**: 文書を読み込んだとき、システムは、**その文書の形式の版**を判別し、読めない版であればその旨を知らせること。
...
**STATEMENT**: 作成者が求めたとき、システムは、文書の基本情報（表題・作成者・会社・改訂番号・全体の開始日など）を画面で確認でき、編集できるようにすること。

**ORIGIN**: UC-011 / UC-010

**RATIONALE**: 表 T-032 の MG-4 は「プロジェクトの基本情報が衝突したら選ばせる」と前提にしているのに、**その基本情報を人が見る・直す要求が無かった。** 透かしと版の管理にも使う値である。**どの項目を出すかは Chapter 5 で絞る。**
...
**STATEMENT**: 作成者がコメントボックスの本文を入力または変更したとき、システムは、それを保持してコメントボックスの中に描くこと。**入口は表 T-023 の `MK-13` に従うこと（MUST）。**

**ORIGIN**: UC-008 拡張 1a

**RATIONALE**: **コメントを置く・動かす・隠す・消す要求はあったが、文字を入れる要求が 1 つも無かった。** 本文の無いコメントでは `UC-008`（注記で伝える）を達成できない。入口を `MK-13` 1 つに寄せるのは `FR-029`（同じ機能の入口を 2 か所に置かない）に従うためである。
...
**STATEMENT**: 作成者が書き出しを選んだとき、システムは、**表 T-024 の形式**から選ばせ、選ばれた形式の規約に従って書き出すこと。**入口を 1 つとすること（MUST）。形式ごとに別の入口を設けてはならない（MUST NOT）。**

**ORIGIN**: UC-010 手順 5 ／ UC-011 手順 1

**RATIONALE**: **開く側は `FR-087` が入口を 1 つと定めたのに、出す側に対になる要求が無かった。** `FR-024`（JSON）も `FR-057`（MSPDI）も「書き出したとき」から始まる規約であり、**どちらが起きるかを決める要求がどこにも無い。** そのため表 T-036 の `SK-11`（保存する）が、MSPDI から開いた文書に対して何をするのかが決まらなかった。上書き保存との関係は `FR-060` が持つ。
...
**STATEMENT**: 作成者が新しく始めることを選んだとき、システムは、**開いている文書を捨てて表 T-034 の `BT-4` と同じ状態に戻すこと**。捨てる前に、未保存の編集について表 T-024a の `OP-4` と同じ確認を行うこと（MUST）。

**ORIGIN**: UC-011 拡張 1b

**RATIONALE**: **表 T-041 の `WY-1` が判定の手順に「初期化」を置いているのに、それを行う要求が無かった。** 判定条件が指す操作が存在しないと、`GL-004`（WYSIWYG）の達成を確かめられない。出すものは `FR-027` のテンプレートであり、本要求は別のテンプレートを持たない。
...
**STATEMENT**: システムは、`Agent API` を**表 T-035 の規約**を満たす形で提供すること。

**ORIGIN**: UC-012 手順 2 〜 4、拡張 3a

**RATIONALE**: `FR-028` は「値で返す」ことしか定めていなかった。細目は表に置く（1.9）。 前プロジェクトの実測で、基準版の食い違いは 1 セッションに 6 回起きている。
...
| AG-2 | 書き込む側が「どの版を読んで書いているか」を申告でき、**食い違えば書き込みを拒否して現在の文書を返すこと（MUST）** |
...
| AG-6 | 監視は「**自分が最後に読んだ版より後で、かつ自分以外の書き手が確定した変更だけ**」を通知すること。対話に書いただけでは起きない。**自分の書き込みで自分が起きてはならない（MUST NOT）** |
| AG-7 | 書き出しは、ダウンロードのダイアログを出さずに値で返せること |
| AG-8 | 画像化に失敗したときも、呼び出した側が**失敗を値で受け取れること** |
| AG-9a | **拒否の値には、拒否された対象・理由の区分・現在の版数を含めること（MUST）。** 人向けには「どの項目が、なぜ誤りか」を示すと定めている（`FR-076` の NT-1）のに、**同格を謳う機械向けにだけ対応物が無かった。** そのまま再試行に使える形にする（`UC-012` 拡張 3a） |
...
**STATEMENT**: システムは、人が `Agent API` を画面上で有効にできるようにし、**有効であるあいだ、そのことを画面上に示すこと（MUST）。**
...
**STATEMENT**: `Agent API` が有効であるあいだ、システムは、画面上で AI と言葉をやり取りする欄を出すこと。**会話そのものを文書に保存してはならない（MUST NOT）。** 文書に残すのは変更の理由だけとし、**その理由を、適用された版数（`FR-063`）と対にして持つこと（MUST）。書き出す JSON にも載せること（MUST）。**

**ORIGIN**: UC-013 手順 2・4、拡張 4a

**RATIONALE**: 会話を保存すると、文書が対話の履歴で膨らみ、往復無損失の対象にも入ってしまう。**残す価値があるのは「なぜその版でその変更をしたか」だけである。** ⚠️ **本要求の実装は後の節目へ回す**（確定 2026-08-12）。仕様と設計を先に置くのは、`Agent API` の形が対話を後から載せられるものであることを保証するためである。
...
**STATEMENT**: システムは、本体と文書を合わせた 1 つの `.html` を作れるようにすること。
...
**STATEMENT**: 作成者が求めたとき、システムは、AI へ渡す文書を画面上で確認できるようにし、それをクリップボードへ複写できるようにすること。
...
**STATEMENT**: システムは、**本体のライセンス全文・著作権表示・第三者ライブラリの帰属表示**を成果物の `.html` の中に持ち、ヘルプから読めるようにすること。
...
**STATEMENT**: システムは、**表 T-040 の手立て**をすべて備えること。

**ORIGIN**: `GL-006`（達成を判定する手段が無かった）

**RATIONALE**: **`GL-002` は「表 T-025 の `MC-7` が定める目標規模で収まること」という判定条件を持つのに、`GL-006` には対応物が無かった。** 「マニュアルを読まずに主要な操作へ到達できる」は、そのままでは合否を決められない。**手立てを全数で挙げ、1 つずつ在ることを確かめる形にする**（利用者判断、2026-08-13）。
...
**STATEMENT**: システムは、各種メニューの用途を言葉ではなくアイコンで伝え、**掴めない端点を薄く描いて理由をツールチップで示すこと（MUST）。** **アイコンの図形と画面上の配置は `previous-project-result/03-ui-naming/appheader-palette-icons.html` を正とすること（MUST）。**
...
**アイコンを本仕様書に写さないのは、絵だからである。** 22 個の図形と 5 群の並び順を文字で書き取ると、原本と写しのどちらが正か分からなくなる。**外部の文書を正とするのは本仕様書で唯一ここだけであり、対象はアイコンの図形と配置に限る**（確定 2026-08-12）。振る舞いの要求は本書が持つ。
...
**RATIONALE**: 本仕様書には「知らせる」と書いた要求が 10 件以上あるが、**知らせ方の規則がどこにも無かった。** `NFR-008` が適合の対象に「エラー表示」を挙げているのに、それを受ける要求も無い。**知らせ方を各所で個別に決めると、同じ製品の中で伝え方が食い違う。**
...
**STATEMENT**: 利用者がヘルプを開いたとき、システムは、キーボードショートカットとポインタ操作の一覧を画面上で示すこと。

**ORIGIN**: `GL-006`（横断。特定のユースケースから来ていない）

**RATIONALE**: `GL-006` を受ける経路がアイコンとツールチップしか無く、**操作の全体像に到達する道が 1 本も無かった。** 一覧は表 T-023a（判定順序）と表 T-023b（構え）と表 T-023 と表 T-036 と同じ内容を人が読む形で出すものであり、別の規則を作らない。**最も誤解を招くのは、何にも当たらない場所のドラッグが範囲選択になることと、当たったものが常に勝つことである。**
...
**STATEMENT**: 利用者がスクロールバーへポインタを乗せたとき、システムは、同じことをより速く行える操作割当をその場に示し、ポインタが離れたら消すこと。**常時表示してはならない（MUST NOT）。**
...
**STATEMENT**: 利用者が表示言語を選んだとき、システムは、メニューとパネルの文字をその言語で示すこと。対象は `ja` と `en` の 2 言語とする。**タスク名と行名は翻訳の対象ではない。**
...
**STATEMENT**: 読む人が文字サイズまたは明暗テーマを変えたとき、システムは、その指定で描くこと。**文字サイズの 3 段階が何 px かは `_assets/tbl-settings.md` の表 T-215 が持つ。****文書に保存された値が読む人の指定を強制してはならない（MUST NOT）。**
...
⚠️ **保存値は初期値であり、`fontScale` を変えたときに追随して書き換わることが「文書の編集」になる** —— 既定値の欄にだけ書くと、一度書き出した文書は実値を持つので追随せず、上の MUST NOT を破る。**目盛の文字と帯の高さは、それぞれ独立したキーとして保つこと（MUST）** —— UI を試したうえで固定の定数へ戻す道を残すためである（利用者判断、2026-08-13）。
...
**STATEMENT**: システムは、ポインタとキーボードの入力を**表 T-028 の作法**で扱うこと。
...
| IN-5a | **単文字キーと `Delete` / `Backspace` は、文字入力を確定していない間は効かないこと（MUST NOT）** —— 名称・担当者名・行名・文書名はその場で入力するので、`SK-3`（選択の削除）と 1 文字消す操作が同じキーに重なる。状態の呼び名は表 T-035 の `AG-9` と同じである |
...
**ORIGIN**: GL-003 / 表 T-007 の D-1（基準機）（基準機）
...
**STATEMENT**: 表 T-025 の条件で**ズーム・パン・ドラッグ・範囲選択・ポインタの移動**を行っている間、システムは、平均フレームレートを毎秒 60 フレーム以上に保つこと。

**ORIGIN**: GL-003（測定条件は表 T-025）

**RATIONALE**: **操作が引っかかった時点で日程表ツールとして失格である。** これが最優先事項である。**ポインタの移動を対象に含めるのは、掴めるものを探す当たり判定が毎フレーム走りうるためで、前プロジェクトが「測る」と決めて測らずに終わった項目である。**
...
**STATEMENT**: 表 T-025 の条件で**ズーム・パン・ドラッグ・範囲選択・ポインタの移動**を行っている間、システムは、フレーム時間の 95 パーセンタイルを 16.7 ミリ秒以内に保つこと。
...
**STATEMENT**: 利用者が操作していない間、システムは、画面を描き直さないこと。
...
**STATEMENT**: 起動して最初に表示するとき、システムは、**空白のまま残る画面も、内容が欠けたまま出る画面も出さないこと（MUST NOT）。**
...
**STATEMENT**: システムは、**表 T-039 に挙げる将来の拡張**を、既存の構造を作り直さずに足せる形であること。
...
**RATIONALE**: 表 T-003 の CN-1・CN-6 を要求として置いたものである。ネットワークを遮断した状態で全機能が動くことをもって満たしたと判定する。**判定は、ファイルを直接開いた状態でも行うこと（MUST）** —— 内容セキュリティ方針と単一ファイルの組み合わせには、その開き方でだけ落ちる形がある。
...
**STATEMENT**: システムは、キーボードでフォーカスを移せる要素について、**フォーカスの位置が視覚的に判別できること**（WCAG 2.1 の 2.4.7）。
...
⚠️ **2.1.1（キーボード操作可能）を適合範囲から外した経緯と、代わりに残る手段は `LM-2b` が持つ。** 本要求はフォーカスの判別（2.4.7）だけを扱う。
...
| RC-4 | ~~`fontScale` の S / M / L が何 px か未定義で、表 T-205 の時間軸のしきい値がフォント 12px の解であること~~ | **決着（2026-08-13）。** px は表 T-215 が持ち、しきい値は固定のまま**判定を実効フォントサイズで割った比で行う**と定めた（表 T-205 の注） | — | **解決済み** |
...
| RC-8 | **行 ID の接頭辞が文書をまたいで衝突している** —— `S-` が範囲の表（表 T-002）と設定値の表（表 T-201 ほか）の両方に、`X-` が範囲外の表と用語の由来の表の両方にある | 「表の第 1 列は行 ID であり、テストが落ちたとき行 ID が仕様の 1 行を指す」という方針が、この 2 系統では成り立たない | テストが `S-1` と書いても、どちらの表の 1 行かが決まらない | **決着（2026-08-13）。範囲の表を `SO-`、範囲外の表を `XO-` へ改名した**（設定値の `S-` は 123 行あり参照も多いので残した）。**行 ID の重複を機械検査に加えた** —— この誤りは執筆中に 2 度踏んだ（`GL-` と `GOAL`、`SC-` とスクロール表） |
| RC-9 | ~~タイムルーラーの上でのポインタ操作に受け皿が無い~~ | **決着（2026-08-13）。ポインタ操作を持たないと明記した**（表 T-023a の適用外の面） | — | **解決済み** |
...
| RC-6 | 依存線の**自動配線**が使うアンカーを 2 種に絞ったこと（要望 47 は 9 点を求めている） | **性能を理由に狭めた。** 9 点から候補を探索する方式は横切り 0 / 交差 9 で最良だったが、**再レイアウトが 13.3ms** かかり、毎秒 60 フレームの予算 16.7ms に余裕が残らなかった。2 種なら表引き 1 回と分岐 2 つで直接計算でき、探索が要らない | 交差の本数が増える。ただし**どちらが手前かが読めれば実用上は成立する**（`FR-009`） | **2 種のまま進める。** ⚠️ **`Task` が 9 点のアンカーを表現できること自体は残す** —— 表現力を捨てると、後から探索方式へ戻す道が閉じる |
