...
⚠️ **「入口が 2 か所」に当たらないものを先に区別する。** **同じ値を、画面上で直接掴む面と、値として編集する面（プロパティパネル（表 T-016）・`Agent API`（`FR-028`））の両方から編集できることは、本禁止の対象外とする（MUST NOT 適用しない）。** 本書はこれを各所で明示的に許している（`FR-011` / `FR-006` / `FR-049`）—— `shapeKind` は `PR-11` とパレット（`FR-083`）、`start` / `finish` は `PR-3` と掴み領域（表 T-023d）、`nameAnchor` / `nameAlign` は `PR-13` と `FR-028` から編集できる。⚠️ **`GR-10` を本注記の例から外した**（利用者の裁定 2026-08-27）—— **掴む面でなくなったので、値として編集する面しか残っていない。**⭐ **例が減っても禁止そのものは動かない。****禁じているのは、同じ面の同じ操作が 2 か所にあることである。** 対象外をここに書かないと、**意図した二面編集と本当の違反を読者が区別できない。**
