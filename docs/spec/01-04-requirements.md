...
**1 回のドラッグ操作を 1 段にまとめること（MUST）。** 押してから離すまでの途中経過を段に刻むと、1 本のバーを動かしただけで履歴が数十段埋まる。
...
| MK-9a | 掴む対象が重なった | **優先順位を定めること（MUST）** —— 端点 ＞ 名称ラベル ＞ バー本体。定めないと、同じ場所を押すたびに違うものを掴む |
...
**いま構えている図形が画面上で読めること（MUST）。** 構えたまま置く操作（表 T-036 の SK-1）があるので、何を構えているかが読めないと置くまで分からない。
...
**RATIONALE**: 狭い画面に最大の情報量を出すという方針（`GL-002`）の最も直接的な手段である。**入って戻れないと道具として使えない**ので、同じ入口で解除できることを要求に含める。`Esc` が全画面から戻る鍵として効くかどうかはブラウザ側の挙動であり、**アプリが `Esc` を使わないなら握らない**（表 T-028 の IN-4）。
...
**RATIONALE**: **囲み枠の線色を指定でき、指定が無ければ注記用の固定色で描くこと（MUST）。** その固定色は、テーマの色相・依存線・イナズマ線のいずれからも離した色とする —— 注記が日程の要素と見分けられなくなる。**囲み枠に透明を選ばせてはならない（MUST NOT）** —— 枠線が唯一の表現なので、透明にすると何も残らない。**コメントは引出し四角と折れ線の 2 種から選べること（MUST）** —— 指す先が近いときと遠いときで、読みやすい形が違う。順番で持つと並べ替えで別の行を指す。**コメントは点を指し、囲み枠は範囲を囲む。必要な情報が違うので別のものとして持つ。** 吹き出しのずれだけは画面上の距離で持ち、ズームしても見た目の距離が変わらないようにする。角丸の半径もズームによらず一定に描く。指している行が畳まれたらコメントも隠し、囲んだ範囲の一部が非表示になったら見えている行だけを囲う。
...
**ガイドカーソルが無効なとき、ポインタが動いただけで再描画してはならない（MUST NOT）。** 追従する線が 1 本も無いのに毎フレーム描き直すことになり、`GL-003` を崩す。
...
| AG-5 | **UI と同じ検証・同じ制限を通ること（MUST）。** 透かしの合言葉の照合を迂回できてはならない |
...
| AG-9 | **人が画面でドラッグしている間は、書き込みを拒否すること（MUST）。** 途中の状態へ書き込むと、離した瞬間に人の操作が上書きする |
...
**RATIONALE**: `GL-006` を受ける経路がアイコンとツールチップしか無く、**操作の全体像に到達する道が 1 本も無かった。** 一覧は表 T-023 と表 T-036 と同じ内容を人が読む形で出すものであり、別の規則を作らない。
...
| 行 ID | 作法 |
| --- | --- |
| IN-1 | ポインタ操作は押した時点で実行せず、**離した時点で確定すること。** 途中で領域の外へ出せば中断できること |
| IN-2 | ポインタの形が、その場所で掴めるか・いま掴んでいるかを示すこと（背景の上は開いた手、パン中は握った手） |
| IN-3 | ツールチップは、消せること・ポインタを乗せられること・**勝手に消えないこと** |
| IN-4 | `Esc` は閉じる対象または取り消す対象があるときだけ 1 階層ぶん消費し、無ければブラウザへ渡すこと |
...
**STATEMENT**: 表 T-025 の条件で**ズーム・パン・ドラッグ・ポインタの移動**を行っている間、システムは、平均フレームレートを毎秒 60 フレーム以上に保つこと。
...
**STATEMENT**: 表 T-025 の条件で**ズーム・パン・ドラッグ・ポインタの移動**を行っている間、システムは、フレーム時間の 95 パーセンタイルを 16.7 ミリ秒以内に保つこと。
...
**RATIONALE**: テーマの色相を変えても満たすこと。**重ね描きの文字（透かし・コメント）も対象に含める（MUST）** —— 透かしは薄く重ねると定めている（`FR-020`）ので、放置すると本要求と衝突したまま残る。**モノクロ表示時のコントラストは未検証であり、本プロジェクトで測る**（彩度を落とすと色相差が明度差に化けないため、人の指定色と地の明度が近いと 3 : 1 を割りうる）。
