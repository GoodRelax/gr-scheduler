...
**STATEMENT**: `Task` にフェードイン日数またはフェードアウト日数が入っているとき、システムは、**バーの端をその日数ぶんぼかして描くこと。** 作成者がその日数を画面上で編集できるようにすること。

**ORIGIN**: UC-001 / UC-003

**RATIONALE**: 「この辺りから始まる」「この辺りで終わる」という**確定していない日付**を表す唯一の手段である。表 T-016 の PR-14 にデータ項目としては載っており、**拡張領域を使う 2 つはこれだけ**なのに、描く要求も編集する要求も無かった。
...
**担当者の名前を変えたときは、その担当者が付いている全タスクの表示が変わることを、対象の件数とともに知らせること（MUST）** —— 1 か所を直したつもりで日程表の各所が変わるのは、知らせなければ事故である。**割当を解除しても担当者そのものは残すこと（MUST）。参照されなくなった担当者を自動で消してはならない（MUST NOT）** —— 消すと、割当をやり直すたびに名前を打ち直すことになる。
...
| MK-9a | 掴む対象が重なった | **優先順位を定めること（MUST）** —— フェード掴み点 ＞ 端点 ＞ 進捗マーカー ＞ 再開アイコン ＞ 実績入力の入口 ＞ 名称ラベル ＞ 担当ラベル ＞ 実績バー ＞ 予定バー本体 ＞ 依存線 ＞ コメントボックス ＞ ハイライトボックス。定めないと、同じ場所を押すたびに違うものを掴む |
...
**RATIONALE**: **`FR-011` と `FR-012` はどちらも「稼働日」で計算すると述べているのに、稼働日が何かを定める要求が無かった。** 暦は取り込んだ MSPDI が持っており、曜日ごとの稼働可否と例外日からなる。**繰り返し指定の例外日（毎年の元日など）は実日付へ展開しないので、未対応であることを利用者に知らせること。** **週の目盛は週の始まりの日付を書き、週の始まりも文書の暦に従うこと（MUST）** —— 曜日を決め打ちしない。
...
**資源の上限（ファイルサイズ・件数・ネストの深さ）を持ち、超えた入力は取り込まずに知らせること（MUST）。部分的に適用してはならない（MUST NOT）** —— 途中まで取り込んだ状態は、利用者から見て「壊れた文書」と区別がつかない。
