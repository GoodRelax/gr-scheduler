...
| HF-11 | **配下をすべて閉じる操作子は、その行の配下をすべて畳むこと（MUST）** —— 表 T-015 の `HR-4` である。⭐ **`HF-2` と対になる**（あちらは配下をすべて開く）。⛔ **その行自身を畳んではならない（MUST NOT）** —— 行自身を畳むのは `HF-3` であり、両方を書くと同じ操作の入口が 2 つになる（`FR-029`）。⚠️ **`HR-1a` は畳んだ行の配下を描かないので、畳んだ行の下でさらに畳んでも絵は変わらない** —— それでも書くのは、開いたときに畳んだ状態が残っているためである |
