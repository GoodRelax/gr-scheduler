...
**刻印を順序として読んではならない（MUST NOT）。どの判定も等値で行うこと（MUST）** —— **刻印が答えるのは「同じ文書か」だけであり、「どちらが新しいか」ではない。** ⚠️ **取り消しは以前の文書を刻印ごと復元する**（`FR-031`）ので、**順序で読むと「戻った文書」を「新しくない」と読み、`AG-6` が通知を落とす。** ⛔ **壁時計は `NTP` 補正や夏時間で逆行しうるため、順序の根拠にできない。**
...
| AG-6 | 監視は「**自分がまだ受け取っていない、自分以外の書き手が確定した変更と発話だけ**」を通知すること。**日程データの変更は、その書き込みが日程データの群を変えたかどうかで選ぶこと（MUST。判定は 表 T-067 の `WS-5` が既に下している）。購読し直したときは、最後に手渡した日程データの群の刻との等値で選ぶこと（MUST）。確定した発話（`AG-11`）は刻に依らず選ぶ（MUST）** —— 人は「待った」「なぜそうしたか」とだけ言うこともあり、そのときも起きなければ `UC-013` の手順 2 から 3 へ進まない。⛔ **見せ方の群だけが動いた書き込みで起きてはならない（MUST NOT）** —— 見せ方の群は 1 秒に何十回も動くので、起きると「だけ」が意味を失う。**自分の書き込みで自分が起きてはならない（MUST NOT）。確定していない下書きで起きてはならない（MUST NOT）** |
