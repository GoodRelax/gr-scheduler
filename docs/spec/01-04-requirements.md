...
**STATEMENT**: `GRS` は、**いま開いているファイルの名前**と、**そのファイルへ最後に書いた時刻**を画面上に示すこと（MUST）。**名前を時刻の上に置くこと（MUST）**（利用者の裁定 2026-08-29）。**まだ 1 度もファイルへ書いていないときは、時刻の代わりにその旨を示すこと（MUST）** —— 空欄では「書けたのに読めない」と区別がつかない。
