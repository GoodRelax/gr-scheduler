...
⛔ **パネルの文字を宿主の既定の大きさで描いてはならない（MUST NOT）**（利用者の裁定 2026-08-27）—— **画面のほかのどこよりも大きい字でパネルだけが描かれ、同じ幅に入る項目が減る。** 大きさは `S-197` を 表 T-215 の `fontScaleSizes[fontScale]` に掛けて求め、項目名にはさらに `S-198` を掛けること（MUST）。⚠️ **読む人の文字サイズ設定（`FR-039`）に追随する形で持つのは、`NFR-007` が求める拡大に耐えるためである** —— px で固定すると、文字を大きくした人の画面でパネルだけが取り残される。
...
**`GR-3` / `GR-4` / `GR-12` / `GR-15` を掴んでいるあいだ、置くことになる予定バーまたはマイルストーンを、ポインタに追従させて描いて示すこと（MUST）**（利用者の裁定 2026-08-27）—— **離すまで元の位置に描かれたままだと、どこへ置くことになるのかを見ないまま手を離すことになる。** ⚠️ **`GR-12` は縦にも追従すること（MUST）** —— 行の載せ替え（表 T-015a の `HM-3`）は縦の移動そのものなので、縦を止めるとどの行へ載るのかが見えない。⚠️ **確定は 表 T-028 の `IN-1` に従う**（離した時点）。⛔ **掴んでいるあいだ日付を文書へ書いてはならない（MUST NOT）**（`FR-031`）—— **追従は絵であって編集ではない。** ⭐ **`GR-1` / `GR-2` と `GR-9` / `GR-17` / `GR-18` は、上の 2 つの規則が同じことを既に求めている。** ⛔ **これで表 T-023d の掴み領域のうち、掴んで動かすものはすべて追従する** —— 追従するものとしないものが混ざっていると、動かないほうが壊れて見える。
