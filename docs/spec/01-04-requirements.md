...
| SL-8 | 見せ方 | **選択されていることを、色以外の手掛かりでも示すこと（MUST）**（`FR-030`）。**その手掛かりは、対象の外接矩形に沿った破線の枠とすること（MUST）。`SL-1` が挙げる対象のすべてに同じ規則を当てること（MUST）** —— 形状ごとに描き方を分けると、選べる 5 種に対して規則が 5 つになる。**対象自身の輪郭をなぞってはならない（MUST NOT）** —— 面を持たない対象ではなぞる輪郭が無い。**倍率に追随させてはならない（MUST NOT）**。太さと破線の刻みは `_assets/tbl-settings.md` の 表 T-206 の `S-174` ／ `S-175`、色は同書の 表 T-236 の `S-151` が持つ |
