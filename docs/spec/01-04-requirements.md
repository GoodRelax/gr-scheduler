...
| HF-8 | **人が全体表示（`FR-055`）を求めたとき、人が畳んだ状態をすべて捨てること（MUST）。起動のときは働かせてはならない（MUST NOT。表 T-024a の `OP-10`）** —— これが自動の表示量へ戻す唯一の操作である。**捨てるのは畳みだけであり、隠した状態は残す** |
...
⚠️ **取り違えないための鍵に、表 T-227 の `DI-1` を用いてはならない（MUST NOT）** —— **`DI-1` はファイル名を含むが、1 度もファイルへ保存していない文書にも自動保存は掛かる。** ⛔ **本要求が使う鍵はまだ決まっていない。**
