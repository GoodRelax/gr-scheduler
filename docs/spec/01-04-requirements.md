...
**STATEMENT**: 作成者がタスク形状を構えた状態で、どのアイテムにも当たらない場所をドラッグしたとき、システムは、構えている形状でその期間のタスクを作ること（表 T-023a の PD-4）。**タスク形状の全数は表 T-012 に従うこと。** ドラッグせずにクリックしたときは、**開始日と終了日が同じタスクを作ること（MUST）。**
...
⚠️ **描いた直後のタスクは選択されたまま残る。** したがって続けて別の形を描きたいときは、**いったん何にも当たらない場所をクリックして選択を外してから**（表 T-023 の `MK-11`）パレットを押す。**描いた直後に別の形を押す操作は「形を間違えたので変える」を意味する**（利用者判断、2026-08-13）。**選択されていることは色以外の手掛かりでも見えている**（`FR-081` の `SL-8`）ので、押す前にどちらが起きるかを読める。
...
**STATEMENT**: 作成者が行（`TaskGroup`）に色または高さを指定したとき、システムは、その指定で帯を描くこと。**色**の指定が無い行は、**深さと行の位置**からテーマ色に基づいて解いた帯色で描くこと。**高さ**の指定が無い行は、段数から自動で決めること（`FR-003` の ST-9）。
...
**依存の種別ごとにラグを保持し、作成者が編集できること（MUST）。** ラグは日付に効くので経路の規則を増やさない。
...
**タスクの本体をドラッグしたとき、予定の日付だけをずらし、実績の日付を変えてはならない（MUST NOT）。** 行をまたぐ移動では予定も実績も新しい行へ移るが、**どちらの日付も変わらない（MUST NOT）。** —— **一度入力された実績は、担当者が置いた事実である。** 予定を動かす操作で書き換わると、それを置いた者以外が気づかないうちに履歴が変わる。**実績の日付を書き込む経路の全数は表 T-023d が持つ**（端点 `GR-5` / `GR-6`、実績入力の入口 `GR-9`、進捗マーカー `GR-7`）。**それ以外の操作で実績の日付を変えてはならない（MUST NOT）。**
...
| 行 ID | 事項 | 規則 |
| --- | --- | --- |
| GD-1 | 引く条件 | 予定と実績が**時間軸上で重ならない**とき。重なっているあいだは引かない |
...
**進捗マーカーの大きさは実績バーに追随させ、実績バーより大きく描いてはならない（MUST NOT）。** 低いズームでは実績バーが数 px まで縮むので、**追随させないと注釈が本体より大きくなり、何の印なのか読めなくなる。**
...
**ORIGIN**: UC-003 / UC-006

**RATIONALE**: 期限は「この日までに終える」という独立した約束であり、予定の終了日とは別物である。表 T-016 の PR-10 にデータ項目としては載っていたが、**描く要求が無かった。**
...
| PD-3 | **何かに当たった**（判定の順と優先は MK-9a） | **そのものへの操作。** 構えが依存線のときに限り、当たった対象は依存の端点になる |
| PD-4 | 何にも当たらない かつ **図形を構えている**（表 T-023b の AR-2 / AR-3） | 構えている形状で**作る** |
...
| タイムルーラー | **受け皿が無い。** 目盛の粒度は `FR-017` が定めるが、その上でのポインタ操作は未定義である |
...
**構えは持続すること（MUST）。** 続けて何本も引けなければ道具として重い。**解除は `Esc` かパレットの同じ入口の再押下とすること（MUST）。** 構えの 4 値は排他であり、依存線を構えれば図形の構えは外れる。**`Dual Cursor` モード（表 T-029a）に入るときも構えを外すこと（MUST）** —— 同モードは作成・移動・編集を受け付けないので、構えたまま入れると何も起きない構えが残る。
...
| MK-10 | 修飾キーの付いた入力 | **ブラウザの既定動作を画面全体で止めること（MUST）。** キャンバスの上だけでは足りない —— `Ctrl+S` が素通りすると、利用者の保存操作がブラウザの HTML 保存にすり替わる |
...
| GR-12 | 予定バー本体 | 端点を除いた中間 | **予定の日付だけを平行移動する。実績の日付は変えない**（`FR-011`） |
| GR-13 | 依存線 | 線の上 | 選ぶ |
...
| KB-8 | **構えを選び、解除する**（表 T-023b の 4 値） |
...
**いま構えているものが画面上で読めること（MUST）。** 構えは 4 値（表 T-023b）であり、**依存線は図形ではない。** 依存線を構えていることが読めないと、`AR-4` の「当たった対象が依存の端点になる」が予告なく起きる。構えたまま置く操作（表 T-036 の SK-1）もあるので、何を構えているかが読めないと置くまで分からない。
...
**STATEMENT**: 閲覧者が全体表示を求めたとき、システムは、1 つの操作で、縦横の倍率と表示位置を全体が収まる側へ合わせること。**ただし必ず収まることを保証しない。収まらない軸にはスクロールを残すこと。**
...
**STATEMENT**: 作成者がコメントまたはハイライトボックスを置いたとき、システムは、その位置を日付と**行の識別子**で持つこと。**行を順番で参照してはならない（MUST NOT）。**
...
| 行 ID | 名前 | 何をするもの | 備考 |
| --- | --- | --- | --- |
| CU-1 | `Today Line`（本日線） | 今日の日付に縦線を 1 本引く（固定） | **表示状態を文書に保存しない**（今日は読む人の環境である） |
...
**表 T-202 の表示可否は、当該の要素を描くすべての要求の状態条件に加わること（MUST）。ただし操作の入口を閉じてはならない（MUST NOT）** —— 隠した要素を編集する経路まで消えると、`progressMarkerVisible` を `false` にしただけで着手済みタスクの状態を変える手段が無くなる。**隠している要素の掴み領域（表 T-023d）は残すか、代わりの入口を用意すること。** 表示の切り替えは取り消しの対象外（`UN-7`）なので、失った操作を履歴から戻せない。 例えば `FR-014`（イナズマ線）は「基準日が置かれているとき」と書いているが、`progressLineVisible` が `false` のあいだは描かない。**この 1 文で一括して閉じる** —— 個別の要求文に条件を書き足すと、表 T-202 に行を足すたびに要求の追記が要る。
...
**階層の循環（あるタスクの親が、その子孫である状態）を検出して取り込まないこと（MUST NOT）** —— `FR-005` が禁じているのは作成者が画面で行う移動だけで、**外から来た循環は素通りする。循環では深さが確定しないので、ネストの深さの上限でも検出できない。** **資源の上限（ファイルサイズ・件数・ネストの深さ）を `_assets/tbl-settings.md` の表 T-211 に従って持ち、超えた入力は取り込まずに知らせること（MUST）。部分的に適用してはならない（MUST NOT）** —— 途中まで取り込んだ状態は、利用者から見て「壊れた文書」と区別がつかない。
...
**STATEMENT**: `Agent API` が有効であるあいだ、システムは、画面上で AI と言葉をやり取りする欄を出すこと。**会話そのものを文書に保存してはならない（MUST NOT）。** 文書に残すのは変更の理由だけとすること。
...
**STATEMENT**: 読む人が文字サイズまたは明暗テーマを変えたとき、システムは、その指定で描くこと。**文書に保存された値が読む人の指定を強制してはならない（MUST NOT）。**
...
**ORIGIN**: GL-004 / 表 T-002 の S-12（範囲外だが設計の余地は残す）

**RATIONALE**: いずれも範囲外だが（表 T-002 の S-12）、**設計の余地だけは残すと決めている。** `Agent API` が 1 つに揃っていれば、サーバー側はその薄い包みで済む。**逆に UI が直接ストアを書き換える形にすると、後から `Agent API` を生やすことになり、そこは取り返せない。**
...
| RC-8 | **行 ID の接頭辞が文書をまたいで衝突している** —— `S-` が範囲の表（表 T-002）と設定値の表（表 T-201 ほか）の両方に、`X-` が範囲外の表と用語の由来の表の両方にある | 「表の第 1 列は行 ID であり、テストが落ちたとき行 ID が仕様の 1 行を指す」という方針が、この 2 系統では成り立たない | テストが `S-1` と書いても、どちらの表の 1 行かが決まらない | **接頭辞を割り当て直す。** 併せて**接頭辞の衝突を機械検査に加える**（この誤りは本文書の執筆中にも 1 度踏んだ） |
| RC-9 | **タイムルーラーの上でのポインタ操作に受け皿が無い** | 表 T-023a が判定順序を適用しない面として挙げたが、**そこでの操作を定める要求が存在しない** | ルーラーを掴んだときの挙動が実装ごとに決まる | **要求を立てるか、操作を持たないと明記する** |
