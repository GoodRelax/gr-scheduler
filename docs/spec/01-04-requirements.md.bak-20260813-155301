...
**STATEMENT**: 作成者が日程表の任意の場所にコメントボックスとハイライトボックスを置き、システムがそれらを行の識別子で結び付けて描くこと。
...
**STATEMENT**: 作成者がコメントボックスまたはハイライトボックスを構えて置いたとき、システムは、それを作り、その位置を日付と**行の識別子**で持つこと。**置く手段は表 T-023b の構え（AR-5 / AR-6）とする。****行を順番で参照してはならない（MUST NOT）。**
...
**STATEMENT**: 作成者が求めたとき、システムは、文書の基本情報（作成者・会社・改訂番号・全体の開始日など）を画面で確認でき、編集できるようにすること。⚠️ **文書名は対象に含めない（MUST NOT）** —— 入口は `FR-035` 1 つとする（`FR-029`）。
...
**RATIONALE**: **コメントボックスを置く・動かす・隠す・消す要求はあったが、文字を入れる要求が 1 つも無かった。** 本文の無いコメントボックスでは `UC-008`（注記で伝える）を達成できない。入口を `MK-13` 1 つに寄せるのは `FR-029`（同じ機能の入口を 2 か所に置かない）に従うためである。
