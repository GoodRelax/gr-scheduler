...
**エンティティの全数と、層・由来・書き出しの別・責務は `fig-domain-model.md`（`DOC-FIG-DOMAIN-MODEL`）の 表 T-302 が持つ。** 図 F-003 の直下に置いてある —— **関係の図と要素の一覧は、離すと読めなくなる。**

**本書はそこから指される側であり、エンティティごとの列を第 3 節以降に持つ。**
