...
| `PD` | Pointer Down | ポインタを押したときの判定順序の段（台帳の未裁定の決めごとが同じ綴りを使っており、CR-371 が PTD へ改める） | 仕様書（⛔ 台帳にも 212 行。別物であり、2026-09-12 から抑止している） | `T-023a` | 6 |
...
| `PTD` | Pointer Down | ポインタを押したときの判定順序の段。<br>CR-371 が PD から改める綴りであり、改名の巡が来るまで行は 1 つも無い | 仕様書 | —（登録のみ） | 0 |
...
| --- | --- | --- | --- | --- | --- |
| `PD` | 仕様書 ／ 台帳 | 仕様書 6 ／ 台帳 212 | 2026-09-12 | 仕様書の `PD-1` は中ボタンドラッグによるパンであり、台帳の PD-1 は themeHue の彩度と明度である。<br>4 つの綴り（PD-1 〜 PD-4）が両方の木に立って別のものを指し、残る 208 は台帳だけのものである。<br>CR-371 が書かれた理由そのものである | CR-371 の巡 1 が仕様書側を PTD へ、巡 2 が台帳側を PND へ改める。<br>両方が済んだとき、本行も接頭辞 PD も消える |
