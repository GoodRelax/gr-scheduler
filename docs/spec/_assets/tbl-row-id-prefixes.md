...
| `AT` | Attribute | エンティティの列 1 つと、その型・交換相手の対応 | 仕様書 | `T-058` | 143 |
...
| `DFC` | Defect | 台帳に載る欠陥（⛔ `D-` は配置図の機器と紛れるので使わない） | 台帳 | `docs/development-records/defects.md` ／ `docs/development-records/evidence/measured-2026-09-06.md` ／ `docs/development-records/evidence/measured-2026-09-07.md` ／ `docs/development-records/evidence/order-2026-09-06.md` ／ `docs/development-records/fixed-defects.md` | 724 |
...
| `IC` | Icon | 画面に出すアイコン 1 つ | 仕様書（台帳にも 5 行。写しとして申告済み） | `T-109` | 96 |
| `IF` | Interface | 層をまたぐインターフェース 1 本 | 仕様書 | `T-065` | 8 |
| `IN` | Input | 入力の作法の条 | 仕様書 | `T-028` | 10 |
...
| `JDG` | Judgement | 利用者の裁定（⛔ `R-` はデータの経路と紛れるので使わない。<br>`RL` が「実体のあいだの関係」であり `RLG-` は 1 字違い、`RUL-` は「規則」と紛れるので選ばなかった） | 台帳 | `docs/development-records/handoff-state-machine.md` ／ `docs/development-records/rulings.md` | 294 |
...
| `PK` | Pointer Kind | ポインタの形 | 仕様書 | `T-269` | 7 |
...
| `S` | Setting | 設定値 1 つ。<br>一連の通し番号を主題ごとに複数の表へ割っているだけであり、同じ綴りが別のものを指しているのではない | 仕様書 | `T-201` ／ `T-202` ／ `T-203` ／ `T-216` ／ `T-204` ／ `T-205` ／ `T-206` ／ `T-207` ／ `T-236` ／ `T-212` ／ `T-211` ／ `T-213` ／ `T-214` ／ `T-215` ／ `T-210` ／ `T-209` ／ `T-208` ／ `T-217` | 299 |
...
| `UF` | — | ユニット 1 つ —— 1 ファイルと、その純粋性と持ち主 | 仕様書（台帳にも 53 行。写しとして申告済み） | `T-075` | 101 |
| `UN` | Undo | 取り消しの対象と対象外 | 仕様書 | `T-027` | 18 |
| `UT` | — | ユニットを割った理由 | 仕様書 | `T-063` | 11 |
