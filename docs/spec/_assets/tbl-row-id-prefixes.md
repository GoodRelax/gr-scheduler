...
| `D` | 仕様書 ／ 台帳 | 仕様書 5 ／ 台帳 527 | 2026-09-12 | 仕様書の `D-1` は機器（利用者の PC のブラウザ）であり、台帳の D-01 は欠陥である。<br>494 の綴りが台帳だけのもので、機器を指すものは 1 つも無い。<br>いま綴りが衝突していないのは台帳が 0 詰めで書いているからにすぎず、読み手はその差に頼れない | ⛔ 未裁定。<br>CR-371 が名指すのは PD だけであり、本件は登録簿が 2026-09-12 に見つけたものである。<br>片側を改名するか、対のままと決めるかは裁定を要する |
| `R` | 仕様書 ／ 台帳 | 仕様書 9 ／ 台帳 44 | 2026-09-12 | 仕様書の `R-1` は機器と機器のあいだの経路であり、台帳の R-01 は利用者の裁定である。<br>44 の綴りはすべて台帳だけのものである。<br>D と同じ形であり、綴りを分けているのは 0 詰めだけである | ⛔ 未裁定。<br>D と同じ |
