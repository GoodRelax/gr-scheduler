...
| IC-61 | `Command Palette` | 揃える | 依存線を構える（表 T-023b の `AR-4`）。<br>⭐ **本行から `IC-38` までが 1 つの群である** —— ⛔ **並びを決めるのは本表の `群` の欄だけであり**（同表の前文）、**行を動かすのではなく群を書き換える。<br>**⚠️ **群の語は `揃える` のままである** —— 画面には刷らない（同前文の MUST NOT）。ヘルプ（`FR-036`）も群の境目を線で示すだけである | `FR-009` | `AR-4` |
