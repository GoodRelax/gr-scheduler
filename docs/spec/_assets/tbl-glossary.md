...
⚠️ **本表は英名の欄を持たない** —— 持つと 57 個の確定名を新たに作ることになり、`milestoneGlyph` の 8 つの綴りは未決である。**表 T-012 の `SH-5` が既に書いている表記だけを使う。**
⚠️ **`図形` の欄を持たないのも同じ理由である。** 図形を持たない 4 行（`IC-58` 〜 `IC-61`）だけ、入口の欄の末尾に **未描画 🔎** と書く —— 後から選び直す義務は 表 T-026 の `RC-13` が持つ。
...
| IC-3 | `App Header` | 文書 | 画像として書き出す | `FR-025` |
...
| IC-52 | `Help Modal` / `AI Export Modal` | — | 開いている面を閉じる | 表 T-028 の `IN-4` |
| IC-53 | `Command Palette` | — | 掴んで動かせることを示す。**ボタンではない** | `FR-053` |
| IC-54 | `Command Palette` | 構え | 構えた図形を置く打鍵を示す。**ボタンではない** | 表 T-036 の `SK-19` |
...
| IC-58 | `Row Title Panel` | — | 行の配下を 1 段開く。**未描画 🔎** | 表 T-051 の `HF-2` |
| IC-59 | `Row Title Panel` | — | 行の配下をすべて閉じる。**未描画 🔎** | 表 T-051 の `HF-3` |
| IC-60 | `Row Title Panel` | — | 行をピン止めし、同じ入口で外す。**未描画 🔎** | `FR-098` |
| IC-61 | `Command Palette` | 置く | 依存線を構える（表 T-023b の `AR-4`）。**未描画 🔎** —— 構えの入口 6 つのうち、これだけ図形が無い | `FR-009` |
