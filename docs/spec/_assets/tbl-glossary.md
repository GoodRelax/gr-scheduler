...
⚠️ **本表は英名の欄を持たない** —— 持つと 72 個の確定名を新たに作ることになる。**表 T-012 の `SH-5` が既に書いている表記だけを使う。** ⭐ **`milestoneGlyph` の 8 つの綴りは CR-172（版 0.53）が決めており、`_source/erd.json` が持つ** —— 本表に写さない。
⚠️ **`図形` の欄を持たないのも同じ理由である。** ⭐ **図形を持たない行は無い** —— 全 72 行が 図 F-019 に図形を持つ。**新しく起こした図形を実物と見比べて選び直す義務は 表 T-026 の `RC-13` が持つ。**
