...
| 行 ID | 面 | 群 | 何の入口か | 正 | 構え |
| --- | --- | --- | --- | --- | --- |
| IC-7 | `App Header` | パレット | コマンドパレットを出す・しまう（`S-99e`）。⭐ **本行だけが独自の群を持つのは、`App Header` の左端に置くためである**（利用者の指示 2026-08-27「一番左に移動しろ。ファイル読み書きの左だ」）—— ⛔ **並びを決めるのは本表の `群` の欄だけであり**（同表の前文）、**既存の群に入れたまま先頭へ動かすと群がとびとびになり、その欄の役目が壊れる。**⚠️ **群の名は画面に刷らない**（同前文の MUST NOT）ので、辞書の語は増えない | `FR-053` | — |
...
| IC-79 | `Command Palette` | 表示 | 担当ラベルを出す・しまう（`S-60`）| `FR-049` | — |
| IC-80 | `Command Palette` | 表示 | 完了率ラベルを出す・しまう（`S-61`）| `FR-049`（`FR-090`）| — |
| IC-81 | `Command Palette` | 表示 | 依存線を出す・しまう（`S-62`）。⚠️ **隠しているあいだも、依存線を選ぶ・ラグを編集する・削除する経路は別の面に残る** —— 規則と理由は `FR-049` が持つ | `FR-049`（`FR-009`）| — |
