...
| U-53 | `Tooltip` | ツールチップ。何かに添えて出す説明。出す場面は `FR-092` の `EZ-2`（アイコン）と `EZ-6`（タスク）／ `FR-037` が、振る舞いは表 T-028 の `IN-3` が持つ。⚠️ **`FR-085` は場面ではない** —— 同要求は打ち切った行名の全文を出すことを禁じており、`IN-3` が「引き金ではなくなった」と述べている。⚠️ **重ねて開く面ではない**（表 T-028 の `IN-4`） |
| U-54 | `Export Chooser` | 書き出しの形式を選ぶ面。開く規則は `FR-096`、入口は表 T-109 の `IC-2`、鍵は表 T-036 の `SK-12` が持つ |
| U-55 | `Confirmation` | 続けてよいかを問う面。問い方は表 T-037 の `NT-7`、2 択の入口の語は `FR-038` の辞書の `confirmation` が持つ。⛔ **語のボタンであり、図形の入口ではない**（利用者の指示 2026-09-01）—— **本表 T-109 にも 図 F-019 にも行を持たない。**⚠️ **通知（`Notice`）ではない** —— 通知は答えを求めない |
...
| U-58 | `Opened File Name` | 開いているファイル名。⚠️ **`Document Title`（`U-27`）とは別物である** —— あちらは文書が持つ値であり、本行はファイルの名前である。規則は `FR-101` |
| U-59 | `File Saved At` | ファイル保存時刻。開いているファイルへ最後に書いた時刻。規則は `FR-101` |
| U-60 | `Watermark Unlock` | **透かし解除の面**。透かしを消す前に透かし解除パスワードを問う面。立てる規則は `FR-020`、問いの文は表 T-234 の `QN-9`、合わなかったときの理由は表 T-233 の `RS-41` が持つ。⛔ **`Confirmation`（`U-55`）ではない** —— **答えが 2 択ではなく、打ち込む文字である。**⚠️ **入口は表 T-109 の `IC-41`**。⭐ **`_assets/tbl-settings.md` の `S-99g` の「面」であり、`Esc` の第 1 階層で閉じる** |
| U-61 | `Difference Review` | 差分の確認。合流で対応するかもしれないタスクを並べ、重複しているものをどちらのファイルから取るかを選ばせる面。立てる規則は `FR-022`、合流の規則は表 T-032 が持つ。⭐ **この面は開く道（表 T-024a の `OP-3`）から立つ** —— 独立したアイコンの入口は持たない（`FR-022`）。⚠️ **面の上の入口は表 T-109 の `IC-95` 〜 `IC-97` であり、表 T-032a の `MM-1`・`MM-2`・`MM-4` に当たる** —— `MM-3`（1 件ずつ決める）は置かない（利用者の裁定 2026-09-06「複雑なことは要らない」）。⛔ **合流（`FR-022`）で立つときは `Confirmation`（`U-55`）ではない** —— 答えが 2 択ではない（上の 3 つの入口である）。⭐ **読めなかった列を問うとき（`FR-073`）は 2 択（続ける／やめる）である**（利用者の裁定 2026-09-11）—— **面と作法は差分の確認に従うが、択の数だけが用途で変わる。**⭐ **同じ面を、版が新しい文書を開くときにも使う**（`FR-073`）|
| U-62 | `Import Report` | 取り込みが落とした `Task` の名前を並べて告げる面。立てる規則は `FR-023`、運ぶ理由は表 T-233 の `RS-50`、閉じる入口の語は表 T-037 の `NT-8` が持つ。⛔ **`Confirmation`（`U-55`）ではない** —— **答えを求めない。入口は `OK` の 1 つだけである**（利用者の裁定 2026-09-06）。⛔ **`Notification Area`（`U-57`）でもない** —— **表 T-037 の `NT-9` が通知を 1 行に限っており、落とした名前の列挙が入らない。**⚠️ **名前は文書の値であるので訳さない**（`FR-023`） |
| U-57 | `Notification Area` | 通知が立つ場所。作法は表 T-037 が、運ぶ理由は表 T-233 が持つ。⚠️ **`Confirmation` ではない** —— 通知は答えを求めない。⛔ **`S-99g` が持つ「面」ではない** —— **重ねて開くものではない。**⚠️ **`Esc` では 表 T-028 の `IN-4` の第 1 階層が本行を消す**（表 T-037 の `NT-8`）—— **面より先であって、面として閉じるのではない** —— 重ねて開くものではなく、`NT-4` が起動時の用件を 1 枚に集約させるのもここである。⚠️ **値の型は `Notice` のままである** —— 本行が名づけるのは場所であって、立つものではない |
...
⛔⛔ **本表と `tbl-settings.md` の役割の境目はこうである（MUST）** —— **`tbl-settings.md` の表に在る行のうち、`documentSettings` の鍵であるものは、すべて本表に行を持つこと。**⛔ **鍵でないものを本表に載せてはならない（MUST NOT）。**⚠️⚠️ **「`src/` が英語の名前で引くもの」では広すぎる** —— **それは文書の列にも当てはまる。**⭐ **実例**: `cornerRadiusPx` は `_assets/tbl-settings.md` の 表 T-217 に既定値を持つが、`documentSettings` の鍵ではなく `HighlightBox` の列である（`AT-122`）。**2026-09-04 に本表へ入り、同日、どの面にも出ないことが実測されて退いた。**⛔ **列の名前の正は本表ではない。**⛔ **英語の名前を持たず、行 ID で引く行**（表 T-207 の `S-100` / `S-101` と、表 T-236 の色がこれである）**を本表に載せてはならない（MUST NOT）。**⭐ **値そのもの（既定値・下限・上限とその理由）を本表に書いてはならない（MUST NOT）** —— **名前の正だけが本表に在り、値の正はあちらに在る。**⚠️ **1 つの行が名前を 2 つ以上まとめて名乗ってよい**（`K-103` は 2 つ、`K-105` は 3 つ）—— **画面に刷る語は行ごとに 1 つだからである**（`FR-038`）。
...
**本表はアイコンの全数である**（`FR-029`）。**92 行ある。**
⛔ **本表の `群` の欄は、入口を並べる順を決めるためだけに在る。画面に刷ってはならない（MUST NOT）** —— 規則と理由は `FR-053` が持つ。⭐ **図形は 図 F-019 が正であり、本表は図形を語で説明しない**（1.9）—— 語で書き取らない理由は `FR-029` が持つ。**繋ぎ目は行 ID `IC-nn` だけである** —— 図では各図形の下に刷り、本表では第 1 列に立つ。
**`面` の欄は 表 T-103 の確定名である。** 新しい面の名を作らない。
⚠️ **本表は英名の欄を持たない** —— 持つと 92 個の確定名を新たに作ることになる。**表 T-012 の `SH-5` が既に書いている表記だけを使う。** ⭐ **`milestoneGlyph` の綴りは `_source/erd.json` が持つ**（はじめの 8 つは CR-172（版 0.53）が決めた）—— 本表に写さない。
⚠️ **`図形` の欄を持たないのも同じ理由である。** ⭐ **図形を持たない行は無い** —— 全 92 行が 図 F-019 に図形を持つ。**新しく起こした図形を実物と見比べて選び直す義務は 表 T-026 の `RC-13` が持つ。**
...
| 行 ID | 面 | 群 | 何の入口か | 正 | 構え |
| --- | --- | --- | --- | --- | --- |
| IC-7 | `App Header` | パレット | コマンドパレットを表示する・非表示にする（`S-99e`）。⭐ **本行だけが独自の群を持つのは、`App Header` の左端に置くためである**（利用者の指示 2026-08-27「一番左に移動しろ。ファイル読み書きの左だ」）—— ⛔ **並びを決めるのは本表の `群` の欄だけであり**（同表の前文）、**既存の群に入れたまま先頭へ動かすと群がとびとびになり、その欄の役目が壊れる。**⚠️ **群の名は画面に刷らない**（同前文の MUST NOT）ので、辞書の語は増えない | `FR-053` | — |
| IC-98 | `App Header` | 文書 | 文書を新しく始める（開いている文書を捨て、表 T-034 の `BT-4` と同じ状態へ戻す）。⚠️ **捨てる前の確認は `FR-095` が表 T-024a の `OP-4` に揃えている** | `FR-095` | — |
...
| IC-61 | `Command Palette` | 揃える | 依存線を構える（表 T-023b の `AR-4`）。⭐ **本行から `IC-38` までが 1 つの群である**（利用者の裁定 2026-09-01「依存線、コメントボックス、ハイライトボックスは次の行に移せ。順番は 依存線、コメントボックス、ハイライトボックス、開始日揃え、終了日揃え とせよ」）—— ⛔ **並びを決めるのは本表の `群` の欄だけであり**（同表の前文）、**行を動かすのではなく群を書き換える。**⚠️ **群の語は `揃える` のままである** —— 画面には刷らない（同前文の MUST NOT）が、ヘルプ（`FR-036`）は入口を語で並べるので、5 行を 1 語でどう呼ぶかは裁定を要する | `FR-009` | `AR-4` |
...
| IC-41 | `Command Palette` | 表示 | 透かしの表示を切り替える。⛔ **出ているときだけ解除パスワードを求める** —— 戻す側は問わない（`FR-020`、利用者の裁定 2026-09-02）| `FR-020` | — |
...
| IC-81 | `Command Palette` | 表示 | 依存線を表示する・非表示にする（`S-62`）。⚠️ **隠しているあいだも、依存線を選ぶ・ラグを編集する・削除する経路は別の面に残る** —— 規則と理由は `FR-049` が持つ | `FR-049`（`FR-009`）| — |
| IC-99 | `Command Palette` | 表示 | 文字サイズの段を変える（`S-70`。3 値排他）。⭐ **押すたびに 表 T-215 が刷る並びの次の段へ移り、末尾の次は先頭へ戻る** —— **`FR-029` が同じ機能の入口を 2 か所に置くことを禁じているので、入口は 1 つであり、1 つの入口が 3 段すべてに届くには巡るほかない。**⚠️ **段の並びは 表 T-215 のものであって、本行が定めるものではない** | `FR-039`（表 T-108 の `CM-62`）| — |
| IC-100 | `Command Palette` | 表示 | モノクロで描く・色に戻す（`S-74`）。⚠️ **明暗テーマ（`IC-16`）とは別の値である** —— 理由は `FR-041` が持つ。⛔ **`FR-049` の切り替えではない** —— **`S-74` は 表 T-203 の行であり、同要求が見るのは 表 T-202 の真偽の行である** | `FR-041`（表 T-108 の `CM-64`）| — |
| IC-101 | `Command Palette` | 表示 | 積む向きを選ぶ（`S-58`。2 値排他）。⭐ **押すたびにもう一方の向きへ移る** —— **`IC-16`（`S-72`）と同じ形であり、値が 2 つしか無いので巡りは往復になる** | `FR-003`（表 T-108 の `CM-56`）| — |
| IC-44 | `Command Palette` | カーソル | 基準日を置く・動かす・消す。⚠️ **「本日線」と呼んではならない**（表 T-103 の `U-11`）| `FR-046` | — |
| IC-47 | `Command Palette` | カーソル | ガイドカーソルを `'crosshair'` にする（`S-66`。3 値排他）。⛔ **取り消す入口は置かない** —— **本行をもう一度押せば `'none'` に戻る**（`FR-048`） | `FR-048` | — |
...
| IC-82 | `Row Title Panel` | — | 行を削除する。⭐ **消える範囲は 表 T-050 の `CD-2` が持ち、問い方は 表 T-037 の `NT-7` が持つ**（示す文は 表 T-234 の `QN-1`）| `FR-032` | — |
...
| IC-65 | `Resource Roster` | — | どの割当からも参照されていない担当者を選ぶ | `FR-099`（表 T-108 の `CM-43` は、本行と `IC-66` の 2 手で果たす。⭐ **本行は選ぶだけであり、`CM-43` を単独で出す行ではない** —— **2 手で果たす規則と、消す入口をちょうど 1 つにする理由は同要求が持つ**）| — |
...
| IC-76 | `Command Palette` | 表示 | 操作と描画の記録を始め、**同じ入口で止める**（`S-206`）。止めたとき記録はクリップボードへ渡る（表 T-008 の `R-9`）| `FR-102` | — |
...
⭐ **図形は本プロジェクトが起こしたものであり、第三者の素材ではない** —— 幾何のマイルストーンの図形 8 つは 1 つの外接円に内接し、☆ の内外比は `tbl-settings.md` の `S-48` である。**本プロジェクトの設定値と一致する図形が第三者の素材であることはない。**⭐ **あとの 7 つ（`IC-83` 〜 `IC-89`）も、同じ格子の上に同じ線の太さで本プロジェクトが起こした**（2026-08-29）—— 外から持ち込んだ素材は 1 つも無い。 これが 表 T-003 の `CN-7` に対する判断の根拠である。**差し替えの禁止そのものは `FR-029` が持つ。**
