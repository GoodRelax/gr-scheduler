...
| 行 ID | 面 | 群 | 何の入口か | 正 | 構え |
| --- | --- | --- | --- | --- | --- |
| IC-7 | `App Header` | パレット | コマンドパレットを表示する・非表示にする（`S-99e`）。<br>⭐ **本行だけが独自の群を持つのは、`App Header` の左端に置くためである**（利用者の指示 2026-08-27「一番左に移動しろ。<br>ファイル読み書きの左だ」）—— ⛔ **並びを決めるのは本表の `群` の欄だけであり**（同表の前文）、**既存の群に入れたまま先頭へ動かすと群がとびとびになり、その欄の役目が壊れる。<br>**⚠️ **群の名は画面に刷らない**（同前文の MUST NOT）ので、辞書の語は増えない | `FR-053` | — |
...
| IC-61 | `Command Palette` | 揃える | 依存線を構える（表 T-023b の `AR-4`）。<br>⭐ **本行から `IC-38` までが 1 つの群である**（利用者の裁定 2026-09-01「依存線、コメントボックス、ハイライトボックスは次の行に移せ。<br>順番は 依存線、コメントボックス、ハイライトボックス、開始日揃え、終了日揃え とせよ」）—— ⛔ **並びを決めるのは本表の `群` の欄だけであり**（同表の前文）、**行を動かすのではなく群を書き換える。<br>**⚠️ **群の語は `揃える` のままである** —— 画面には刷らない（同前文の MUST NOT）が、ヘルプ（`FR-036`）は入口を語で並べるので、5 行を 1 語でどう呼ぶかは裁定を要する | `FR-009` | `AR-4` |
