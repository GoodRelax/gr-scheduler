...
| U-61 | `Difference Review` | 差分の確認。合流で対応するかもしれないタスクを並べ、重複しているものをどちらのファイルから取るかを選ばせる面。立てる規則は `FR-022`、合流の規則は表 T-032 が持つ。⭐ **この面は開く道（表 T-024a の `OP-3`）から立つ** —— 独立したアイコンの入口は持たない（`FR-022`）。⚠️ **面の上の入口は表 T-109 の `IC-95` 〜 `IC-97` であり、表 T-032a の `MM-1`・`MM-2`・`MM-4` に当たる** —— `MM-3`（1 件ずつ決める）は置かない（利用者の裁定 2026-09-06「複雑なことは要らない」）。⛔ **`Confirmation`（`U-55`）ではない** —— 答えが 2 択ではない。⭐ **同じ面を、版が新しい文書を開くときにも使う**（`FR-073`）|
