...
| U-47 | `Row Expander` | 行の折り畳みの操作子。開く側と、その行を閉じる側と、配下をすべて閉じる側の 3 つで 1 組（置き方は表 T-051 の `HF-1`）|
...
⚠️ **`図形` の欄を持たないのも同じ理由である。** ⭐ **図形を持たない行は無い** —— 全 85 行が 図 F-019 に図形を持つ。**新しく起こした図形を実物と見比べて選び直す義務は 表 T-026 の `RC-13` が持つ。**
