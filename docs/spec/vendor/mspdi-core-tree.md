...
```text
TimephasedData
  ├─ Type? : enum(76種)   # 何の値か。例: 1=割当の残作業, 2=割当の実績作業,
  │                       #   11=タスク進捗率, 9=タスクベースライン作業, 76=物理進捗率 …
  ├─ UID : int
  ├─ Start? / Finish? : dateTime   # この一区間の期間
  ├─ Unit? : enum{0=m,1=h,2=d,3=w,4=mo,5=y}   # 時間粒度
...
graph TB
    ...
    subgraph GR["1行に複数の独立アイテムを横並べする表現"]
        G1["1行（例: 1車種）に<br/>企画◆・設計バー・試作◆・量産バーを<br/>独立要素として横並べ＝全フェーズを一枚絵で俯瞰"]
    end
