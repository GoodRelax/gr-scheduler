...
| SU-3 | **ユニット** | **1 ファイル。<br>** 公開エントリもユニットである | **131。<br>** 全数は 表 T-075、割った理由は 表 T-063 |
...
| UF-5 | `ScheduleLayout` | `schedule-layout.ts` | `pure` | `CP-5` の残り。<br>タスクと行の置き場を 表 T-068 の順に求め、表 T-064 の `PI-5` のとおりに公開する | `FR-003`（`OW-1`）・`FR-055`（`OW-1`）・`FR-109`（`OW-1`） |
