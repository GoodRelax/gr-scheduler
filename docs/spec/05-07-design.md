...
| 行 ID | 契機 | 気づくもの | 正 |
| --- | --- | --- | --- |
| FT-1 | 人の入力（ポインタとキー）と、ファイルのドロップ（01-04 の 表 T-024a の `OP-2`）。<br>⭐ **その入力の、待ち（表 T-066 の `CS-4`）をまたいだ続きを含む** | `DomInputSource`（`CP-27`）が 表 T-065 の `IF-2` で渡す。<br>⚠️ **待ちをまたいだ続きを起こすのはシェル自身である** —— `IF-2` の供給物は広げない | `NFR-010` |
