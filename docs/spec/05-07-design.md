...
| CP-32 | `documentModel` | `Selection` | 選ばれている対象の集合と、選んだ順序。<br>文書に保存しない | 表 T-023c の `SL-1` / `SL-7b` / `SL-8` |
...
| UF-169 | `ItemHitArea` | `drawn-selection.ts` | `pure` | 選択を、描いているタスクだけに絞る（表 T-023c・`FR-049`） —— 描いているかを 表 T-240 の図から判じる（`isTaskDrawn`）。<br>何も外れないときは同じ物を返し、選んだ順を保つ（`SL-7b`） | — |
...
| PI-32 | `documentModel` | `Selection` | `Selection`（型。<br>順序は 表 T-023c の `SL-7b`）／ `selectionWith` ／ `selectionWithout` ／ `emptySelection` ／ `isSelected` |
