...
**Clean Architecture を採る。** 層は `Entity` / `Use Case` / `Adapter` / `Framework` の 4 つとし、**中核の計算を `Entity` に置き、人向けの画面と `Agent API` を同格の `Adapter` に置く。**
...
| 行 ID | 層 | 置くもの | 部品 |
| --- | --- | --- | --- |
| AH-6 | `Entity` | 保存される事実と、それだけから決まる計算 | `DocumentModel` `TimeAxis` `RowLayout` `AlignmentSolver` `DetailSelector` `DependencyRouter` `ProgressGeometry` `FitCalculator` |
| AH-7 | `Use Case` | 文書を変える操作と、その付随（照合・履歴・版数・通知） | `CommandExecutor` `RevisionGuard` `UndoHistory` `ImportValidator` `ChangeNotifier` |
| AH-8 | `Adapter` | 外の世界と内側の橋渡し | `ViewAdapter` `AgentApi` `SvgSerializer` `JsonCodec` `MspdiCodec` `StorageGateway` |
| AH-9 | `Framework` | 差し替えられない外の仕組み | `BrowserDom` `FileSystemAccess` `LocalStorage` `SingleFileShell` |

**部品の責務は 表 T-046 が持つ。** 部品名は表記の規約 `W-1`（型は PascalCase）に従う。
...
**部品の全体は `_assets/fig-components.md` の 図 F-005 が、書き込みの経路は `_assets/fig-write-path.md` の 図 F-006 が持つ。** 本節は責務と機器の対応を表として持つ。
...
| 行 ID | 部品 | 層 | 責務 | 他へ出す面 |
| --- | --- | --- | --- | --- |
| CP-1 | `DocumentModel` | `Entity` | 保存される事実を保持する。形は `_assets/tbl-datamodel.md` | 読み取りは凍結した複製。**書き込みは `CP-7` からのみ** |
| CP-2 | `TimeAxis` | `Entity` | 時刻と座標を相互に変換する。**空間に名前を与え、対が厳密な逆関数であること** | 値を返す関数だけ。状態を持たない |
| CP-3 | `RowLayout` | `Entity` | 行の中でタスクを縦に積み、行の高さを決める | 同上 |
| CP-4 | `AlignmentSolver` | `Entity` | 選んだものを上下左右に揃える | 同上 |
| CP-5 | `DetailSelector` | `Entity` | 倍率に応じて描くものを絞る（`FR-018`） | 同上 |
| CP-6 | `DependencyRouter` | `Entity` | 依存線の経路を求める。**経路は保存しない** | 同上 |
| CP-7 | `ProgressGeometry` | `Entity` | 予定と実績の差と、遅れを示す線の頂点を求める | 同上 |
| CP-8 | `FitCalculator` | `Entity` | 全体を収める倍率と表示位置を求める（`FR-055`） | 同上 |
| CP-9 | **`CommandExecutor`** | `Use Case` | **書き込みの唯一の関門。** 表 T-047 の順序で実行する | 受理したか否かを値で返す。**例外を投げない**（`FR-028`） |
| CP-10 | `RevisionGuard` | `Use Case` | 申告された基準の版といまの版数を照合する | `CP-9` から呼ばれる |
| CP-11 | `UndoHistory` | `Use Case` | 変更前の状態の複製を積む。上限は `S-94` / `S-95` | `CP-9` からのみ積む |
| CP-12 | `ImportValidator` | `Use Case` | 外から来たものを受け入れてよいか判定する | 同上 |
| CP-13 | `ChangeNotifier` | `Use Case` | 変わったことを購読している側へ知らせる | **外側が購読する。内側は外側の型を知らない** |
| CP-14 | `ViewAdapter` | `Adapter` | 入力を命令へ翻訳し、幾何を画面へ描く | `CP-9` を呼ぶ。`CP-2` 〜 `CP-8` を読む |
| CP-15 | `AgentApi` | `Adapter` | 外から呼べる面。規約は 表 T-035 | `CP-9` を呼ぶ。**既定では公開しない**（`FR-028`） |
| CP-16 | `SvgSerializer` | `Adapter` | 幾何を `SVG` の文字列にする | `Entity` だけを読む。**`Use Case` を通らない** |
| CP-17 | `JsonCodec` | `Adapter` | 本製品の `JSON` と文書のモデルを相互に写す | `CP-9` へ取り込みを依頼する |
| CP-18 | `MspdiCodec` | `Adapter` | `MSPDI` と文書のモデルを相互に写す。対応は `_assets/tbl-mspdi.md` | 同上 |
| CP-19 | `StorageGateway` | `Adapter` | ファイルと保管庫への読み書き | `CP-17` / `CP-18` へ渡す |
| CP-20 | `BrowserDom` | `Framework` | ブラウザと文書オブジェクトモデル | `CP-14` / `CP-16` が触れる |
| CP-21 | `FileSystemAccess` | `Framework` | ファイルへの読み書きの仕組み | `CP-19` が触れる |
| CP-22 | `LocalStorage` | `Framework` | 保管庫 | 同上 |
| CP-23 | `SingleFileShell` | `Framework` | 単一の `.html` の殻。`Agent API` を載せる面 | `CP-15` が触れる |

#### 5.2.1 落としてはならない 1 つの形

**Type**: SECTION

**人が画面から行う編集と、`Agent API` から行う編集が、同じ `CommandExecutor`（`CP-9`）を通ること（MUST）。** そして **`CP-9` を通らずに `DocumentModel`（`CP-1`）を書き換える経路を持ってはならない（MUST NOT）。**
...
**`CP-9` が 1 回の呼び出しで行うことを、表 T-047 に順序として示す。図にしたものが `_assets/fig-write-path.md` の 図 F-006 である。**
...
| CX-3 | 3 | **変更前の文書の複製を履歴へ積む。取り消しは逆命令ではなく状態の複製で行うこと（MUST）** —— 取り込みと合流には逆命令が作れず、そこだけ別の仕組みになる。**取り消しの対象外の命令では積まない** | 表 T-027 ／ 表 T-035 の `AG-10` |
...
⚠️ **`CX-5` と `CX-6` を `CP-9` の外で行ってはならない（MUST NOT）。** 外で行える経路を 1 本でも作ると、**版数が上がったのに誰も知らない状態**が生まれ、表 T-035 の `AG-2` の照合が当てにならなくなる。

⚠️ **`CX-3` が `CX-4` より前でなければならない。** 置き換えた後で複製を取っても、取れるのは変更後の状態であって戻り先ではない。**複製は不変の更新（`CX-4`）と組で使うので、変わらなかった部分は複製されず共有される。** 段数と合計メモリの上限は `FR-032` が定め、値は `tbl-settings.md` の `S-94` / `S-95` が持つ。⚠️ **履歴そのものは文書に保存しない。**
...
**コアの 4 つは `_assets/fig-core-entities.md` の 図 F-002 が、2 つの軸の関係は `_assets/fig-two-axes.md` の 図 F-004 が、全体は `_assets/fig-domain-model.md` の 図 F-003 が持つ。**
