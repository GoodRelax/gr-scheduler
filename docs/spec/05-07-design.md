...
| SU-3 | **ユニット** | **1 ファイル。<br>** 公開エントリもユニットである | **132。<br>** 全数は 表 T-075、割った理由は 表 T-063 |
...
| UT-14 | `ScheduleLayout` | `schedule-layout.ts` ／ `time-axis.ts` ／ `label-width.ts` ／ `assignee-label.ts` ／ `percent-label.ts` ／ `name-label.ts` ／ `shape-cross-sections.ts` ／ `drawn-rows.ts` ／ `group-level-of-detail.ts` ／ `label-placement.ts` ／ `pinned-band.ts` ／ `row-scroll.ts` | **純粋性ではない** —— 表 T-075 のとおり 12 とも `pure` である（`UD-3`）。<br>**段ごとに正が別なので割った**（表 T-276 の `UD-1`）—— 表 T-068 の `LC-1` 〜 `LC-9` は `layoutFromSchedule` の 1 回の通り道で、時間軸は `FR-017` と 表 T-252 が、ラベル幅の概算は `FR-093` が、担当の札は `FR-059` が、完了率の札は `FR-090` と `OC-2` が、名称ラベルの字は `FR-002` と `ND-1` 〜 `ND-5` が、形の縦の断面は 表 T-271 と `FR-077` が、描く行の選びは `LC-1` と `HR-2` が、描く行の深さは `FR-018` と 表 T-005a の `L-3` が、札の置き場は `FR-109` と 表 T-272・表 T-273 が、ピン止めの帯は `FR-098` と `LF-14` が、縦の表示位置からのずらしは `OP-10a` が変わったときに書き直す。<br>行のループ —— 段の割当（`FR-003`・表 T-014）・占有幅の合算（`LC-7`）・帯高（`LC-9`）—— は公開エントリに置き、兄弟へ出したのはループが呼ぶ関数だけである。<br>兄弟が共有する語彙（形の種類・置き場の型・生成の定数）と、兄弟の名の出し直しは公開エントリに置いた |
...
| UF-5 | `ScheduleLayout` | `schedule-layout.ts` | `pure` | `CP-5` の残り。<br>タスクと行の置き場を 表 T-068 の順に求め、表 T-064 の `PI-5` のとおりに公開する | `FR-003`（`OW-1`）・`FR-055`（`OW-1`） |
