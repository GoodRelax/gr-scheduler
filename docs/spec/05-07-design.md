...
| IV-21 | `actualStart` と `actualDuration` がともに非 `null` の `Task` で、`actualDuration` が 0 を下回らないこと。<br>⭐ `IV-10` が予定の 2 列について定めるものを、実績の 2 列について定める —— 下回る値は実績の両端が入れ替わった置き方であり、拒んで告げる。<br>⚠️ **0 以上で床を下回る値は本行の対象ではない** —— 着手しているタスクの床（`_assets/tbl-settings.md` の `S-129`）は `FR-011` が持つ | その 2 列 | 組合せ |
