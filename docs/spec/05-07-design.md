...
| SU-3 | **ユニット** | **1 ファイル。<br>** 公開エントリもユニットである | **145。<br>** 全数は 表 T-075、割った理由は 表 T-063 |
...
| UT-17 | `DocumentCodec` | `json-codec.ts` ／ `grs-json-schema.ts` ／ `mspdi-codec.ts` ／ `mspdi-xml.ts` | **純粋性ではない** —— 表 T-075 のとおり、割ったユニットはどれも `pure` である（`UD-3`）。<br>**1 つの形式の中でも、変更の理由が別なので割った**（表 T-276 の `UD-1`）—— `UT-5` で形式ごとに割ったユニットを、さらに割った。<br>`GRS JSON` の文字列と文書の変換（形式の版の判じ・古い版の補い）は `FR-024` と `FR-073` が、値をスキーマに照らす歩きと節の表は 表 T-220 の前文のスキーマ（`grs-document.schema.json`）が変わったときに書き直す。<br>`MSPDI` の要素の木と文書の写しは `FR-021`・`FR-057` と交換相手のスキーマが、XML の文字列と要素の木の変換は XML の構文と `FR-023` の拒みと `CN-5` の書き方が変わったときに書き直す。<br>生成の区画は、それを歩く関数と同じユニットに刷る —— `json-codec.ts` は生成の定数を直に読まず、スキーマの根を閉じ込めた関数を兄弟に問う。<br>外れの型は作り手と同じユニットに置き、`json-codec.ts` と `mspdi-codec.ts` が出し直す |
...
| UF-36 | `DocumentCodec` | `mspdi-codec.ts` | `pure` | `MSPDI` と文書の相互変換。<br>XML の文字列と要素の木の変換は `mspdi-xml.ts` に問い、外れの型を出し直す | `FR-021`（`OW-2`）・`FR-057`（`OW-1`） |
