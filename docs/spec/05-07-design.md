...
| 回 | すること |
| --- | --- |
| 1 | 人が畳んだ状態をすべて捨て（表 T-051 の `HF-8`）、帯の高さが `FR-094` の床に達する倍率で、その文書が持つすべての深さを通して `LC-1` 〜 `LC-9` を走らせ、**表 T-038 の実寸を段ごとに得る**。<br>⭐ その床より下では絵が倍率に依らないので、1 回測れば床の内側に収まる段の縦幅は算術で出る |
