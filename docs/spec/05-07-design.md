...
| SU-3 | **ユニット** | **1 ファイル。<br>** 公開エントリもユニットである | **147。<br>** 全数は 表 T-075、割った理由は 表 T-063 |
...
| UT-6 | `SingleHtmlShell` | `single-html-shell.ts` ／ `frame-loop.ts` ／ `session-effects.ts` | **純粋性ではない** —— 表 T-075 のとおり 3 つとも同じである。<br>起動は `FR-067` と `FR-065` が、フレームの走行は 表 T-060 の `LY-5` と 5.6 の ADR-001 が縛るので、変更の理由が別である。<br>`session-effects.ts` は、副作用の名の全数（表 T-280 ・ 表 T-286 ・ 表 T-289 ・ 表 T-290 ・ 表 T-292 ・ 表 T-293 の状態遷移表の升）と、返った順に 1 つずつ実行すること（表 T-249 の `SF-6`）を負い、フレームの回し方とは変更の理由が互いに素である（表 T-276 の `UD-1`）—— 「副作用が 2 度実行された」「種類の漏れが型検査を通った」は本ファイル、「書き込みの中身が違う」は `frame-loop.ts` の実行の行である（`UD-5`）。<br> ⚠️ **割らないと 1 つのユニットが複数の事柄を負い、`R2.2` に反する** —— 5.2 の分割基準が、ユニットの側でも同じことを言う |
...
| UT-17 | `DocumentCodec` | `json-codec.ts` ／ `grs-json-schema.ts` ／ `mspdi-codec.ts` ／ `mspdi-xml.ts` ／ `mspdi-child-placement.ts` ／ `mspdi-imported-rows.ts` | **純粋性ではない** —— 表 T-075 のとおり、割ったユニットはどれも `pure` である（`UD-3`）。<br>**1 つの形式の中でも、変更の理由が別なので割った**（表 T-276 の `UD-1`）—— `UT-5` で形式ごとに割ったユニットを、さらに割った。<br>`GRS JSON` の文字列と文書の変換（形式の版の判じ・古い版の補い）は `FR-024` と `FR-073` が、値をスキーマに照らす歩きと節の表は 表 T-220 の前文のスキーマ（`grs-document.schema.json`）が変わったときに書き直す。<br>`MSPDI` の要素の木と文書の写しは `FR-021`・`FR-057` と交換相手のスキーマの列が、XML の文字列と要素の木の変換は XML の構文と `FR-023` の拒みと `CN-5` の書き方が、書き出す要素の子の並びは交換相手のスキーマの子の順（`EX-10`、`mspdi-child-order.json`）が、取り込んだタスクから作る行と所属は `FR-058` と `AT-51` が変わったときに書き直す。<br>生成の区画は、それを歩く関数と同じユニットに刷る —— `json-codec.ts` は生成の定数を直に読まず、スキーマの根を閉じ込めた関数を兄弟に問う。<br>外れの型は作り手と同じユニットに置き、`json-codec.ts` と `mspdi-codec.ts` が出し直す |
...
| UF-36 | `DocumentCodec` | `mspdi-codec.ts` | `pure` | `MSPDI` と文書の相互変換。<br>XML の文字列と要素の木の変換は `mspdi-xml.ts` に、書き出す要素の子の並び（`EX-10`）は `mspdi-child-placement.ts` に、取り込んだタスクの行と所属（`FR-058`）は `mspdi-imported-rows.ts` に問い、外れの型を出し直す | `FR-021`（`OW-2`）・`FR-057`（`OW-1`） |
