...
⚠️⚠️ 実測（2026-09-09、出荷ビルド）: 告げは 15.4 秒後も立っており、`Esc` を押したときにだけ消えた。  
⇒ **この造りは前の枝を選んでいる。**  
⛔⛔ **だから、本行を根拠に期限の値を実装が持つことはできない** —— `_assets/tbl-settings.md` は通知の期限を持つ行を 1 つも持たず、⇒ 数を置けば、それは発明である。  
⭐ **後の枝へ移るなら、先に設定値の行が要る。**  
⚠️⚠️ **2026-09-09 まで、本注は期限を 無条件に 数えると読めた** —— 台帳はその読みから「期限を計る仕掛けが `src/` に 1 つも無い」を欠陥として数えていた。  
⇒ ⭐ **欠陥は造りではなく本注に在った。**  
...
⭐⭐ `ET-` / `AT-` / `RL-` / `DV-` の番号は席番号である。一度与えた番号を振り直してはならない（MUST NOT）。 原稿（`_source/erd.json`）の行がそれぞれ自分の `seat` を持ち、生成器はそれを印字する。**行を足しても、それ以降の番号は動かない。**⭐ **要求の `UID` と同じ規則である**（Chapter 1）—— **番号が動くとトレースの鎖が切れる。**  
⚠️ かつては表の中の位置であった。⛔ そのあいだに 2 度、参照が黙ってずれた記録が残っている（`docs/development-records/changelog.md` の版 0.19 と 0.46 ②）。
