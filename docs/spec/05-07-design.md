...
| PI-19 | `Adapter` | `SvgRenderer` | `SvgSurface`（表 T-065）／ `svgFromSchedule`（`FR-080`）／ `colourOf`（表 T-236 の行 ID と、いまの色の好みから 1 色を返す。`EP-9` が書き出しに畫面と同じ線を引けと定めるので、`ImageExporter` が同じ色を問えなければならない。⚠️ **直値は 2026-09-06 まで 3 つに割れていた**） |
