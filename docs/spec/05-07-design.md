...
| SU-3 | **ユニット** | **1 ファイル。<br>** 公開エントリもユニットである | **120。<br>** 全数は 表 T-075、割った理由は 表 T-063 |
...
| UT-13 | `Schedule` | `schedule.ts` ／ `schedule-entities.ts` ／ `plan-actual-state.ts` ／ `calendar-day.ts` ／ `working-calendar.ts` ／ `task-delay.ts` ／ `stored-colour.ts` | **純粋性ではない** —— 表 T-075 のとおり 7 つとも `pure` である（`UD-3`）。<br>**変更の理由が別なので割った**（表 T-276 の `UD-1`）—— エンティティの型と列と既定の暦は `erd.json` と 表 T-209 が変わったときに `npm run gen` が書き直し、予実の状態の判別は 表 T-019a が、暦の日付の算術は 表 T-033 の `EX-` の行と 表 T-029a の `DC-` の行が、稼働日の数えは `FR-054` と 表 T-214 が、遅れの判別は 表 T-021b が、色の保存の綴りは 表 T-017b が、文書の不変条件は 表 T-220 が変わったときに人が書き直す。<br>生成の区画を公開エントリではなく兄弟に置いたのは、`working-calendar.ts` がモジュールの頂上で既定の暦を読むからである —— 区画が公開エントリにあると、兄弟が公開エントリから取り込み、公開エントリが兄弟を出し直す輪ができ、読み込みの順によっては読む時に定数がまだ無い。<br>`taskByUid` と、兄弟の名の出し直しは公開エントリに置いた |
