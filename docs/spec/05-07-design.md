...
| SU-3 | **ユニット** | **1 ファイル。<br>** 公開エントリもユニットである | **138。<br>** 全数は 表 T-075、割った理由は 表 T-063 |
...
| UT-15 | `ScheduleGeometry` | `schedule-geometry.ts` ／ `highlight-box.ts` ／ `plan-actual-guides.ts` ／ `progress-line.ts` ／ `comment-box.ts` ／ `dual-cursor.ts` | **純粋性ではない** —— 表 T-075 のとおり 6 つとも `pure` である（`UD-3`）。<br>**描くものごとに正が別なので割った**（表 T-276 の `UD-1`）—— ハイライトボックスの箱は `FR-019` と `CM-52` が、予実の補助線は `FR-084` と 表 T-020a が、イナズマ線は `FR-014` と 表 T-022 が、コメントボックスは `FR-097` と `S-182` が、`Dual Cursor` は `FR-082` と `CU-2` が変わったときに書き直す。<br>ハイライトボックスはタスクの図と共に変わってきたが、共に変えた変更はどれも多くのファイルに一斉に触れた手直しだったので、名が体を表すように別のユニットにした —— 割ったあと、どちらかに触れる次の 2 件の変更のうち 1 件でも両方に触れたら、1 つに戻す。<br>頂点を 1 度に組み立てる通り道（`LC-10`・`LC-11`）と、兄弟が共有する語彙（頂点の型・1 回の入力 `GeometryInputs`・`point`）と、兄弟の名の出し直しは公開エントリに置いた |
...
| UF-6 | `ScheduleGeometry` | `schedule-geometry.ts` | `pure` | `CP-6` の残り。<br>配置から、日程表に描くものの頂点を 1 度に組み立て（`LC-10`・`LC-11`）、表 T-064 の `PI-6` のとおりに公開する | `FR-009`（`OW-1`）・`FR-020`（`OW-2`）・`FR-043`（`OW-2`）・`FR-045`（`OW-2`）・`FR-079`（`OW-4`）・`FR-094`（`OW-4`）・`NFR-013`（`OW-4`） |
