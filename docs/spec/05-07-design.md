...
| SU-3 | **ユニット** | **1 ファイル。<br>** 公開エントリもユニットである | **146。<br>** 全数は 表 T-075、割った理由は 表 T-063 |
...
| UF-36 | `DocumentCodec` | `mspdi-codec.ts` | `pure` | `MSPDI` と文書の相互変換。<br>XML の文字列と要素の木の変換は `mspdi-xml.ts` に、書き出す要素の子の並び（`EX-10`）は `mspdi-child-placement.ts` に問い、外れの型を出し直す | `FR-021`（`OW-2`）・`FR-057`（`OW-1`） |
