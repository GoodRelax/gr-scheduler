...
**図の原稿は `_assets/source/model.json` である**（コンポーネント・辺・層の木）。**`_assets/source/build.py` が `.svg` と `docs/review/components/components.md` を書き出す。`.svg` と `.drawio` を手で直してはならない（MUST NOT）** —— 次の生成で消える。**図とその一覧を同じ原稿から起こすことで、両者の食い違いが起きないようにしている。**
