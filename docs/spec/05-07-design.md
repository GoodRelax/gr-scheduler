...
| SU-3 | **ユニット** | **1 ファイル。<br>** 公開エントリもユニットである | **117。<br>** 全数は 表 T-075、割った理由は 表 T-063 |
...
| UT-13 | `Schedule` | `schedule.ts` ／ `schedule-entities.ts` ／ `plan-actual-state.ts` ／ `calendar-day.ts` | **純粋性ではない** —— 表 T-075 のとおり 4 つとも `pure` である（`UD-3`）。<br>**変更の理由が別なので割った**（表 T-276 の `UD-1`）—— エンティティの型と列と既定の暦は `erd.json` と 表 T-209 が変わったときに `npm run gen` が書き直し、予実の状態の判別は 表 T-019a が、暦の日付の算術は 表 T-033 の `EX-` の行と 表 T-029a の `DC-` の行が、文書の不変条件は 表 T-220 が変わったときに人が書き直す。<br>`taskByUid` と、兄弟の名の出し直しは公開エントリに置いた |
...
| --- | --- | --- | --- | --- | --- |
| UF-1 | `Schedule` | `schedule.ts` | `pure` | `CP-1` の残り。<br>日程データの群を 表 T-064 の `PI-1` のとおりに公開する | `FR-047`（`OW-1`）・`FR-054`（`OW-2`） |
