...
> **初期テンプレートはアプリのコード側に持つ**（`handover/NEXT-STEPS-ja.md` **2-6** の管轄）。
...
> （`handover/NEXT-STEPS-ja.md` 2-6）。**入れ口に置いてはならない**（§5-1 規約 7）。
...
> **ただし同時更新の方針（基準版の照合のみ／自動調停）は未決**（`OPEN-ITEMS-ja.md` O-4。**語彙は決定-6 で確定**）。
