---
type: Open Items
title: Agent Interface の未決 3 件（＋ 実測で決着 7 件・決定 4 件）
description: file:// と公開の可否を 7 件実測で決着、D1-D4 は決定済み。残るは設計で決める 3 件。
tags: [agent-interface, open-items]
phase: proof-of-concept
status: draft
---
# Agent Interface の未決 3 件（＋ 実測で決着 7 件・決定 4 件）
...
>
> **D5（O-7）へ送るもの**: 人間が起動したときの有効化を**毎回聞くか記憶するか**。
...
| | |
|---|---|
| **争点** | 埋込文書がある状態で起動したとき、**localStorage の復旧確認を出すのか、黙って埋込文書を優先するのか** |
| **既存** | `user-order.md` 60（自動保存とクラッシュ復旧）／`handover/05-security-a11y/security-design.md` §5（**破損時はサイレントに破棄せず通知**） |
| **決め方** | 「埋め込みは明示的な意思」と見るなら埋込文書優先＋復旧は通知のみ。安全側なら常に確認を出す |
| **併せて決める（決定-4 から）** | **人間が起動したときの API 有効化を、毎回聞くか記憶するか。** 決着-1 でファイルアクセス復帰の 1 クリックが**どのみち省けない**ので、**起動時の 1 枚にまとめるか**をここで決める |
