---
type: Open Items
title: Agent Interface の未決 2 件（＋ 実測で決着 8 件・決定 5 件）
description: file:// と公開・保管庫を 8 件実測で決着、D1-D5 は決定済み。残るは設計で決める 2 件。
tags: [agent-interface, open-items]
phase: proof-of-concept
status: draft
---
# Agent Interface の未決 2 件（＋ 実測で決着 8 件・決定 5 件）
...
測定用のページは **GRS ではなく最小の代役**（テキスト 1 面 ＋ `revision` ＋ 楽観ロック）である。
...
4. **楽観ロックはこの経路でもそのまま働く**（P5）。拒否時に現在の文書が返るので読み直しが要らない。
...
| 項目 | 結果 |
|---|---|
| ~~**O-2 残件**（人間が開いている画面に外から入れるか）~~ | **決着-7 で成立を確認。** 人間が起動した Edge の `file://` ページへ外部から接続し、**読み・書き・楽観ロックの拒否まで通した**。拡張の経路は未測定だが、問うていた「入れるか」の答えは出ている |
...
> **この決定が縛るもの**: `handover/NEXT-STEPS-ja.md` **3-3 モジュール構成**（公開点は 1 か所だけ）／
> **2-1 視覚モック**（有効化の面が乗る）／
> `handover/05-security-a11y/security-design.md` の脅威モデルに 1 行。
>
...
| **壊れている** | **必ず通知し、退避して残す** | `handover/05-security-a11y/security-design.md` §5 |
...
> **これは `handover/02-data-model/grs-native-erd-ja.md`（データ構造の正）の管轄である。**
> **本ドラフトは「要る理由と最小の形」だけを書き、正への追加は組込み時に change-manager を通す。**
> **ドラフトの側から正へ書き足さない**（`handover/README.md` §`authority`）。

> **ブラウザの権限ダイアログは 1 枚に統合できない。** クリックを起点にしてしか出せないため、
> 「復帰」を押した**後に**ブラウザのダイアログが出る順になる。

> **この決定が縛るもの**: `handover/NEXT-STEPS-ja.md` **3-5 localStorage のキー設計**（文書ごと）／
