...
（`handover/06-background/refactor-gui-data-separation-ja.md`）。**機能を作る前に語を分けるのは安い。**
...
> 構造も挙動も変えないが、**組込み時に命名の正（`handover/03-ui-naming/handover-ui-parts-ja.md`）へ通す**
> （`MERGE-PLAN-ja.md` §4）。**ドラフトの側から正を書き換えない。**
