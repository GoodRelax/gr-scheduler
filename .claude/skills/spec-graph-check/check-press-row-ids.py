...
type: `export type PressRow = 'PD-1' | ... | 'PD-5'` is a hand-written copy of
...
⭐⭐ AND IT IS ABOUT TO MATTER. CR-371 abolishes the `PD-` prefix, because the
specification's press rows and the ledger's pending decisions both spell
themselves `PD-n` and mean unrelated things. On 2026-09-12 a classifier read
...
(表 T-023a -> `PTD-`) has to move the manuscript, the type, the tests and the
generated rosters together, and this check is what makes "together" mechanical
rather than remembered.

⛔ IT IS DELIBERATELY INSTALLED BEFORE THE RENAME, AND GREEN. A check added
afterwards would let one rename through unmeasured -- the check would be born
guarding something already broken.
