...
const EXPECTED_POINTER_IDS = Array.from({ length: 9 }, (_one, at) => `PK-${at + 1}`)
...
describe('table T-269 -- the manuscript shape of the nine pointers', () => {
  it('holds exactly PK-1 through PK-9', () => {
  ...
  it(`keeps white for the plan and black for the actual: ${T_269_WHITE_IS_PLAN}`, () => {
    expect(cellOf('T-269', 'PK-1', '中 ／ 縁')).toContain('白')
    expect(cellOf('T-269', 'PK-2', '中 ／ 縁')).toContain('黒')
    expect(cellOf('T-269', 'PK-5', '中 ／ 縁')).toContain('白')
    expect(cellOf('T-269', 'PK-6', '中 ／ 縁')).toContain('黒')
  })
})
...
describe(`the pointer column of table T-266: ${FR_106_TELLS_WHAT}`, () => {
  ...
  it('gives every plan end a white pointer and every actual or dummy end a black one', () => {
    ...
    for (const id of white) expect(pointerIdsIn(grabRow(id).pointer), `${id}: ${T_269_WHITE_IS_PLAN}`).toEqual(['PK-1'])
    for (const id of black) expect(pointerIdsIn(grabRow(id).pointer), `${id}: ${T_269_WHITE_IS_PLAN}`).toEqual(['PK-2'])
  })
