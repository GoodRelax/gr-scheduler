...
const TRANSITIONS = transitionsOf(SCREEN)
...
describe('the generated table of region screen against state-machines.json', () => {
  it('holds the rows of the manuscript, in its order', () => {
    ...
    expect(SCREEN_VALUES_TRANSITIONS.map(idOf)).toEqual(TRANSITIONS.map((row) => row.id))
  })

  it.each(TRANSITIONS.map((row) => [row.id, row] as const))(
    '%s carries its source, event, guard, target and effect unchanged',
    (id, row) => {
      const printed = SCREEN_VALUES_TRANSITIONS.find((one) => idOf(one) === id)
