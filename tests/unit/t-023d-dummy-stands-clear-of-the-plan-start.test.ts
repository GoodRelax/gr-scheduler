...
//
// CR-382 put the dummy back on the plan start day (T-240 DM-1); GA-1 and GA-5
...
//           GA-1 stands above the dummies, GA-6 above GA-5 (swapped by the
//           user's ruling of 2026-09-08), and all of them above GA-9
//   T-266   GA-1 「`===` | 予定の開始 | 予定の左端 | 外側 `S-250` ／ 内側 `S-251`
//           | 予定の帯 ＋ `S-252` | 箱の矢印 ← 白（`PK-1`）」
//   T-266   GA-2 「`===` | 予定の終了 | 予定の右端 | 外側 `S-253` ／ 内側 `S-254`
//           | 予定の帯 ＋ `S-255` | 箱の矢印 → 白（`PK-1`）」
...
//   T-023d  GA-5 / GA-6 / GA-17, the three dummies
//   T-023d  GA-9 「予定バー本体 | 端点を除いた中間」 and the warning under the
//           table, which limits `GA-5` to the left half of the mark `FR-043`
//           draws so that `GA-9` stays reachable on a not-started task
...
//   T-206   S-250 the plan start's outward reach, 12px
...
// ---------------------------------------------------------------------------
//
//   * HOW FAR the not-started marker stands from GA-6. LF-11 now settles it
...
//     below asserts only the RELATION: the marker hangs off GA-6 rather than
...
// `Hit` / `GrabArea` / `GrabSizes` / `itemAtPointer` / `NOT_STORED_SIZES`,
...
//     UNTIL 2026-09-10 this read `GrabSizes.dummyWidth` / `dummyHeight` as a
//     HALF-box around the point, which is what `SLOP` below had to be built to
//     match. `GrabSizes` carries no dummy figure at all now; the row walk
...
//     GA-5 exists at all and that `previewOfHeldPress` is a closure with no
...
/** Table T-268 in its PRINTED order -- 「上の行ほど先に応えること（MUST）」. */
const T_268_ORDER = specTable('T-268').rows.map((row) => row.id)
...
const HT_4_THE_LATER_DAY_WINS =
  '距離が同じときは、日付の遅いほう（終了の側）が応えること（MUST）'
const FR_043_GR_17_NAMES_THE_DAY =
  '終了点の掴みシロ（`FR-104` の 表 T-266 の `GA-6`）が指す日は、実績開始日の後に来る稼働日を `S-129` − 1 個数えた日（`FR-011` の床の日であり、既定の `S-129` ＝ 1 では実績開始日と同じ日）とすること（MUST）'
...
/**
 * `zoomX` (`S-75`), chosen so that ONE DAY is wider than `S-250`.
 *
 * ⭐ WHY THE ZOOM HAS TO BE STATED. At the default `zoomX` of 1 a day is
 * exactly `S-1` = `S-250` px, and the two grab points a day apart would still be
 ...
 * distinguishable halves (GA-5's and GA-6's); the case that measures that
 ...
 *
 * ⛔ `GrabSizes` HOLDS NO `dummyHeight`. The hold reads the band off
 ...
 * `working-calendar.ts` and is deliberately not called: a test that walked the calendar
...
/** The reading FR-043 rejects for the other handle: CR-198 「暦日で進めると非稼働日に置く」. */
...
/**
 * How far the finish handle is pulled out, in worked days from GA-5's own day.
 */
const PULLED_WORKED_DAYS = 4

/** Where the finish handle is let go: `PULLED_WORKED_DAYS` past GA-5's day. */
const PULLED_TO_DAY = workedDaysAfter(PLAN_START, PULLED_WORKED_DAYS)

/** A milestone that has not been started -- table T-023d's GA-17. */
...
/** A milestone nobody has started. Table T-023d's GA-17, and FR-043's exception. */
 ...
 * 印は、その横幅の中央で左右に割ること（MUST）。左半分を実績の開始側（`GA-5`）、
 * 右半分を実績の終了側（`GA-6`）とすること（MUST）」.
...
/** And the RIGHT half, which is GA-6's by the same clause. */
...
describe('the fixture stands where these cases think it does', () => {
  ...
  it('is built on a Friday, so the calendar-day answer and the working-day answer differ', () => {
    ...
    const inkWidth = dummyNamed(taskDrawn(drawn), 'GA-5').ink.width
    expect(
  ...
  it('draws one day wider than S-250, and ink wide enough to split into two halves', () => {
    ...
    expect(
      layout.pxPerDay,
      'S-250 reaches this far outside GA-1, so one day must be wider than it',
    ...
    // is wide enough for its own two halves (GA-5's and GA-6's, split at its
    ...
    // unmeasurable.
    const ink = dummyNamed(taskDrawn(draw(notStarted())), 'GA-5').ink
    expect(
      ink.width,
  ...
  it('prints the dummies above GA-1, GA-6 above GA-5, and all of them above GA-9 (MUST)', () => {
    ...
    // ⚠️ GA-6 AND GA-5 SWAPPED ON 2026-09-08, by the user's ruling 「実績の
    // 開始日と実績の終了日のどちらをつかむか難しいことになるが、実績の終了日を
    // 優先しろ」. The table used to close GA-6's row with docs/development-records/pending-decisions.md 「重なったら開始点
    ...
    const at = (row: string): number => {
      const found = T_268_ORDER.indexOf(row)
      if (found < 0) throw new Error(`table T-268 no longer prints ${row}`)
      return found
    }
    expect(at('TY-4')).toBeLessThan(at('TY-6'))
    expect(at('TY-6')).toBeLessThan(at('TY-9'))
    // ⭐ Two dummy ends are one kind, so the finish side is settled by HT-4.
    expect(HT_4_THE_LATER_DAY_WINS).toBeTruthy()
    expect(REQUIREMENTS).toContain(HT_4_THE_LATER_DAY_WINS)
  })
  ...
  it('carries an S-129 of at least one worked day, which the dummy\'s length assumes', () => {
    ...
    // すると幅が 0 になる」. ⚠️ A GUARD: the case that asserts GA-6 stands to
    // the RIGHT of GA-5 is only meaningful while the placed period has a length,
  ...
  it('draws the two dummies FR-043 asks for, and no more (MUST)', () => {
    ...
    expect(taskDrawn(draw(notStarted())).dummies.map((one) => one.grab)).toEqual(['GA-5', 'GA-6'])
  })
...
describe('table T-240 DM-1 / T-023d GA-5 / GA-6 (CR-382): the dummy stands on the plan start day', () => {
  it('puts GA-5 on the plan start day itself, not on the working day after it', () => {
    const drawn = draw(notStarted())
    const start = dummyNamed(taskDrawn(drawn), 'GA-5')
  ...
  it('puts GA-6 on the worked day S-129 - 1 past GA-5, so the dummy is still S-129 long', () => {
    const drawn = draw(notStarted())
    const task = taskDrawn(drawn)
    const start = dummyNamed(task, 'GA-5')
    const end = dummyNamed(task, 'GA-6')
    ...
    expect(
      ...
      'FR-011 counts GA-5 day through GA-6 day as the S-129 of DM-2',
    ).toBe(ACTUAL_INITIAL_DURATION)
    expect(end.at.x - start.at.x).toBeCloseTo(
      xOfDay(drawn, DUMMY_END_DAY) - xOfDay(drawn, PLAN_START),
      6,
    )
    expect(end.at.x, 'not S-129 worked days past GA-5, which would make the actual S-129 + 1 days')
    ...
    expect(
      ...
      "T-266's closing rule: the day is not the place -- GA-6 holds the right half of the one mark",
    ).toBe('GA-6')
  })

  it('stands the mark inside the plan bar\'s left end, where GA-1 answers only outside it', () => {
    const drawn = draw(notStarted())
    ...
    expect(grabAt(drawn, planLeft - SLOP['S-250'] / 2, middleY)).toBe('GA-1')
    expect(grabOnTheStartHalfOfTheMark(drawn, start)).toBe('GA-5')
  })
})
...
describe('table T-023 MK-9a: a press on each point answers a different row', () => {
  ...
  it('answers GA-5 on its own band, not GA-1 (MK-9a)', () => {
    // ⛔ RED WHEN THIS WAS WRITTEN. Both rows claimed the same x, and 「上の行ほ
    // ど優先すること（MUST）」 gave it to GA-1 -- so there was no point at all
    // where GA-5 could be reached at its own place, which is what the user
    // reported (DFC-56).
    // ⚠️ THE PRESS MOVED TWICE, THE CLAIM DID NOT. On 2026-09-09 the manuscript
    // first handed every pixel of the one mark to GA-6 and the press moved
    // past the mark; later the same day it cut the mark down the middle
    // instead, and the LEFT half is GA-5's own. What is asked is still that
    // GA-5 can be reached at its own place, and GA-1 must still not answer
    // there.
    const built = drawn()
    const task = taskDrawn(built)
    expect(grabOnTheStartHalfOfTheMark(built, dummyNamed(task, 'GA-5'))).toBe('GA-5')
  })
  ...
  // GA-5 at 6px a day, 0 of 12 at 12.9 and at 27.6 -- the whole mark was the
  // finish's, which is what these two cases now refuse.

  it('DFC-415 ⭐ MUST: the LEFT half of the one mark answers the actual START', () => {
    const built = drawn()
    const mark = dummyNamed(taskDrawn(built), 'GA-5').ink
    expect(mark.width, 'FR-043 draws one mark').toBeGreaterThan(1)
    // ⭐ EVERY pixel of the left half, so a build that gave GA-5 one edge pixel
  ...
  it('DFC-415 ⭐ MUST: the RIGHT half of the same mark answers the actual FINISH', () => {
    ...
    // to GA-5 -- the mirror of the fault -- would pass the case above.
    const built = drawn()
    const mark = dummyNamed(taskDrawn(built), 'GA-5').ink
    ...
    expect(grabOnTheFinishHalfOfTheMark(built, dummyNamed(taskDrawn(built), 'GA-5')))
      .toBe('GA-6')
  })

  // ⛔⛔ WHAT DFC-415's SECOND HALF ASKS AND WHY NO CASE STANDS FOR IT HERE.
  // Table T-023d's GA-6 row (MUST) wants the finish handle to pin `actualStart`
  // at GA-5's day -- 「掴めば `actualDuration` を置く（`actualStart` は `GA-5` の
  ...
  // 日 ＝ 掴みシロを離した日」: 「左半分を掴めば `GA-5` が答え、離した日が実績開始
  ...
  it('still answers GA-1 just outside the plan bar\'s left end -- the other half of the pair', () => {
    // ⭐ THE CASE THAT MUST NOT REGRESS. DFC-56 「実績開始部の左側が予定、右側がダミー
    // の実績として掴めるはず」: moving the dummy is only half of what was asked,
    // and widening the plan to the left has to keep working.
    // ⚠️ PRESSED OUTSIDE THE EDGE, NOT ON IT. Table T-240's `DM-1` makes the
    // bar's end the fence -- 「予定の開始の掴み代は端の外側だけ、ダミーの印
    // は端の内側」 -- and the mark begins ON the edge, where table T-268
    // seats it above the plan's end.
    const built = drawn()
    const task = taskDrawn(built)
    const box = boxOfBar(task.plan)
    expect(grabAt(built, box.x0 - 1, middleOf(box))).toBe('GA-1')
  })

  it('still answers GA-2 on the plan bar\'s right end', () => {
    const built = drawn()
    const box = boxOfBar(taskDrawn(built).plan)
    expect(grabAt(built, box.x1, middleOf(box))).toBe('GA-2')
  })

  it('still answers GA-9 in the plan bar\'s middle (the warning under table T-023d)', () => {
    // The warning under table T-023d limits `GA-5` to the left half of the mark
    // `FR-043` draws, so that `GA-9` stays reachable on a not-started task.
    // Moving the dummy one day along must not turn into widening it.
    const built = drawn()
    const box = boxOfBar(taskDrawn(built).plan)
    expect(grabAt(built, (box.x0 + box.x1) / 2, middleOf(box))).toBe('GA-9')
  })
  ...
  it('leaves neither dummy past its own ink, so nothing answers GA-5 or GA-6 beyond it', () => {
    ...
    const built = drawn()
    const end = dummyNamed(taskDrawn(built), 'GA-6')
    const past = grabAt(built, end.ink.x + end.ink.width + 1, end.ink.y + end.ink.height / 2)
    expect(past).not.toBe('GA-5')
    expect(past).not.toBe('GA-6')
  })
})
...
describe('FR-043 (MUST): grabbing GA-5 places the day it was let go on, S-129 and resumeValid', () => {
  ...
  it('pins the start at GA-5 の日 from GA-6, and counts the length out to the release', () => {
    ...
    // drawn mark instead -- GA-6's hold, where the same closing rule splits
    // the mark.
    const built = draw(notStarted())
    expect(grabOnTheFinishHalfOfTheMark(built, dummyNamed(taskDrawn(built), 'GA-6'))).toBe(
      'GA-6',
    )
    const after = taskIn(
      run(notStarted(), {
    ...
    expect(dayOf(after.actualStart), DM_1_NOT_THE_WORKED_DAY_AFTER)
      .not.toEqual(dayNamed(WORKED_DAY_AFTER_START))
    expect(dayOf(after.actualStart), 'GA-6 does not write the day the hand let go on')
      .not.toEqual(dayNamed(PULLED_TO_DAY))
    expect(dayOf(after.stop), 'GA-6: the released day is the last day itself')
    ...
    expect(dayOf(after.stop), 'and it is NOT the floor day, which is GA-5\'s answer')
      .not.toEqual(dayNamed(workedDaysAfter(PLAN_START, ACTUAL_INITIAL_DURATION - 1)))
    expect(after.resumeValid).toBe(true)
  })

  it('⭐ THE CONTRAST: the same release through GA-5 writes the other two values', () => {
    // ⛔ WITHOUT THIS PAIR EITHER ARM COULD ANSWER FOR BOTH. The case above is
    // green over a build that pinned EVERY grab at GA-5's day; this one is green
  ...
  it('starts the actual bar where the hand let go, NOT where GA-5 is drawn', () => {
    // ⛔⛔ THE TWO RULES ARE SEPARATE AND THIS IS WHERE THAT IS ASKED. Until
    // 2026-09-02 the write took the day GA-5 is DRAWN on, so the actual bar came
    ...
    const handleX = dummyNamed(taskDrawn(drawnBefore), 'GA-5').at.x
...
describe('table T-023d GA-17 (CR-382): the milestone\'s dummy stands on the figure\'s own day, where GA-5 would', () => {
  it('draws exactly one dummy, and it is GA-17 (exception ①)', () => {
    // FR-043's first surviving exception: 「実績バーを持たないので（表 T-023d の
    // `GA-16`）、ダミーは点として 1 つだけ出すこと（MUST）」. ⭐ THIS ONE IS A
    // REAL EXCEPTION -- a milestone has no actual bar, so it cannot have two
    // ends -- and the ruling of 2026-09-02 left it standing.
    expect(taskDrawn(draw(milestone())).dummies.map((one) => one.grab)).toEqual(['GA-17'])
  })

  it('stands GA-17 on the plan start day, exactly where GA-5 stands', () => {
    const built = draw(milestone())
    const dummy = dummyNamed(taskDrawn(built), 'GA-17')
    ...
    expect(grabOn(built, dummy)).toBe('GA-17')
  })

  it('follows the same rule GA-5 does, day for day, on the same plan start', () => {
    const shapeAt = (schedule: Schedule, grab: 'GA-5' | 'GA-17'): number =>
      dummyNamed(taskDrawn(draw(schedule)), grab).at.x
    const asARectangle = notStarted({
      start: stored(MILESTONE_DAY),
      finish: stored(PLAN_FINISH),
    })
    expect(shapeAt(milestone(), 'GA-17')).toBeCloseTo(shapeAt(asARectangle, 'GA-5'), 6)
  })
  ...
  it('leaves the FIGURE on the plan day, and the dummy stands in that same day column', () => {
    ...
    const dummy = dummyNamed(task, 'GA-17')
...
// ===========================================================================
// Table T-023d GA-18: the not-started marker follows GA-6, wherever it stands
// ===========================================================================

describe('table T-023d GA-18: the not-started marker hangs off GA-6, not off the plan', () => {
  it('stands outside GA-6 and moves with it', () => {
    ...
    expect(marker!.centre.x).toBeGreaterThan(dummyNamed(task, 'GA-6').at.x)
  })

  it('does not follow the plan\'s right end: two plans of different length share a marker x', () => {
    // ⭐ THE CLEANEST STATEMENT OF GA-18'S 未着手 CLAUSE. GA-6 depends on the
    ...
    expect(shortTask.marker!.centre.x - dummyNamed(shortTask, 'GA-6').at.x).toBeCloseTo(
      longTask.marker!.centre.x - dummyNamed(longTask, 'GA-6').at.x,
      6,
    )
  })

  it('keeps the marker clear of the two dummies, so GA-18 does not swallow them', () => {
    // MK-9a again: GA-18 is printed ABOVE GA-5 and GA-6, so a marker drawn on
    ...
    expect(grabOnTheFinishHalfOfTheMark(built, dummyNamed(task, 'GA-6'))).not.toBe('GA-18')
  })
})
