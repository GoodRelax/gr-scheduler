...
// ⛔ TWO EXCEPTIONS THAT MUST BE DECLARED, because they are more than a
// signature:
...
describe('FR-031 -- 対象外 の値は、無関係な編集の取り消しで戻ってはならない', () => {
  ...
  it('UN-3 writes nothing in the 見せ方の群, so undoing it may move none of it', () => {
    ...
    expect(settingsOf(back.next.document)).toBe(beforeUndo)
  })
})
