...
describe('tables T-206 / T-210 -- the four values CR-416 moved, and S-92 it did not', () => {
  it('holds S-174 1px, S-175 2 x 1px, S-109 3.5px and S-110 1.0px', () => {
    expect(S_174).toBe(1)
    expect([S_175_ON, S_175_OFF]).toEqual([2, 1])
    expect(S_109).toBe(3.5)
    expect(S_110).toBe(1)
  })

  it('keeps the S-92 grab at 15px, wider than the 7px point', () => {
    expect(S_92).toBe(15)
...
describe('S-109 / S-110 -- the fade grab points of the selected Task', () => {
  it('draws each as a 7 x 7 square centred on its point, with a 1px border', () => {
...
describe('S-92 -- a press outside the drawn point but inside 15 x 15 still takes the fade', () => {
  it('takes GR-1 or GR-2 at 7px right of each point centre', () => {
