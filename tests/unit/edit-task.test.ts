...
describe('EditDocument (PI-9) -- CM-13 setTaskPlanActualState', () => {
  ...
  it('IV-21 must not store an actual whose end is before its start', () => {
    ...
    //
    // ⭐ THE ROW BROKEN IS TABLE T-220's IV-21: 「`actualStart` と
    // `actualDuration` がともに非 `null` の `Task` で、`actualDuration` が 0 を
    // 下回らないこと。」 A negative length is refused there before it can even
    // reach AT-39's column type in `_assets/fig-erd-detail.md`.
    //
