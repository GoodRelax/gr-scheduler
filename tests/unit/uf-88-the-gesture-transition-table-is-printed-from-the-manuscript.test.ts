// UF-88: tools/generate_state_machine_types.py prints the transition table of the manuscript, row by row.
...
import { GESTURE_VALUES_TRANSITIONS } from '../../src/use-case/advance-screen-session/gesture-values'
...
const GESTURE = MANUSCRIPT.regions.find((one) => one.region === 'gesture')
...
const TRANSITIONS = transitionsOf(GESTURE)

describe('the generated table of region gesture against state-machines.json', () => {
  it('holds the rows of the manuscript, in its order', () => {
    expect(GESTURE).toBeDefined()
    expect(GESTURE_VALUES_TRANSITIONS.map(idOf)).toEqual(TRANSITIONS.map((row) => row.id))
  })

  it.each(TRANSITIONS.map((row) => [row.id, row] as const))(
    '%s carries its source, event, guard, target and effect unchanged',
    (id, row) => {
      const printed = GESTURE_VALUES_TRANSITIONS.find((one) => idOf(one) === id)
...
// WHY: R4.4 has every case of a cell stated; a fall-through is printed only where the
// guards leave a case open, judged over every truth assignment of the guard terms.
describe('the printed table of region gesture states every case of a cell once', () => {
  ...
  const printedRow = (event: string, firstTarget: string): string =>
    printed.split(/\r?\n/).find((line) => line.startsWith(`| \`gesture/${event}\` | → \`${firstTarget}\``)) ?? ''

  it('prints no fall-through where three conjunctions cover all four cases', () => {
    const row = printedRow('pointerPressed', 'changingDocument')
    expect(row).not.toBe('')
    expect(row).not.toContain('それ以外')
  })

  it('prints the fall-through where one guard leaves its negation open', () => {
    const row = printedRow('pointerPressed', 'axisUndecided')
