...
describe('ScheduleLayout (PI-5) -- LC-1 and LC-2', () => {
  ...
  it('FR-018 drops the deeper rows as zoomY falls, and never depth 1', () => {
    // threshold(depth) = 0.32 x 1.5^(depth - 2), so depth 4 wants 0.72 and depth 5 wants 1.08.
    expect(groupDepthLimit(settingsOf({ ...LAYOUT_SETTINGS, zoomY: 0.7 }))).toBe(3)
    expect(groupDepthLimit(settingsOf({ ...LAYOUT_SETTINGS, zoomY: 1 }))).toBe(4)
    expect(groupDepthLimit(settingsOf({ ...LAYOUT_SETTINGS, zoomY: 1.1 }))).toBe(5)
...
describe('ScheduleLayout (PI-5) -- labels, shapes and fit', () => {
  ...
  it('FR-077 carries the drawn type size on the placement, with S-8 applied last', () => {
    ...
    // A rectangle: basePlanHeight x actualOfPlan x fontOfActual, clear of S-8.
    // ⛔ THE THREE VALUES ARE READ, NOT TYPED. `actualOfPlan` moved from 0.73
...
describe('ItemHitArea (PI-7)', () => {
  ...
  it('JDG-35 ⭐ MUST: outside the left end is the PLAN\'s, inside it is the ACTUAL\'s', () => {
    ...
    // and nothing but the ruling can separate them. A premise, not the claim.
    const actualXs = outlinePoints(geometry.tasks[0]!.actual).map((one) => one.x)
