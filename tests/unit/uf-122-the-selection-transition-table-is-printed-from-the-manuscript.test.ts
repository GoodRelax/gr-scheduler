// UF-122: tools/generate_state_machine_types.py prints the transition table of the manuscript, row by row.
...
import { SELECTION_VALUES_TRANSITIONS } from '../../src/use-case/advance-screen-session/selection-values'
...
const SELECTION = MANUSCRIPT.regions.find((one) => one.region === 'selection')
...
const TRANSITIONS = transitionsOf(SELECTION)

describe('the generated table of region selection against state-machines.json', () => {
  it('holds the rows of the manuscript, in its order', () => {
    expect(SELECTION).toBeDefined()
    expect(SELECTION_VALUES_TRANSITIONS.map(idOf)).toEqual(TRANSITIONS.map((row) => row.id))
  })

  it.each(TRANSITIONS.map((row) => [row.id, row] as const))(
    '%s carries its source, event, guard, target and effect unchanged',
    (id, row) => {
      const printed = SELECTION_VALUES_TRANSITIONS.find((one) => idOf(one) === id)
...
// WHY: R4.4 has every case of a cell stated; a guard that covers one case leaves the
// other open, and the printed cell says what happens to it.
describe('the printed table of region selection states every case of a cell once', () => {
  const printed = readFileSync(join(process.cwd(), 'docs', 'spec', '_assets', 'tbl-state-machines.md'), 'utf8')
  // WHY: the event list has a row of the same key; a cell of the machine's table opens with a target or a dash.
  const printedRow = (event: string): string =>
    printed
      .split(/\r?\n/)
      .find((line) => [' → ', ' — '].some((open) => line.startsWith(`| \`selection/${event}\` |${open}`))) ?? ''

  it('prints the fall-through where nothing is selected and the pick is empty', () => {
    const row = printedRow('objectsPicked')
    expect(row.split('それ以外 → —')).toHaveLength(2)
  })

  it('prints the fall-through beside the rung guard', () => {
    expect(printedRow('selectionEscapePressed')).toContain('それ以外 → —')
  })

  it('prints no fall-through where a cell has no guard', () => {
    const row = printedRow('emptyAreaClicked')
    expect(row).not.toBe('')
