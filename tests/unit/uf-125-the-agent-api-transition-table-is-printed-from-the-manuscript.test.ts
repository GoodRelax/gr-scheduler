// UF-125: tools/generate_state_machine_types.py prints the transition table of the manuscript, row by row.
...
import { AGENT_API_VALUES_TRANSITIONS } from '../../src/use-case/advance-screen-session/agent-api-values'
...
const AGENT_API = MANUSCRIPT.regions.find((one) => one.region === 'agentApi')
...
const TRANSITIONS = transitionsOf(AGENT_API)

describe('the generated table of region agentApi against state-machines.json', () => {
  it('holds the rows of the manuscript, in its order', () => {
    expect(AGENT_API).toBeDefined()
    expect(AGENT_API_VALUES_TRANSITIONS.map(idOf)).toEqual(TRANSITIONS.map((row) => row.id))
  })

  it.each(TRANSITIONS.map((row) => [row.id, row] as const))(
    '%s carries its source, event, guard, target and effect unchanged',
    (id, row) => {
      const printed = AGENT_API_VALUES_TRANSITIONS.find((one) => idOf(one) === id)
