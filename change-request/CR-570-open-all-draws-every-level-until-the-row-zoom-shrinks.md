...
> ⚠️ 問いが 2 つ残る（11 節）。問い 1（段 0 の畳みを文書に保存するか）の答えで、表 B（14.4 節）の 2 つの行（B8・B9）と、3.2 節の末尾の段落に挙げた編集が動く。本書の本文は、今の仕様のまま「保存しない」（問い 1 の案 A）で書き、案 B で増える編集を 3.2 節と 8 節に別に挙げる。
...
>
> 閉じるもの: `DFC-983`（すべて開く入口が、倍率で描かれていない行を開かない）と、12 節の仮の 2 行（`DFC-984`・`DFC-985`）。利用者の裁定 `JDG-590` 〜 `JDG-599`。
...
| `JDG-595` | 「② Fit で戻す とせよ。」 | 全体表示は段 0 の畳み（`S-211`）も戻す | 生きている（表 B の B5） |
...
- `R1.1`（抜けが無い）—— 旧の 8 つの組み合わせのうち、入口が作らない 3 つ（畳み＋印、隠し＋印、隠し＋畳まない）が文書に現れたときの意味が、どこにも無かった。1 つの値にまとめると組み合わせそのものが消え、古い文書の読み替え（3.4 節）だけが残る。
...
| 決定 12 | 取り消し・やり直しは、段が覚えた文書へ戻すので、値もそのときの値へ戻る。段 0 の畳みは戻さない（問い 1 が案 A のとき） | 表 T-027 の `UN-14`。段 0 は画面の値（`S-211`） | 頭の [^^] を取り消しても、段 0 は畳まれたまま（行は 1 つも描かれない）—— 今の仕様と同じ。問い 1 の理由の 1 つ |
...
| 決定 18 | 古い `GRS JSON`（3 つの真偽を持ち `treeState` を持たない）は読み替えて開く（3.4 節）。読み替えは `AT-153` の意味の欄に置き、3 つの列は表 T-297 に退いた列として足す | `FR-073`（古い版は読める版）、表 T-297、`AT-143` の「この列を持たない `GRS JSON` は … として読む」の先例 | 新しい造りで保存したファイルは、古い造りでは開けない（問い 2） |
...
| 決定 22 | 段 0 の畳みは、全体表示（`JDG-595`）と文書の置き換えでも開く | 置き換えで開かないと、畳んだ段 0 が次の文書に持ち越され、開いた文書の行が 1 つも描かれない（今の仕様とコード。`DFC-984`、仮） | 問い 1 が案 B なら、置き換えではファイルの値になる |
...
| R8 古い文書を読む | `AT-153` の意味、表 T-297 に `RK-3` 〜 `RK-5`、6.1 節の「捨てる前に読み替える」、`FR-018` の `isKeptOpen` を持たない版の段を消す | 4.1、4.11 |
| R9 段 0 | 表 T-206 の `S-211` の注、状態機械 `levelZeroFoldStateMachine` の出来事 `levelZeroOpened` の出どころ | 4.13 |
| R10 頭の [vv] の理由 | 表 T-233 の `RS-31` の場面と語 | 4.14 |
| R11 命令 | 表 T-108 の `CM-33`・`CM-34`・`CM-75` を退かせ `CM-85` を足し、`CM-72` の名を替える | 4.15 |
...
| 種類 | 仮の番号・名 | 何か | 測った |
|---|---|---|---|
| 列 | `AT-153` `TaskGroup.treeState` | 行の木の状態（列挙 5 値、`null` 不可、既定 `auto`） | 仕様・台帳・本書以外の CR に `AT-153` は 0。進行中の CR が `AT-152` まで取っている |
| 表 | `T-328` | 行の木の状態機械（生成。`_assets/tbl-state-machines.md`） | 木と CR に 0（CR は `T-327` まで） |
| 図 | `F-043` | 行の木の状態遷移（生成） | 木と CR に 0 |
| 表 | `T-329` | 行を描くか（`FR-018`） | 同上 |
| 行 | `TD-1` 〜 `TD-7` | 表 T-329 の行。⚠️ 新しい接頭辞 `TD`（Tree Drawn）を `row-id-prefixes.json` に登録する | 登録簿に `TD` は無い（`PTD` とは別の綴り） |
| 行 | `SD-5` | 表 T-250 の行（文書のデータの状態のうち、原稿に置くもの） | 木は `SD-4` まで |
| 行 | `RK-3` 〜 `RK-5` | 表 T-297 の行（`isCollapsed`・`isHidden`・`isKeptOpen`） | 木は `RK-2` まで |
| 行 | `N-26` | 用語 `treeState`（`N-15` `isKeptOpen` に代わる） | 進行中の CR が `N-25` まで |
| 行 | `CM-85` | 命令 `setTaskGroupTreeState` | 進行中の CR が `CM-84` まで |
| 原稿の名 | 領域 `rowTree`、状態機械 `treeStateMachine`、出来事 10（4.12 節）、ガード `isPressedRow`・`isChildOfPressedRow`・`isBelowPressedRow`・`isLeafRow`・`isTopLevelRow` | 表 T-328 の中身 | 原稿の出来事のキーと重ならない（生成器は領域をまたいだ重なりを拒む） |
| 台帳 | `DFC-984`・`DFC-985`（12 節） | 本書が閉じる、今の仕様の穴 2 つ | 帯の中 |

⚠️ 退く番号（`retired.py` に載せる）: `AT-56`・`AT-57`・`AT-142`・`T-254`・`KO-1` 〜 `KO-7`・`N-15`・`CM-33`・`CM-34`・`CM-75`。
...
| `FR-018` の `:4488`〜`:4496`（開いたままの印の段 —— 印の行と祖先と直下の子を外す、印は畳みに勝たない、単調性、表 T-254 に従う、行を作るときは偽、`isKeptOpen` を持たない版は偽） | 表 T-329 を指す段と、表 T-328 を指す段（4.3 節）。古い版の段は `AT-153` の意味へ移る | 決定 2・13・18 |
...
| 同 `HR-2` の「⛔ `AT-56` も `AT-57` も動かさない（MUST NOT） —— 列を増やしても減らしても、保存する文書の形が変わる」 | 「⛔ 段 0 のために `treeState` の値も列も足さない（MUST NOT）」 | 決定 1（問い 1 が案 B なら、この文は「段 0 の値は `documentSettings` の鍵が持つ」に替わる） |
...
| 同 段 0 の段「⇒ `AT-56` に当たるものを … `S-211` が持ち、`AT-57` に当たるものを持たない」（`:1693`）、「⚠️ 段 0 は `AT-142` に当たるものも持たない」（`:1698`）、「⇒ `HF-16` は 表 T-254 の印を立てない」（`:1699`）、「⇒ `HF-10` は … 印を外す（同表の `KO-3`）」（`:1700`） | 「段 0 は `collapsed` に当たるもの（`S-211`）だけを持ち、ほかの 4 つの値に当たるものを持たない」「⇒ `HF-10` は `HF-2` を段 0 に対して行う —— すべての行が表 T-328 の同じ升に従う」 | 決定 4・13 |
...
| `05-07-design.md` 6.1 節の表 T-297（`RK-1`・`RK-2`） | `RK-3` 〜 `RK-5` を足す。「スキーマを走らせる前に、表 T-297 の列を捨てて読む」の後に「捨てる前に、同表の理由が名指す読み替えを済ませる」 | 決定 18 |
| 同 5.x 節「⚠️ 図 F-018 は文書のデータの状態なので、原稿に入れない。」と、表 T-250 | 「文書のデータの状態は、`SD-5` の行の木を除いて原稿に入れない」と `SD-5` | 決定 17 |
| 同 表 T-068 の `LC-2`「開いたままの印（表 T-254）」、表 T-075 の `UF-139`「開いたままの印が残す行を答え（表 T-254）」、`UF-79`「行の畳み・隠し・開いたままの印を…書き換える」、`UF-96`「表 T-015・表 T-051・表 T-254 に従って」、`UF-35`「古い版の欠けた列を埋め（`AT-141`〜`AT-143`・`GP-1`）」 | 4.16 節の文 | 決定 13・17・18 |
...
| 状態機械 `levelZeroFoldStateMachine`（画面の値、表 T-280）の出来事 `levelZeroOpened` の出どころ `HF-16`・`HF-10`・`HF-17`・`S-211` | 出どころに `FR-055`・`HF-8`（全体表示）と `OP-3`・`RD-4`・`RD-6`・`RD-7`（置き換えと新しい文書）を足し、`folded → unfolded` の升の根拠にも足す。状態機械そのもの・出来事の数は変えない | `JDG-595`・決定 22。先例: `HF-17` は行を作った後に書き込みの無い `levelZeroOpened` を送る |
| 表 T-206 の `S-211` の注「⭐ 戻す道は 3 つある —— `HF-16`・`HF-10`・`HF-17`」 | 「戻す道は 5 つ —— `HF-16`・`HF-10`・`HF-17`・全体表示（`FR-055`）・文書の置き換え」 | 同上 |
| 表 T-280（画面の値）の出来事 32・状態機械 12 | 変えない | 旧の設計が足した 4 つの出来事と 1 つの状態機械は `refactor` に無い |
| `display-words.json` の `RS-31` の語 | 4.14 節の語 | 決定 19 |
| `row-id-prefixes.json` の `KO`（開いたままの印を立てる入口と外す入口） | 行を外す。`TD` を足す | 決定 13・表 T-329 |

⚠️ **問い 1 が案 B（段 0 の畳みを保存する）のときに増える編集**: `S-211` を表 T-206（保存しないもの）から退かせ、`documentSettings` に鍵（仮 `levelZeroTreeState`、値 `auto` ／ `collapsed`）を足す（`erd.json` の見せ方の群、表 T-104 の `K-` 行 1 つ）。`levelZeroFoldStateMachine` を画面の値の原稿から外し、段 0 の畳みと開きを表 T-328 の出来事（`everyRowFoldPressed`・`topLevelOpenPressed`・`everyRowOpenPressed`・`fitPressed`）の副作用の書き込みにする。取り消しの 1 段に行の値と同じく入る（`UN-14`）。置き換えでは開いた文書の値になる。古い文書（鍵が無い）は `auto` として読む（`OP-6` の欠けた鍵）。
...
| `.claude/skills/spec-graph-check/retired.py` | `AT-56`・`AT-57`・`AT-142`・`T-254`・`KO-1` 〜 `KO-7`・`N-15`・`CM-33`・`CM-34`・`CM-75` を、理由（本書・`JDG-596`）とともに足す | 名簿は「まだ何かが名指す番号」を持つ —— 本書と台帳が名指す |
...
### 3.4 古い `GRS JSON` を読む（3 つの真偽を持ち、`treeState` を持たない文書）

仕様が既に許している所:

- `FR-073`: 形式の版がこの造りの知る版より古い文書は、読める版である（拒むのは新しすぎる版だけ）。
- 表 T-297（`05-07-design.md` 6.1 節）: 仕様が落とした列を、スキーマを走らせる前に黙って捨てる（「読む人の側に直せることが何も無い」ので告げない）。
- `AT-143`（`Project.sourceFormat`）の意味: 「この列を持たない `GRS JSON` は、… として読む」—— 足した列の値を、古い文書のほかの値から導く先例。
- `FR-018` の `:4496`: 「以前の版の `GRS JSON` が `isKeptOpen` を持たないときは、すべての行の印を偽として開く」—— 列を足したときの先例（本書で `AT-153` の意味へ移る）。コードの先例は `json-codec.ts` の `FIRST_SCHEMA_VERSION_WITH_KEPT_OPEN_MARK`。
- `FR-024`・`FR-073`: 書き出す文書はこの造りのスキーマと版で書く ⇒ 読み替えた文書を保存すれば、新しい形になる。

⇒ 読み替えは仕様の枠の中で決められる（利用者の裁定は要らない）。本書の決定（決定 18）:

| 古い文書の行の値 | 読む値 | 注 |
|---|---|---|
| `isHidden` が `true` | `hidden` | ほかの 2 つが何であっても |
| そうでなく、`isCollapsed` が `true` | `collapsed` | `isKeptOpen` が `true` でも（入口はこの組み合わせを作らない） |
| そうでなく、`isKeptOpen` が `true` | `expanded` | |
| どれでもない（`null` は `false` と同じ。鍵が無いのも同じ） | `auto` | |
| `treeState` を持つ | `treeState` の値 | 3 つの鍵も持っていれば、それは捨てる（表 T-297） |
| `treeState` が 5 つの値のどれでもない | 文書ごと拒む | 日程データの群はスキーマに合わなければ拒む（`FR-073`、今の真偽の列と同じ） |

- 置き場: 読み替えは `AT-153` の意味の欄に書き（`AT-143` と同じ形）、3 つの列は表 T-297 の `RK-3` 〜 `RK-5`（捨てる理由「`treeState`（`AT-153`）へ読み替えた後に捨てる」）。6.1 節に「捨てる前に読み替える」順を 1 文足す（先に捨てると畳みと隠しが消える）。
- 版: この造りが書く形式の版を上げる（`FR-073` の日付の版）。読み替えは版ではなく鍵の有無で行う —— `treeState` を持たない文書だけが読み替えの相手であり、版の境目を 1 つ覚える必要が無い。
- ⚠️ 失うもの: 「畳みつつ印」（`fK`）と「隠しつつ印」の印、全体表示が作った「隠して畳まない」（`h·o`）の区別。どれも入口が作らない組み合わせか、`DFC-985` の穴が作った組み合わせである。
- ⚠️ 逆向きは開けない: 新しい造りが保存した文書を古い造りで開くと、日程データの群が古いスキーマに合わない（`isCollapsed` ほかが無く、知らない `treeState` がある）ので、古い造りは文書ごと拒む（`RS-64`）。⇒ 11 節の問い 2 で利用者に確かめる。
- 仕様の外の文書: `sample-schedule/No Name.json`・`sample-schedule/Three-Year Product Plan.json`、起動テンプレート（`tools/generate_startup_template.py` が刷る `startup-template.json`）、`docs/guides/schedule-to-grs-json/` の骨組みと依頼文 —— 新しい形へ書き直す（9 節）。読み替えがあるので、書き直さなくても開けるが、依頼文が古い鍵を教え続けるのは避ける。

### 3.5 試験（`e3b6d50c` の木で数えた）

| 旧 | 新 | 持ち場 |
|---|---|---|
| 3 つの真偽の名を持つ `tests/` のファイル 117 | 下の判断の要るファイル（約 20）を除き、機械的に `treeState` へ替える（写像は 3.4 節の表） | 波 2c（機械の体） |
...
```json
{
 ...
 "exchange": "",
 "meaning": {
  "ja": "行の木の状態。`auto` ／ `collapsed` ／ `expanded` ／ `temporarilyExpanded` ／ `hidden`。値が描かせる行は `FR-018` の 表 T-329、値を書き換える入口と先の値は `_assets/tbl-state-machines.md` の 表 T-328 が持つ。行を作るときは `auto`。この列を持たない `GRS JSON` は、`isHidden` が真なら `hidden`、そうでなく `isCollapsed` が真なら `collapsed`、そうでなく `isKeptOpen` が真なら `expanded`、どれでもなければ `auto` として読む（`null` は偽と同じ。3 つの列は 表 T-297 のとおり読んだ後に捨てる）"
 }
}
```

⚠️ `"default"` を列挙の列に書けるかは `erd.json` のスキーマで確かめる（真偽の列 `isKeptOpen` は持っていた）。書けなければ既定は意味の欄だけが持つ。
...
| 行 ID | 種類 | 条件 |
| --- | --- | --- |
| TD-1 | すべて要る | 段 0 が畳まれていない（`_assets/tbl-settings.md` の 表 T-206 の `S-211`） |
...
```text
⇒ 段 0 は `collapsed` に当たるものを `_assets/tbl-settings.md` の 表 T-206 の `S-211` に持ち、ほかの 4 つの値に当たるものを持たない（MUST） —— 段 0 には畳み込む先の親が無いので隠せず、その直下の最も浅い段は `FR-018` が倍率で落とさないので開いたままにする必要も無い。  
...
- `HR-2` の「⛔ **`AT-56` も `AT-57` も動かさない（MUST NOT）** —— 列を増やしても減らしても、保存する文書の形が変わる。」→「⛔ **段 0 のために `treeState`（`AT-153`）の値も列も足さない（MUST NOT）** —— 足すと、保存する文書の形が変わる。」（問い 1 が案 B なら 3.2 節の別の文）。
...
- `UN-14`: 「畳みと非表示の変更」→「行の木の状態（`treeState`）の変更」。注「⚠️ 畳みと非表示が `UN-7` でないのは、どちらも `TaskGroup` の列だからである」→「⚠️ 行の木の状態が `UN-7` でないのは、`TaskGroup` の列だからである —— 書き換えを起こしたのが縦を縮める入力でも同じである（`FR-018`）」。
- `UN-17`: 「**全体表示が人の畳んだ状態を捨てること**」→「**全体表示が行の木の状態を戻すこと**（表 T-051 の `HF-8`）」。「⚠️ **戻るのは畳みだけであり、…**」→「⚠️ **戻るのは行の木の状態だけであり、倍率と表示位置は `UN-8` のまま対象外である**」。印の文を消す。
- 全体表示の段: 「⚠️ **戻るのは畳みだけである**（`UN-17`）。⭐ 同じ押下が外した開いたままの印…」→「⚠️ **戻るのは行の木の状態だけである**（`UN-17`）。」。「② 畳んだ行をすべて開き、開いたままの印をすべて外す（同 `CM-72`。」→「② `hidden` 以外の行の木の状態を `auto` へ戻す（同 `CM-72`。」。
- 段を 1 つ足す（全体表示の段の後）:
...
### 4.11 `05-07-design.md` 6.1 節と表 T-297

```text
| RK-3 | `TaskGroup` | `isCollapsed` | `treeState`（`_assets/fig-erd-detail.md` の `AT-153`）へ読み替えた後に捨てる —— 読み替えは同列の意味の欄が持つ |
| RK-4 | `TaskGroup` | `isHidden` | 同上 |
| RK-5 | `TaskGroup` | `isKeptOpen` | 同上 |
```

「捨てた列を通知してはならず、書き戻してもならない（MUST NOT）」の後に: 「⭐ 捨てる前に、本表の捨てる理由が名指す読み替えを済ませること（MUST） —— 先に捨てると、古い文書が持つ行の畳みと隠しが消える。」
...
- ⚠️ 文書のデータの領域であることを原稿に言わせる欄（仮 `"holds": "documentData"`）を `state-machines.schema.json` に足す。その領域について生成器は ① 判別共用体と根の型を刷らない（状態の型は `erd.json` の `AT-153` の列挙が持つ）② 遷移表の定数だけを刷る ③ 状態機械の状態のキーの並びが `AT-153` の値の並びと一致しなければ拒む ④ 生成した文書の前文に「保存する値である」と書く。
...
| `childRowAddPressed` | 入力: `IC-91`・`HF-14`・`HR-8` | `pressedRowId` |
...
### 4.13 段 0（`S-211` と `levelZeroOpened`）

- 表 T-206 の `S-211` の注: 「⭐ **戻す道は 3 つある** —— 表 T-051 の `HF-16`（段 0 を 1 階層開く）と `HF-10`（すべて開く）、そして `HF-17`（最も浅い段へ行を足す）である。」→「⭐ **戻す道は 5 つある** —— 表 T-051 の `HF-16`（段 0 を 1 階層開く）と `HF-10`（すべて開く）と `HF-17`（最も浅い段へ行を足す）、全体表示（`FR-055`、同表の `HF-8`）、そして文書の置き換え（表 T-024a の `OP-3`）である。」。「⚠️ 置き換えで開くのは、保存しない値を次の文書へ持ち越さないためである。」を足す。
- `state-machines.json` の出来事 `levelZeroOpened` の `source.rows` に `FR-055`・`HF-8`・`OP-3` を足し、`levelZeroFoldStateMachine` の `folded → unfolded` の升の `evidence` にも足す。出来事の数・状態機械は変えない。
...
- 表 T-108: `CM-33`・`CM-34`・`CM-75` の行を消し、`CM-85` を足す: 「| CM-85 | `TaskGroup` | `setTaskGroupTreeState` | — | 行の木の状態を置く | `FR-004`（表 T-328）|」。`CM-72`: 「| CM-72 | `TaskGroup` | `resetTaskGroupTreeStates` | ⭐ | 隠した行を除くすべての行の木の状態を `auto` へ戻す | `FR-055`（表 T-051 の `HF-8`、表 T-328）|」。
...
- `UF-35` の「古い版の欠けた列を埋め（`AT-141`〜`AT-143`・`GP-1`）」→「古い版の欠けた列を埋め（`AT-141`〜`AT-143`・`AT-153`・`GP-1`）」。
...
| 段 0（波 2b） | `frame-loop.ts`: 全体表示の書き込みの後と、置き換え・新しい文書の後に、段 0 が畳まれていれば `{ type: 'levelZeroOpened', writes: [] }` を送る（先例は `HF-17` の行を作った後の送り） |
| 読み替え（波 2b） | `json-codec.ts`: 行が `treeState` を持たなければ 3.4 節の表で作り、その後で表 T-297 の 3 列を捨てる。`FIRST_SCHEMA_VERSION_WITH_KEPT_OPEN_MARK` の埋めは消える。`mspdi-imported-rows.ts` は `treeState: 'auto'` |
| 変えない値 | `S-87`・`S-88`・`groupDepthLimit` の式、`HF-18` の数え方、`RS-28`・`RS-29`・`RS-30`・`RS-32`、`levelZeroFoldStateMachine` の状態と出来事 |

⭐ 試験の体へ渡す継ぎ目は、上の名のうち仕様に現れるもの（`treeState` と 5 つの値、`setTaskGroupTreeState`、`resetTaskGroupTreeStates`、表 T-328 の出来事のキーとガードの名）だけ。試験の体はコードを読まない。
...
| figures | 27 | 28 | ＋`F-043` |
| rows | 2362 | 2362 | −7（`KO-*`）＋7（`TD-*`）＋3（`RK-3` 〜 `RK-5`）−2（`AT-*` の 3 が 1）−2（`CM-*` の 3 が 1）＋1（`SD-5`）、`N-*` は ±0 |
| uids | 162 | 162 | — |
| 原稿の領域 ／ 状態機械 ／ 出来事 | 8 ／ 24 ／ 81 | 9 ／ 25 ／ 91 | ＋`rowTree` ／ ＋`treeStateMachine` ／ ＋10 |
| 表 T-280（画面の値）の出来事 ／ 状態機械 | 32 ／ 12 | 32 ／ 12 | `levelZeroOpened` の出どころが 3 つ増えるだけ |
| `erd.json` の列 | 141 | 139 | −3 ＋1 |
| 検査 23 の免除（`erd.json` の型 ／ `null` の欄 ／ 合計） | 128 ／ 141 ／ 312 | 126 ／ 139 ／ 308 | 3.3 節 |
| 表 T-108 の命令 | — | −2 | −`CM-33` −`CM-34` −`CM-75` ＋`CM-85` |

⚠️ 問い 1 が案 B なら: `S-` の行 −1（`S-211`）、`K-` の行 ＋1、画面の値の状態機械 −1。
...
| 波 | 持ち場 | 中身 |
|---|---|---|
| 0 | 調整役 | 起草のコミットと本書を `refactor` へ摘み取る（`0d49e737` は摘み取らない）。本書の 11 節の答えを畳み込む |
...
⚠️ 問い 1 が案 B なら、波 1 に 3.2 節の編集、2a に段 0 の値を書く命令、2b に `frame-loop.ts` の段 0 の読み替え（画面の値から文書の値へ）が加わる。
...
- 試験（2d の新しい試験 `tests/unit/cr-570-tree-state-*.test.ts` の主張。どれも 表 T-328・表 T-329 と新しい `HF-*` の逐語を引く）:
  ...
  11. 古い `GRS JSON`（3 つの真偽）を 3.4 節の表のとおりに読み、保存すると `treeState` だけを書く
...
- 図 F-018 を原稿へ移さない。
- 段 0 の畳みを文書に保存しない —— 問い 1 の答えが出るまで。
...
| `docs/development-records/rulings.md` | `JDG-590` 〜 `JDG-599`（どれも「指示 —— 本書が当てる」）。問い 1・2 の答えは `JDG-600` 以降（調整役から受ける） |
...
| A27 | 文書を開く（置き換え `OP-3`・`RD-4`、新しい文書 `RD-7`、起動テンプレート `BT-4`） | すべての行 | ファイルが持つ値（古い版は 3.4 節で読み替える）。開いた直後の全体表示（`OP-10`）は値を変えない | | | | |
...
### 14.4 表 B —— 段 0（`levelZeroFoldStateMachine`。問い 1 が案 A のとき）
...
| B7 | 縦・横のズーム、スクロール | — | — |
| B8 | 取り消し・やり直し | — | —（問い 1 が案 B なら、段が覚えた値） |
| B9 | 文書の置き換え・新しい文書（`OP-3`・`RD-4`・`RD-6`・`RD-7`） | — | → `unfolded` ⚠️（決定 22。問い 1 が案 B なら、開いた文書の値） |
| B10 | 起動 | 初期 | （起きない） |
...
| B9 | 置き換えは段 0 に触れない | 開く | 決定 22、`DFC-984` |
...
⭐ 今と同じ升: A1（`temporarilyExpanded` の列を除く）、A3、A7 〜 A13（`KO-4`・`KO-5` と `HR-6`・`HR-4` —— `JDG-594` が確かめた）、A16、A18 〜 A21、A24 〜 A30、B1 〜 B4、B6 〜 B8、B10。
...
### 14.8 今の 8 つの組み合わせ → 5 つの値（比べるための写像。3.4 節の読み替えと同じ）
