...
>
> 閉じるもの: `DFC-983`（すべて開く入口が、倍率で描かれていない行を開かない）。利用者の裁定 `JDG-590` 〜 `JDG-593`（逐語は 0.1 節）。
...
| `docs/development-records/rulings.md` | `JDG-590`・`JDG-591`（状態「指示 —— 本書が当てる（起草のみ）」。本書と同じコミットで記入）、`JDG-592`・`JDG-593`（11 節の問いの答え。答えを本書に畳み込んだコミットで記入） |
