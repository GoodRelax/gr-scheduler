...
> ✅ **適用済み（2026-08-17）。利用者の裁定を受けた。**
> **閉じるもの**: `erd_json_to_schema.py --report` が挙げる「メンバの綴られていない列挙 4 件」。
> ⚠️ **「型を読めない設定値 3 件」は本 CR では閉じない** —— 裁定 4（`dualCursor`）と裁定 5
> （`planActualGuidePattern`）は「生成器を触るかどうか」を訊いており、**利用者は裁定 C で
> 「生成器の射程を広げる」を選んだ。よって 2 件は CR-175 が引き取る。**
...
| | 訊いたこと | 裁定 | 本 CR で閉じたか |
|---|---|---|:-:|
| 1 | 11 の綴り | **`shapeKind` の流儀（lowerCamelCase）に倣う。** `triangleUp` / `triangleDown` / `calloutBox` の 3 つの発明を含めて採る | ✅ |
| 2 | 綴りの置き場 | **`erd.json` だけに置く。** 値の表は新設しない | ✅ |
| 3 | `AT-99` / `AT-104` の員数の文 | **3 値で確定。** 型欄を素の総数へ直す | ✅ |
| 4 | `dualCursor`（`S-65`）| **生成器側で解く**（裁定 C = 案 (a)）| ⛔ CR-175 |
| 5 | `planActualGuidePattern`（`S-104`）| **同上** | ⛔ CR-175 |

### 実測（適用後）

`erd_json_to_schema.py --report` は **`enumerations with no spelled-out members (0)`** を返す。
残るのは `settings keys with no readable type (3)` の 3 件であり、**上の裁定 4 / 5 のぶんである。**

### ⭐ 適用が明るみに出したもの —— **コードが 4 か所で嘘をついていた**

| | 直したもの | 何が起きていたか |
|---|---|---|
| 1 | `edit-task.ts` の 3 つの ⛔ MISSING | 「綴りがどこにも無いので所属を検査できない」と書いていた。**3 件とも古くなった。** 型に所属を持たせ（`TaskMilestoneGlyph` / `TaskLineWeight` / `TaskNameAlign`）、`shapeKind` の先例に合わせた |
| 2 | `edit-annotation.ts` の `CM-49` | ⛔ **入口が丸ごと拒否だった** ——「2 種の綴りが無いので `FR-019` の MUST に守らせるものが無い」。**綴りが出たので入口が動くようになった。** ⚠️ **払い出しは `null` を取らない** —— `FR-019` は 2 種からの選択を求めており、**戻す入口は仕様書のどこにも無い**（`CM-46` の ⛔ が待っているのと同じ穴。W4 の裁定に載っている）|
| 3 | `edit-annotation.test.ts` | ⛔ **`CM-49` をわざと検査から外していた** ——「値が拒まれたのか id が拒まれたのか区別できない」。**区別できるようになったので入れた。** 2 値それぞれを書けることも検査に足した（669 件） |
| 4 | `schedule-geometry.ts` の注記 | 「綴りが存在しないので全マイルストーンを ◇ で描く」。⛔ **理由が消えた**（欠けているのは名前ではなく**残り 7 つの幾何**である）。⚠️ **`SH-5` が最初に挙げるのは 〇 であって ◇ ではない**という事実誤認も直した |

⭐ **4 件とも 20 検査は 1 つも知らせなかった。** ⛔ **コードの注記は仕様が動いた瞬間に腐り、機械はそれを見ない。**
...
⭐ **裁定 2 が「`erd.json` だけに置く」だったので、表が 1 つも増えず、行も増えなかった。**
裁定 3 は**既存の 2 つの欄を書き換えるだけ**なので、これも行を動かさない。
...
### 実測（`git diff --unified=0`）

| 生成物 | 予測 | 実測 |
|---|---|---|
| `fig-erd-detail.md` | 表 T-058 の `AT-99` / `AT-104` と 図 F-011 の 2 行 | **その 4 行だけ** ✅ |
| `grs-document.schema.json` | 4 件が `enum` へ狭まる | **4 件とも狭まった** ✅ |
| `schedule.ts` | 4 件が直和へ | **4 件とも直和へ** ✅ |

### 検査の結果

`check.sh` **ALL GREEN**（20 本）／ `audit-ch5.py` **PASS** ／ `check-render.py` **PASS** ／
`npm run typecheck` **緑** ／ `npm test` **669 件緑**（`CM-49` の 1 件を足した）／
