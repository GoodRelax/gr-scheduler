...
| 項目 | 行き先 | 覆すときの戻し方 |
|---|---|---|
| 入口（11 節の問い 1。推奨は案 A） | 案 A: `FR-068` の面（`U-30` の `AI Export Modal`）に「中身の切替」の行を 表 T-109 に 1 行（`IC-115`。取りまとめ役が 2026-09-24 に予約した）。閉じる入口 `IC-52` は、いま見せている中身を写す（今の「閉じると写す」、`frame-loop.ts:3933` をそのまま広げる） | 行を消し、`FR-068` の 2 つ目の中身の段を消す |
