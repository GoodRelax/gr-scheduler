...
⚠️ **本書の誤り**: ① 第 1 節の 31 行は 2 行を落としていた —— `05-07-design.md` の 表 T-069 の前文「交換相手にも、実績バーの右端（`RV-1`）のほかは書かない」と、Chapter 6.2 の「実績バーの右端（`RV-1`）だけは、交換相手へ書き出す行を持つ —— それが `DV-9`」（正規表現が `DV-9` と `RV-1` を拾わない）。② 表 T-244 の `TE-2`「期間を置く」も同じ読みで変わる。③ `FR-006` の MUST NOT は 01-04 の 1767 行（本書は 1766 と書いた）。④ 決定 8 の置き場は `FR-054` ではなく `FR-011` にした（数える規則の 1 か所に置くため。`FR-054` は閉路の中で書き換えない側）。⑤ 語 `P-4` の `stop` の日本語は、退けた `FR-006` の文が使っていた「中断日」を運んだ（語の見直しに回す）。
⚠️ 決定 11（`schemaVersion` を上げる）は仕様に値を持たない（`FR-073` は書式だけを持つ）ので、仕様の側では何も書いていない —— 実装の波で上げる。
