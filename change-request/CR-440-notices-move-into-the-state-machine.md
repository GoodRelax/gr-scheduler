...
> - 行 ID の接頭辞の登録簿は、当てる直前には 163 行だった（2026-09-21 実測、`"prefix"` を数えた）。`SM` ・ `EV` ・ `TN` は登録済みだった。**本書は新しい接頭辞を登録しない。**
...
## 改訂の記録 —— 番号を名に改めた（`JDG-286`、2026-09-21）

原稿の行の番号を、利用者の裁定 `JDG-286` が退けた。本書の付録と改訂の記録の番号は、当てた日の履歴としてそのまま残す。

| # | 起草時の文 | 直した文 | 理由 |
|---|---|---|---|
| 12 | 付録 A.1 〜 A.3 と本文の `SM-34` 〜 `SM-38` ・ `EV-31` 〜 `EV-35` ・ `TN-54` 〜 `TN-63` | 本書では直さない。いまの名は、状態機械 `noticeDisplayStateMachine`（軸 `onScreen`）と `changeDeliveryStateMachine`（軸 `delivery`）、状態 `noticeDisplayStateMachine.shown`（もとの `standing`）のような `状態機械.キー`、出来事 `notices/noticeRaised` のような `領域/キー`、遷移はその升（状態機械・いまの状態・出来事）である。全数は `docs/spec/_assets/tbl-state-machines.md` の 表 T-286。対応の規則は `CR-436` の「波 A2」の節 | `JDG-286`（原稿と印字を状態機械ごとに組み替え、番号をやめて名で指す）。接頭辞 `SM` ・ `EV` ・ `TN` は登録簿から外した。表 `T-287` ・ `T-288` は 表 T-286 に畳まれ、いまは使われていない |
| 13 | 識別子の段の登録簿の大きさと「登録済み」（現在形） | 「当てる直前には 163 行だった」「登録済みだった」 | 本書を直すと検査 62 が本書を読み、いまの登録簿（160）と食い違う現在形の主張として赤にする。`CR-436` の改訂の記録の 11 と同じく過去形の履歴へ |
