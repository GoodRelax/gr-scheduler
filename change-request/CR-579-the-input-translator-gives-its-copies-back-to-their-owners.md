...
> ⛔ **最終の割り方の表（4 節）は、まだ利用者の了承を取らない。**  
>  調整役の指示（2026-09-26）により、`src/` 全体の関数の重複の調査が報告するまで待つ。  
> その調査の群のうちこのファイルに落ちるものは、本書に統合する（2 つの写しのまま動かさない）。
...
| `DFC-1041` | 表 T-109 は、表示の切り替えの入口が書き換える設定の行と `Guide Cursor` の型を文の中にしか持たない。<br>コードの `VISIBLE_ELEMENT_BY_ENTRY`（11 行）と `GUIDE_CURSOR_MODE_BY_ENTRY`（2 行）はそれを打ち直している（`03-implementation.md:22`）。<br>`IC-48` には設定の行 ID も無い | `tbl-glossary.md:513-571` を読む。<br>10 節の `JDG-691` の答え次第 |
...
## 10. 利用者に問う問い（⛔ 重複の調査の報告の後にまとめて問う。4 節の表の了承と同じ一通）
...
`JDG-692` 〜 `JDG-694` は予備（使わなければ調整役へ返す）。
