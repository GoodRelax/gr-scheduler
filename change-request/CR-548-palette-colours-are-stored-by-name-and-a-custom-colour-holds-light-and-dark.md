...
- 接頭辞の登録簿は起草の日に 160 件だった。当てる直前（2026-09-22、`577231a8` の上）に測り直すと 161 件で、`CV` を足して 162 件になった。表 T-294 と、行 ID の 2 つの帯（S の 314 〜 324 と、新しい接頭辞の 1 〜 9）、表 T-017b、DFC の 728 も、同じ日に 67 のブランチの先端でどの木も使っていないことを測り直してから取った（表 T-294 は `handoff-state-machine.md` が「使わずに返した」と記すだけであった）。
...
| CV-6 | 描く値 | パレット色は、表 T-294 がその名に持つ、いま描いている明暗の値で描くこと（MUST）。<br>どの欄の値かは描く所で決める: `fillColor` は予定バーの塗りを塗りの値で、実績バーの塗りを実績の塗りの値で描く。<br>`strokeColor` は予定バーと実績バーの縁を、`HighlightBox.strokeColor` は枠の線を、縁の値で描く。<br>`TaskGroup.color` は行の帯の値で描く。<br>⭐ 白と黒は暗いテーマで入れ替わる（表 T-294 の `S-314` ・ `S-315`）。<br>カスタムカラーは、実績バーの塗りを除くどの所にも、`CV-3` で決まった値をそのまま使うこと（MUST）。<br>カスタムカラーの実績バーの塗りは、その値から試作の予定と実績の差で導くこと（MUST）: 明るいテーマは HSL の彩度に 16 を足して明度から 46 を引き、暗いテーマは彩度に 30、明度に 38 を足し、どちらも 0 〜 100 に収める（`previous-project-result/08-poc/poc-integrated.html` の `:53-55` / `:93-95`、`_assets/tbl-settings.md` の 表 T-236 の予定の塗り `S-155` と実績の塗り `S-157` の差に同じ）。<br>⭐ 導くのは実績という別の形であり、`CV-8` の補正ではない —— 選んだ値は動かさず、表 T-017a の結びの「実績を濃く描くこと」を保つ（`CT-3`） |
...
旧（`:1996-1997`、`FR-041` の RATIONALE の末尾。当てるときに、前の文との間に挟まず末尾へ置いた —— 2 文を続けて引く試験の注記があるため）
```text
...
## 13. 当てた後の測定（2026-09-22、`origin/refactor` の `577231a8` の上）

| 数 | 前（`577231a8`） | 後 | 予測との差 |
|---|--:|--:|---|
| tables | 184 | 186 | ＋2（予測どおり） |
| figures | 27 | 27 | — |
| rows | 2262 | 2282 | ＋20（予測どおり） |
| uids | 162 | 162 | — |
| 接頭辞 | 161 | 162 | ＋1（予測どおり） |
| `tbl-settings.md` の `\| S-` の行 | 301 | 312 | ＋11（予測どおり） |

⚠️ 前の数は 7 節の測りと違う（7 節は `4015736e` の上、ここは `CR-530` の波 A の後の `577231a8` の上）。差だけが予測と一致する。

**4 節から離れて当てたもの（理由つき）**

- E-03: 表 T-017b を「係数そのものは Chapter 5 が定める…」の段落の後（表 T-017a の直前）に置いた。表 T-017 の結びの直後に置くと、その段落が表 T-017b の下に数えられ、重複の検査（検査 11）が新しい組として読んだ。表の文は 4 節のまま。
- E-03 の `CV-6`: カスタムカラーの実績の塗りの文（11.1 節の 1・5）に、「表 T-236 の `S-155` と `S-157` の差に同じ」を添えた —— コードは差を表 T-236 の 2 つの行から読み、数を写さない。
- E-05: 新しい文を `FR-041` の RATIONALE の 2 文の間ではなく末尾に置いた。2 文を続けて引く試験の注記が 2 つあり（検査 42）、間に挟むと引用が切れる。
- 生成器（E-20）: 表 T-294 の欄のうち「—（行の色には選ばせない）」は、生成される定数では `false`（その形はテーマの値のまま）、「描かない」は `null`（描かない）とした。列の形（`COLUMN_SHAPES`）の `color` の列は、表 T-294 の名を選択肢として持つ（`AT-121` は透明を除く）。

**台帳**: `defects.md` に `DFC-728`（`仕様待ち`）を足した（11.1 節の 4）。`PND-494` ・ `DFC-566` ・ `DFC-684` の閉じは本書の仕事ではない（10 節）。

**門**（同じ木で）: `npm run gen:check` 緑。`npx tsc --noEmit -p .` の誤りは `tests/unit/uf-45-46.test.ts` の 8 つだけ（本書の前から在る）。`npx vitest run` の赤は本書の前と同じ 4 件と、読み込めない 11 ファイル（`docs/reference/mspdi/` が無い）だけ。`check.sh` の赤は検査 39（逐語の試験を持たない MUST が 1334 → 1355。本書の `CV-` の行と `FR-007` の文の分）だけ —— 基準線は動かしていない。仕様だけを読む試験者が逐語の試験を足すまで赤のまま残る（8 節）。
