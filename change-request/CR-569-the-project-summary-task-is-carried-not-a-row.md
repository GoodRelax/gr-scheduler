...
> 着地の状態（2026-09-25）: 波 1 は `c1c71807`（E-01 〜 E-08、`npm run gen`）、波 2a は `5529a0de`（`mspdi-codec.ts`）。波 2b（仕様だけを読む試験の体）と波 3 の実物確認の記録は `DFC-982` の行にある。検査 39 は 1305 → 1311 —— 波 2b が新しい 6 条項を逐語で引くまで赤（基準値は動かさない）。
