...
> 起草の状態: 起草した（2026-09-26）。利用者の裁定 `JDG-603`（仕分けの変更要求を起こす）と、本書のために問うた `JDG-620` 〜 `JDG-625`（逐語は 0.1 節）を受けた。⛔ 仕様・コード・見本はまだ 1 文字も変えていない。
...
| `JDG-621` | 「明暗テーマ・モノクロ箱のみ。 文字サイズは.json  ※理由: 適切な文字サイズはプロジェクトによる。」 | 明暗テーマとモノクロは見る人の好み。文字サイズ（S/M/L）は文書の中身としてファイルに残す | 3.1 節。⚠️「箱のみ」は「好みのみ」と読んだ（11 節の確認 1） |
...
| `JDG-625` | 「定数だけ」（モノクロ・ガイドカーソル・プロパティパネルの幅の 3 問に同じ答え） | 3 つは覚えない。起動のたびに定数の初期値から始まる | 4.3 節 |
...
2. **反証 —— 「どの値も文書に保存する」と決めた記録は在った。**
   ...
   - 同書は「`themeHue`・`themeMonochrome` を保存する理由 —— 色のテーマはプロジェクトのテーマカラーである（ユーザー判断、確定 2026-08-01）」「`themePreference` も同じ枠」と書く。⭐ `JDG-621` はこのうち `themeMonochrome` と `themePreference` を覆す。`themeHue` は `DR-5` のまま日程データの群に残る（本書は触らない）。
   ...
   - `WY-1` は「書き出した JSON を読み直すと描画領域が同じ（倍率と表示位置を含む）」を求め、例外を「読む人の環境に属する表示状態（全数は表 T-206）」とする。⭐ 本書は例外の側に 5 鍵を移すだけであり、`WY-1` の文は変えない。
...
3. **読み替えは要らない。** `OP-6` は「`documentSettings` は、この造りのスキーマが解釈できるものだけを読む。知らない鍵（退役した鍵を含む）は捨てる」と定める（`CR-565`、`JDG-425`）。今のファイルを新しい版で開くと、82 鍵と 5 鍵はそのまま捨てられる。
...
- `R2.1`（命名）: 動かす 5 鍵は名を変えない。今の綴りのまま画面の値になる。新しい名は生成する定数の束 1 つだけである（2 節、仮）。
- `R2.9`（YAGNI）: 好みを覚える別枠（`localStorage`）を足す案は、利用者が項目ごとに退けた（`JDG-624`・`JDG-625`）。⇒ 本書は別枠を 1 つも足さない。
- `R4.3`・`R4.4`（状態遷移）: 5 鍵が文書から画面の値へ移ると、取り消し（`UN-13`）と未保存の印（`FR-100`）と刻印（`FR-063`）から外れる。⇒ 3.3 節で行ごとに挙げた。状態機械は足さない（どれも 1 つの値を置き換えるだけで、状態の組み合わせを持たない）。
...
3. **`Agent API` から 5 つの命令を外す。** `CM-59`（ガイドカーソル）・`CM-60`/`CM-61`（2 本カーソル）・`CM-63`（明暗）・`CM-64`（モノクロ）は、文書への命令（表 T-108 の見せ方の群）から画面の命令へ移る。`CM-67`（パネル幅）は行見出しパネルの幅だけを書く命令になる。`Agent API` は文書を書く口なので（`AG-8`）、見る人の画面の値には届かない。⚠️ 今、Agent から明暗を切り替えている呼び手があれば効かなくなる —— 運用前なので呼び手は無い（`JDG-601`）。
...
| `erd_json_to_schema.py` と `grs-document.schema.json` | 表 → 群の対応を付け替え、スキーマを作り直す（111 → 24 鍵。`CR-570` の 1 鍵を足して 25） | 4.1 |
| `FR-063` | 「見せ方」の定義を「人が文書に対して選んだもの」に絞り、道具の定数を除く | 4.2 |
| `FR-039` | 明暗を文書の値から外す。文字サイズと倍率は今のまま | 4.3 |
| `FR-041`・`FR-048`・`FR-052`・`FR-082` | モノクロ・ガイドカーソル・プロパティパネルの幅・2 本カーソルの置き場の文 | 4.3・4.4 |
...
| 表 T-108（`_assets/tbl-glossary.md`） | `CM-59`・`CM-60`・`CM-61`・`CM-63`・`CM-64` の群を画面の命令へ。`CM-67` は行見出しパネルの幅だけ | 4.7 |
| `05-07-design.md` | 表 T-064 の `PI-2`（公開する定数の束）、`PI-39` の `ScreenSession`（5 つの値）、6.1 節 | 4.8 |
| `display-words.json` | 設定パネルに並ぶ 87 鍵の語を消す（パネルは 24 鍵を並べる） | 4.8 |
...
| 分類 | 数 | 鍵（行） | 行き先 |
|---|--:|---|---|
| 内容 | 24 | 表示の切り替え 11（`S-60`〜`S-64`・`S-67`〜`S-69`・`S-227`・`S-228`・`S-232`）・`stackDirection`（`S-58`）・`fontScale`（`S-70`）・`rulerFont`（`S-3`）・`rulerHeight`（`S-2`）・`displayScale`（`S-234`）・`zoomX`/`zoomY`（`S-75`・`S-76`）・スクロール 4（`S-77`・`S-78`・`S-176`・`S-177`）・`rowTitlePanelWidth`（`S-79`）・`pinnedGroupIds`（`S-126`） | ファイルに残る。⭐ `CR-570` が足す段 0 の畳みの鍵（`JDG-600`）もここに入る |
| 好み | 4 | `themePreference`（`S-72`）・`themeMonochrome`（`S-74`）・`guideCursorMode`（`S-66`）・`propertyPanelWidth`（`S-80`） | 画面の値（`ScreenSession`）。覚えない |
...
| 表 T-203 の `S-72`・`S-74` | 表 T-206（保存しない） |
...
| 旧 | 新 |
|---|---|
| 表 T-108 の `CM-59`・`CM-60`・`CM-61`・`CM-63`・`CM-64`（群「見せ方の群」） | 画面の命令。文書を書かない |
...
| `UN-13`「文書全体の設定の変更 —— テーマ（`FR-041`）・文字サイズ・積む向き・ガイドカーソル（`FR-048`）…」 | テーマとガイドカーソルを外す。残りは今のまま |
| `UN-12`（2 本カーソルの位置は保存するが戻さない） | 行を退かせるか、「保存も取り消しもしない」へ書き直す（当てる体が表 T-027 を読んで選ぶ。どちらでも同じ動き） |
| `UN-16`「パネル幅」 | 行見出しパネルの幅だけ（保存する・戻さない、は今のまま） |
| `FR-100` の未保存の印 | 明暗・モノクロ・ガイド・2 本カーソル・プロパティパネルの幅を変えても立たない（文書を書かないので、規則の文は変えずに済む —— 当てる体が確かめる） |
| `FR-063` の「どちらの群であれ動いた刻」 | 同上。5 つの値を変えても動かない ⇒ `AG-6` の監視に通知されない |

### 3.4 試験（`refactor` 70be9db6 で数えた）

- 定数になる 82 鍵を名で書く所: `tests/` に 1,354 か所、`src/` に 279 か所（生成物を除く、`git grep -c -w <鍵>`）。好みと画面だけの 5 鍵: `tests/` 287・`src/` 55。⭐ 多くは `SETTINGS_DEFAULTS` から設定を組む試験なので、鍵の読み先が変われば試験の組み立ても動く。
- 5 鍵を文書の編集として確かめている試験（取り消し・未保存・刻印）は、新しい規則では偽になる。⇒ 書き直すのは実装の体、新しい規則を書くのは仕様だけの試験の体（8 節）。

### 3.5 見本・起動テンプレート・骨組み（`JDG-601`: 読み替えず書き直す）

| ファイル | 今の鍵の数 | 新 |
|---|--:|---|
| `src/framework/single-html-shell/startup-template.json` | 111 | 24（`CR-570` の 1 を足して 25） |
...
- 生成器 `erd_json_to_schema.py` の対応表（`'T-201': ('documentSettings', None)` ほか 10 行）を `('notStored', '<見出しの語>')` へ。`--report` で鍵の数が 24 になることを確かめる。
...
### 4.3 明暗・モノクロ・ガイドカーソル・プロパティパネルの幅（`FR-039`・`FR-041`・`FR-048`・`FR-052`）
...
- モノクロ・ガイドカーソル・プロパティパネルの幅（`JDG-625`）: 起動のたびに表 T-206 の初期値（モノクロは偽、ガイドは `none`、幅は `S-171`）から始める。覚えない。
- 書き出す画像（`FR-080`）: 明暗とモノクロは、書き出した人の画面の値で描く —— `JDG-123` の「見たままを書き出す」のとおり。
...
`CM-59`・`CM-60`・`CM-61`・`CM-63`・`CM-64` の群の欄を、画面の命令の群（表 T-108 に在る語を当てる体が選ぶ）へ。`CM-67` の要旨を「行見出しパネルの幅を変える」へ。

### 4.8 設計（`05-07-design.md`）と語

- 表 T-064 の `PI-2`: 公開するものに `SETTINGS_CONSTANTS` を足す。
- `PI-39` の `ScreenSession`: 5 つの値（`themePreference`・`themeMonochrome`・`guideCursorMode`・`dualCursor`・`propertyPanelWidth`）を持つ。
- 6.1 節（文書スキーマ）: 鍵の数を書いている文があれば直す（生成器の `--report` の数で）。
- `display-words.json` の `settings` の語: 設定パネルは `SETTINGS_DEFAULTS` の鍵を並べる（`properties-panel.ts` の `settingsFields`）ので、87 鍵の語は使われなくなる。消す。
...
- DocumentSettings keeps exactly the stored keys: the 24 of section 3.1
...
- SETTINGS_CONSTANTS (provisional name) is generated into
  ...
  from a document.
- ScreenSession (PI-39) gains themePreference, themeMonochrome,
  guideCursorMode, dualCursor, propertyPanelWidth, same spellings.
  themePreference starts from prefers-color-scheme (light when unread);
  the other four start from table T-206. None is written to a file,
  localStorage, the undo history, the unsaved mark or the stamp.
- CM-59, CM-60, CM-61, CM-63, CM-64 move to screen commands; CM-67 writes
...
| 何を | 今 | 後 |
|---|--:|--:|
| 文書スキーマの `documentSettings` の鍵 | 111 | 24（`CR-570` の後なら 25） |
| そのうち画面の操作で書き換わる鍵 | 29 | 24（`CR-570` の後なら 25） |
| 表 T-206 の行 | 147 | 151（`S-65`・`S-66`・`S-72`・`S-74` を足す。`S-80` は退かせるので足さない） |
...
| 2b | `src/entity/**`（生成物を除く）・`src/use-case/**` とその試験 | `edit-document-settings.ts` から 5 命令を外す。`screen-values.ts` の `ScreenSession` に 5 値。定数の読み先 | 実装の体 |
| 2c | `src/adapter/**` とその試験 | 描く側・書き出す側・入力の訳し・ツールチップ（`tooltips.ts`）の読み先。`image-exporter.ts` は画面の値の明暗 | 実装の体 |
...
| 3 | `tests/`（新しいファイルだけ） | 仕様だけを読んで書く試験: 保存した JSON の `documentSettings` の鍵の全数、古い鍵を持つファイルを開くと捨てる（`OP-6`）、明暗は起動時にブラウザに従い・書かず・戻さず・未保存にしない、モノクロ・ガイド・幅は開き直すと初期値、`S-124` を変えると古いファイルでも効く | ⭐ 仕様だけの試験の体。実装の体と別。`docs/spec` だけを読む |
...
1. `JDG-621` の「明暗テーマ・モノクロ箱のみ」を「明暗テーマとモノクロは好み」と読んだ。違えば 3.1 節の好み 4 のうち 2 つが内容へ戻る。
2. `JDG-621` は、前プロジェクトで利用者が決めた「色のテーマはプロジェクトのテーマカラー（`themePreference`・`themeMonochrome` を保存する、2026-08-01）」を覆す。問うたときにこの記録を示していない —— 利用者に一度見せて、覆すことを確かめる。
...
| `JDG-620` 〜 `JDG-625` | 0.1 節の裁定 | `rulings.md` に記録済み（着地先は本書） |
...
| 103 | `themeMonochrome` | `S-74`（T-203） | 好み | 表 T-206 へ移す（定数だけ） | 8 | 18 |
